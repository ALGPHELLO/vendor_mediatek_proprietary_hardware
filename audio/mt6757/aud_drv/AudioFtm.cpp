...
int AudioFtm::EarphoneTest(char bEnable) {
    ...
    if (bEnable) { // enable
        mStreamManager->setHeadsetVolumeMax();
        mHardwareResourceManager->openAddaOutput(32000);
...
int AudioFtm::HDMI_SineGenPlayback(bool Enable, int Freq) {
    ...
    if (Enable) { // enable
        ...
        mHardwareResourceManager->setSgenMode(SGEN_MODE_O03_O04);
        mHardwareResourceManager->setSgenMode(SGEN_MODE_O00_O01);
...
int AudioFtm::SetSpkMonitorParam(void *pParam) {
    ALOGD("%s()", __FUNCTION__);
#if defined(MTK_SPEAKER_MONITOR_SUPPORT)
    return AudioCustParamClient::GetInstance()->SetSpeakerMonitorParamToNVRam((AUDIO_SPEAKER_MONITOR_PARAM_STRUCT *)pParam);
#else
    return 0;
#endif
}
...
int AudioFtm::GetSpkMonitorParam(void *pParam) {
    ALOGD("%s()", __FUNCTION__);
#if defined(MTK_SPEAKER_MONITOR_SUPPORT)
    return AudioCustParamClient::GetInstance()->GetSpeakerMonitorParamFromNVRam((AUDIO_SPEAKER_MONITOR_PARAM_STRUCT *)pParam);
#else
    return 0;
#endif
}
