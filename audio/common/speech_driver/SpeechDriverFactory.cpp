...
status_t SpeechDriverFactory::CreateSpeechDriverInstances() {
...
#ifdef MTK_AURISYS_PHONE_CALL_SUPPORT
    ALOGD("Create SpeechDriverOpenDSP for MODEM_2");
    mSpeechDriver2 = SpeechDriverOpenDSP::GetInstance(MODEM_2);
#else
    ...
    char configMultiSim[PROPERTY_VALUE_MAX];
    property_get("persist.radio.multisim.config", configMultiSim, "0"); //"0": default not support

    ALOGD("%s(), configMultiSim = %s", __FUNCTION__, configMultiSim);
    if (strcmp(configMultiSim, "dada") == 0) {
        ALOGD("Create SpeechDriverDSDA for MODEM_EXTERNAL");
        mSpeechDriverExternal = SpeechDriverDSDA::GetInstance(MODEM_EXTERNAL);
    } else {
#ifdef MTK_AURISYS_PHONE_CALL_SUPPORT
        ALOGD("Create SpeechDriverOpenDSP for MODEM_EXTERNAL");
        mSpeechDriverExternal = SpeechDriverOpenDSP::GetInstance(MODEM_EXTERNAL);
#else
