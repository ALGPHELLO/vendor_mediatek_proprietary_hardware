...
status_t WCNChipController::InitAudioBTInfo() {
    ...
    if (libHandle && mtk_bt_op) {
        ...
        for (int trycnt = 1; trycnt <= 10; trycnt++) {
            ...
            ret = mtk_bt_op(BT_MTK_OP_AUDIO_GET_CONFIG, &result);//mtk_bt_op(req, &result);
            //result->param.audio_config.hw_if
            ALOGD("%s(), query BT info status = %d", __FUNCTION__, ret);//ALOGD("%s(), query BT info status = %d", __FUNCTION__, result.status);
