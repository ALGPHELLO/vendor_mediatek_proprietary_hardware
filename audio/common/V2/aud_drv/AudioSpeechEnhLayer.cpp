...
SPELayer::SPELayer() {
    ...
    handle = dlopen("libspeech_enh_lib.so", RTLD_LAZY);
    if (!handle) {
    ...
    mSphEnhOps.ENH_API_Get_Memory = (Word32(*)(SPH_ENH_ctrl_struct * Sph_Enh_ctrl)) dlsym(handle, funName);
    error = dlerror();
    if (error != NULL) {
    ...
    mSphEnhOps.ENH_API_Alloc = (Word16(*)(SPH_ENH_ctrl_struct * Sph_Enh_ctrl, Word32 * mem_ptr)) dlsym(handle, funName);
    error = dlerror();
    if (error != NULL) {
        ALOGE("%s(), dlsym %s fail. (%s)\n", __FUNCTION__, funName, error);
        ASSERT(0);
    ...
    funName = "ENH_API_Rst";
    mSphEnhOps.ENH_API_Rst = (Word16(*)(SPH_ENH_ctrl_struct * Sph_Enh_ctrl)) dlsym(handle, funName);
    error = dlerror();
    if (error != NULL) {
        ALOGE("%s(), dlsym %s fail. (%s)\n", __FUNCTION__, funName, error);
        ASSERT(0);
    ...
    mSphEnhOps.ENH_API_Process = (void (*)(SPH_ENH_ctrl_struct * Sph_Enh_ctrl)) dlsym(handle, funName);
    error = dlerror();
    if (error != NULL) {
        ALOGE("%s(), dlsym %s fail. (%s)\n", __FUNCTION__, funName, error);
        ASSERT(0);
    ...
    mSphEnhOps.ENH_API_Free = (Word16(*)(SPH_ENH_ctrl_struct * Sph_Enh_ctrl)) dlsym(handle, funName);
    error = dlerror();
    if (error != NULL) {
        ALOGE("%s(), dlsym %s fail. (%s)\n", __FUNCTION__, funName, error);
        ASSERT(0);
    ...
    if (dir == DOWNLINK) {
        ...
        if (mFirstVoIPDownlink) {
            ...
            if (bRemainInfo) {
                ...
                if ((TimeDifference(BInputInfo->time_stamp_queued, mPreDownlinkQueueTime) > (mDLLatencyTime * (unsigned long long)1000000)) || mDLlateAdjust) {
                    //if(!mDLlateAdjust)
                    {
                        //two downlink queue interval is larger than hardware buffer latency time, this buffer is playing directly since no previous data in the hardware buffer
...
void SPELayer::CompensateBuffer(size_t BufLength, struct timespec CompenStartTime) {
    ...
    memset(newInBuffer->pBufBase, 0xCC, BufLength);
...
bool SPELayer::PrepareProcessData() {
    ...
    if (mDLNewStart || (DLdataPrepareCount > 0)) { //the first queue downlink buffer is not ready to play yet, only need to process uplink data, so add zero data on downlink
        ...
        ALOGD("PrepareProcessData, DL data is not ready yet, size %d", mDLInBufferQ.size());
...
void SPELayer::EPLTransVMDump() {
    ...
    if (bflag || mVMDumpEnable) {
        ...
        if (bflag) {
            audio_strncpy(mVMDumpFileName, filename, VM_DUMP_FILE_NAME_SIZE);
        }
