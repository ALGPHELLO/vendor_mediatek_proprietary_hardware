...
public:
    ...
    static void instantiate() {
        SLOGV("+VUnlockSharedBuffer::instantiate");
...
AudioVUnlockRingBuf::AudioVUnlockRingBuf() {
    ...
    if (ret != 0) {
        SLOGE("Failed to initialize AudioVUnlockRingBuf mBufMutex!");
    }

    ret = pthread_cond_init(&mBuf_Cond, NULL);
    if (ret != 0) {
        SLOGE("Failed to initialize AudioVUnlockRingBuf mBuf_Cond!");
    }
...
int32_t AudioVUnlockRingBuf:: Write(void *buf, uint32_t datasz) {
    ...
    while (leftsz > 0) {
        ...
        if (loopCount == 5 && leftsz != 0) {
            SLOGD("[AudioVUnlockRingBuf:: Write] Drop Stream out data! data droped %d , write %d", leftsz, writeAmount);
            break;
        }
    }
...
uint32_t AudioVUnlockRingBuf:: WaitBufData(void) {
    ...
    if (leftdata <= 0) { // wait till buffer has 92 ms data.
        SLOGD("[WaitBufData]Buffer empty , wait %d", leftdata);
...
void *ReadRoutine(void *hdl) {
    ...
    if (VInstance == NULL || ringBuf_in == NULL || ringBuf_out == NULL) {
        SLOGD(" [ReadRoutine] Buffer NULL,  Exit");
        pthread_mutex_lock(&mVUnlockReadMutex);
    ...
    if (result == -1) {
        ...
        if (0 != sched_setscheduler(0, SCHED_RR, &sched_p)) {
            SLOGE("[%s] failed, errno: %d", __func__, errno);
        } else {
            sched_p.sched_priority = RTPM_PRIO_AUDIO_I2S;
            sched_getparam(0, &sched_p);
            SLOGD("sched_setscheduler ok, priority: %d", sched_p.sched_priority);
        }
    }
#endif
    ...
    while (!VInstance->mReadThreadExit) {
        ...
        if (VInstance->mDLtime.tv_sec == 0 && VInstance->mNewDLtime.tv_sec  != 0) {
            ...
            SLOGV("[ReadRoutine] switch to new DL time %d %d", VInstance->mDLtime.tv_sec, VInstance->mDLtime.tv_nsec);
        }
        ...
        if (dataread <= 0 || VInstance->mDLtime.tv_sec == 0) { // do not read data if mDLtime is ready.
            ...
            if (dataConsumed > 0) {
                ...
                SLOGV("[ReadRoutine] write to ring out, datawritten %d", datawritten);
            } else {
                datawritten = 0;
            }
            if (datawritten <= 0) { //  write fail
                rwfailcount++;
                if (rwfailcount > 100) {
                    SLOGD("[ReadRoutine] Fail, write fail");
                    break;
                }
                SLOGD("[ReadRoutine] No space to write, sleep");
                usleep(10 * 1000); //10ms
                ...
                if (VInstance->mInRemaining != 0) {
                    SLOGV("[ReadRoutine] last playback in buffer remaining %d", VInstance->mInRemaining);
                    VInstance->mInRemaining -= dataConsumed;
                    VInstance->mOutRemaining +=  datawritten;
                }
                ringBuf_out->SignalBufData();
                SLOGV("[ReadRoutine] advance Read poniter %d ", (uint32_t)dataConsumed);
                rwfailcount = 0;
            }
        }
    }
    ...
    SLOGD("[ReadRoutine] exit ");

    VInstance->ClearState(VPWStreamIn_READ_START);

    SLOGD("stop and signal ReadRefFromRing to stop");
    ringBuf_out->SignalBufData();

    int cnt_val = 50;
    while ((VInstance->mReadFunctionActive == true) && (cnt_val > 0)) {
        SLOGD("[ReadRoutine]Signal ReadRefFromRing to stop and wait (%d) ", cnt_val);
        cnt_val--;
        ringBuf_out->SignalBufData();
        usleep(1000 * 10);
    }
...
AudioVUnlockDL::AudioVUnlockDL() {
    ...
    if (ret != 0) {
        SLOGE("Failed to initialize AudioVUnlockDL mMutex!");
    }

    ret = pthread_mutex_init(&mSRCMutex, NULL);
    if (ret != 0) {
        SLOGE("Failed to initialize AudioVUnlockDL mSRCMutex!");
    }
...
int32_t AudioVUnlockDL::SetUplinkStartTime(struct timespec uplinkStartTime) {
    if (mULtime.tv_sec  == 0 && mULtime.tv_nsec == 0) {
        ...
        SLOGD("[SetUplinkStartTime] mULtime sec %ld nsec %ld", mULtime.tv_sec, mULtime.tv_nsec);
    } else {
        SLOGD("[SetUplinkStartTime] already get UL time :mULtime sec %ld nsec %ld", mULtime.tv_sec, mULtime.tv_nsec);
    }
    return 0;
...
int32_t AudioVUnlockDL:: GetSRCInputParameter(
    ...
    SLOGD("[GetSRCInputParameter] mOutputSampleRate %d,  mInputSampleRate %d, mInChannel %d, mOutChannel, %d ", (int)mOutputSampleRate, (int)inSR, (int)inChannel, (int)mOutChannel);
    ...
    if (!mpSrcHdl) {
        SLOGD("[GetSRCInputParameter] create SRC handle fail");
...
int32_t AudioVUnlockDL:: SetSRC(
    ...
    SLOGD("[SetSRC] mOutputSampleRate %d,  mInputSampleRate %d, mInChannel %d, mOutChannel, %d ", (int)outSR, (int)mInputSampleRate, (int)mInChannel, (int)outChannel);
    pthread_mutex_lock(&mSRCMutex);
    ...
    if (!mpSrcHdl) {
        SLOGD("[SetSRC] create SRC handle fail");
        pthread_mutex_unlock(&mSRCMutex);
        return -1;
    }
...
int32_t AudioVUnlockDL:: DoSRC(uint8_t *inbuf, uint32_t *datasz, uint8_t *outbuf, uint32_t *outlength) {
    ...
    pthread_mutex_lock(&mSRCMutex);
    if (mpSrcHdl == NULL) {
        SLOGD("[DoSRC] SRC not created");
        pthread_mutex_unlock(&mSRCMutex);
        return -1;
    }

    while (count) {
        SLOGV("count %d ,in offset %d, datasz %d, out offset %d, outlength %d ", count, inPtr - inbuf, *datasz, outPtr - outbuf, *outlength);
        ...
        SLOGV("after count %d , datasz %d,  outlength%d ", count,  *datasz, *outlength);
    ...
    {
        SLOGD("[DoSRC] do not finish SRC *datasz %d, &outlength %d", *datasz,*outlength );
        //return -1;
    }*/
...
int32 AudioVUnlockDL::GetFirstDLTime() {
    ...
    if (mGetTime == true) {
        ...
        SLOGD("[GetFirstDLTime] DL time  %d,  %d", mNewDLtime.tv_sec, mNewDLtime.tv_nsec);
        mGetTime = false;
        if (mDLtime.tv_sec != 0) {
            mInRemaining = mRingBufIn.GetBufDataSz();
            mOutRemaining = mRingBufOut.GetBufDataSz();
            SLOGD("[GetFirstDLTime] input buf never cleared IN remaining %d, Out remaining %d", mInRemaining, mOutRemaining);
        }
    }
    return 0;
}
...
int32_t AudioVUnlockDL::SetDownlinkStartTime(int remainMs) {
    ...
    if (mGetTime == true && mInputStandby == false) {
        ...
        SLOGD("[SetDownlinkStartTime] get DL time: mNewDLtime.tv_sec %d, mNewDLtime.tv_nsec %d, remainMs %d", mNewDLtime.tv_sec, mNewDLtime.tv_nsec, remainMs);
        mGetTime = false;
        if (mDLtime.tv_sec != 0) {
            mInRemaining = mRingBufIn.GetBufDataSz();
            mOutRemaining = mRingBufOut.GetBufDataSz();
            SLOGD("[SetDownlinkStartTime] input buf never cleared IN remaining %d, Out remaining %d", mInRemaining, mOutRemaining);
        }
    }
...
int32_t AudioVUnlockDL::WriteStreamOutToRing(const void *buf, uint32_t datasz) {
    ...
    if (buf == NULL || datasz == 0) {
        SLOGD("[WriteStreamOutToRing] input buf and datasz null");
        return -1;
    }
...
#endif
    SLOGD("[WriteStreamOutToRing] datawritten %d", datawritten);
    return datawritten;
}
void AudioVUnlockDL::SetInputStandBy(bool val) {
    mInputStandby = val;
    if (val) {
        SLOGD("[SetInputStandBy] val %d", val);
        int cnt_val = 30;
        mRingBufOut.SignalBufData();
        while ((mReadFunctionActive == true) && (cnt_val > 0) && (mNeedBlock == false)) {
            mRingBufOut.SignalBufData();
            SLOGD("[SetInputStandBy] wait ReadRefFromRing to exit (%d) ", cnt_val);
            cnt_val--;
            usleep(1000 * 3);
        }
        SLOGD("[SetInputStandBy] ReadRefFromRing to exit? (%d) ", mReadFunctionActive);
        mGetTime = true;
    } else {
        SLOGV("[SetInputStandBy] val %d", val);
    }
}
...
int32_t AudioVUnlockDL::ReadRefFromRing(void *buf, uint32_t datasz, void *DLtime) {
    ...
    if (/*mRingBufIn == NULL ||*/ !StateInputStart()) {
        SLOGD("[ReadRefFromRing] AudioVUnlockDL is not initialized");
        return -1;
    }

    if (buf == NULL || datasz == 0 || DLtime == NULL) {
        SLOGD("[ReadRefFromRing] buf and datasz null");
        return -1;
    }
    ...
    struct timespec *ptr;
    if (mNeedBlock == true) {
        SLOGD("[ReadRefFromRing] mNeedBlock %d ", mNeedBlock);
    ...
    while (count) {
        ...
        if (!mInputStandby && StateInputStart()) {
            SLOGV("[ReadRefFromRing] mNeedBlock %d ", mNeedBlock);
            mRingBufOut.WaitBufData();
        }
        ...
        *ptr =  mDLtime;
        ...
        if (mOutRemaining != 0) {
            ...
            SLOGV("[ReadRefFromRing] last playback remaining %d", mOutRemaining);
        }
        bufptr += dataread;
        finaldataout += dataread;
        dataproduced = datasz - finaldataout;
        if (dataproduced == 0) {
            SLOGV("[ReadRefFromRing] output buffer filled. break");
            break;
        }
        if (mInputStandby) {
            if ((mRingBufOut.GetBufDataSz() != 0) && count == 1) {
                count ++;
                SLOGE("[ReadRefFromRing] Playback in standby mode,  but ringbuf still has data, mRingBufOut.GetBufDataSz() %d, mRingBufIn.GetBufDataSz() %d", mRingBufOut.GetBufDataSz(), mRingBufIn.GetBufDataSz());
            }
        }
        count --;
    }
    ...
    SLOGD("[ReadRefFromRing] end finaldataout %d time %d", finaldataout, mDLtime.tv_sec);
...
bool AudioVUnlockDL::startInput() {
    ...
    int ret = 0;
    if (StateInputStart()) {
        SLOGD("[startInput] already start thread");
        return true;
    }

    SLOGD("[startInput] +create AudioVUnlockDL ReadRoutine thread");
    ret = pthread_create(&mReadThread, NULL, ReadRoutine, this);
    SLOGD("[startInput] -create AudioVUnlockDL ReadRoutine thread");
    int cnt_val = 10;
    while ((mReadThreadActive != true) && (cnt_val > 0)) {
        SLOGD("[startInput] wait thread to exit (%d) ", cnt_val);
        cnt_val--;
        usleep(1000 * 10);
    }
    ...
    if (mOutFile == NULL) {
        SLOGD("Fail to Open File %x ", mOutFile);
    }
    ...
    if (mOutFile_1 == NULL) {
        SLOGD("Fail to Open File %x ", mOutFile_1);
    }
    ...
    if (mOutFile_2 == NULL) {
        SLOGD("Fail to Open File %x ", mOutFile_2);
    }
    mSampleCount_Dump = 0;
#endif
    return true;
...
bool AudioVUnlockDL::stopInput() {
    ...
    if (!StateInputStart()) {
        SLOGD("[stopInput] mState != VPWStreamIn_READ_START mState = %d", mState);
        return false;
    }
    ...
    while ((mReadThreadActive == true) && (cnt_val > 0)) {
        SLOGD("[stopInput] wait thread to exit (%d) ", cnt_val);
        cnt_val--;
        usleep(1000 * 50);
    }
    mULtime.tv_sec = 0;
    ...
    if (cnt_val <= 0) {
        SLOGD("[stopInput] mReadThreadActive:%d, cnt_val:%d ", mReadThreadActive, cnt_val);
    }
#ifdef DUMP_VPW_StreamIn_DATA
    if (mOutFile != NULL) {
        SLOGD("[stopInput]  Close dump file");
        fclose(mOutFile);
        mOutFile = NULL;
    }
    ...
    if (mReadThreadActive) {
        SLOGD("[stopInput]  mReadThreadActive is false,  stop fail");
        return false;
    }
...
int32_t  AudioVUnlockDL::GetVoiceUnlockULTime(void *ULtime) {
    ...
    *ptr = mULtime ;
    SLOGD("ULtime->tv_sec %d, ULtime->tv_nsec %d", ptr->tv_sec, ptr->tv_nsec);
    return 0;
}
...
int32_t  AudioVUnlockDL::GetLatency() {
    ...
    if (/*mRingBufOut == NULL || mRingBufIn == NULL ||*/ !StateInputStart()) {
        SLOGD("[WriteStreamOutToRing] AudioVUnlockDL is not initialized");
        return -1;
    }
    Inlatency = mRingBufIn.GetBufDataSz();
    SLOGD("[GetLatency]  mRingBufIn buffer data %d bytes", Inlatency);
    Outlatency = mRingBufOut.GetBufDataSz();

    SLOGD("[GetLatency]  mRingBufOut data %d bytes", Outlatency);
...
void AudioVUnlockDL::ClearState(uint32 state) {
    SLOGD("clear AudioVUnlockDL state = %d", state);
    mState &= (~state);
    SLOGD("clear AudioVUnlockDL mState = %d", mState);
}

void AudioVUnlockDL::SetState(uint32 state) {
    SLOGD("Set AudioVUnlockDL state = %d", state);
    mState |= state;
    SLOGD("Set AudioVUnlockDL mState = %d", mState);
}
...
void AudioVUnlockDL:: freeInstance() {
    SLOGV("+delete UniqueVUnlockDLInstance");
...
void *WriteRoutine(void *hdl) {
    ...
    if (OutFile == NULL) {
        SLOGD("Fail to Open File %x ", OutFile);
    }
#if MTK_USE_RT_PRIORITY
...
#ifdef DUMP_VPW_StreamIn_DATA
            if (OutFile != NULL) {
                SLOGV("[DumpData]  Dump file");
                fwrite(buf, sizeof(char), dataSRC, OutFile);
            }
#endif
        } else if (dataSRC == -1) {
            SLOGD("[write routine] dataSRC %d, exit", dataSRC);
            break;
        }
        //latency = VInstance->GetLatency();
        //SXLOGD("[write routine] latency %d ms", latency);
        SLOGD("[write routine] dataSRC %d, sleep", dataSRC);
        usleep(10 * 1000);
    }
...
bool AudioVUnlockDL::startWrite() {
    ...
    SLOGD("[startwrite] +create AudioVUnlockDL WriteRoutine thread");
...
bool AudioVUnlockDL::stopWrite() {
    ...
    if (!StateStartwrite()) {
        SLOGD("[stopWrite] mState != VPWStreamIn_READ_START mState = %d", mState);
        return false;
    }
    ...
    while ((mWriteThreadActive == true) && (cnt_val > 0)) {
        SLOGD("[stopWrite] wait thread to exit (%d) ", cnt_val);
        cnt_val--;
        usleep(1000 * 50);
    }

    if (cnt_val <= 0) {
        SLOGD("[stopWrite] mWriteThreadActive:%d, cnt_val:%d ", mWriteThreadActive, cnt_val);
    }


    if (mReadThreadActive) {
        SLOGD("[stopWrite]  mWriteThreadActive is false,  stop fail");
        return false;
    }
    // stop read in from Stream out
    //delete ring buffer
