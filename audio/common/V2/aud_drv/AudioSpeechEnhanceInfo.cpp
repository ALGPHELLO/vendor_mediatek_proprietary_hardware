...
AudioSpeechEnhanceInfo::AudioSpeechEnhanceInfo() {
    ...
    ALOGD("mVoIPSpeechEnhancementMask = 0x%x", mVoIPSpeechEnhancementMask.dynamic_func);
...
void AudioSpeechEnhanceInfo::GetHDRecVMFileName(char *VMFileName, size_t string_size) {
    Mutex::Autolock lock(mHDRInfoLock);
...
void AudioSpeechEnhanceInfo::GetPreLoadBesRecordSceneTable(AUDIO_HD_RECORD_SCENE_TABLE_STRUCT *pPara) {
    ALOGD("%s+", __FUNCTION__);
    Mutex::Autolock lock(mHDRInfoLock);
    memcpy(pPara, &mPreLoadBesRecordSceneTable, sizeof(mPreLoadBesRecordSceneTable));
    ALOGD("%s-", __FUNCTION__);
}
void AudioSpeechEnhanceInfo::GetPreLoadBesRecordParam(AUDIO_HD_RECORD_PARAM_STRUCT *pPara) {
    ALOGD("%s+", __FUNCTION__);
    Mutex::Autolock lock(mHDRInfoLock);
    memcpy(pPara, &mPreLoadBesRecordParam, sizeof(mPreLoadBesRecordParam));
    ALOGD("%s-", __FUNCTION__);
}
void AudioSpeechEnhanceInfo::GetPreLoadAudioVoIPParam(AUDIO_VOIP_PARAM_STRUCT *pPara) {
    ALOGD("%s+", __FUNCTION__);
    Mutex::Autolock lock(mHDRInfoLock);
    memcpy(pPara, &mPreLoadVOIPParam, sizeof(mPreLoadVOIPParam));
    ALOGD("%s-", __FUNCTION__);
}
