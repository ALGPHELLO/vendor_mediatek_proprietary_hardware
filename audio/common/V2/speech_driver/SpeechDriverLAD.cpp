...
int SpeechDriverLAD::RttConfig(int rttMode) {
    ALOGW("%s(), rttMode = %d", __FUNCTION__, rttMode);
    return INVALID_OPERATION;
}
...
{
    ALOGD("%s(), enableLpbk = %d not support", __FUNCTION__, enableLpbk);
    return NO_ERROR;
}
 ...
 *                     Speech Enhancement Parameters
 *============================================================================*/
status_t SpeechDriverLAD::SetDynamicSpeechParameters(const int type __unused, const void *param_arg __unused) {
    return INVALID_OPERATION;
}
