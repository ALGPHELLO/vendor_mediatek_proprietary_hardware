...
#undef LOG_TAG
#endif
#define LOG_TAG "aurisys_utility"
...
typedef void (*match_mask_by_string_fp_t)(uint32_t *p_mask, const char *string, const uint32_t string_size);
...
static const string_to_enum_pair_t g_s2e_aurisys_scenario[] = {
    STRING_TO_ENUM("AURISYS_SCENARIO_PLAYBACK_NORMAL", AURISYS_SCENARIO_PLAYBACK_NORMAL),
    STRING_TO_ENUM("AURISYS_SCENARIO_PLAYBACK_LOW_LATENCY", AURISYS_SCENARIO_PLAYBACK_LOW_LATENCY),
    STRING_TO_ENUM("AURISYS_SCENARIO_RECORD_LOW_LATENCY", AURISYS_SCENARIO_RECORD_LOW_LATENCY),
    STRING_TO_ENUM("AURISYS_SCENARIO_RECORD_WITHOUT_AEC", AURISYS_SCENARIO_RECORD_WITHOUT_AEC),
    STRING_TO_ENUM("AURISYS_SCENARIO_RECORD_WITH_AEC", AURISYS_SCENARIO_RECORD_WITH_AEC),
    STRING_TO_ENUM("AURISYS_SCENARIO_VOIP", AURISYS_SCENARIO_VOIP),
    STRING_TO_ENUM("AURISYS_SCENARIO_VOIP_WITHOUT_AEC", AURISYS_SCENARIO_VOIP_WITHOUT_AEC),
    STRING_TO_ENUM("AURISYS_SCENARIO_PHONE_CALL", AURISYS_SCENARIO_PHONE_CALL),
    STRING_TO_ENUM("AURISYS_SCENARIO_ALL", AURISYS_SCENARIO_ALL),
};
...
const char* get_string_by_enum_aurisys_scenario(uint32_t enum_value) {
    return get_string_by_enum(g_s2e_aurisys_scenario,
                              ARRAY_SIZE(g_s2e_aurisys_scenario),
                              enum_value);
}

uint32_t get_enum_by_string_aurisys_scenario(const char *string) {
    return get_enum_by_string(g_s2e_aurisys_scenario,
                              ARRAY_SIZE(g_s2e_aurisys_scenario),
                              string);
}
...
void dump_task_config(const struct arsi_task_config_t *p_arsi_task_config) {
    ...
    AUD_LOG_D("input dev: 0x%x, fmt = 0x%x, fs: %u, max fs: %u, ch: %d, max ch: %d, ch maks: 0x%x, hw_info_mask: 0x%x"
              ...
              "; task_scene: %d, audio_mode: %d, output_flags: 0x%x, input_source: %u, input_flags: 0x%x, enhancement_feature_mask: 0x%x",
...
void dump_lib_config(const struct arsi_lib_config_t *p_arsi_lib_config) {
    ...
    if (p_arsi_lib_config->p_ul_buf_in && p_arsi_lib_config->p_ul_buf_out) {
        AUD_LOG_D("ul in[type:%d], ch: %d, ch_mask: 0x%x, buf fs: %u, read data fs: %u, fmt: 0x%x"
                  "; ul out[type:%d], ch: %d, ch_mask: 0x%x, buf fs: %u, read data fs: %u, fmt: 0x%x",
    ...
    if (p_arsi_lib_config->p_ul_ref_bufs) {
        for (i = 0; i < p_arsi_lib_config->num_ul_ref_buf_array; i++) {
            AUD_LOG_D("ul ref[type:%d], ch: %d, ch_mask: 0x%x, buf fs: %u, read data fs: %u, fmt: 0x%x",
    ...
    if (p_arsi_lib_config->p_dl_buf_in && p_arsi_lib_config->p_dl_buf_out) {
        AUD_LOG_D("dl in[type:%d], ch: %d, ch_mask: 0x%x, buf fs: %u, read data fs: %u, fmt: 0x%x"
                  "; dl out[type:%d], ch: %d, ch_mask: 0x%x, buf fs: %u, read data fs: %u, fmt: 0x%x",
    ...
    if (p_arsi_lib_config->p_dl_ref_bufs) {
        for (i = 0; i < p_arsi_lib_config->num_dl_ref_buf_array; i++) {
            AUD_LOG_D("dl ref[type:%d], ch: %d, ch_mask: 0x%x, buf fs: %u, read data fs: %u, fmt: 0x%x",
...
static void match_support_format_mask(uint32_t *p_mask, const char *format, const uint32_t string_size)
{
...
static void match_support_frame_ms_mask(uint32_t *p_mask, const char *frame_ms, const uint32_t string_size __unused)
{
...
static void match_support_channel_number_mask(uint32_t *p_mask, const char *channel_number, const uint32_t string_size __unused)
{
...
uint32_t parse_segment_in_string(const char *string, match_mask_by_string_fp_t match_func)
{
    uint32_t mask = 0;
...
uint32_t get_dedicated_format_from_mask(const uint32_t masks, const uint32_t the_audio_format)
{
    uint32_t audio_format = 0;
    uint32_t the_mask = 0;
...
uint8_t get_dedicated_frame_ms_from_mask(const uint32_t masks, const uint8_t the_frame_ms)
{
    uint32_t frame_ms = 0;
    uint32_t the_mask = 0;
...
uint8_t get_dedicated_channel_number_from_mask(const uint32_t masks, const uint8_t the_channel_number)
{
