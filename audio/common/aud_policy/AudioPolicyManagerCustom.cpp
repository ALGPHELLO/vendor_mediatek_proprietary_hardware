...
float AudioPolicyManagerCustom::mapVoltoCustomVol(unsigned char array[], int volmin, int volmax, float &vol, int stream) {
    ...
    if (vol == 0) {
        ...
        if (vol < fCUSTOM_VOLUME_MAPPING_STEP / mVolumeCurves->getVolumeIndexMax((audio_stream_type_t) stream)) {
            volume = array[0];
            vol = volume;
            return volume;
        }
...
float AudioPolicyManagerCustom::computeCustomVolume(int stream, int index, audio_devices_t device) {
    ...
    float volInt = (fCUSTOM_VOLUME_MAPPING_STEP * (index - mVolumeCurves->getVolumeIndexMin((audio_stream_type_t)stream)))
                   / (mVolumeCurves->getVolumeIndexMax((audio_stream_type_t)stream) - mVolumeCurves->getVolumeIndexMin((audio_stream_type_t)stream));
    ...
    MTK_ALOGVV("stream = %d after computeCustomVolume , volInt = %f volume = %f", stream, volInt, volume);
    return volume;
}
