...
int32_t SpeechMessengerDSDA::SendMessage(const ccci_buff_t &ccci_buff) {
    ...
    if (message_id == MSG_A2M_EM_NB ||
        ...
        message_id == MSG_A2M_SMARTPA_PARAMETER) {
    ...
    while (pCCCI->CCCIEnable) {
        ...
        case MSG_M2A_PCM_REC_DATA_NOTIFY: { // meaning that we are recording, modem have some data
            ...
            if (pCCCI->mLad->GetApSideModemStatus(RECORD_STATUS_MASK) == false) {
                ...
                SLOGV("MSG_M2A_VM_REC_DATA_NOTIFY(0x%x), data_length: %zd", ccci_buff.message, pCCCI->GetMessageLength(ccci_buff) - CCCI_SHARE_BUFF_HEADER_LEN);
        ...
        case MSG_M2A_PNW_UL_DATA_NOTIFY: { // Get Microphone data from Modem
            ...
            if (pCCCI->mLad->GetApSideModemStatus(P2W_STATUS_MASK) == false) {
                ...
                SLOGV("MSG_M2A_PNW_UL_DATA_NOTIFY(0x%x), data_length: %zd", ccci_buff.message, pCCCI->GetMessageLength(ccci_buff) - CCCI_SHARE_BUFF_HEADER_LEN);
        ...
        case MSG_M2A_CTM_DEBUG_DATA_NOTIFY: {
            if (pCCCI->mLad->GetApSideModemStatus(TTY_STATUS_MASK) == false) {
                ...
                SLOGV("MSG_M2A_CTM_DEBUG_DATA_NOTIFY(0x%x), data_length: %zd", ccci_buff.message, pCCCI->GetMessageLength(ccci_buff) - CCCI_SHARE_BUFF_HEADER_LEN);
...
status_t SpeechMessengerDSDA::SetPcmRecordType(record_type_t type_record __unused) {
    ALOGD("%s(), Not Support", __FUNCTION__);
    return NO_ERROR;
}

#ifdef USE_CCCI_SHARE_BUFFER
void SpeechMessengerDSDA::InitA2MRawParaRingBuffer() {
    return ;
}
status_t SpeechMessengerDSDA::GetA2MRawParaRingBuffer(uint16_t *offset, uint16_t *avail) {
    return NO_ERROR;
}

status_t SpeechMessengerDSDA::AdvanceA2MRawParaRingBuffer(int datalength) {
    return NO_ERROR;
}

status_t SpeechMessengerDSDA::WriteA2MRawParaRingBuffer(char *data, int datalength) {
    return NO_ERROR;
}
#endif
