...
#undef LOG_TAG
#define LOG_TAG "SpeechMessengerVendEVDO"


#define ENABLE_XLOG_SPEECH_VendEVDO_MSG
#ifdef ENABLE_XLOG_SPEECH_VendEVDO_MSG
#include <log/log.h>
#define SPEECH_MSG_VEB(fmt, arg...)  SLOGV(fmt, ##arg)
#define SPEECH_MSG_DBG(fmt, arg...)  SLOGD(fmt, ##arg)
#define SPEECH_MSG_INFO(fmt, arg...) SLOGI(fmt, ##arg)
#define SPEECH_MSG_WARN(fmt, arg...) SLOGW(fmt, ##arg)
#define SPEECH_MSG_ERR(fmt, arg...)  SLOGE("Err: %5d:, "fmt, __LINE__, ##arg)
#else
#include <utils/Log.h>
...
//constructor
SpeechMessengerVendEVDO::SpeechMessengerVendEVDO() {
...
status_t SpeechMessengerVendEVDO::Initial() {
    SPEECH_MSG_DBG("%s", __FUNCTION__);
...
status_t SpeechMessengerVendEVDO::DeInitial() {
    SPEECH_MSG_DBG("%s", __FUNCTION__);
    if (mEVDOEnable) {
        mEVDOEnable = false;
...
// static
void *SpeechMessengerVendEVDO::ReadATCmdThread(void *me) {
    SPEECH_MSG_DBG("%s", __FUNCTION__);
    prctl(PR_SET_NAME, (unsigned long)__FUNCTION__, 0, 0, 0);
    return (void *) static_cast<SpeechMessengerVendEVDO *>(me)->ReadATCmdLooper();
}


status_t SpeechMessengerVendEVDO::InitATCmdQueue() {
    SPEECH_MSG_DBG("+%s", __FUNCTION__);
...
status_t SpeechMessengerVendEVDO::PostATCmdItem(int type, int arg1, int arg2) {
    atcmd_req_t cmd;
    cmd.type = type;
    cmd.arg1 = arg1;
    cmd.arg2 = arg2;
...
status_t SpeechMessengerVendEVDO::PostATCmd(atcmd_req_t *cmd) {
...
status_t SpeechMessengerVendEVDO::GetCurrentATCmd(atcmd_req_t *cmd) {
    SPEECH_MSG_DBG("%s", __FUNCTION__);
...
status_t SpeechMessengerVendEVDO::ReadATCmdLooper() {
    SPEECH_MSG_DBG("%s", __FUNCTION__);
...
int SpeechMessengerVendEVDO::SetCPAudioVoiceRecord(int on) {
    SPEECH_MSG_DBG("%s", __FUNCTION__);
    return mClientPtr->RPC_send(REQ_VOICE_RECORD, on);
}

int SpeechMessengerVendEVDO::SetCPAudioPlayDtmf(int mode, int dtmf) {
...
int SpeechMessengerVendEVDO::SetCPAudioPlayTone(int toneId) {
...
bool SpeechMessengerVendEVDO::Spc_SetAudioMode(int cdma_audio_mode) {
    SPEECH_MSG_DBG("%s(cdma_audio_mode=%d)", __FUNCTION__, cdma_audio_mode);
    return PostATCmdItem(REQ_AUDIO_MODE, cdma_audio_mode);
}

bool SpeechMessengerVendEVDO::Spc_SetOutputVolume(uint32_t Gain) {
    SPEECH_MSG_DBG("%s(%d)", __FUNCTION__, Gain);
    return PostATCmdItem(REQ_AUDIO_VOLUME, Gain);
}


int SpeechMessengerVendEVDO::Spc_Default_Tone_Play(uint8_t toneIdx) {
...
int SpeechMessengerVendEVDO::Spc_Default_Tone_Stop(void) {
    SPEECH_MSG_DBG("%s", __FUNCTION__);
    return PostATCmdItem(REQ_PLAY_DTMF);
}

bool SpeechMessengerVendEVDO::Spc_MuteMicrophone(uint8_t enable) {
    SPEECH_MSG_DBG("%s", __FUNCTION__);
    return PostATCmdItem(REQ_MUTE, enable);
}


status_t SpeechMessengerVendEVDO::Spc_OpenNormalRecPath(uint32_t sampleRate) {
    ...
    void *dlHandle;
    dlHandle = dlopen("libviatelutils.so", RTLD_NOW);
    if (dlHandle == NULL) {
...
status_t SpeechMessengerVendEVDO::Spc_CloseNormalRecPath() {
    SPEECH_MSG_DBG("%s", __FUNCTION__);
    int ret = 0;
#ifdef RECORD_FAKE
    mRecHdl = -1;
...
int  SpeechMessengerVendEVDO::Spc_ReadRNormalRecData(uint16_t *pBuf, uint16_t u2Size) {
    SPEECH_MSG_DBG("%s", __FUNCTION__);
...
status_t SpeechMessengerVendEVDO::OpenRecDevice(char *deviceName) {
...
void SpeechMessengerVendEVDO::CloseRecDevice(void) {
    SPEECH_MSG_DBG("%s", __FUNCTION__);
    if (mRecHdl >= 0) {
        close(mRecHdl);
    }
...
void *SpeechMessengerVendEVDO::RecordVoiceThread(void *me) {
    SPEECH_MSG_DBG("%s", __FUNCTION__);
    prctl(PR_SET_NAME, (unsigned long)__FUNCTION__, 0, 0, 0);
    return (void *) static_cast<SpeechMessengerVendEVDO *>(me)->RecordVoiceLooper();
}


status_t SpeechMessengerVendEVDO::RecordVoiceLooper() {
    SPEECH_MSG_DBG("%s", __FUNCTION__);
        ...
        if (result > 0) {
            if (FD_ISSET(mRecHdl, &readfd)) {
                ...
                if (n > 0) {
                    ...
                    AudioALSACaptureDataProviderVoice::getInstance()->provideModemRecordDataToProvider(ul_ring_buf);
                }
            }
        } else if (result < 0) {
            // error
...
void SpeechMessengerVendEVDO::EnableDualMic(bool enable) {
    PostATCmdItem(REQ_AUDIO_MODE, enable ? 32 : 33);
}

void *SpeechMessengerVendEVDO::Record_main_fake(void *me) {
    SPEECH_MSG_DBG("%s", __FUNCTION__);
    prctl(PR_SET_NAME, (unsigned long)__FUNCTION__, 0, 0, 0);
    return (void *) static_cast<SpeechMessengerVendEVDO *>(me)->Record_fake();
}

status_t SpeechMessengerVendEVDO::Record_fake() {
...
bool SpeechMessengerVendEVDO::Spc_SetMicrophoneVolume(uint8_t mic_volume) {
    SPEECH_MSG_DBG("%s(mic_volume=%d)", __FUNCTION__, mic_volume);
    return true;
}
bool SpeechMessengerVendEVDO::Spc_SetOutputDevice(uint8_t device) {
    SPEECH_MSG_DBG("%s(device=%d)", __FUNCTION__, device);
    return true;
}

bool SpeechMessengerVendEVDO::Spc_SetInputSource(uint8_t src) {
    SPEECH_MSG_DBG("%s(src=%d)", __FUNCTION__, src);
    return true;
}

bool SpeechMessengerVendEVDO::Spc_SetSidetoneVolume(uint8_t sidetone_volume) {
    SPEECH_MSG_DBG("%s(sidetone_volume=%d)", __FUNCTION__, sidetone_volume);
    return true;
}
bool SpeechMessengerVendEVDO::Spc_Speech_On(phone_call_mode_t mode) {
    SPEECH_MSG_DBG("%s(phone_call_mode_t=%d)", __FUNCTION__, mode);
    return true;
}

bool SpeechMessengerVendEVDO::Spc_Speech_Off(void) {
    SPEECH_MSG_DBG("%s", __FUNCTION__);
    return true;
}

void SpeechMessengerVendEVDO::RecordDump(uint8_t *pBuf, int len) {
...
int SpeechMessengerVendEVDO::OpenDevice(char *deviceName) {
