...
status_t SpeechMessengerCCCI::ReadMessage(ccci_buff_t &ccci_buff) {
    ...
    if (read_length != CCCI_BUF_HEADER_SIZE && GetModemCurrentStatus() != MODEM_STATUS_READY) {
        ...
        ALOGV("%s(), read_length=%d, data[0](magic)=0x%x, data[1](message)=0x%x, ch=0x%x, reserved=0x%x", __FUNCTION__, read_length, ccci_buff.magic, ccci_buff.message, ccci_buff.channel, ccci_buff.reserved);
    }
    return NO_ERROR;
}
...
bool SpeechMessengerCCCI::CheckOffsetAndLength(const ccci_buff_t &ccci_buff) {
    ...
    ALOGV("%s(), CheckOffsetAndLength message_id 0x%x , length 0x%x, offset 0x%x", __FUNCTION__, message_id, length, offset);
    ...
    while (pCCCI->CCCIEnable) {
        ...
        case MSG_M2A_PCM_REC_DATA_NOTIFY: { // meaning that we are recording, modem have some data
            ...
            if (pCCCI->mLad->GetApSideModemStatus(RECORD_STATUS_MASK) == false) {
                ...
                ALOGV("MSG_M2A_REC_DATA_NOTIFY(0x%x), data_length: %d", ccci_buff.message, pCCCI->GetMessageLength(ccci_buff) - CCCI_SHARE_BUFF_HEADER_LEN);
        ...
        case MSG_M2A_VM_REC_DATA_NOTIFY: { // meaning that we are recording, modem have some data
            ...
            if (pCCCI->mLad->GetApSideModemStatus(VM_RECORD_STATUS_MASK) == false) {
                ...
                ALOGV("MSG_M2A_VM_REC_DATA_NOTIFY(0x%x), data_length: %d", ccci_buff.message, pCCCI->GetMessageLength(ccci_buff) - CCCI_SHARE_BUFF_HEADER_LEN);
        ...
        case MSG_M2A_BGSND_DATA_REQUEST: { // modem request bgs data to play
            ...
            AL_LOCK(pBGSPlayer->mBGSMutex); // avoid AudioMTKStreamOut::standby() destroy BGSPlayBuffer during MSG_M2A_BGSND_DATA_REQUEST
        ...
        case MSG_M2A_PNW_UL_DATA_NOTIFY: { // Get Microphone data from Modem
            ...
            if (pCCCI->mLad->GetApSideModemStatus(P2W_STATUS_MASK) == false) {
                ...
                ALOGD("MSG_M2A_PNW_UL_DATA_NOTIFY(0x%x), data_length:%d", ccci_buff.message, pCCCI->GetMessageLength(ccci_buff) - CCCI_SHARE_BUFF_HEADER_LEN);
...
RingBuf SpeechMessengerCCCI::GetM2AUplinkRingBuffer(const ccci_buff_t &ccci_buff) {
    ...
    if (*(uint16_t *)p_sync_word != CCCI_M2A_SHARE_BUFF_HEADER_SYNC) {
        ...
        snprintf(SphDumpStr, SPH_DUMP_STR_SIZE, "ring_buf[pBase %p, pEnd %p, Len 0x%x, pRead %p, offset 0x%x]= ", ul_ring_buf.pBufBase, mM2AShareBufEnd, mM2AShareBufLen, ul_ring_buf.pRead, ccci_buff.message);
        for (u4I = 0; u4I < idxDump; u4I++) {
            char SphDumpTemp[100];
            snprintf(SphDumpTemp, 100, "[%d, %p]0x%x, ", u4I, (uint16_t *)ul_ring_buf.pRead + u4I, *((uint16_t *)ul_ring_buf.pRead + u4I));
...
RingBuf SpeechMessengerCCCI::GetM2ARawPcmRingBuffer(const ccci_buff_t &ccci_buff) {
    ALOGV("%s(), id(0x%x), mM2AShareBufLen(0x%x), A2M_SHARED_BUFFER_BGS_DATA_SIZE(%d)", __FUNCTION__, GetMessageID(ccci_buff), mM2AShareBufLen, A2M_SHARED_BUFFER_BGS_DATA_SIZE);
    ...
    ALOGV("%s(), before memcpy mM2AShareBufEnd(%p), BytesCopied(0x%x), p_data_len(0x%x), pRead(%p), pWrite(%p)",
          __FUNCTION__, mM2AShareBufEnd, BytesCopied, (*(uint16_t *)p_data_len), ul_ring_buf.pRead, ul_ring_buf.pWrite);
    ...
    if (pcm_ring_buf.pWrite >= mSphPCMBufEnd) {
        ...
        ALOGV("%s(), mSphPCMBufEnd(%p), BytesCopied(0x%x), size_copy1(0x%x), size_copy2(0x%x), pRead(%p), pWrite(%p)",
              __FUNCTION__, mSphPCMBufEnd, BytesCopied, size_copy1, size_copy2, pcm_ring_buf.pRead, pcm_ring_buf.pWrite);
    ...
    ALOGV("-%s(), pBufBase(%p), bufLen(0x%x), BytesCopied(0x%x), pRead(%p), pWrite(%p)",
          __FUNCTION__, pcm_ring_buf.pBufBase, pcm_ring_buf.bufLen, BytesCopied, pcm_ring_buf.pRead, pcm_ring_buf.pWrite);
