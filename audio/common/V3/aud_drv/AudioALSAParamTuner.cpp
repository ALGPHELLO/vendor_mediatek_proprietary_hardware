...
static void *DMNR_Play_Rec_Routine(void *arg) {
    ...
    ALOGD("%s(), pthread_cond_signal(&pDMNRTuning->mDMNRExit_Cond)", __FUNCTION__);
    ...
    ALOGD("%s(), pthread_mutex_lock", __FUNCTION__);
    pthread_mutex_lock(&pDMNRTuning->mDMNRMutex);
    ALOGD("%s(), pthread_cond_signal(&mDMNRExit_Cond)", __FUNCTION__);
    pthread_cond_signal(&pDMNRTuning->mDMNRExit_Cond); // wake all thread
    pthread_mutex_unlock(&pDMNRTuning->mDMNRMutex);
...
AudioALSAParamTuner::AudioALSAParamTuner() :
...
#endif
    {
...
#ifdef MTK_NEW_VOL_CONTROL
        mDualMicTool_micGain[0] = AudioMTKGainController::getInstance()->getMicGainDecimal(Handfree_Mic, GAIN_DEVICE_SPEAKER);
#else
        ...
        if (ret != 0) {
            ALOGE("%s(), Failed to initialize mDMNRMutex!", __FUNCTION__);
        }
        ...
        if (ret != 0) {
            ALOGE("%s(), Failed to initialize mDMNRExit_Cond!", __FUNCTION__);
        }
...
status_t AudioALSAParamTuner::enableDMNRAtApSide(bool bEnable, bool bWBMode, unsigned short outputDevice, unsigned short workMode) {
    ...
    if (bEnable && (isPlaying() == false)) {
        ...
        ALOGD("%s(), +mDMNRExit_Cond wait", __FUNCTION__);
        ret = pthread_cond_timedwait(&mDMNRExit_Cond, &mDMNRMutex, &timeout);
        ALOGD("%s(), -mDMNRExit_Cond receive ret=%d", __FUNCTION__, ret);
