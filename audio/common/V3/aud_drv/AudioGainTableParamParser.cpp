            ...
            for (int device = 0; device < NUM_GAIN_DEVICE; device++) {
                ...
                for (int i = 0; i < arraySize + 1; i++) {
                    ...
                    if (mMapDlAnalogType[device] < 0 || mMapDlAnalogType[device] >= NUM_GAIN_ANA_TYPE) {
                        if (i == 0) {
                            ALOGW("\tcontinue, paramPath = %s, mMapDlAnalogType[%d] = %d", paramPath.c_str(), device, mMapDlAnalogType[device]);
                        }
                        continue;
                    }
...
status_t GainTableParamParser::loadGainTableSpec() {
    ...
    if (AudioSmartPaController::getInstance()->isSmartPAUsed()) {
        ...
        unsigned int spkType = AudioALSAHardwareResourceManager::getInstance()->getNonSmartPAType();
        switch (spkType) {
        case INTAMP:
