...
status_t AudioPreProcess::CheckNativeEffect(void) {
    if (mStreamInAttribute->NativePreprocess_Info.PreProcessEffect_Update == true) {
        ...
        ALOGD("PreProcessEffect_Count %d", mStreamInAttribute->NativePreprocess_Info.PreProcessEffect_Count);
...
struct echo_reference_itfe *AudioPreProcess::start_echo_reference(audio_format_t format __unused,
    ...
    ALOGD("%s()+ channel_count=%d,sampling_rate=%d,mEcho_Reference=%p", __FUNCTION__, channel_count, sampling_rate, mEcho_Reference);
    //AL_AUTOLOCK(mLock);
...
#if 0
        pEchoRefFile = fopen("/sdcard/mtklog/NativeEchoRef.pcm", "wb");
        if (pEchoRefFile == NULL) {
