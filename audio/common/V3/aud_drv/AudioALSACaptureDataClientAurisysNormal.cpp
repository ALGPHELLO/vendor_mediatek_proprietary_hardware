...
static const uint32_t kRawDataBufferSize = 0x40000; // 256k

static const uint32_t kProcessedBufferSize = 0x40000; // 256k
...
AudioALSACaptureDataClientAurisysNormal::AudioALSACaptureDataClientAurisysNormal(
    ...
    if (IsAECEnable()) {
        AUDIO_ALLOC_CHAR_BUFFER(mEchoRefDataBuf.base, kRawDataBufferSize);
        mEchoRefDataBuf.read = mEchoRefDataBuf.base;
        mEchoRefDataBuf.write = mEchoRefDataBuf.base;
        mEchoRefDataBuf.size = kRawDataBufferSize;
    }
    ...
    if (IsAECEnable()) { mCaptureDataProviderEchoRef->attach(this); }
    ...
    CheckNativeEffect();
    AUDIO_ALLOC_CHAR_BUFFER(mNativeEffectBuffer, kRawDataBufferSize);
    //Android Native Preprocess effect ---
...
AudioALSACaptureDataClientAurisysNormal::~AudioALSACaptureDataClientAurisysNormal() {
    ...
    AUDIO_FREE_POINTER(mNativeEffectBuffer);
    //Android Native Preprocess effect ---
...
void *AudioALSACaptureDataClientAurisysNormal::processThread(void *arg) {
    ...
    effect_buf = client->mNativeEffectBuffer;
    ...
    while (client->mEnable == true) {
        ...
        // copy data
        AUDIO_RINGBUF_COPY_FROM_RINGBUF_ALL(&ul_in->ringbuf, raw_ul);
        if (client->IsAECEnable()) {
            AUDIO_RINGBUF_COPY_FROM_RINGBUF_ALL(&ul_aec->ringbuf, raw_aec);
        }
        frame_idx++;
        LATENCY_LOG("S-");
        AL_UNLOCK(client->mRawDataBufLock);
        ...
        if (data_count > kRawDataBufferSize) {
            ALOGW("%s(), Ul Out data_count %u > native effect kRawDataBufferSize %u",
                  __FUNCTION__, data_count, kRawDataBufferSize);
            data_count = kRawDataBufferSize;
        }
        audio_ringbuf_copy_to_linear(effect_buf,
                                     &ul_out->ringbuf,
                                     data_count);
        ...
        ALOGV("data_count %u, free_count %d, %dL", data_count, free_count, __LINE__);
        if (data_count > free_count) {
            AUD_LOG_W("%s(), native effect data_count %u > processed buf free_count %u",
                      __FUNCTION__, data_count, free_count);
    ...
    if (data_buf_type == DATA_BUF_UPLINK_IN) {
        ...
        if (isTimeStampStarted(&mEchoRefDataBufTimeStamp) == false) { // echo ref data not ready
            ...
            if (t1_older_then_t2(&mEchoRefDataBufTimeStamp, &mRawDataBufTimeStamp)) { // ul new
                ...
                free_count = audio_ringbuf_free_space(&mRawDataBuf);
                if (free_count < diff_buf_size) {
                    ALOGW("%s(), free_count %u < diff_buf_size %u", __FUNCTION__, free_count, diff_buf_size);
                    WARNING("free_count < diff_buf_size!!");
                } else {
                ...
                free_count = audio_ringbuf_free_space(&mEchoRefDataBuf);
                if (free_count < diff_buf_size) {
                    ALOGW("%s(), free_count %u < diff_buf_size %u", __FUNCTION__, free_count, diff_buf_size);
                    WARNING("free_count < diff_buf_size!!");
                } else {
...
void AudioALSACaptureDataClientAurisysNormal::CreateAurisysLibManager() {
    ...
    prefer_configs.sample_rate = sample_rate;
    prefer_configs.frame_size_ms = (uint8_t)mLatency;
    prefer_configs.num_channels_ul = mStreamAttributeSource->num_channels;
    prefer_configs.num_channels_dl = 2;
    ...
    int16_t ulDigitalGain = mAudioALSAVolumeController->GetSWMICGain();
    int16_t ulAnalogGain = (mAudioALSAVolumeController->GetULTotalGain() - 192) / 4 + 34 - ulDigitalGain; // (TotalGain - 192) / 4 + 34 - DigitalGain (unit: 0.25 db)
...
void AudioALSACaptureDataClientAurisysNormal::InitArsiTaskConfig(struct aurisys_lib_manager_t *manager) {
    arsi_task_config_t *arsi_task_config = get_arsi_task_config(manager);


    /* input device */
    arsi_task_config->input_device_info.devices = mStreamAttributeTarget->input_device; /* TODO: HAL capture */
    arsi_task_config->input_device_info.audio_format = mStreamAttributeSource->audio_format;
    arsi_task_config->input_device_info.sample_rate = mStreamAttributeSource->sample_rate;
    arsi_task_config->input_device_info.channel_mask = mStreamAttributeSource->audio_channel_mask; /* TODO */
    ...
    arsi_task_config->output_device_info.devices = mStreamAttributeTarget->output_devices;
    arsi_task_config->output_device_info.audio_format = AUDIO_FORMAT_DEFAULT;
    arsi_task_config->output_device_info.sample_rate = 0;
    arsi_task_config->output_device_info.channel_mask = AUDIO_CHANNEL_NONE;
    arsi_task_config->output_device_info.num_channels = 0;
    ...
    /* audio mode */
    arsi_task_config->audio_mode = mStreamAttributeTarget->audio_mode;
    ...
    arsi_task_config->max_input_device_num_channels =
        AudioALSAHardwareResourceManager::getInstance()->getNumPhoneMicSupport();
    arsi_task_config->max_output_device_num_channels = 2; /* TODO */

    /* flag & source */
    arsi_task_config->output_flags = 0;
    arsi_task_config->input_source = mStreamAttributeTarget->input_source;
    arsi_task_config->input_flags  = mStreamAttributeTarget->mAudioInputFlags;

    if (arsi_task_config->output_device_info.devices & AUDIO_DEVICE_OUT_SPEAKER &&
        AudioSmartPaController::getInstance()->isSmartPAUsed()) {
        arsi_task_config->output_device_info.hw_info_mask = OUTPUT_DEVICE_HW_INFO_SMARTPA_SPEAKER;
        ALOGV("%s(), SMART_PA_SUPPORT, arsi_task_config->output_device_info.hw_info_mask = %d",
              __FUNCTION__, arsi_task_config->output_device_info.hw_info_mask);
    }
...
void AudioALSACaptureDataClientAurisysNormal::InitBufferConfig(struct aurisys_lib_manager_t *manager) {
    /* UL in */
    mAudioPoolBufUlIn = create_audio_pool_buf(manager, DATA_BUF_UPLINK_IN, kRawDataBufferSize);
    ...
    /* UL out */
    mAudioPoolBufUlOut = create_audio_pool_buf(manager, DATA_BUF_UPLINK_OUT, kRawDataBufferSize);
    ...
    if (IsAECEnable()) {
        ...
        mAudioPoolBufUlAec = create_audio_pool_buf(manager, DATA_BUF_ECHO_REF, kRawDataBufferSize);
    ...
    if (mAurisysScenario == AURISYS_SCENARIO_VOIP ||
        mAurisysScenario == AURISYS_SCENARIO_VOIP_WITHOUT_AEC) {
        /* DL in */
        mAudioPoolBufDlIn = create_audio_pool_buf(manager, DATA_BUF_DOWNLINK_IN, kRawDataBufferSize);

        if (IsAECEnable()) {
            ASSERT(mStreamAttributeSourceEchoRef != NULL);
            mAudioPoolBufDlIn->buf->b_interleave = 1; /* LRLRLRLR*/
            mAudioPoolBufDlIn->buf->frame_size_ms = 0;
            mAudioPoolBufDlIn->buf->num_channels = mStreamAttributeSourceEchoRef->num_channels;
            mAudioPoolBufDlIn->buf->sample_rate_buffer = mStreamAttributeSourceEchoRef->sample_rate;
            mAudioPoolBufDlIn->buf->sample_rate_content = mStreamAttributeSourceEchoRef->sample_rate;
            mAudioPoolBufDlIn->buf->audio_format = mStreamAttributeSourceEchoRef->audio_format;
        } else {
            mAudioPoolBufDlIn->buf->b_interleave = 1; /* LRLRLRLR*/
            mAudioPoolBufDlIn->buf->frame_size_ms = 0;
            mAudioPoolBufDlIn->buf->num_channels = 2;
            mAudioPoolBufDlIn->buf->sample_rate_buffer = 48000;
            mAudioPoolBufDlIn->buf->sample_rate_content = 48000;
            mAudioPoolBufDlIn->buf->audio_format = AUDIO_FORMAT_PCM_16_BIT;
        }


        /* DL out */
        mAudioPoolBufDlOut = create_audio_pool_buf(manager, DATA_BUF_DOWNLINK_OUT, kRawDataBufferSize);

        if (IsAECEnable()) {
            ASSERT(mStreamAttributeSourceEchoRef != NULL);
            mAudioPoolBufDlOut->buf->b_interleave = 1; /* LRLRLRLR*/
            mAudioPoolBufDlOut->buf->frame_size_ms = 0;
            mAudioPoolBufDlOut->buf->num_channels = mStreamAttributeSourceEchoRef->num_channels;
            mAudioPoolBufDlOut->buf->sample_rate_buffer = mStreamAttributeSourceEchoRef->sample_rate;
            mAudioPoolBufDlOut->buf->sample_rate_content = mStreamAttributeSourceEchoRef->sample_rate;
            mAudioPoolBufDlOut->buf->audio_format = mStreamAttributeSourceEchoRef->audio_format;
        } else {
            mAudioPoolBufDlOut->buf->b_interleave = 1; /* LRLRLRLR*/
            mAudioPoolBufDlOut->buf->frame_size_ms = 0;
            mAudioPoolBufDlOut->buf->num_channels = 2;
            mAudioPoolBufDlOut->buf->sample_rate_buffer = 48000;
            mAudioPoolBufDlOut->buf->sample_rate_content = 48000;
            mAudioPoolBufDlOut->buf->audio_format = AUDIO_FORMAT_PCM_16_BIT;
        }
    }
}
