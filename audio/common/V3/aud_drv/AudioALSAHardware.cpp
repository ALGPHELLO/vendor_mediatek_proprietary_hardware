...
static char aurisys_set_key_prefix_dsp_smartpa[] = "AURISYS_SET_PARAM,DSP,SMARTPA";
static char aurisys_set_key_prefix_hal_smartpa[] = "AURISYS_SET_PARAM,HAL,SMARTPA";
static char aurisys_get_key_prefix_dsp_smartpa[] = "AURISYS_GET_PARAM,DSP,SMARTPA";
#endif

#ifdef MTK_AURISYS_PHONE_CALL_SUPPORT
static char aurisys_set_key_prefix_dsp_call[] = "AURISYS_SET_PARAM,DSP,PHONE_CALL";
static char aurisys_set_key_prefix_hal_call[] = "AURISYS_SET_PARAM,HAL,PHONE_CALL";

static char aurisys_get_key_prefix_dsp_call[] = "AURISYS_GET_PARAM,DSP,PHONE_CALL";
#endif
...
#if defined(MTK_AURISYS_PHONE_CALL_SUPPORT) || defined(MTK_AURISYS_FRAMEWORK_SUPPORT)
static String8 aurisys_set_param_result = String8("AURISYS_SET_PARAM_RESULT");
static bool b_opendsp_set_param;
#endif
...
static const char PROPERTY_KEY_BTSCO_WB_ON[PROPERTY_KEY_MAX] = "streamout.btscowb";
...
AudioALSAHardware::AudioALSAHardware() :
    ...
    property_get(PROPERTY_KEY_BTSCO_WB_ON, property_value, "0");
    mAudioHalBtscoWB = (bool)atoi(property_value);
...
#ifdef MTK_SPEAKER_MONITOR_SPEECH_SUPPORT
    status_t status_change = CheckDisableSpeechStream(static_cast<audio_mode_t>(mode));
#endif

    status = mStreamManager->setMode(static_cast<audio_mode_t>(mode));

#ifdef MTK_SPEAKER_MONITOR_SPEECH_SUPPORT
    CheckEnableSpeechStream(static_cast<audio_mode_t>(mode), status_change);
#endif
    return status;
}
...
status_t AudioALSAHardware::setParameters(const String8 &keyValuePairs) {
...
#ifdef MTK_AUDIO_SMARTPASCP_SUPPORT
    if (strncmp(aurisys_set_key_prefix_dsp_smartpa,
                keyValuePairs.string(),
                strlen(aurisys_set_key_prefix_dsp_smartpa)) == 0 ||
        strncmp(aurisys_set_key_prefix_hal_smartpa,
                keyValuePairs.string(),
                strlen(aurisys_set_key_prefix_hal_smartpa)) == 0)
    {
...
#ifdef MTK_AURISYS_PHONE_CALL_SUPPORT
    if (strncmp(aurisys_set_key_prefix_dsp_call,
                keyValuePairs.string(),
                strlen(aurisys_set_key_prefix_dsp_call)) == 0 ||
        strncmp(aurisys_set_key_prefix_hal_call,
                keyValuePairs.string(),
                strlen(aurisys_set_key_prefix_hal_call)) == 0) {
        b_opendsp_set_param = true;

        SpeechDriverOpenDSP *pSpeechDriver =
            static_cast<SpeechDriverOpenDSP *>(
                SpeechDriverFactory::GetInstance()->GetSpeechDriver());
        status = pSpeechDriver->SetParameter(keyValuePairs.string());
        goto SET_PARAM_EXIT;
    }
#endif
    ...
    if (param.get(keySetBTMode, value_str) == NO_ERROR) {
        ...
        if (value_str == "on") {
            property_set(PROPERTY_KEY_BTSCO_WB_ON, "1");
            WCNChipController::GetInstance()->SetBTCurrentSamplingRateNumber(16000);
            AudioBTCVSDControl::getInstance()->BT_SCO_SetMode(true);
            mSpeechPhoneCallController->setBTMode(true);
        } else if (value_str == "off") {
            property_set(PROPERTY_KEY_BTSCO_WB_ON, "0");
            WCNChipController::GetInstance()->SetBTCurrentSamplingRateNumber(8000);
            AudioBTCVSDControl::getInstance()->BT_SCO_SetMode(false);
            mSpeechPhoneCallController->setBTMode(false);
        }
    ...
    if (param.getInt(keyBesLoudnessSet, value) == NO_ERROR) {
        ...
        snprintf(strbuffer, sizeof(strbuffer),
                 "AURISYS_SET_PARAM,HAL,PLAYBACK_NORMAL,MTKBESSOUND,KEY_VALUE,SetBesLoudnessStatus,%d=SET", value);
    ...
    if (param.getInt(keySET_CUST_XML_ENABLE, value) == NO_ERROR) {
        ...
        value = (value == 1 ? 1 : 0);
        if (mStreamManager->GetCustXmlEnable() != value) {
            mStreamManager->SetCustXmlEnable((unsigned short)value);
            AppOps *appOps = appOpsGetInstance();
            if (appOps != NULL) {
                appOps->appHandleCustXmlEnableChanged(appOps->appHandleGetInstance(), value);
            } else {
                ALOGE("%s(), Error: AppOps == NULL", __FUNCTION__);
                ASSERT(0);
            }
    ...
    if (param.getInt(keySET_SUPER_VOLUME_ENABLE, value) == NO_ERROR) {
        ...
        mStreamManager->SetSpeechParamEnable("ParamSphSV", (bool) value);
    }
#endif
...
String8 AudioALSAHardware::getParameters(const String8 &keys) {
...
#ifdef MTK_AUDIO_SMARTPASCP_SUPPORT
    ALOGD("+++%s(), key = %s", __FUNCTION__, keys.string());
    if (strncmp(aurisys_get_key_prefix_dsp_smartpa,
                keys.string(),
                strlen(aurisys_get_key_prefix_dsp_smartpa)) == 0)
    {
    ...
    if (param.get(aurisys_set_param_result, value) == NO_ERROR) {
        ...
        if (b_opendsp_set_param == true) {
            b_opendsp_set_param = false;
#ifndef MTK_AURISYS_PHONE_CALL_SUPPORT
            WARNING("b_opendsp_set_param should not be true!!");
#else
            SpeechDriverOpenDSP *pSpeechDriver =
                static_cast<SpeechDriverOpenDSP *>(
                    SpeechDriverFactory::GetInstance()->GetSpeechDriver());
            retval = pSpeechDriver->GetRetvalSetParameter();
#endif
        }
#ifdef MTK_AURISYS_FRAMEWORK_SUPPORT
        else {
            retval = get_retval_of_aurisys_set_rarameter();
        }
#endif
        returnParam.add(
...
#ifdef MTK_AURISYS_PHONE_CALL_SUPPORT
    if (strncmp(aurisys_get_key_prefix_dsp_call,
                keys.string(),
                strlen(aurisys_get_key_prefix_dsp_call)) == 0) {
        SpeechDriverOpenDSP *pSpeechDriver =
            static_cast<SpeechDriverOpenDSP *>(
                SpeechDriverFactory::GetInstance()->GetSpeechDriver());
...
//    if (param.get(keyACCESS_VOW_PARAM, value) == NO_ERROR) {
      if (strncmp(keyACCESS_VOW_PARAM,
                  keys.string(),
                  strlen(keyACCESS_VOW_PARAM)) == 0) {
        //parse format
        int par_type = -1;
        int par_value = -1;
        value = String8(keys.string() + strlen(keyACCESS_VOW_PARAM) + 1);
        ...
        if ((par_type < 1) || (par_type > 10)) {
            ...
            par_value = mAudioCustParamClient->GetVOWCustParamFromNV(par_type - 1);
        }
        //return value
        char buf[32];
...
//    if (param.get(keyACCESS_VOW_KR_PARAM, value) == NO_ERROR) {
      if (strncmp(keyACCESS_VOW_KR_PARAM,
                  keys.string(),
                  strlen(keyACCESS_VOW_KR_PARAM)) == 0) {
        //parse format
        int par_type = -1;
        int par_value = -1;
        value = String8(keys.string() + strlen(keyACCESS_VOW_KR_PARAM) + 1);
        sscanf(value.string(), "%d", &par_type);
        param.remove(keyACCESS_VOW_KR_PARAM);
        //get value from nvram
        if ((par_type < 1) || (par_type > 4)) {
            ...
            par_value = mAudioCustParamClient->GetVOWCustParamFromNV(par_type + 9);
        }
        //return value
        char buf[32];
        sprintf(buf, "%d", par_value);
...
//    if (param.get(keyACCESS_VOW_KRSR_PARAM, value) == NO_ERROR) {
      if (strncmp(keyACCESS_VOW_KRSR_PARAM,
                  keys.string(),
                  strlen(keyACCESS_VOW_KRSR_PARAM)) == 0) {
        //parse format
        int par_type = -1;
        int par_value = -1;
        value = String8(keys.string() + strlen(keyACCESS_VOW_KRSR_PARAM) + 1);
        sscanf(value.string(), "%d", &par_type);
        param.remove(keyACCESS_VOW_KRSR_PARAM);
        //get value from nvram
        if ((par_type < 1) || (par_type > 4)) {
            ...
            par_value = mAudioCustParamClient->GetVOWCustParamFromNV(par_type + 17);
        }
        //return value
        char buf[32];
        sprintf(buf, "%d", par_value);
    ...
    if (param.get(keyGetPhoneCallStatus, value) == NO_ERROR) {
        ...
        uint32_t result_int = mSpeechPhoneCallController->isModeInPhoneCall();
    ...
    if (param.get(keyGET_SUPER_VOLUME_ENABLE, value) == NO_ERROR) {
        bool supervolume_status = mStreamManager->GetSpeechParamEnable("ParamSphSV");
        value = (supervolume_status) ? "1" : "0";
    ...
    if (param.get(keyGetDumpAEECheck, value) == NO_ERROR) {
        ...
        property_get("streamout.aee.dump", valueDump, "0");
    ...
    if (param.get(keyGetDumpAudioStreamOut, value) == NO_ERROR)
    {
        ...
        char valueDump[PROPERTY_VALUE_MAX];
        property_get("streamout.pcm.dump", valueDump, "0");
        int isDumpEnable = atoi(valueDump);
    ...
    if (param.get(keyGetDumpAudioMixerBuf, value) == NO_ERROR)
    {
        ...
        char valueDump[PROPERTY_VALUE_MAX];
        property_get("af.mixer.pcm", valueDump, "0");
        int isDumpEnable = atoi(valueDump);
    ...
    if (param.get(keyGetDumpAudioTrackBuf, value) == NO_ERROR)
    {
        ...
        char valueDump[PROPERTY_VALUE_MAX];
        property_get("af.track.pcm", valueDump, "0");
        int isDumpEnable = atoi(valueDump);
    ...
    if (param.get(keyGetDumpA2DPStreamOut, value) == NO_ERROR)
    {
        ...
        char valueDump[PROPERTY_VALUE_MAX];
        property_get("a2dp.streamout.pcm", valueDump, "0");
        int isDumpEnable = atoi(valueDump);
    ...
    if (param.get(keyGetDumpAudioStreamIn, value) == NO_ERROR)
    {
        ...
        char valueDump[PROPERTY_VALUE_MAX];
        property_get("streamin.pcm.dump", valueDump, "0");
        int isDumpEnable = atoi(valueDump);
...
#if defined(MTK_AUDIO_HD_REC_SUPPORT)
        char valueDump[PROPERTY_VALUE_MAX];
        property_get("streamin.vm.dump", valueDump, "0");
    ...
    if (param.get(keyGetDumpApSpeechEPL, value) == NO_ERROR)
    {
        ...
        char valueDump[PROPERTY_VALUE_MAX];
        property_get("streamin.epl.dump", valueDump, "0");
        int isDumpEnable = atoi(valueDump);

        char valueDump2[PROPERTY_VALUE_MAX];
        property_get("streamin.epl.dump", valueDump2, "0");
    ...
    if (param.get(String8("hifi_dac"), value) == NO_ERROR) {
        ...
        bool benable = mStreamManager->getHiFiStatus();
        if (benable == true) {
            returnParam.add(String8("hifi_dac"), String8("on"));
        } else {
            returnParam.add(String8("hifi_dac"), String8("off"));
        }
    }
...
#if 0
    char filenameL[] = "/sdcard/record_tdm.pcm";
...
status_t AudioALSAHardware::SetAudioCommand(int par1, int par2) {
    ...
    case SPEAKER_MONITOR_TEMP_UPPER_BOUND: {
    ...
    case SPEAKER_MONITOR_TEMP_LOWER_BOUND: {
...
status_t AudioALSAHardware::GetAudioCommonData(int par1, size_t len, void *ptr) {
    ...
    case VOICE_RECOG_CUST_PARAM: {
        mAudioCustParamClient->GetVoiceRecogCustParamFromNV((VOICE_RECOGNITION_PARAM_STRUCT *)ptr);
        break;
    }
...
String8 AudioALSAHardware::GetAudioEncodedBuffer(int TypeAudioData, size_t ByteAudioData) {
    ...
    ALOGD("%s(), before encode (%s), sz_in(%zu), sz_out(%zu)", __FUNCTION__, pAudioData, sz_in, sz_out);
...
int AudioALSAHardware::SetVoiceUnlockSRC(uint32_t outSR, uint32_t outChannel) {
    ALOGD("%s()", __FUNCTION__);
    AudioVUnlockDL *VInstance = AudioVUnlockDL::getInstance();
...
void AudioALSAHardware::onAudioParameterChangedCallback(const char *audioType) {
    ...
    for (size_t i = 0; i < mAudioParameterChangedHidlCallbackList.size(); i++) {
        ALOGD("%s(),  Invoke hidle callback[%zu/%zu]: %s", __FUNCTION__, i + 1, mAudioParameterChangedHidlCallbackList.size(), audioType);
        mAudioParameterChangedHidlCallbackList[i]->mHidlCallback(audioType, mAudioParameterChangedHidlCallbackList[i]->mHidlCookie);
    }
}
