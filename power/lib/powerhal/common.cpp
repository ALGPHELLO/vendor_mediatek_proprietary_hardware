...
void get_cputopo_cpu_info(int cluster_num, int *p_cpu_num, int *p_first_cpu) // find first cpu of each cluster
{
    FILE *ifp;
    char  buf[128] = {0}, *str;
    int   i = 0, scn, mask, cluster = 0, count, index;
...
int write_to_file(const char* path, const char* buf, int size)
{
    ...
    if (fd == -1) {
        ALOGE("Could not open '%s'\n", path);
        char *err_str = strerror(errno);
        if (strcmp(path, PATH_WIPHY_CAM) == 0) {
            ALOGE("no support wiphy CAM");
        }
        ALOGE("error : %d, %s\n", errno, err_str);
        return 0;
    }
    ...
    if (count != size) {
        ALOGE("write file (%s) fail, count: %d\n", path, count);
        char *err_str = strerror(errno);
        ALOGE("error : %d, %s\n", errno, err_str);
        close(fd);
        return 0;
...
void set_value(const char * path, const int value_1, const int value_2)
{
    char buf[32] = {0};
...
void set_value(const char * path, const int value)
{
    char buf[32] = {0};
    sprintf(buf, "%d", value);
    write_to_file(path, buf, strlen(buf));
...
void set_value(const char * path, const char *str)
{
    write_to_file(path, str, strlen(str));
}

void set_value(const char * path, const string *str)
{
    write_to_file(path, str->c_str(), str->length());
}
...
void get_ppm_cpu_freq_info(int cluster_index, int *p_max_freq, int *p_count, int **pp_table) // max freq, freq level counts, freq table
{
    ...
    int *tbl = NULL, count=0, i=0, timeout=0;
...
void set_wiphy_cam_level(int level)
{
    char buf[32];
    sprintf(buf, "CAM %d", level);
    write_to_file(PATH_WIPHY_CAM, buf, strlen(buf));
}
