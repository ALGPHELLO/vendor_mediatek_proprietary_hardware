...
status_t AsyncBliterHandler::setDstDpConfig(PrivateHandle& dst_priv_handle, BufferConfig* config)
{
    ...
    switch (dst_priv_handle.format)
    {
        ...
        case HAL_PIXEL_FORMAT_RGB_888:
            config->dst_dpformat = DP_COLOR_RGB888;
            config->dst_pitch    = dst_priv_handle.width * getBitsPerPixel(dst_priv_handle.format) / 8;
            config->dst_pitch_uv = 0;
            config->dst_plane    = 1;
            config->dst_size[0]  = config->dst_pitch * config->dst_height;
...
void AsyncBliterHandler::process(DispatcherJob* job)
{
    ...
    for (uint32_t i = 0; i < total_num; i++)
    {
        ...
        if (NO_ERROR == err)
        {
            ...
            bool pq_enhance = (HWCMediator::getInstance().m_features.enhance) &&
            ...
            {
                m_bliter_node->setSrc(config, hw_layer->priv_handle, &layer->acquireFenceFd);
                m_bliter_node->setDst(0, &param, disp_buffer.out_ion_fd,
                                                 disp_buffer.out_sec_handle,
                                                 &disp_buffer.release_fence);
                err = m_bliter_node->invalidate(hw_layer->mdp_job_id);

                passFenceFd(&disp_buffer.acquire_fence, &layer->releaseFenceFd);
            }

            Rect src_crop = dst_roi;
            src_crop.offsetTo(ALIGN_FLOOR(src_crop.left, 2), src_crop.top);
            disp_buffer.data_info.src_crop   = src_crop;
            disp_buffer.data_info.dst_crop   = Rect(*(Rect *)&(layer->displayFrame));
        ...
        else
        {
            ...
            {
                m_bliter_node->cancelJob(hw_layer->mdp_job_id);
            }
        ...
        {
            IONDevice::getInstance().ionCloseAndSet(&hw_layer->priv_handle.ion_fd);
        }
...
void AsyncBliterHandler::nullop(uint32_t job_id)
{
    m_bliter_node->cancelJob(job_id);
}
...
void AsyncBliterHandler::boost()
{
    ...
    if (m_disp_id >= HWC_DISPLAY_VIRTUAL &&
        m_disp_data->subtype == HWC_DISPLAY_WIRELESS)
    {
        PerfServiceNative_setBoostThread();
    }
#endif // MTK_BASIC_PACKAGE
}
...
void AsyncBliterHandler::cancelMirror(DispatcherJob* job)
{
    ...
    {
        closeFenceFd(&hw_mirbuf->mir_in_rel_fence_fd);
    }

    nullop(job->mdp_job_output_buffer);
    clearMdpJob(job->fill_black);

    if (hw_mirbuf->mir_in_acq_fence_fd != -1) closeFenceFd(&hw_mirbuf->mir_in_acq_fence_fd);
}
...
void AsyncBliterHandler::processPhyMirror(DispatcherJob* job)
{
    ...
    if (NO_ERROR == err)
    {
        ...
        m_bliter_node->setSrc(config, src_priv_handle, &src_buf.mir_in_acq_fence_fd);
        m_bliter_node->setDst(0, &param, disp_buffer.out_ion_fd,
                                         disp_buffer.out_sec_handle,
                                         &disp_buffer.release_fence);
        err = m_bliter_node->invalidate(job->mdp_job_output_buffer);
    ...
    else
    {
        ...
        m_bliter_node->cancelJob(job->mdp_job_output_buffer);
    }
...
void AsyncBliterHandler::processVirMirror(DispatcherJob* job)
{
    ...
    if (NO_ERROR == err)
    {
        ...
        BliterNode::Parameter param = {src_roi, dst_roi, config, xform, false};
        m_bliter_node->setSrc(config, src_priv_handle, &src_buf.mir_in_acq_fence_fd);
        m_bliter_node->setDst(0, &param, dst_priv_handle.ion_fd,
                                         dst_priv_handle.sec_handle,
                                         &dst_buf.mir_in_rel_fence_fd);
        m_bliter_node->invalidate(job->mdp_job_output_buffer);
    }
    else
    {
...
void AsyncBliterHandler::processFillBlack(PrivateHandle* dst_priv_handle, int* fence, MdpJob &job)
{
    ...
    if (NO_ERROR == err)
    {
        ...
        m_bliter_node->setSrc(&config, src_priv_handle);
        m_bliter_node->setDst(0, &param, dst_priv_handle->ion_fd, dst_priv_handle->sec_handle, fence);
        m_bliter_node->invalidate(job.id);
    ...
    else
    {
        ...
        m_bliter_node->cancelJob(job.id);
    }
...
void AsyncBliterHandler::clearBackground(buffer_handle_t handle, int curr_orient, int* fence, MdpJob* job)
{
