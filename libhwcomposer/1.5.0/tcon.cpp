    ...
    : mTcon(NULL)
{
    ...
    sprintf(path, "%s%s_%s.so", TCON_LIBRARY_PATH, TCON_LIBRARY_NAME, HWCMediator::getInstance().m_features.epaper_vendor);
    handle = dlopen(path, RTLD_NOW);
    if (handle == NULL)
    {
        ALOGE("failed to load %s_%s", TCON_LIBRARY_NAME, HWCMediator::getInstance().m_features.epaper_vendor);
        return;
    }
