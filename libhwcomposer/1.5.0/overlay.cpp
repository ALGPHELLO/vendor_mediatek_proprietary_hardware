    ...
    , m_ignore_av_grouping(false)
{
    ...
    snprintf(m_thread_name, sizeof(m_thread_name), "OverlayEngine_%d", dpy);
...
int OverlayEngine::getAvailableInputNum()
{
    ...
    DispDevice::getInstance().getOverlaySessionInfo(m_disp_id, &info);
...
status_t OverlayEngine::preparePresentFence(OverlayPrepareParam& param)
{
    ...
    if (HWC_DISPLAY_PRIMARY == m_disp_id)
    {
        DispDevice::getInstance().prepareOverlayPresentFence(m_disp_id, &param);
    }
    else
    {
        param.fence_fd = -1;
...
status_t OverlayEngine::configMirrorOutput(HWBuffer* outbuf, bool secure)
{
    ...
    unsigned int if_fence_idx = 0;
    {
        ...
        DispDevice::getInstance().prepareOverlayOutput(m_disp_id, &prepare_param);
    ...
    {
        int err = getPrivateHandle(outbuf->handle, &outbuf->priv_handle);
        if (err < 0)
            OLOGV("getPrivateHandle fail:%d", err);
    }

    if (DisplayManager::m_profile_level & PROFILE_TRIG)
    {
        OLOGD("HWC->OVL: config output (rel_fd=%d acq_fd=%d/idx=%u)",
...
{
    return DispDevice::getInstance().setOverlaySessionMode(m_disp_id, mode);
}

DISP_MODE OverlayEngine::getOverlaySessionMode()
{
    return DispDevice::getInstance().getOverlaySessionMode(m_disp_id);
}
...
void OverlayEngine::trigger(bool ovl_valid, int num_layers,
                            int present_fence_idx, int prev_present_fence,
                            bool av_grouping, EXTD_TRIGGER_MODE trigger_mode)
{
    if (present_fence_idx != DISP_DO_NOTHING)
    {
    ...
    else
    {
        ...
        logger.printf("(%d) Trigger with idx: %d/ do_nothing", m_disp_id, present_fence_idx);
    }
...
status_t OverlayEngine::loopHandler(sp<FrameInfo>& info)
{
    ...
    {
        DbgLogger logger(DbgLogger::TYPE_HWC_LOG, 'D');
        logger.printf("(%d) triggerOverlaySession ovlp:%d idx:%d prev_present_fence:%d",
    ...
    if (DisplayManager::m_profile_level & PROFILE_TRIG)
    {
        ...
        err = DispDevice::getInstance().triggerOverlaySession(m_disp_id, fence_idx, ovlp_layer_num,
                                                              prev_present_fence);
    }
    else
    {
...
void OverlayEngine::setPowerMode(int mode)
{
        ...
        case HWC_POWER_MODE_OFF:
            {
                ...
                if (DisplayManager::getInstance().m_data[m_disp_id].subtype != HWC_DISPLAY_EPAPER)
                {
                    ...
                    if (HWC_DISPLAY_VIRTUAL > m_disp_id)
                    {
                        DispDevice::getInstance().disableOverlaySession(
                            m_disp_id, m_input_params.array(), num);
                    }
    ...
    if (HWC_DISPLAY_VIRTUAL > m_disp_id)
    {
        DispDevice::getInstance().setPowerMode(m_disp_id, mode);
    }
}
...
void OverlayEngine::updateInput(int id)
{
    ...
    if (m_inputs[id]->connected_state == OVL_PORT_ENABLE)
    {
        ...
        OverlayPrepareParam prepare_param;
        {
            ...
            DispDevice::getInstance().prepareOverlayInput(m_disp_id, &prepare_param);
...
void OverlayEngine::updateInput(int id, sp<GraphicBuffer>& buf, int *releaseFd, int sequence)
{
    ...
    getPrivateHandleInfo(buf->handle, &priv_handle);
    getPrivateHandleBuff(buf->handle, &priv_handle);
    ...
    if (m_inputs[id]->connected_state == OVL_PORT_ENABLE)
    {
        ...
        param->alpha          = 0xff;
        param->blending       = HWC_BLENDING_NONE;
...
bool OverlayEngine::threadLoop()
{
    ...
    while (1)
    {
        ...
        if (!DispDevice::getInstance().isFenceWaitSupported())
        {
...
void OverlayEngine::closeOverlayFenceFd(FrameOverlayInfo* info)
{
    ...
    {
        closeFenceFd(&info->output.fence);
        info->output.fence = -1;
    }
}
...
void OverlayEngine::closeAllFenceFd(const sp<FrameInfo>& info)
{
    ...
    {
        closeFenceFd(&info->prev_present_fence);
...
void OverlayEngine::waitAllFence(sp<FrameInfo>& info)
{
    char tag[128];
    DbgLogger logger(DbgLogger::TYPE_HWC_LOG, 'D');
    logger.printf("(%d) Wait present fence for idx: %d", m_disp_id, info->present_fence_idx);
...
void OverlayEngine::packageFrameInfo(sp<FrameInfo>& info, bool ovl_valid, int num_layers,
                                     int present_fence_idx, int prev_present_fence,
                                     bool av_grouping, EXTD_TRIGGER_MODE trigger_mode)
{
    DbgLogger logger(DbgLogger::TYPE_HWC_LOG, 'D');
    logger.printf("(%d) Trigger with idx: %d", m_disp_id, present_fence_idx);
...
void OverlayEngine::setInputsAndOutput(FrameOverlayInfo* info)
{
    ...
    if (DisplayManager::m_profile_level & PROFILE_TRIG)
    {
        ...
        DispDevice::getInstance().updateOverlayInputs(
            m_disp_id, info->input.array(), info->num_layers);
    }
    else
    {
        DispDevice::getInstance().updateOverlayInputs(
    ...
    if (info->enable_output)
    {
        if (DisplayManager::m_profile_level & PROFILE_TRIG)
        {
            ...
            DispDevice::getInstance().enableOverlayOutput(m_disp_id, &info->output);
        }
        else
        {
            DispDevice::getInstance().enableOverlayOutput(m_disp_id, &info->output);
    ...
    {
        DispDevice::getInstance().disableOverlayOutput(m_disp_id);
    }
}
