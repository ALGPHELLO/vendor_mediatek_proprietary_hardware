    ...
    DispatcherJob* job)
{
    ...
    for (uint32_t i = 0; i < total_num; i++)
    {
        ...
        const uint32_t& num_rect = layer->surfaceDamage.numRects;
        if (!job->is_full_invalidate)
        {
            hwc_rect_t* job_dirty_rect = hw_layer->surface_damage_rect;
            if (num_rect == 0 || num_rect > DispDevice::MAX_DIRTY_RECT_CNT)
            {
            ...
            {
                memcpy(job_dirty_rect, layer->surfaceDamage.rects, sizeof(hwc_rect_t) * num_rect);
...
void ComposerHandler::cancelLayers(DispatcherJob* job)
{
    ...
    for (uint32_t i = 0; i < job->num_layers; ++i)
    {
        ...
        if (hw_layer->layer.acquireFenceFd != -1) closeFenceFd(&hw_layer->layer.acquireFenceFd);
    }
}
