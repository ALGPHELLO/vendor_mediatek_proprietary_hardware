...
status_t OverlayEngine::setInputs(int num)
{
    ...
    if (DisplayManager::m_profile_level & PROFILE_TRIG)
    {
        ...
        HWC_ATRACE_NAME(atrace_tag);
        OLOGI("HWC->OVL: set inputs (max=%d)", num);
...
status_t OverlayEngine::trigger(int present_fence_idx)
{
    AutoMutex l(m_lock);
    ...
    if (DisplayManager::m_profile_level & PROFILE_TRIG)
    {
        ...
        err = DispDevice::getInstance().triggerOverlaySession(m_disp_id, fence_idx);
    }
    else
    {
        err = DispDevice::getInstance().triggerOverlaySession(m_disp_id, fence_idx);
