    ...
    PrivateHandle* priv_handle, BufferConfig* config, int ovl_in)
{
    ...
    if (HAL_PIXEL_FORMAT_UFO == priv_handle->format ||
        ...
        HAL_PIXEL_FORMAT_UFO_10BIT_H == priv_handle->format ||
        GRALLOC_EXTRA_BIT_CM_UFO_10BIT_H == curr_private_format ||
        HAL_PIXEL_FORMAT_UFO_10BIT_V == priv_handle->format ||
        GRALLOC_EXTRA_BIT_CM_UFO_10BIT_V == curr_private_format)
    {
    ...
    if (config->gralloc_stride == priv_handle->y_stride &&
            ...
            // set dst buffer information
            {
                ...
                switch (priv_handle->format)
                {
                    case HAL_PIXEL_FORMAT_RGB_888:
                        ...
                        config->gralloc_color_range = DP_PROFILE_FULL_BT601;
                        break;
