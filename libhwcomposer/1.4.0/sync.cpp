...
void SyncControl::setOverlay(DispatcherJob* job)
{
    if (job->need_sync)
    {
        HWC_LOGD("(%d) Pending disable unused layers...", job->disp_ori_id);
        job->need_sync = false;
        m_condition.signal();
        return;
    }
    ...
    if (!job->triggered)
    {
        HWC_LOGD("(%d) Set overlay layers", job->disp_ori_id);
    ...
    {
        HWC_LOGD("(%d) Overlay layers was set", job->disp_ori_id);
