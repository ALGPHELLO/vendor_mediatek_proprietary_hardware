...
const char* g_present_vali_state_log_prefix = "[HWC] ";
...
HWCDisplay::HWCDisplay(const int64_t& disp_id, const int32_t& type)
    ...
    , m_present_vali_state_log(DbgLogger::TYPE_HWC_LOG, 'D', g_present_vali_state_log_prefix)
{
    switch (disp_id)
    {
    ...
    m_present_vali_state_log.printf("(%" PRIu64 ") ", getId());
}
...
{
    editPresentValiStateLog().printf("(L%d) set s:%s=>", line, getPresentValiStateString(val));
...
void HWCMediator::deviceDump(hwc2_device_t* /*device*/, uint32_t* outSize, char* outBuffer)
{
    ...
    else
    {
        ...
        property_get("debug.hwc.async_bliter", value, "-1");
        if (-1 != atoi(value))
        {
        ...
        property_get("debug.hwc.disp_support_decompress", value, "-1");
        if (-1 != atoi(value))
        {
            Platform::getInstance().m_config.disp_support_decompress = atoi(value);
        }

        property_get("debug.hwc.mdp_support_decompress", value, "-1");
        if (-1 != atoi(value))
        {
        ...
        dump_str.appendFormat("  enableUBL(debug.hwc.enableUBL):%d\n", Platform::getInstance().m_config.use_async_bliter_ultra);
        ...
        dump_str.appendFormat("  async_bliter(debug.hwc.async_bliter):%d\n", Platform::getInstance().m_config.use_async_bliter);
        ...
        dump_str.appendFormat("  disp_support_decompress(debug.hwc.disp_support_decompress):%d\n", Platform::getInstance().m_config.disp_support_decompress);

        dump_str.appendFormat("  mdp_support_decompress(debug.hwc.mdp_support_decompress):%d\n", Platform::getInstance().m_config.mdp_support_decompress);
        dump_str.appendFormat("\n");
#else // MTK_USER_BUILD
        ...
        dump_str.appendFormat("%.2f,%d,%d,%d\n\n",
                Platform::getInstance().m_config.mdp_scale_percentage,
                Platform::getInstance().m_config.extend_mdp_capacity,
                Platform::getInstance().m_config.disp_support_decompress,
                Platform::getInstance().m_config.mdp_support_decompress);
    ...
    int32_t* out_retire_fence)
{
    ...
    m_displays[display]->editPresentValiStateLog().printf("%s s:%s=>", __func__, getPresentValiStateString(m_displays[display]->getValiPresentState()));
    if (!DisplayManager::getInstance().m_data[display].connected)
    {
        HWC_LOGE("%s: the display(%" PRIu64 ") is not connected", __func__, display);
        return HWC2_ERROR_BAD_DISPLAY;
    ...
    m_validate_seq = 0;
    ++m_present_seq;
    m_displays[display]->editPresentValiStateLog().flushOut();
    m_displays[display]->editPresentValiStateLog().printf(g_present_vali_state_log_prefix);
    m_displays[display]->editPresentValiStateLog().printf("(%" PRIu64 ") ", m_displays[display]->getId());
    ...
    for (auto& display : displays)
    {
        ...
        for (auto& layer : layers)
        {
            ...
            layer_config->src_fmt       =
                (Platform::getInstance().m_config.pass_dim_to_hrt &&
                         ...
                         const int32_t& sink_disp)
{
    ...
    set<uint64_t> src_layers_alloc_id;
    set<uint64_t> src_layers_sideband;
...
void calculateMdpDstRoi(sp<HWCLayer> layer, const double& mdp_scale_percentage, const int32_t& z_seq)
{
        ...
        layer->getHwlayerType() == HWC_LAYER_TYPE_MM_HIGH))
    {
        ...
        else if (!(priv_hnd.format != HAL_PIXEL_FORMAT_RGBA_8888 &&
            priv_hnd.format != HAL_PIXEL_FORMAT_BGRA_8888 &&
            priv_hnd.format != HAL_PIXEL_FORMAT_RGBX_8888 &&
            priv_hnd.format != HAL_PIXEL_FORMAT_BGRX_8888 &&
            priv_hnd.format != HAL_PIXEL_FORMAT_RGB_888 &&
