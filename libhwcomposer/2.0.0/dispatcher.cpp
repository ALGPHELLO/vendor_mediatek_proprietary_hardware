...
void HWCDispatcher::setJob(const sp<HWCDisplay>& display)
{
...

    {
        ...
        if (job->num_ui_layers == 0 && job->num_mm_layers == 0 && !job->fbt_exist && !job->mm_fbt && dpy == HWC_DISPLAY_VIRTUAL)
        {
        ...
        logger->printf("(%" PRIu64 ") SET job=%d/max=%d/fbt=%d(%s)/ui=%d/mm=%d/mir=%d/ult=%d/flush=%d/black=%d",
