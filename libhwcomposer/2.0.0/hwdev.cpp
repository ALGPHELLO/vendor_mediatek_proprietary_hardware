...
{
    dump_str->appendFormat("[HWC OvlInSizeArbitrator] config(H:%d,L:%d)", m_hr_config, m_lr_config);
...
status_t DispDevice::legacySetInputBuffer(int dpy)
{
    ...
    static disp_session_input_config input_config;
    memset(&input_config, 0, sizeof(disp_session_input_config));

    input_config.session_id = m_frame_cfg[dpy].session_id;
    input_config.config_layer_num = m_frame_cfg[dpy].input_layer_num;
    int size = input_config.config_layer_num * sizeof(disp_input_config);
    memcpy(input_config.config, m_frame_cfg[dpy].input_cfg, size);
    memcpy(&input_config.ccorr_config, &m_frame_cfg[dpy].ccorr_config, sizeof(m_frame_cfg[dpy].ccorr_config));

    int err = WDT_IOCTL(m_dev_fd, DISP_IOCTL_SET_INPUT_BUFFER, &input_config);
    if (err < 0)
    {
...
status_t DispDevice::legacySetOutputBuffer(int dpy)
{
    ...
    static disp_session_output_config output_config;
    memset(&output_config, 0, sizeof(disp_session_output_config));

    output_config.session_id = m_frame_cfg[dpy].session_id;
    memcpy(&output_config.config, &m_frame_cfg[dpy].output_cfg, sizeof(disp_output_config));

    int err = WDT_IOCTL(m_dev_fd, DISP_IOCTL_SET_OUTPUT_BUFFER, &output_config);
    if (err < 0)
    {
    ...
    int dpy, OverlayPortParam* const* params, int num, sp<ColorTransform> color_transform)
{
    ...
    {
        bool enable_ccorr = !is_fbt_exist || (is_fbt_exist && non_fbt_layer_num != 0);
        if (enable_ccorr)
        {
            if (color_transform != nullptr)
            {
                m_frame_cfg[dpy].ccorr_config.is_dirty = true;
                m_frame_cfg[dpy].ccorr_config.mode = color_transform->hint;
                const int32_t dim = 4;
                for (int32_t j = 0; j < dim * dim ; ++j)
                    m_frame_cfg[dpy].ccorr_config.color_matrix[j] = static_cast<int32_t>(color_transform->matrix[j / 4][j % 4] * 1024.f + 0.5f);
            }
            else
            {
                if ((m_color_transform_info[dpy].prev_enable_ccorr == false && m_color_transform_info[dpy].last_valid_color_transform != nullptr) ||
                    (m_color_transform_info[dpy].resend_color_transform == true && m_color_transform_info[dpy].last_valid_color_transform != nullptr))
                {
                    m_frame_cfg[dpy].ccorr_config.is_dirty = true;
                    m_frame_cfg[dpy].ccorr_config.mode = m_color_transform_info[dpy].last_valid_color_transform->hint;
                    const int32_t dim = 4;
                    for (int32_t j = 0; j < dim * dim ; ++j)
                        m_frame_cfg[dpy].ccorr_config.color_matrix[j] = static_cast<int32_t>(m_color_transform_info[dpy].last_valid_color_transform->matrix[j / 4][j % 4] * 1024.f + 0.5f);
                    m_color_transform_info[dpy].resend_color_transform = false;
                }
                else
                {
        ...
        {
            m_frame_cfg[dpy].ccorr_config.is_dirty = true;
            m_frame_cfg[dpy].ccorr_config.mode = HAL_COLOR_TRANSFORM_ARBITRARY_MATRIX;
            memset(&m_frame_cfg[dpy].ccorr_config.color_matrix, 0, sizeof(m_frame_cfg[dpy].ccorr_config.color_matrix));
            const int dim = 4;
            for (int32_t j = 0; j < dim; ++j)
            {
                m_frame_cfg[dpy].ccorr_config.color_matrix[j * dim + j] = 1024;
            }
        }
        m_color_transform_info[dpy].prev_enable_ccorr = enable_ccorr;
        if (color_transform != nullptr)
            m_color_transform_info[dpy].last_valid_color_transform = color_transform;

        if (m_frame_cfg[dpy].ccorr_config.is_dirty)
        {
            DbgLogger logger(DbgLogger::TYPE_HWC_LOG, 'I', "[DEV] ");
            logger.printf("ccorr enable:%d (is_fbt_exist:%d non_fbt_layer_num:%d) mode:%d",
                enable_ccorr, is_fbt_exist, non_fbt_layer_num,
                m_frame_cfg[dpy].ccorr_config.mode);
            if (enable_ccorr)
            {
                const int dim = 4;
                for (int32_t j = 0; j < dim; ++j)
                {
                    ...
                    {
                        logger.printf("%4d ", m_frame_cfg[dpy].ccorr_config.color_matrix[j*dim + k]);
                    }
                }
            }
            else
            {
                logger.printf(" (Send identity matrix)");
            }
        }
    }
    else
