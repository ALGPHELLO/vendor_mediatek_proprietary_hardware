...
{
    // [NOTE] width and height for DP should be 2 byte-aligned in special case

    bool src_x_align_2 = false;
    bool src_y_align_2 = false;
    bool dst_x_align_2 = false;
    bool dst_y_align_2 = false;

    // determine source buffer alignment
    if (DP_COLOR_GET_H_SUBSAMPLE(src_buf.dpformat))
    {
        src_x_align_2 = true;

        if (DP_COLOR_GET_V_SUBSAMPLE(src_buf.dpformat) &&
            !DP_COLOR_GET_INTERLACED_MODE(src_buf.dpformat))
        {
            src_y_align_2 = true;
        }
    }
    ...
    src_roi->x = src_x_align_2 ?
                 ALIGN_CEIL(dst_param.src_crop.left, 2) :
                 dst_param.src_crop.left;
    src_roi->y = src_y_align_2 ?
                 ALIGN_CEIL(dst_param.src_crop.top, 2) :
                 dst_param.src_crop.top;
    src_roi->w = src_x_align_2 ?
                 ALIGN_FLOOR(dst_param.src_crop.getWidth() - (dst_param.src_crop.left & 0x1), 2) :
                 dst_param.src_crop.getWidth();
    src_roi->w = (src_x_align_2 && src_roi->w > src_buf.rect.getWidth()?
                 src_roi->w - 2: src_roi->w);

    src_roi->h = src_y_align_2 ?
                 ALIGN_FLOOR(dst_param.src_crop.getHeight() - (dst_param.src_crop.top & 0x1), 2) :
                 dst_param.src_crop.getHeight();
    src_roi->h = (src_y_align_2 && src_roi->h > src_buf.rect.getHeight()?
                 src_roi->h - 2: src_roi->h);
    ...
    dst_roi->w = dst_x_align_2 ?
                 ALIGN_CEIL(dst_param.dst_crop.getWidth() + (dst_param.dst_crop.left & 0x1), 2) :
                 dst_param.dst_crop.getWidth();
    dst_roi->h = dst_y_align_2 ?
                 ALIGN_CEIL(dst_param.dst_crop.getHeight() + (dst_param.dst_crop.top & 0x1), 2) :
                 dst_param.dst_crop.getHeight();
    ...
    {
        HWC_LOGW("out of boundary dst W %d+%d>%d", dst_roi->x, dst_roi->w, dst_buf.rect.getWidth());
        dst_roi->w -= 2;
    }

    if ((dst_roi->y + dst_roi->h) > dst_buf.rect.getHeight())
    {
        HWC_LOGW("out of boundary dst H %d+%d>%d", dst_roi->y, dst_roi->h, dst_buf.rect.getHeight());
        dst_roi->h -= 2;
    }\
    return NO_ERROR;
}
...
status_t BliterNode::invalidate(uint32_t job_id, Rect* cal_dst_roi)
{
    ...
    for (uint32_t i = 0; i < MAX_OUT_CNT; i++)
    {
        ...
        calculateROI(&src_roi, &dst_roi, src_param, dst_param, src_buf, dst_buf);

        if (cal_dst_roi != nullptr)
        {
            cal_dst_roi->left = dst_roi.x;
            cal_dst_roi->top = dst_roi.y;
            cal_dst_roi->right = dst_roi.w + dst_roi.x;
            cal_dst_roi->bottom = dst_roi.h + dst_roi.y;
        }
        ...
        WDT_BL_STREAM(setDstConfig, i, dst_roi.w, dst_roi.h, dst_buf.pitch, dst_buf.pitch_uv,
