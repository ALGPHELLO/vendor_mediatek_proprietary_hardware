...
enum {
    ...
    HWC_LAYER_TYPE_TMP        = 7,
    HWC_LAYER_TYPE_UIPQ_DEBUG = 8,
    HWC_LAYER_TYPE_UIPQ       = 9,
};
...
struct DispatcherJob
{
    DispatcherJob()
        ...
        , timestamp(0)
        , hw_layers_pass_max(1)
        , hw_layers_pass_now(0)
        , hw_layers_pass_cnt(1)
    ...
    // MULTIPASS
    int hw_layers_pass_max;
    int hw_layers_pass_now;
    int hw_layers_pass_cnt;
    HWLayer* hw_layers_multi[MAX_OVL_PASS];
    HWBuffer tmp_buf[MAX_OVL_PASS - 1];
...
public:
    ...
    // getSessionMode() is used to get display session mode
    int getSessionMode(int dpy);
