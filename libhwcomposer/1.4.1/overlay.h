...
struct OverlayPrepareParam
{
    ...
    int if_fence_fd;
    // special usage for MDP behaves as OVL in WFD extension mode
    bool secure;
};
...
public:
    ...
    status_t trigger(int present_fence_idx, DISP_DC_TYPE type = DISP_OUTPUT_UNKNOWN);
    ...
    OverlayEngine(int dpy, sp<IDevice> device);
    ~OverlayEngine();
