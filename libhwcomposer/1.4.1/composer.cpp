    ...
    DispatcherJob* job)
{
    ...
    // MULTIPASS: prepare input - prepare all pass input
    ...
    for (uint32_t i = 0; i < total_num; i++)
    {
        ...
        if (HWC_LAYER_TYPE_TMP == hw_layer->type)
        {
            ...
            {
                MULPASSLOGE("pre in", "tmp buffer is supposed to be without fence");
        ...
        // get release fence from display driver
        {
            ...
            hw_layer->fence_index = prepare_param.fence_index;
            // special usage for MDP behaves as OVL
            // replace ion_fd by new imported ion_fd
            ...
            MULPASSLOGV("pre in", "pass:%d, i:%d ,idx:%d, fd:%d", pass, i, prepare_param.fence_index, prepare_param.fence_fd);
        }
        memcpy(&hw_layer->layer, layer, sizeof(hwc_layer_1_t));
...
void ComposerHandler::process(DispatcherJob* job)
{
    ...
    int pass_now = job->hw_layers_pass_now;

    // wait until each layer is ready, then set to overlay
    for (i = 0; i < total_num; i++)
    {
        // MULTIPASS: wait acq fence for hw_layers_pass_now
        HWLayer* hw_layer = &job->hw_layers_multi[pass_now][i];
        ...
        // MULTIPASS: no need to wait acq fence of temp buffer
    ...
    for (i = 0; i < total_num; i++)
    {
        // MULTIPASS: fill overlay engine setting for hw_layers_pass_now
        HWLayer* hw_layer = &job->hw_layers_multi[pass_now][i];
        ...
        // MULTIPASS: fill overlay engine setting for temp buffer
        if (HWC_LAYER_TYPE_TMP == hw_layer->type)
        {
            OverlayPortParam* param = ovl_params[i];
            PrivateHandle* priv_handle = &hw_layer->priv_handle;
            Rect src_crop(priv_handle->width, priv_handle->height);
            ...
            param->protect      = false; //(priv_handle->usage & GRALLOC_USAGE_PROTECTED);
            param->secure       = false; //(priv_handle->usage & GRALLOC_USAGE_SECURE);
            param->alpha_enable = true;
            param->alpha        = 0xFF;
            param->blending     = HWC_BLENDING_PREMULT;
            param->dim          = false;
