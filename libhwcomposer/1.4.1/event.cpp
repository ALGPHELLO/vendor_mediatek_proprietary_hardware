...
void VSyncThread::initialize(bool force_sw_vsync, nsecs_t refresh)
{
    ...
    property_get("ro.sf.hwvsync.disable", value, "0");
    if (atoi(value))
    {
        HWC_LOGI("Force to disable hw vsync");
        m_fake_vsync = true;
    }
    else if (force_sw_vsync)
    {
        HWC_LOGI("Force to use sw vsync");
        m_fake_vsync = true;
    }

    if (refresh > 0) m_refresh = refresh;

    run(m_thread_name, PRIORITY_URGENT_DISPLAY);

    HWC_LOGW("HW VSync State(%d)", !m_fake_vsync);
}
...
bool VSyncThread::threadLoop()
{
    ...
    if (!use_fake_vsync)
    {
        ...
        int err = DispDevice::getInstance().waitVSync(m_disp_id, &next_vsync);
        if (err == NO_ERROR)
        {
#ifdef DEBUG_VSYNC_TIME
    ...
    if (use_fake_vsync)
    {
        ...
        m_next_fake_vsync = next_vsync + period;
...
void VSyncThread::setProperty()
{
    ...
    property_get("debug.hwc.period_io", value, "0");
    if (atoi(value))
    {
    ...
    property_get("debug.hwc.period_req", value, "0");
    if (atoi(value))
    {
...
void UEventThread::handleUevents(const char *buff, int len)
{
    ...
    if (!change_hdmi && !change_hdmi_res)
       return;
...
void UEventThread::setProperty()
{
    ...
    property_get("debug.hwc.test_hdmi_connect", value, "0");
