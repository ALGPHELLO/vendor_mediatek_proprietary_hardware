...
DispDevice::DispDevice()
{
    ...
    status_t err = setCapsInfo();
    if (NO_ERROR != err)
    {
        HWC_LOGE("Failed to get display device cap", strerror(err));
        abort();
    }
...
status_t DispDevice::setCapsInfo()
{
    // query device Capabilities
    int err = ioctl(m_dev_fd, DISP_IOCTL_GET_DISPLAY_CAPS, &m_caps_info);
    MULPASSLOGD("query cap", "mode:%d / pass:%d / num:%d / dispFea:%d", m_caps_info.output_mode,
                                                                        m_caps_info.output_pass,
                                                                        m_caps_info.max_layer_num,
                                                                        m_caps_info.disp_feature);

    if (NO_ERROR == err)
    {
        char value[PROPERTY_VALUE_MAX];
        property_get("debug.hwc.SinglePassOnly", value, "1");
        if (atoi(value) == 1)
        {
            MULPASSLOGD("query cap", "force single pass only");
            m_caps_info.output_pass = DISP_OUTPUT_CAP_SINGLE_PASS;
        }
    }

    if (DISP_OUTPUT_CAP_DIRECT_LINK == m_caps_info.output_mode)
        DevicePlatform::m_config.mirror_state = MIRROR_DISABLED;

    if (0 != (m_caps_info.disp_feature & DISP_FEATURE_TIME_SHARING))
        DevicePlatform::m_config.overlay_cap |= OVL_CAP_TIMESHARING;
...
status_t DispDevice::createOverlaySession(int dpy, int mode)
{
    ...
    if (DevicePlatform::m_config.alloc_sec_decouple_buf &&
        (HWC_DISPLAY_PRIMARY == dpy) &&
        (1 == HWCMediator::getInstance().m_features.svp) &&
        (DISP_FEATURE_TIME_SHARING == m_caps_info.disp_feature))
    {
        memset(m_decouple_buf, 0, sizeof(m_decouple_buf));
        disp_session_buf_info buf_info;
        disp_session_info info;
        buf_info.session_id = config.session_id;

        info.session_id = config.session_id;
        err = ioctl(m_dev_fd, DISP_IOCTL_GET_SESSION_INFO, &info);

        HWC_LOGI("Width:%d, Height:%d", info.displayWidth, info.displayHeight);
        for (int i = 0; i < 3; i++)
        {
            GrallocDevice::AllocParam param;
            param.width  = info.displayWidth;
            param.height = info.displayHeight;
            param.format = HAL_PIXEL_FORMAT_RGB_888;
            param.usage  = GRALLOC_USAGE_HW_COMPOSER | GRALLOC_USAGE_SECURE;

            // allocate
            if (NO_ERROR == GrallocDevice::getInstance().alloc(param))
            {
                m_decouple_buf[i] = param.handle;
                SECHAND sec_handle;
                err = gralloc_extra_query(param.handle, GRALLOC_EXTRA_GET_SECURE_HANDLE, &sec_handle);
                if (NO_ERROR != err)
                {
                    HWC_LOGE("decouple sbuf - query priv handle fail");
                }
                else
                {
                    buf_info.buf_hnd[i] = sec_handle;
                    HWC_LOGI("decouple sbuf (%d) - w:%d h:%d", i, param.width, param.height);
                }
            }
            else
            {
                HWC_LOGE("decouple sbuf - allocate buf fail");
            }
        }
        err = ioctl(m_dev_fd, DISP_IOCTL_INSERT_SESSION_BUFFERS, &buf_info);
        if (NO_ERROR != err)
        {
            HWC_LOGE("decouple sbuf - set buf info fail");
        }
    }

    m_input_config[dpy].session_id = config.session_id;
    m_session_mode[dpy] = (DISP_MODE)mode;
...
void DispDevice::destroyOverlaySession(int dpy)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        if (0 != m_decouple_buf[i])
        {
            GrallocDevice::getInstance().free(m_decouple_buf[i]);
        }
    }
...
status_t DispDevice::triggerOverlaySession(int dpy, int present_fence_idx, DISP_DC_TYPE /*type*/)
{
    int session_id = m_input_config[dpy].session_id;
    if (DISP_INVALID_SESSION == session_id)
    {
...
status_t DispDevice::setOverlaySessionMode(int dpy, int mode)
{
    int session_id = m_input_config[dpy].session_id;
    if (DISP_INVALID_SESSION == session_id)
    {
    ...
    HWC_LOGD("(%d) Set DispSessionMode (id=0x%x mode=%s)", dpy, session_id, toString((DISP_MODE)mode));

    return NO_ERROR;
}

int DispDevice::getOverlaySessionMode(int dpy)
{
    int session_id = m_input_config[dpy].session_id;
    if (DISP_INVALID_SESSION == session_id)
    {
    ...
    int dpy, OverlayPortParam* const* params, int num)
{
    ...
    for (int i = 0; i < m_ovl_input_num; i++)
    {
        ...
        if (params[i]->src_crop.getWidth() < 1 || params[i]->src_crop.getHeight() < 1)
        {
            input->layer_id      = i;
            input->layer_enable  = 0;
            input->next_buff_idx = params[i]->fence_index;

            HWC_LOGD("(%d:%d) Layer- (idx=%d,w/h=0) ", dpy, i, input->next_buff_idx);
            continue;
        }
        ...
        logger.printf("+%d,ion:%d,idx:%d/ ", i,params[i]->ion_fd, params[i]->fence_index);
...
void DispDevice::prepareOverlayOutput(int dpy, OverlayPrepareParam* param)
{
    ...
    buffer.index      = -1;
    buffer.fence_fd   = -1;
    MULPASSLOGV("pre out", "(D%d)(L%d)ion_fd:%d", dpy, buffer.layer_id, buffer.ion_fd);
    ...
    MULPASSLOGV("pre out", "fence_id:%d fence_fd:%d", buffer.index, buffer.fence_fd);
...
{
    // 0x534B4950 is a magic number to skip controlling fb.
    // Engineer mode will use setPowerMode to stop drawing buffer and power down panel by ioctl.
    // Therefore, we should not control fb in here.
    if (HWCMediator::getInstance().m_features.control_fb && mode != 0x534B4950)
    {
        AUTO_WDT(1000);
        char filename[32] = {0};
