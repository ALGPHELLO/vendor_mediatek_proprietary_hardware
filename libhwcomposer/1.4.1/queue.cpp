    ...
    , m_listener(NULL)
{
    ...
    else
    {
        ...
        m_client_name = String8::format("%s", (m_queue_type == QUEUE_TYPE_BLT) ?
                                                "blt" : "ovl");
#endif
        QLOGI("Buffer(%p) queue is created with size(%d)", this, m_buffer_count);
    }
}

DisplayBufferQueue::~DisplayBufferQueue()
{
    QLOGI("Buffer(%p) queue is destroyed", this);
    ...
    for (int i = 0; i < m_buffer_count; i++)
    {
        ...
        if (slot->release_fence != -1) ::close(slot->release_fence);
...
status_t DisplayBufferQueue::reallocate(int idx)
{
    ...
    // allocate new buffer
    {
...

        {
            ...
            slot->inval_roi = Rect(0, 0, m_buffer_param.width, m_buffer_param.height);
        ...
        status_t err = getPrivateHandle(slot->out_handle, &priv_handle);
        if (NO_ERROR != err)
        {
            GrallocDevice::getInstance().free(slot->out_handle);
            slot->out_handle = NULL;
    ...
    DisplayBuffer* buffer, bool async, bool is_secure)
{
    ...
    buffer->index                = idx;
    buffer->inval_roi            = m_slots[idx].inval_roi;
...
status_t DisplayBufferQueue::queueBuffer(DisplayBuffer* buffer)
{
...

    {
        ...
        m_slots[idx].inval_roi            = buffer->inval_roi;
...
void DisplayBufferQueue::dumpLocked(int idx)
{
    if (idx < 0 || idx >= m_buffer_count)
    {
        QLOGE("Failed to dump buffer with invalid index(%d)", idx);
        return;
    }

    if (m_slots[idx].out_handle == NULL)
    {
        QLOGE("Failed to dump buffer(%d) with invalid source", idx);
        return;
    }
    ...
    {
        case QUEUE_TYPE_BLT:
            file_name = String8::format("q_blt_");
            break;

        case QUEUE_TYPE_OVL:
            file_name = String8::format("q_ovl_");
            break;

        default:
            file_name = String8::format("q_none_");
            break;
    };

    getGraphicBufferUtil().dump(
        m_slots[idx].out_handle, file_name.string());
}
