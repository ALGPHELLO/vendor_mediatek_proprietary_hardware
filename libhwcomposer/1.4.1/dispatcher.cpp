...
DispatcherJob* HWCDispatcher::getJob(int dpy)
{
    ...
    DispatcherJob* job = NULL;
    {
        ...
        else
        {
            ...
            // MULTIPASS: allocate hw_layers_multi
            DISP_CAP_OUTPUT_PASS query_pass = DispDevice::getInstance().getCapabilitiesPass();
            job->hw_layers_pass_max = (DISP_OUTPUT_CAP_SINGLE_PASS == query_pass) ? 1 : MAX_OVL_PASS;
            for (int i = 0; i < job->hw_layers_pass_max; i++)
                job->hw_layers_multi[i] = (HWLayer*)calloc(job->num_layers, sizeof(HWLayer));
            job->hw_layers = job->hw_layers_multi[0];
        }
...
void HWCDispatcher::setSessionMode(int dpy, bool mirrored)
{
    ...
    // get previous and current session mode
    {
        ...
        if (mirrored)
        {
            curr_session_mode = DISP_SESSION_DECOUPLE_MIRROR_MODE;
        }
        else
        {
            DisplayData* disp_data = &DisplayManager::getInstance().m_data[HWC_DISPLAY_VIRTUAL];
            if (disp_data->connected && disp_data->timeSharing)
            {
                curr_session_mode = DISP_SESSION_DECOUPLE_MODE;
            }
            else
            {
                DISP_CAP_OUTPUT_MODE query_mode = DispDevice::getInstance().getCapabilitiesMode();
                DISP_CAP_OUTPUT_PASS query_pass = DispDevice::getInstance().getCapabilitiesPass();

                curr_session_mode = (DISP_OUTPUT_CAP_DECOUPLE == query_mode)
                                  ? DISP_SESSION_DECOUPLE_MODE
                                  : DISP_SESSION_DIRECT_LINK_MODE;

                if ((DISP_OUTPUT_CAP_SWITCHABLE == query_mode)
                        && (DISP_OUTPUT_CAP_SINGLE_PASS != query_pass))
                {
                    if (m_curr_jobs[dpy]!= NULL)
                        if (m_curr_jobs[dpy]->hw_layers_pass_cnt != 1)
                            curr_session_mode = DISP_SESSION_DECOUPLE_MODE;
                }
            }
        }
...
int HWCDispatcher::getSessionMode(int dpy)
{
    DISP_MODE session_mode = DISP_INVALID_SESSION_MODE;
    ...
    // get current session mode
    {
        ...
        if (m_workers[dpy].enable && m_workers[dpy].ovl_engine != NULL)
        {
            session_mode = m_workers[dpy].ovl_engine->getOverlaySessionMode();
        }
    }

    return (int)session_mode;
}
    ...
    int dpy, struct hwc_display_contents_1* list)
{
...

    {
        ...
        // and WFD didn't use MDP instead of OVL
            ...
            (HWC_MIRROR_SOURCE_INVALID == job->disp_mir_id)) // extension mode
        {
            ...
            if (NULL != mir_job)
            {
                // record main display orientation in disp_mir_rot for MHL/WFD extension mode
                job->disp_mir_rot = mir_job->disp_ori_rot;
                if (job->disp_ori_rot != mir_job->disp_ori_rot)
                {
                    HWC_LOGD("change dpy(%d) ori %d->%d", job->disp_ori_id,
                        job->disp_ori_rot, mir_job->disp_ori_rot);
                }
            }
        }
        ...
        if (job->fbt_exist)
        {
            ...
            {
                int final_pass = job->hw_layers_pass_cnt - 1;
                for (int idx = 0; idx < job->num_layers; idx++)
                {
                    // MULTIPASS: check fbt_exist in the final pass
                    HWLayer* hw_layer = &job->hw_layers_multi[final_pass][idx];
                    if (HWC_LAYER_TYPE_FBT == hw_layer->type)
                    {
                        hw_layer->enable = false;
#ifdef MTK_HWC_PROFILING
                        hw_layer->fbt_input_layers = 0;
                        hw_layer->fbt_input_bytes  = 0;
#endif
                        job->fbt_exist = false;
        ...
        HWC_ATRACE_JOB("set", job->sequence, job->fbt_exist, job->num_ui_layers, job->num_mm_layers);

        DbgLogger* logger = Debugger::getInstance().m_logger->set_info[dpy];

        logger->printf("(%d) SET job=%d/max=%d/fbt=%d/ui=%d/mm=%d/mir=%d",
            dpy, job->sequence, job->num_layers, job->fbt_exist, job->num_ui_layers,
...
void HWCDispatcher::onPlugIn(int dpy)
{
...

    {
        ...
        if ((HWC_DISPLAY_VIRTUAL == dpy) &&
            (HWC_DISPLAY_WIRELESS == DisplayManager::getInstance().m_data[dpy].subtype) &&
            (0 != (DevicePlatform::m_config.overlay_cap & OVL_CAP_TIMESHARING)) &&
            (m_workers[HWC_DISPLAY_PRIMARY].ovl_engine->isEnable()))
        {
            HWC_LOGD("TS Eable");
            DisplayManager::getInstance().m_data[dpy].timeSharing = true;
        }

        if ((HWC_DISPLAY_VIRTUAL != dpy) ||
            (!HWCMediator::getInstance().m_features.copyvds) ||
            (DisplayManager::getInstance().m_data[dpy].timeSharing))
        {
            device = &(DispDevice::getInstance());
        }
        else
        {
            device = new BlitDevice();
        }

        m_workers[dpy].ovl_engine = new OverlayEngine(dpy, device);
        ...
        if (HWC_DISPLAY_PRIMARY < dpy)
        {
                               ...
                               HAL_PIXEL_FORMAT_RGBA_8888 : HAL_PIXEL_FORMAT_YUYV;
            {
                ...
                if (0 != atoi(value))
                {
                    HWC_LOGI("force RGB output format!!");
...
bool DispatchThread::threadLoop()
{
    ...
    while (1)
    {
...

        {
            ...
            if( m_job_queue.size() > 5)
            {
                HWC_LOGW("(%d) Job is late(%d)...", m_disp_id, m_job_queue.size());
            }
        }
        ...
        // if yes, there is no need to wait for next vsync for handling next composition
        {
            ...
            if (m_disp_id < HWC_DISPLAY_VIRTUAL && (need_check_next_period || !m_vsync_signaled))
            {
                ...
                {
                    char atrace_tag[128];
                    sprintf(atrace_tag, "dispatcher_wait(%d)\n", job->sequence);
        ...
        // 4. clear used job
        {
            ...
            HWC_LOGD("(%d) Handle job seq=%d", m_disp_id, job->sequence);
            ...
            if (job != NULL)
            {
                // MULTIPASS: free hw_layers_multi
                for (int i = 0; i < job->hw_layers_pass_max; i++)
                    free(job->hw_layers_multi[i]);
                delete job;
            }
        }
...
void DispatchThread::waitNextVSyncLocked(int dpy)
{
    ...
    if (m_vsync_cond.waitRelative(m_vsync_lock, ms2ns(18)) == TIMED_OUT)
    {
    ...
    struct hwc_display_contents_1* list, DispatcherJob* job)
{
    ...
    if (is_dirty)
    {
        // MULTIPASS: scan all pass
        for (int pass = 0; pass < job->hw_layers_pass_cnt; pass++)
        for (uint32_t i = 0; i < total_num; i++)
        {
            HWLayer* hw_layer = &job->hw_layers_multi[pass][i];
            ...
            if (HWC_LAYER_TYPE_TMP == hw_layer->type) continue;
        ...
        // ---------------------------------------------------------------
        // MULTIPASS: prepare for output temp buffer
        // ---------------------------------------------------------------
        for (int pass = 0; pass < job->hw_layers_pass_cnt - 1; pass++)
        {
            HWBuffer* hw_outbuf = &job->tmp_buf[pass];
            PrivateHandle* priv_handle = &hw_outbuf->priv_handle;
            OverlayPrepareParam prepare_param;
            {
                MULPASSLOGV("pre out", "pass:%d hand:%d ion_fd:%d", pass, hw_outbuf->handle, priv_handle->ion_fd);
                prepare_param.ion_fd        = priv_handle->ion_fd;
                prepare_param.is_need_flush = 0;

                int err = m_ovl_engine->prepareOutput(prepare_param);
                if (NO_ERROR != err)
                {
            ...
            if (prepare_param.fence_fd != -1)
            {
                MULPASSLOGE("pre out", "tmp buffer is supposed to be without fence");
                close(prepare_param.fence_fd);
                prepare_param.fence_fd = -1;
            }

            hw_outbuf->out_retire_fence_fd  = prepare_param.fence_fd;
            hw_outbuf->out_retire_fence_idx = prepare_param.fence_index;
            MULPASSLOGV("pre out", "fd:%d, idx:%d", prepare_param.fence_fd, prepare_param.fence_index);
        }
        // ---------------------------------------------------------------
    ...
    if (is_dirty)
    {
        ...
        if (DevicePlatform::m_config.overlay_cap & OVL_CAP_P_FENCE)
        {
...
void HWCDispatcher::PhyPostHandler::process(DispatcherJob* job)
{
    ...
    {
        // MULTIPASS: disable input unused layers for hw_layers_pass_now
        if (!job->hw_layers_multi[job->hw_layers_pass_now][i].enable)
    ...
    // ---------------------------------------------------------------
    // MULTIPASS: set output tmp buffer
    // ---------------------------------------------------------------
    int pass = job->hw_layers_pass_now;

    if (pass != job->hw_layers_pass_cnt - 1)
    {
        HWBuffer* hw_outbuf = &job->tmp_buf[pass];
        PrivateHandle* priv_handle = &hw_outbuf->priv_handle;
        ...
        MULPASSLOGV("set out", "fenceID:%d", param.fence_index);
        m_ovl_engine->setOutput(&param);

        // trigger overlay engine
        m_ovl_engine->trigger(DISP_NO_PRESENT_FENCE, DISP_OUTPUT_MEMORY);
        return;
    }
    // ------------------------------------------------------
    ...
    struct hwc_display_contents_1* list, DispatcherJob* job)
{
    ...
    else
    {
        ...
        OverlayPrepareParam prepare_param;
        {
            ...
            prepare_param.secure        = (priv_handle->usage & GRALLOC_USAGE_SECURE);
            ...
            // special usage for MDP behaves as OVL
            // replace ion_fd by new imported iob_fd
            if (priv_handle->ion_fd != prepare_param.ion_fd)
                priv_handle->ion_fd = prepare_param.ion_fd;
        }
...
void HWCDispatcher::VirPostHandler::process(DispatcherJob* job)
{
    if (HWC_MIRROR_SOURCE_INVALID == job->disp_mir_id)
    {
        ...
        for (int i = 0; i < job->num_layers; i++)
        {
            // MULTIPASS: vir display
            if (!job->hw_layers_multi[0][i].enable)
                m_ovl_engine->disableInput(i);
        }
