...
BlitDevice::BlitDevice()
    ...
    , m_blit_stream(new DpBlitStream())
    , m_session_id(BLIT_INVALID_SESSION)
    , m_cur_params(new InvalidParams())
...
status_t BlitDevice::createOverlaySession(int dpy, int mode)
{
...
status_t BlitDevice::triggerOverlaySession(int dpy, int /*present_fence_idx*/, DISP_DC_TYPE /*type*/)
{
    checkValidDpyRetVal(dpy, INVALID_OPERATION);
    ...
    else
    {
        ...
        if (OVL_IN_PARAM_ENABLE == m_state)
...
status_t BlitDevice::setOverlaySessionMode(int dpy, int mode)
{
    checkValidDpyRetVal(dpy, INVALID_OPERATION);

    checkValidSessionRetVal(dpy, m_session_id, INVALID_OPERATION);

    HWC_LOGD("(%d) Set BlitSessionMode (id=0x%x mode=%d)", dpy, m_session_id, mode);

    return NO_ERROR;
}

int BlitDevice::getOverlaySessionMode(int dpy)
{
    ...
    int dpy, OverlayPrepareParam* param)
{
    ...
    else
    {
        ...
        IONDevice::getInstance().ionImport(&param->ion_fd);
        {
            AutoMutex l(m_vector_lock);
...
void BlitDevice::enableOverlayInput(
