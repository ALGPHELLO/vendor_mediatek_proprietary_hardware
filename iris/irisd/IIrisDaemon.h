...
class IIrisDaemon : public IInterface, public IBinder::DeathRecipient {
    public:
        enum {
           ...
           POST_ENROLL = IBinder::FIRST_CALL_TRANSACTION + 5,
           REMOVE = IBinder::FIRST_CALL_TRANSACTION + 6,
           GET_AUTHENTICATOR_ID = IBinder::FIRST_CALL_TRANSACTION + 7,
           SET_ACTIVE_GROUP = IBinder::FIRST_CALL_TRANSACTION + 8,
           SET_PREVIEW_QUEUE = IBinder::FIRST_CALL_TRANSACTION + 9,
           OPEN_HAL = IBinder::FIRST_CALL_TRANSACTION + 10,
           CLOSE_HAL = IBinder::FIRST_CALL_TRANSACTION + 11,
           INIT = IBinder::FIRST_CALL_TRANSACTION + 12,
           CAPTURE = IBinder::FIRST_CALL_TRANSACTION + 13,
           CANCEL_CAPTURE = IBinder::FIRST_CALL_TRANSACTION + 14,
        };
        ...
        virtual int32_t capture(const uint8_t *pPidData, ssize_t pidLen, int32_t pidType,
                int32_t bioType, const uint8_t *pCertChain, ssize_t certLen, int32_t groupId) = 0;
        virtual int32_t stopCapture() = 0;
        virtual int32_t remove(int32_t irisId, int32_t groupId) = 0;
        virtual uint64_t getAuthenticatorId() = 0;
        virtual int32_t setActiveGroup(int32_t groupId, const uint8_t *pPath, ssize_t pathLen) = 0;
        virtual int32_t setPreviewSurface(int32_t groupId, const sp<Surface>& surface) = 0;
...
class BnIrisDaemon: public BnInterface<IIrisDaemon> {
    public:
