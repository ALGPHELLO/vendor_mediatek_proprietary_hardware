...
void hal2_geofence_recover_geofences() {
    ...
    for(i = 0; i < MAX_GOEFENCE; i++) {
        if(geofence_property[i].alive == 1) {
            ...
            LOGE("FLP HAL RECOVER: id=%ld,type=%d,%d,%d,%lf,%d",geofence_property[i].geofence_id,geofence_property[i].last_transition,geofence_property[i].monitor_transition,
...
int mnl2gfchal_hdlr (char *buff) {
    ...
    len = get_binary(buff, &offset, data);
    if((len > 0) && (len<=1024)) {
