...
void mtk_hal_geofence_init(GpsGeofenceCallbacks* callbacks) {
    hal2_geofence_init();
    LOGD("init geofence, cb function : %x",callbacks);
...
void mtk_geofence_transition_callbacks_proc(MTK_FLP_MSG_T *prmsg) {
    ...
    LOGD("geo transition: id=%ld, transition=%ld, latlon=%lf,%lf",geofence_id,transition,location.latitude,location.longitude);
