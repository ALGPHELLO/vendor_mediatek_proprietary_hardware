...
static int insmod_by_path(char *name_buf, char * module_path, char *prefix, char *postfix )
{
    ...
    strncat(name_buf, module_path, strlen(module_path));
    strncat(name_buf, prefix, strlen(prefix));
    strncat(name_buf, postfix, strlen(postfix));
    strncat(name_buf, WMT_MODULES_SUFF, strlen(WMT_MODULES_SUFF));
