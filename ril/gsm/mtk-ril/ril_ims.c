...
void imsInit(RILSubSystemId subsystem, RIL_SOCKET_ID rid) {
    ...
    property_get("persist.mtk_ct_volte_support", ctVolteSupport, "0");
    if (atoi(ctVolteSupport) == 1) {
        at_send_command("AT+ECFGSET=\"mtk_ct_volte_support\",\"1\"", NULL, getDefaultChannelCtx(rid));
        RLOGD("VoLTE over IMS is support and AT+ECFGSET=...,1 is sent");
    } else {
        at_send_command("AT+ECFGSET=\"mtk_ct_volte_support\",\"0\"", NULL, getDefaultChannelCtx(rid));
    ...
    if (isImsSupport()) {
        at_send_command("AT+EIMSSMS=1", NULL, getDefaultChannelCtx(rid));
        RLOGD("SMS over IMS is support and AT+EIMSSMS=1 is sent");
        at_send_command("AT+EIMSESS=1", NULL, getDefaultChannelCtx(rid));
        RLOGD("Enable IMS ECC support capability report");
    } else {
        at_send_command("AT+EIMSSMS=0", NULL, getDefaultChannelCtx(rid));
    ...
    if (isImsAndVolteSupport() || isWfcSupport()) {
        ...
        if (((atoi(volteEnable) & (1 << checkingBit)) == 0) &&
            ...
            at_send_command("AT+EIMSVOLTE=0", NULL, getDefaultChannelCtx(rid));
            at_send_command("AT+EIMSWFC=0", NULL, getDefaultChannelCtx(rid));
            at_send_command("AT+EIMSVOICE=0", NULL, getDefaultChannelCtx(rid));
            at_send_command("AT+EIMS=0", NULL, getDefaultChannelCtx(rid));
            at_send_command("AT+EVADSREP=0", NULL, getDefaultChannelCtx(rid));
            RLOGD("volte setting and wfc setting are both disable");
        } else {
            if ((atoi(volteEnable) & (1 << checkingBit)) > 0) {
                at_send_command("AT+EIMSVOLTE=1", NULL, getDefaultChannelCtx(rid));
            ...
            at_send_command("AT+EIMSVOICE=1", NULL, getDefaultChannelCtx(rid));
            at_send_command("AT+EIMS=1", NULL, getDefaultChannelCtx(rid));
            at_send_command("AT+EVADSREP=1", NULL, getDefaultChannelCtx(rid));
    ...
    if (isRcsUaProxySupport()) {
        ...
        at_send_command(tempStr, NULL, getDefaultChannelCtx(getMainProtocolRid()));
    }
}
...
extern int rilImsUnsolicited(const char *s, const char *sms_pdu, RILChannelCtx* p_channel)
{
    ...
    if (isImsSupport()) {
       if (strStartsWith(s, "+EIMS: 0")) {
           ...
           LOGD("EIMSESS URC:%s",s);
           onImsEccUpdated((char *)s, rid);
           return 1;
       }
   }
   return 0;
...
void requestSetImsVoiceEnabled(void * data, size_t datalen, RIL_Token t)
{
    UNUSED(datalen);
    char * cmd;
    ATResponse *p_response = NULL;
    int err;
    int enable = ((int *)data)[0];
    LOGD(" requestSetImsVoiceEnabled enable:%d", enable);
    asprintf(&cmd, "AT+EIMSVOICE=%d", enable);
    err = at_send_command(cmd, &p_response, OEM_CHANNEL_CTX);
...
void onImsRtpReport(char *urc, const RIL_SOCKET_ID rid) {
    ...
    RIL_UNSOL_RESPONSE(RIL_UNSOL_IMS_RTP_INFO, data, 5 * sizeof(char *), rid);
    return;
error:
