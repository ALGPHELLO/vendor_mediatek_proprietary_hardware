...
void requestModemPowerOn(void * data __unused, size_t datalen __unused, RIL_Token t) {
    RILChannelCtx* p_channel = getRILChannelCtx(RIL_SIM, getMainProtocolRid());
    ...
    LOGD("SET ril.ipo.radiooff to 0");

    // isDualTalkMode return 0 in c2k project after revert isEVDODTSupport.
    if (!isDualTalkMode() && !isCdmaSupport()) {
        property_set("ril.ipo.radiooff","0");
    }else{
        for (int i = 0; i < SIM_COUNT; i++) {
            upadteSystemPropertyByCurrentMode(i,"ril.ipo.radiooff","ril.ipo.radiooff.2","0");
        }
    }
    if (isCCCIPowerOffModem()) {
        ...
        if (getMdMaxSimCount() >= 2) {
            if (RIL_queryMyChannelId(t) >= RIL_CHANNEL_OFFSET)
                p_channel = getDefaultChannelCtx(RIL_SOCKET_1);
        }
...
void requestModemPowerOff(void * data __unused, size_t datalen __unused, RIL_Token t) {
    ...
    if (s_enhanceModemOffVersion == 1 && isCdmaSupport()) {
        char c2kPowerOffSync[PROPERTY_VALUE_MAX] = { 0 };
        int c2kPowerOffSyncVal = 0;
        property_get("ril.cdma.offsync", c2kPowerOffSync, "0");
        ...
        LOGE("no need to sync ril.cdma.offsync");
    }
    ...
    LOGD("requestRadioPowerOff SET ril.ipo.radiooff to -1");
    // isDualTalkMode return 0 in c2k project after revert isEVDODTSupport. @{
    if (!isDualTalkMode() && !isCdmaSupport()) {
        property_set("ril.ipo.radiooff","-1");
    }else{
        for (int i = 0; i < SIM_COUNT; i++) {
            upadteSystemPropertyByCurrentMode(i,"ril.ipo.radiooff","ril.ipo.radiooff.2","-1");
        }
    }
    ...
    if (s_enhanceModemOffVersion == 1) {
        ...
        property_set("ril.cdma.offsync", "0");
        RLOGD("requestRadioPowerOff set ril.cdma.emdstatus.send to 0");
    }
...
void requestConfigModemStatus(void *data, size_t datalen, RIL_Token t)
{
    ...
    if ((modemStatus < 0 || modemStatus > 2) || (remoteSimProtocol < 0 || remoteSimProtocol > 2)) {
        ...
        if (isSvlteSupport()) {
            //SVLTE need to send to both slot of GSM.
            p_channel = getRILChannelCtxFromToken(t);
        } else {
            // [RIL_CC]Align with mtk_ril_commands.h -> RIL_CMD_PROXY_2
            p_channel = getRILChannelCtx(RIL_SIM, getMainProtocolRid());
        }
        ...
        if(err >= 0) {
            ...
            if ( !(err < 0 || p_response->success == 0) ) {
                ...
                property_set("ril.cdma.emdstatus.protocol", lastProtocol);
                LOGD("ril.cdma.emdstatus.protocol set to %d,%d", remoteSimProtocol, cdmaslot);
                free(lastProtocol);
            }
        }
        if (cmd != NULL) {
...
void requestModeSwitchSetTrm(void * data, size_t datalen, RIL_Token t) {
    ...
    RILChannelCtx* p_channel = getRILChannelCtxFromToken(t);
    if (getMdMaxSimCount()>= 2) {
        p_channel = getRILChannelCtx(RIL_OEM, getMainProtocolRid());
    }
...
RIL_Errno resetRadioForSvlte() {
    if (getSimCount() >= 2) {
        ...
        if (s_enhanceModemOffVersion == 1 && isCdmaSupport()) {
            ...
            RLOGI("%s: set ril.cdma.emdstatus.send to 0, cdma.ril.eboot to 1", __FUNCTION__);
        }
...
static void *mdStatusReaderLoop(void* params __unused) {
    LOGD("mdStatusReaderLoop - md status working thread");
    int count = -1;
    ...
    */
    ...
    LOGD("mdStatusReaderLoop: fd = %d", sMdStatusFd);
    getResetLock();
    ...
    do {
        ...
        LOGE("mdStatusReaderLoop: count: %d", count);
        LOGE("mdStatusReaderLoop: modem id: %d, event type: %d, reason: %s",
        ...
        if (MD_STA_EV_RESET_REQUEST == buffer->event_type
            ...
            LOGE("mdStatusReaderLoop: RESET/STOP/ENTER_FLIGHT, break");
            break;
        }
    } while (1);
    ...
    if (MD_STA_EV_RESET_REQUEST == buffer->event_type) {
        ...
        RIL_UNSOL_RESPONSE(RIL_UNSOL_MODEM_RESTART,
                buffer->reason, sizeof(buffer->reason), RIL_SOCKET_1);
        sleep(1);
    }
    releaseResetLock();
    free(buffer);
