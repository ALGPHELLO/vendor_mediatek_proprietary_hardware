...
int at_send_command_raw (const char *command,
                         ATResponse **pp_outResponse,
                         RILChannelCtx * p_channel);
int at_send_command_raw_ack (const char *command,
                         ATResponse **pp_outResponse,
                         RILChannelCtx * p_channel,
                         RIL_Token ackToken);
...
typedef enum {
    ...
    CME_PROVISION_NO_DEFAULT_VALUE = 604,
    ...
    // [Network][C2K] Add new fail cause for ERAT fail in Call.
    CME_ERAT_NOT_ALLOWED_CONFLICT_WITH_CALL = 4119
} AT_CME_Error;
