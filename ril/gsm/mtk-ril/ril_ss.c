        ...
        RIL_Token t, CallForwardOperationE op)
{
    ...
    if (CCFC_E_QUERY == op) {
        ...
        for (p_cur = p_response->p_intermediates; p_cur != NULL; p_cur = p_cur->p_next) {
            ...
            if (checkUserLoad() == 1) {
                LOGD("CfInfoResponse status:%d class:%d toa:%d time:%d",
...
static void onUssd(char *s, RIL_SOCKET_ID rid)
{
    ...
    /* For O version, ril_service only accepts response length == 2 */
...
/// M: Send USSI request AT command @{
static void requestSendUSSI(void * data, size_t datalen, RIL_Token t)
{
    const char** strings = (const char**)data;
    ...
    if (p_ussdRequest == NULL) {
        LOGE("requestSendUSSI:p_ussdRequest malloc fail");
        goto error;
    }
...
static void onCfuNotify(char *s, RIL_SOCKET_ID rid)
{
    ...
    RIL_UNSOL_RESPONSE (RIL_UNSOL_CALL_FORWARDING, response, 2 * sizeof(int *), rid);
    return;
    ...
    if (strStartsWith(s, "+ECMCCSS:")) {
       ...
       if (at_tok_start(&line) < 0) {
        ...
        if (checkUserLoad() != 1) {
            ...
            LOGD("[onLteSuppSvcNotification] \
                    svcNotify.notificationType = %d, svcNotify.code = %d",
    ...
    while ((begin != NULL) && (end != NULL)) {
        ...
        if ((begin != NULL) && (end != NULL)) {
            ...
            if ((len > 0) && (len < 256)) {
                ...
                LOGD("[retrieveCallForwardedToNumber]number = %s", number);
            }
            begin = end + 1;
        }
    }
...
extern int rilSsMain(int request, void *data, size_t datalen, RIL_Token t)
{
    switch (request)
    {
        ...
        case RIL_REQUEST_SEND_USSI:
            requestSendUSSI(data,datalen,t);
        break;
