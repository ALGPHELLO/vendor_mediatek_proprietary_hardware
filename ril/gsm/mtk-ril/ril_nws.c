...
void resetRegSuspend() {
    int simCount = getSimCount();
    for (int i = 0; i < simCount; i++) {
        TimedCallbackParam* mParam = (TimedCallbackParam*)malloc(sizeof(TimedCallbackParam));
        assert(mParam != NULL);
        mParam->rid = RIL_SOCKET_1 + i;
        mParam->urc = NULL;
        RIL_requestProxyTimedCallback(onResetRegSuspend, mParam, &TIMEVAL_0,
                getRILChannelCtx(RIL_SIM, RIL_SOCKET_1 + i)->id, "onResetRegSuspend");
    }
}

void onResetRegSuspend(void *param) {
    TimedCallbackParam* pParam = (TimedCallbackParam*)param;
    char* urc = pParam->urc;
    RIL_SOCKET_ID rid = (RIL_SOCKET_ID)(pParam->rid);
    char * cmd1;
    char * cmd2;
    int err;
    ...
    err = at_send_command(cmd1, &p_response, getChannelCtxbyProxy(rid));
    free(cmd1);
    ...
    if (err < 0 || p_response->success == 0) {
        ...
        err = at_send_command(cmd2, &p_response, getChannelCtxbyProxy(rid));
        free(cmd2);
