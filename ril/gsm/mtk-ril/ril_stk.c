...
bool isChipTestMode_Stk();
char* composeChipModeTR(char* cmd);
void sendChipModeTR(void *param);
// for BT_SAP
...
void resetAllStkServiceRunningFlagDelayed(RIL_SOCKET_ID rid) {
    ...
    RIL_requestProxyTimedCallback(resetAllStkServiceRunningFlag, NULL, &TIMEVAL_2S,
        getChannelCtxbyProxy(rid)->id, "resetAllStkServiceRunningFlag");
}
...
void sendTerminalResponseForSimPlugOut() {
    ...
    err = at_send_command(cmd, &p_response, getChannelCtxbyProxy(rid_menu));
    free(cmd);
...
void freeStkQueuedProactivCmdByRid(RIL_SOCKET_ID rid) {
    ...
    if (NULL != pProactive_cmd[simId]) {
        ...
        RIL_requestProxyTimedCallback(sendTerminalResponseForSimPlugOut, NULL, &TIMEVAL_0S,
                                          getChannelCtxbyProxy(rid)->id, "sendTerminalResponseForSimPlugOut");
    }
    freeProactiveCmd(simId);
}
...
void onStkAtSendSmsFromUrc(void *param)
{
    ...
    at_send_command("AT+STKSMS=0", NULL, getChannelCtxbyProxy(g_sms_at[rid].rid));
}
    ...
    switch(urc[9]) { // t point to cmd_deatil tag t[9] mean refresh type
        case '0':
            ...
            RIL_requestProxyTimedCallback(resetRefreshCount, NULL, &TIMEVAL_60S,
                getChannelCtxbyProxy(rid)->id, "resetRefreshCount");
            /// @}
            break;
        case '1':
...
void onStkProactiveCommand(char* urc, RILChannelCtx* p_channel)
{
    ...
    if (!isChipTestMode_Stk() && false == isStkServiceRunning) {
    ...
    if (isChipTestMode_Stk()) {
        LOGD("ChipTestMode ProactiveCommand: %s", &(temp_str[0]));
        char* chipModeTerminalResponseString;
        chipModeTerminalResponseString = composeChipModeTR(&(temp_str[0]));
        LOGD("ChipTestMode: TR: %s", chipModeTerminalResponseString);
        g_stk_at.rid = rid;
        RIL_requestProxyTimedCallback(sendChipModeTR,
                chipModeTerminalResponseString, &TIMEVAL_0,
                getChannelCtxbyProxy(rid)->id, "sendChipModeTR");
        return;
    }
...
char* composeChipModeTR(char* cmd) {
    LOGD("composeChipModeTR()");

    // Terminal currently unable to process command (0x20)
    char terminalResponseString[64] = "810301400182028281830120";
    ...
    if (cmd != NULL) {
        ...
        // append BEARER_DESCRIPTION if found, for OPEN CHANNEL
        ...
        // append BUFFER_SIZE if found, for OPEN CHANNEL
        ...
        returnString = (char*) calloc(1, stringLength+1);
        if (returnString != NULL) {
            memcpy(returnString, terminalResponseString, stringLength+1);
        }
    }
...
void sendChipModeTR(void *param) {
    ...
    ATResponse *p_response = NULL;
    int err;
    char* chipModeTerminalResponseString = (char*) param;

    asprintf(&cmd, "AT+STKTR=\"%s\"", chipModeTerminalResponseString);

    err = at_send_command(cmd, &p_response, getChannelCtxbyProxy(g_stk_at.rid));
    free(cmd);
    free(chipModeTerminalResponseString);
