...
void requestAddImsConferenceCallMember(void *data, RIL_Token t) {
    ...
    */
    ...
    LOGD("[requestAddImsConferenceCallMember]%d, %s, %d", confCallId, strings[1], callIdToAdd);
...
void requestRemoveImsConferenceCallMember(void *data, RIL_Token t) {
    ...
    */
    ...
    LOGD("[requestRemoveImsConferenceCallMember]%d, %s, %d", confCallId, strings[1], callIdToRemove);
...
void requestImsEct(void *data, size_t datalen, RIL_Token t) {
    ...
    LOGD("[requestImsEct] ect type = %s, target number = %s", type, number);
...
void requestPullCall(void *data, RIL_Token t) {
    ...
    LOGD("[requestPullCall] target uri = %s, call mode = %s", targetUri, callMode);
