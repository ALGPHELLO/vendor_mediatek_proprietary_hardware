                  ...
                  size_t *out_len)
{
    ...
    *pos = '\0';
    if (out_len) {
        *out_len = pos - out;
        RLOGD("[base64] encode olen %zu, out %s, %zu", olen, out, (*out_len));
    } else {
        RLOGD("[base64] encode olen %zu, out %s", olen, out);
    }
    return out;
}
                  ...
                  size_t *out_len)
{
    ...
    *out_len = pos - out;
    RLOGD("[base64] decode olen %zu, out %s, %zu", olen, out, (*out_len));
    return out;
}
...
unsigned char* byteArrayToHexStringB64(unsigned char* array,int length)
{
    ...
    if (NULL == buf) {
        RLOGE("[byteArrayToHexStringB64] OOM");
        return NULL;
    }
    for (i = 0 ; i < length; i++)
    {
...
unsigned char * base64_decode_to_str(const unsigned char *src, size_t len)
{
    ...
    RLOGD("[base64] decode src %s, %zu", src, len);
    out = base64_decode(src, len, &out_len);
    if (!out) {
        return NULL;
    }

    out_str = byteArrayToHexStringB64((unsigned char *)out, out_len);
    free((void*)out);
    return out_str;
}
...
int hexStringToByteArrayB64(unsigned char* hexString, unsigned char ** byte)
{
    ...
    if (NULL == buffer) {
        RLOGE("[hexStringToByteArrayB64] OOM");
        return -1;
    }
...
unsigned char * base64_encode_to_str(const unsigned char *src, size_t len)
{
    ...
    RLOGD("[base64] encode, src %s, %zu", src, len);

    out_len = hexStringToByteArrayB64((unsigned char *)src, (unsigned char **)&out);

    if (-1 == out_len) {
        return NULL;
    }
    RLOGD("[base64] byte array %s, %zu", out, out_len);
    ...
    free((void*)out);
    RLOGD("[base64] encode done %s, %zu", out_str, encode_len);
    return out_str;
}
