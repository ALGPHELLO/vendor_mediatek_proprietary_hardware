...
bool isHvolteDisable() {
    ...
    property_get("persist.embms.hvolte.disable", prop, "0");
...
bool isRjilSupport() {
    ...
    property_get("persist.operator.optr", property_value, "0");
...
bool isAtCmdEnableSupport() {
    ...
    property_get("persist.radio.embms.atenable", prop, "1");
...
void requestAtGetSaiList(char * data, size_t datalen, RIL_Token t) {
    ...
    // %MBMSCMD:<SAI_COUNT>,[<csai1>,<csai2>, <csai3>,¡K], <INTER_FREQUENCY_COUNT>,
...
void postSessionListUpdate(void* param) {
    ...
    RILChannelCtx* pDataChannel = getRILChannelCtx(RIL_OEM, rid);
...
void onEmbmsSessionListUpdate(char *urc, RIL_SOCKET_ID rid) {
    ...
    */
    ...
    if (num == index) {
        RILChannelCtx* pDataChannel = getRILChannelCtx(RIL_OEM, rid);
        LOGD("trigger postSessionListUpdate rid=%d, dataChannelId=%d", rid, pDataChannel->id);
        ...
        RIL_requestProxyTimedCallback(postSessionListUpdate, param, &TIMEVAL_0,
            pDataChannel->id, "postSessionListUpdate");
    }
    return;
}
...
void postSaiListUpdate(void* param) {
    ...
    err = at_send_command_multiline("AT+EMSAIL?", "+EMSAIL", &p_response, getChannelCtxbyProxy(rid));
...
void onEmbmsSaiListUpdate(char *urc, RIL_SOCKET_ID rid) {
    ...
    if (num == index) {
        RILChannelCtx* pDataChannel = getRILChannelCtx(RIL_OEM, rid);
        LOGD("trigger postSaiListUpdate rid=%d, dataChannelId=%d", rid, pDataChannel->id);
        embmsTimeCallbackParam *param = (embmsTimeCallbackParam*)malloc(sizeof(embmsTimeCallbackParam));
        if (param == NULL) {
            LOGE("malloc fail");
            return;
        ...
        RIL_requestProxyTimedCallback(postSaiListUpdate, param, &TIMEVAL_0,
            pDataChannel->id, "postSaiListUpdate");
    }
}
...
void postEpsNetworkUpdate(void* param) {
    ...
    */
    ...
    err = at_send_command_singleline("AT+EOPS?", "+EOPS:", &p_response, getChannelCtxbyProxy(rid));
    if (err != 0 || p_response->success == 0 ||
            p_response->p_intermediates == NULL) {
        goto error;
    }
...
void onEpsNetworkUpdate(char *urc, RIL_SOCKET_ID rid) {
    ...
    RILChannelCtx* pDataChannel = getRILChannelCtx(RIL_OEM, rid);
    LOGD("trigger postEpsNetworkUpdate rid=%d, dataChannelId=%d", rid, pDataChannel->id);
    embmsTimeCallbackParam *param = (embmsTimeCallbackParam*)malloc(sizeof(embmsTimeCallbackParam));
    if (param == NULL) {
        LOGE("malloc fail");
        return;
    ...
    RIL_requestProxyTimedCallback(postEpsNetworkUpdate, param, &TIMEVAL_0,
        pDataChannel->id, "postEpsNetworkUpdate");
}
...
void onEmbmsHvolteUpdate(char *urc, RIL_SOCKET_ID rid) {
    ...
    RILChannelCtx* pDataChannel = getRILChannelCtx(RIL_OEM, rid);
    LOGD("trigger postHvolteUpdate rid=%d, dataChannelId=%d", rid, pDataChannel->id);
    embmsTimeCallbackParam *param = (embmsTimeCallbackParam*)malloc(sizeof(embmsTimeCallbackParam));
    if (param == NULL) {
        LOGE("malloc fail");
        return;
    ...
    RIL_requestProxyTimedCallback(postHvolteUpdate, param, &TIMEVAL_0,
        pDataChannel->id, "postHvolteUpdate");
}
