...
static SbpTable s_mtk_sbp_table[] = {
    ...
    // OP18
    {40409, 40409, 18},    //India
    {40418, 40418, 18},    //India
    {40436, 40436, 18},    //India
    {40450, 40452, 18},    //India
    {40467, 40467, 18},    //India
    {40483, 40483, 18},    //India
    {40485, 40485, 18},    //India

    {40501, 40501, 18},    //India
    {40503, 40523, 18},    //India
    ...
    // OP187
    {63902, 63902, 187},   // A1 - Austria

    // OP188
    {23201, 23201, 188},   // Safaricom - Kenya
...
static void setRadioSbpProperty(RIL_SOCKET_ID rid) {
    if (multi_ps == 1) {
        if (RIL_SOCKET_1 == rid) {
            property_set("persist.radio.sim.opid", sExecutionContext[rid].sbpId);
        } else if (RIL_SOCKET_2 == rid) {
            property_set("persist.radio.sim.opid_1", sExecutionContext[rid].sbpId);
        }
    } else {
        property_set("persist.radio.sim.opid", sExecutionContext[rid].sbpId);
    }
...
static int getSBPIdFromIccid(RIL_SOCKET_ID rid) {
    ...
    RILChannelCtx *p_channel = getRILChannelCtx(RIL_SIM,
            multi_ps == 1 ? rid : getMainProtocolRid());
...
static int setupSBP(RIL_SOCKET_ID rid) {
    ...
    if (multi_ps == 1 && RIL_SOCKET_2 == rid) {
        property_get("persist.radio.sim.opid_1", prevSbp_1, "");
    } else {
        property_get("persist.radio.sim.opid", prevSbp, "");
    }
    // Set property
    setRadioSbpProperty(rid);
    ...
    if (err < 0 || p_response->success == 0) {
        ...
        if (multi_ps == 1 && RIL_SOCKET_2 == rid) {
            property_set("persist.radio.sim.opid_1", prevSbp_1);
        } else {
            property_set("persist.radio.sim.opid", prevSbp);
        }
        free(cmd);
        at_response_free(p_response);
        return E_RETRY;
        ...
        if (ret == S_OK) {
            ...
            if (ret == E_RETRY) {
                RIL_requestProxyTimedCallback(
                        ...
                        getRILChannelCtx(RIL_SIM, rid)->id, "executeDynamicSBP");
...
static void startDynamicSBP(void *param) {
    ...
    if (sExecutionContext[rid].executionPhase == DSBP_PHASE_DONE) {
        ...
        RIL_requestProxyTimedCallback(
                ...
                getRILChannelCtx(RIL_SIM, rid)->id, "startDynamicSBP");
    }
...
void setupDynamicSBP(RIL_SOCKET_ID rid) {
    ...
    property_get("persist.radio.mtk_dsbp_support", dsbp_prop, "");
    ...
    property_get("ro.mtk_data_config", multi_ps_prop, "0");
    multi_ps = atoi(multi_ps_prop);

    property_get("persist.mtk_mims_support", multi_ims_prop, "0");
