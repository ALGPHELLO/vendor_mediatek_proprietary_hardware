...
extern void RIL_UpdateToVT(RIL_VT_MsgType type, RIL_VT_MsgParams param);
/// @}
...
error:
    ...
    if (isOp07Support() && !isCdmaSupport()) {
        if (bEmergencyMode == true) {
            ...
            RIL_requestProxyTimedCallback(stopEmergencySession, param, &TIMEVAL_0,
                    getRILChannelCtx(RIL_CC, rid)->id, "stopEmergencySession");
...
void requestUdub(void *data, size_t datalen, RIL_Token t) {
    ...
    // check if current project has c2k support
    ...
    if (isC2KSupport == 0 && inCallNumberPerSim[rid] == 0 && isSimInserted(rid) == 0) {
...
extern int rilCcMain(int request, void *data, size_t datalen, RIL_Token t) {
    ...
    case RIL_REQUEST_VT_DIAL:
        requestDial(data, datalen, t, 0, 1);
        break;
    ...
    case RIL_REQUEST_CURRENT_STATUS:
        requestCurrentStatus(data, datalen, t);
        break;
...
static void requestSwitchAntenna(void *data, size_t datalen, RIL_Token t) {
    ...
    at_send_command(cmd, NULL, getRILChannelCtx(RIL_CC, getMainProtocolRid()));
    free(cmd);
    RIL_onRequestComplete(t, RIL_E_SUCCESS, NULL, 0);
}
/// @
...
// Support EF ECC @{
static void onHandleEfEccUrc(const char *s, RIL_SOCKET_ID rid) {
    int count = 0, i = 0, remain = 255, tempStrLen = 0;
...
void rejectInComingCall(void *param);
...
void onIncomingCallIndication(char *urc, RIL_SOCKET_ID rid) {
    ...
    /// M: CC: For 3G VT only @{
    if (atoi(data[3]) == 10) {  // URC call_mode ==10 means 3G VT call
      vtCallCount++;
    }
    // IMS VoLTE refactoring
...
/// reject incoming call from SIM2 while SIM1 is active. @{
void rejectInComingCall(void *param)
{
    if(param == NULL) return;

    RejectInCallParam *data = (RejectInCallParam *) param;
    ...
    ATResponse *p_response = NULL;
    int err;
    LOGD("Reject incoming call from skt id: %d, active skt is: %d", (int)rid, activeRilSktID);

    // EAIC=1X, disallow incoming call
    asprintf(&cmd, "AT+EAIC=1,%s,%s", callId, seqNo);
    err = at_send_command(cmd, &p_response, getRILChannelCtx(RIL_CC, rid));

    if (err < 0 || p_response->success == 0) {
        LOGD("Reject incoming call failed: send AT failed");
    }
    ...
    if (strStartsWith(urc, "+CEN2")) {
        ...
        if (ret >= 0) {
            ...
            if (ptr == NULL) {  // Not found
                ...
                len = ptr - list;
                strncpy(newList, list, len);

                ptr = strstr(ptr + 1, ";");
                if (ptr != NULL) {
                    // Copy the mapping string after finding number
                    len = sizeof(newList) - strlen(newList);
                    strncat(newList, ptr, len - 1);
                }

                sprintf(newList, "%s;%s,%d", newList, number, category);
...
void onMccMncIndication(RIL_SOCKET_ID slotId, char* mccmnc) {
    ...
    property_get(PROPERTY_ECC_MCC[slotId], oldMcc, "0");
    if (strcmp(oldMcc, mcc) != 0) {
        property_set(PROPERTY_NW_ECC[slotId], "");
        LOGD("[onMccMncIndication]reset %s. MCC[%d] from %s to %s",
                PROPERTY_NW_ECC[slotId], slotId, oldMcc, mcc);
    }
}
...
void onCallProgressInfoCallStateChange(char *urc, RIL_SOCKET_ID rid) {
    ...
    /// M: For 3G VT only @{
    call_mode = atoi(data[5]);
    if (call_mode == 10 && call_id != 254) {  // VT && call_id valid
        RIL_VT_MsgParams param;
        param.callId = (unsigned char) call_id;
        LOGD("vtCallCount = %d", vtCallCount);
        if (vtCallCount > 0 && msg_type == 133) {
            RIL_UpdateToVT(MSG_ID_WRAP_3GVT_RIL_CONFIG_DEINIT_IND, param);
            LOGD("VT DEINIT: id = %d", param.callId);
            vtCallCount--;
        } else {
            switch (msg_type) {
                case 130: //MO
                    vtCallCount++;
                    // Fall through
                case 0: //MT, callcount depends on EAIC instead
                    RIL_UpdateToVT(MSG_ID_WRAP_3GVT_RIL_CONFIG_INIT_IND, param);
                    LOGD("VT INIT: id = %d", param.callId);
                    break;
                case 6:
                case 132:
                    RIL_UpdateToVT(MSG_ID_WRAP_3GVT_RIL_CONFIG_UPDATE_IND, param);
                    LOGD("VT UPDATE: id = %d", param.callId);
                    break;
                default:
                    break;
            }
...
*/
void requestSetEccServiceCategory(void *data, size_t datalen, RIL_Token t) {
    int ret;
...
void requestDtmfStart(void *data, size_t datalen, RIL_Token t) {
    ...
    if (vtCallCount > 0) {
        RIL_VT_MsgParams param;
        param.dtmfKey = c;
        RIL_UpdateToVT(MSG_ID_WRAP_3GVT_RIL_ON_USER_INPUT, param);
    }
    /// @}
...
/// M: For 3G VT only @{
void requestVoiceAccept(void *data, size_t datalen, RIL_Token t) {
    int *p_line;
    char *cmd;
    ...
    asprintf(&cmd, "AT+CHLD=9%d", p_line[0]);
    at_send_command(cmd, NULL, CC_CHANNEL_CTX);
...
void requestReplaceVtCall(void *data, size_t datalen, RIL_Token t) {
    int *c = (int *)data;
    char *cmd;
    int ret;
    ATResponse *p_response = NULL;
...
void onVtStatusInfo(char *urc, RIL_SOCKET_ID rid) {
    int ret;
    int info = 0;

    LOGD("onVtStatusInfo:%s", urc);

    if (strStartsWith(urc, "+EVTSTATUS")) {
        ret = at_tok_start(&urc);
        if (ret < 0) goto error;
    ...
    LOGD("VT_status = %d, ", info);
    RIL_UNSOL_RESPONSE(RIL_UNSOL_VT_STATUS_INFO, &info, sizeof(int), rid);
    return;

error:
    LOGE("There is something wrong with the +EVTSTATUS");
}
/// @}
...
void stopEmergencySession(void *param) {
    ...
    err = at_send_command("AT+EMCS=0", &p_response, getRILChannelCtx(RIL_CC, rid));
 ...
 *  1 : on
 */
void requestCurrentStatus(void *data, size_t datalen, RIL_Token t) {
    int ret;
    int airplaneMode = ((int *)data)[0];
    int imsReg = ((int *)data)[1];
    char *cmd;
    ATResponse *p_response = NULL;
    ...
    LOGD("[requestCurrentStatus]cmd = %s", cmd);
