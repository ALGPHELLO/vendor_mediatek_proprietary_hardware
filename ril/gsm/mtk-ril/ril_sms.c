...
static void rild_sms_sendSMS(int request, void * data, size_t datalen, RIL_Token t) {
    ...
    ATResponse *p_response = NULL;
    RIL_Errno ril_errno = RIL_E_RADIO_NOT_AVAILABLE;
...
extern int rilSmsUnsolicited(const char *s, const char *sms_pdu, RILChannelCtx* p_channel) {
    ...
    property_get("persist.op12.ccp.mode", chipCertification, "0");
...
void requestGSMGetBroadcastSMSConfigEx(void *data, size_t datalen, RIL_Token t) {
    RLOGD("requestGSMGetBroadcastSMSConfigEx");
    ...
    CB_CONFIG_NODE *config_node, *tem = NULL, config_head, **config_head_link;
    config_head.next = NULL;
    config_node = &config_head;
    config_head_link = &config_node;
    err = rild_sms_queryCBInfo(&mode, &ch_string, &dcs_string, &alllanguageon, t);
    if (err < 0) goto error;

    if (mode == 1) {
        node->next = calloc(1, sizeof(RIL_SMS_Linked_list));
        if (node->next == NULL) goto error;
        node = node->next;

        node->data = calloc(1, sizeof(RIL_GSM_BroadcastSmsConfigInfo));
        if (node->data == NULL) {
            free(node->next);
            node->next = NULL;
            goto error;
        ...
        if(alllanguageon == 1) {
            RLOGD("Here we receive alllanguage == 1");
            // get the id table
            err = rild_sms_handleCBConfig(config_head_link, ch_string, 1);
            if(err < 0) goto error;

            config_node = config_head.next;
            while (config_node != NULL || count == 0)
            {
                node->next = calloc(1, sizeof(RIL_SMS_Linked_list));
                if (node->next == NULL) {
                    RLOGE("requestGSMGetBroadcastSMSConfigEx, failed to allocate memroy");
                    break;
                }
                node = node->next;
                node->next = NULL;
                ...
                if (config_node == NULL) {
                    ...
                    info->fromServiceId = config_node->value;
                    /* start to check if any consequence value and set to one RIL_GSM_BroadcastSmsConfigInfo */
                    CB_CONFIG_NODE *next_config = NULL;
                    while (1)
                    {
                        next_config = config_node->next;
                        if (next_config == NULL) {
                            /* no more config */
                            break;
                        } else if (next_config->value == (config_node->value + 1)) {
                            /* move forward */
                    ...
                    info->toServiceId = config_node->value;
                    config_node = config_node->next;
                }

                RLOGD("CB Setting: channel id: %d-%d", info->fromServiceId, info->toServiceId);
                count++;
            }
        } else {
            // get the id table
            ...
            do {
                ...
                RLOGD("dcs: %d %d %d", dcs_begin, dcs_end, dcs_cur);

                //create a structure of RIL_GSM_BroadcastSmsConfigInfo
                if (dcs_begin == -1 && count > 0) {
                    break;
                } else {
                    if (node->next == NULL) {
                        node->next = calloc(1, sizeof(RIL_SMS_Linked_list));
    ...
    RLOGD("Here we receive get all the cb and dcs info");
    // pack the output data
    node = head.next;
    infos = calloc(1, sizeof(RIL_GSM_BroadcastSmsConfigInfo*)*count);
    if (infos != NULL) {
        for (i = 0; i < count ; i++) {
            infos[i] = node->data;
            node = node->next;
        }
        RIL_onRequestComplete(t, RIL_E_SUCCESS, infos,
                sizeof(RIL_GSM_BroadcastSmsConfigInfo*)*count);
        ...
        // release output data
        RLOGE("rild_sms_handleCBConfig, failed to allocate memory. Ready to release output data");
        RIL_SMS_Linked_list *freePtr = NULL;
        while (node != NULL) {
            if (node->data != NULL) {
                free(node->data);
            }
    ...
    free(ch_string);
    free(dcs_string);
    for (config_node = config_head.next; config_node != NULL; ) {
        tem = config_node;
        config_node = config_node->next;
        free(tem);
    }
...
void requestGSMSetBroadcastSMSConfigEx(void *data, size_t datalen, RIL_Token t) {
    RLOGD("requestGSMSetBroadcastSMSConfigEx");
    char *line, *ch_string = NULL, *dcs_string = NULL;
    char *ch_new_str = NULL, *dcs_new_str = NULL;
    ATResponse *p_response = NULL;
    int err, mode, count, i, j, alllanguageon, len;
    int ch_begin, ch_end, dcs_begin, dcs_end;

    RIL_GSM_BroadcastSmsConfigInfo *info;
    RIL_GSM_BroadcastSmsConfigInfo **infos;
    char dcsID_table[MAX_CB_DCS_ID];
    CB_CONFIG_NODE *node, *tem = NULL, head, **head_link;
    head.next = NULL;
    node = &head;
    head_link = &node;
    ...
    ch_new_str = calloc(1, sizeof(char)*1000);
    if (ch_new_str == NULL) goto error;
    dcs_new_str = calloc(1, sizeof(char)*1000);
    if (dcs_new_str == NULL) goto error;

    err = rild_sms_queryCBInfo(&mode, &ch_string, &dcs_string, &alllanguageon, t);
    if (err < 0) goto error;

    if (mode == 1) {
        RLOGE("Should not set CB info when CB is disabled");
        goto error;
    }
    ...
    if(infos[0]->fromServiceId == -1 && infos[0]->toServiceId == -1 &&
        ...
        // set the new setting
        RLOGD("Set Channel and Dcs settings : AT+CSCB=0, \"\", \"\", 1");
        //asprintf(&line, "AT+CSCB=0, \"%s\", \"\", 1", ch_string);
        asprintf(&line, "AT+CSCB=0, \"\", \"\", 1");
        err = at_send_command(line, &p_response, SMS_CHANNEL_CTX);
        free(line);
        if(err < 0 || p_response->success == 0) {
            RLOGE("Fail to set all language on");
            goto error;
        }
    ...
    rild_sms_handleCBConfig(head_link, ch_string, 1);
    ch_new_str[0] = '\0';
    //combine the current setting and input setting
    err = rild_sms_getIDTable(dcsID_table, dcs_string, MAX_CB_DCS_ID);
    if(err < 0) goto error;
    RLOGD("Get current CB setting number: %d", count);
    for (i = 0; i < count; i++) {
        ch_begin = dcs_begin = 0;
        ch_end = dcs_end = -1;

        RLOGD("Cell broadcast setting, index: %d", i);
        ...
        if(infos[i]->fromServiceId >= 0) {
            ch_begin = infos[i]->fromServiceId;
        }
        if(infos[i]->toServiceId >= 0) {
        ...
        RLOGD("Cell broadcast individual setting, channel begin:%d, end:%d DCS begin: %d, end: %d",
                ch_begin, ch_end, dcs_begin, dcs_end);

        if (ch_begin == ch_end) {
            sprintf(ch_new_str, "%d", ch_begin);

        } else if (ch_end > ch_begin) {
            sprintf(ch_new_str, "%d-%d", ch_begin, ch_end);
        }
        rild_sms_handleCBConfig(head_link, ch_new_str, infos[i]->selected);
    ...
    /*
     *  If dcs_string is empty, the all language can't be reset since modem CSCB design
     *  add the fake dcs_string(0) instead to set the all language first.
     */
    if (strlen(dcs_string) == 0) {
        // reset the all language setting
        asprintf(&line, "AT+CSCB=1, \"%s\", \"0\"", ch_string);
        err = at_send_command(line, &p_response, SMS_CHANNEL_CTX);
        free(line);
        if (err < 0 || p_response->success == 0)
            goto error;
    ...
    ********************************************/
    ...
    RLOGD("New dcs Setting: %s %d", dcs_new_str, count);
    ...
    ch_new_str[0] = '\0';
    while (node != NULL) {
        CB_CONFIG_NODE *next_node = NULL;
        int begin_value = 0;
        int end_value = 0;
        if (node->selected > 0) {
            /* select & start to part sequence values */
            begin_value = node->value;
            while (1) {
                next_node = node->next;
                if (next_node == NULL) {
                    /* no more config */
                    break;
                } else if (next_node->value == (node->value + 1) && next_node->selected > 0) {
                    /* move forward */
                    node = node->next;
                } else {
                    break;
            ...
            if (begin_value == end_value) {
                /* use the single setting */
                sprintf(ch_new_str, "%s%d,", ch_new_str, begin_value);
            } else {
                /* use the range setting, ex. 20-50 */
                sprintf(ch_new_str, "%s%d-%d,", ch_new_str, begin_value, end_value);
            }
        }
        node = node->next;
    }
    if (strlen(ch_new_str) > 0) {
        ch_new_str[strlen(ch_new_str)-1] = '\0';
    }

    RLOGD("New ch Setting: %s", ch_new_str);
    ...
    // set the new setting
    RLOGD("alllanguageon: %d", alllanguageon);
    if (alllanguageon) {
        asprintf(&line, "AT+CSCB=0, \"%s\"", ch_new_str);
    } else {
    ...
    if (err < 0 || p_response->success == 0) {
        ...
        ********************************************/
        ...
        free(line);
        if (err < 0 || p_response->success == 0) {
            RLOGE("Restore CB Setting ERROR");
        }
        goto error;
    }
    ...
    ********************************************/
    ...
    if(alllanguageon) {
        RLOGD("re-set all language on, because we change cid");
        asprintf(&line, "AT+CSCB=0, \"\", \"\", 1");
        err = at_send_command(line, &p_response, SMS_CHANNEL_CTX);
        free(line);
        if(err < 0 || p_response->success == 0) {
            RLOGE("Fail to re-set all language on");
            goto error;
        }
    }
...
error:
    RLOGD("here we go to error setBroadcastEx");
    if (ch_string != NULL)
        free(ch_string);
    if (dcs_string != NULL)
        free(dcs_string);
