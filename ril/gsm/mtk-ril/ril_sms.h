...
extern void requestGetGsmSmsBroadcastActivation(void *data, size_t datalen, RIL_Token t);
...
extern void requestGsmSetBroadcastLanguage(void *data, size_t datalen, RIL_Token t);
extern int rild_sms_queryCBInfo(int *mode, char **ch_str, char **dcs_str, int *alllanguageon, RIL_Token t);
extern int rild_sms_getIDTable(char *table, char *id_string, int maximum_id);

typedef struct CB_CONFIG{
    int value;
    int selected;
    struct CB_CONFIG* next;
}CB_CONFIG_NODE;

extern int phone_storage_status;

#define SMS_UNUSED(x) (x)   //eliminate "warning: unused parameter"
