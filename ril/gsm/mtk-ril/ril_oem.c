...
extern void enforceQueryAllIccId();
...
static void sendATWithRetry(int times, int interval/*ms*/,
...
static void clearIaCacheAndDefineInvalidIa()
{
    ...
    if (proj_type != GEMINI_MODE_L_AND_L) {
        ...
        RILChannelCtx *pChannel = getRILChannelCtx(RIL_SIM, getMainProtocolRid());
        ...
        for (iaSimId = 0; iaSimId < SIM_COUNT; iaSimId++) {
            ...
            RILChannelCtx *pChannel = getRILChannelCtx(RIL_SIM, iaSimId);
            if (getAttachApnSupport() == ATTACH_APN_PARTIAL_SUPPORT) {  // partial solution
...
extern int queryMainProtocol(RILSubSystemId subsystem)
{
    ...
    if (SIM_COUNT == 1) {
        ...
        err = at_send_command_singleline("AT+ES3G?", "+ES3G:", &p_response, getRILChannelCtx(subsystem,getMainProtocolRid()));
...
static int simSwitchGetSIMInitState() {
    int ret;
    pthread_mutex_lock(&s_simInitStateMutex);
    ret = s_simInitState;
    pthread_mutex_unlock(&s_simInitStateMutex);
...
int doSimSwitchWithoutSimReset(RIL_Token t) {
    int i;
    RILChannelCtx *p_channel_old, *p_channel_cmd;
    int err;
    char *cmd = NULL;
    ATResponse *p_response = NULL;
    ...
    p_channel_cmd = getRILChannelCtx(RIL_SIM, getMainProtocolRid());
    ...
    for (i = 0; i < getSupportChannels(); i++) {
        p_channel_old = getChannelCtxbyId(i);
        RLOGI("lock reader mutex[%d]", p_channel_old->id);
        if (p_channel_cmd == p_channel_old) {
            p_channel_cmd_idx = i;
            RLOGI("skip terminate cmd channel");
            continue;
        }
        err = pthread_mutex_lock(&p_channel_old->readerMutex);
        pthread_kill(p_channel_old->tid_reader, SIGUSR1);
        at_close(p_channel_old);
    }
    // cmd channel need to be locked later
    if (p_channel_cmd_idx >= 0) {
        p_channel_old = getChannelCtxbyId(p_channel_cmd_idx);
    }
    ...
    int retry = 1;
    RILChannelCtx *p_channel_tmp = NULL;
    int kill_rc;
    while (retry) {
    ...
    for(i = RIL_URC; i < getSupportChannels(); i += RIL_CHANNEL_OFFSET) {
        p_channel_old = getChannelCtxbyId(i);
        RLOGI("unlock channel[%d]", p_channel_old->id);
        pthread_mutex_unlock(&p_channel_old->readerMutex);
    }
    ...
    if (isCdmaLteDcSupport()){
        // for IR reset suspend enable state before SIM switch
        resetRegSuspend();
    }
...
int doSimSwitchWithoutMdReset() {
    int i;
    RILChannelCtx *p_channel_sim, *p_channel_old, *p_channel_cmd;
    int err;
    char *cmd = NULL;
    ATResponse *p_response = NULL;
    int isSwitchSuccess = 1;
    ...
    for (i = 0; i < getSupportChannels(); i++) {
        ...
        err = pthread_mutex_lock(&p_channel_old->restartMutex);
        err = pthread_mutex_lock(&p_channel_old->readerMutex);
        ...
        pthread_kill(p_channel_old->tid_reader, SIGUSR1);
        at_close(p_channel_old);
        //pthread_join(p_channel_old->tid_reader, NULL);
    }
    // cmd channel need to be locked later
    if (p_channel_cmd_idx >= 0) {
        p_channel_old = getChannelCtxbyId(p_channel_cmd_idx);
  ...
  //  int modem_setting_value = CAPABILITY_3G_SIM1 << (targetSimRid);
    ...
    // kill cmd channel
    if (p_channel_cmd_idx >= 0) {
        err = pthread_mutex_lock(&p_channel_cmd->restartMutex);
        err = pthread_mutex_lock(&p_channel_cmd->readerMutex);
        //RLOGI("******** requestSetRadioCapability: try to lock channel %d, name %s, ret %d, %x",
        //        p_channel_cmd->id, p_channel_cmd->myName, err, &p_channel_old->restartMutex);
        pthread_kill(p_channel_cmd->tid_reader, SIGUSR1);
        at_close(p_channel_cmd);
        // pthread_join(p_channel_cmd->tid_reader, NULL);
    }
    ...
    for(i = 0; i < SIM_COUNT; i++) {
        ...
        p_channel_sim = getRILChannelCtx(RIL_SIM, RIL_SOCKET_1+i);
        ...
        pthread_mutex_unlock(&p_channel_sim->readerMutex);
        pthread_mutex_unlock(&p_channel_sim->restartMutex);
    }
    //enforceQueryAllIccId();
    simSwitchClearProxyList();
    ...
    for (i = 0; i < getSupportChannels(); i++) {
        ...
        if (((i % RIL_CHANNEL_OFFSET) != RIL_CMD_3) && ((i % RIL_CHANNEL_OFFSET) != RIL_URC)) {
            ...
            pthread_mutex_unlock(&p_channel_old->readerMutex);
            pthread_mutex_unlock(&p_channel_old->restartMutex);
        }
    }
    ...
    for (i = RIL_URC; i < getSupportChannels(); i += RIL_CHANNEL_OFFSET) {
        ...
        pthread_mutex_unlock(&p_channel_old->readerMutex);
        pthread_mutex_unlock(&p_channel_old->restartMutex);
        setRadioState(RADIO_STATE_OFF, getRILIdByChannelCtx(p_channel_old));
    }
...
extern ApplyRadioCapabilityResult applyRadioCapability(RIL_RadioCapability* rc, RIL_Token t)
{
    ...
    if (isImsSupport()) {
        ...
        err = at_send_command("AT+EIMS=0", &p_response, getRILChannelCtx(RIL_SIM, getMainProtocolRid()));
        at_response_free(p_response);
        p_response = NULL;
    }
...
void requestSN(RILSubSystemId subsystem, RIL_SOCKET_ID rid) {
    ATResponse *p_response = NULL;
    int err;
    char *line, *sv;
...
extern void triggerMal(int isCapabilitySwitching) {
    ...
    {
        // capability switch start

        /* set MAL stopping flag, then stop MAL */
        property_set(PROPERTY_SIM_SWITCH_MAL_STOPPING_FLAG, "1");
        property_set(PROPERTY_SIM_SWITCH_CONTROL_MAL, "0");
    }
    else
    {
        // capability switch done

        /* start MAL */
        property_get(PROPERTY_SIM_SWITCH_DONGLE_MODE, prop_val, "0");
        dongleMode = atoi(prop_val);
        LOGD("get prop persist.chiptest.enable = %d", dongleMode);
...
extern void flightModeBoot(RILSubSystemId subsystem, RIL_SOCKET_ID rid)
{
    ATResponse *p_response = NULL;
...
extern void bootupGetIccid(RILSubSystemId subsystem, RIL_SOCKET_ID rid) {
    int result = 0;
    ATResponse *p_response = NULL;
    RILChannelCtx *p_channel = getRILChannelCtx(subsystem, rid);
...
extern void bootupGetImeisv(RILSubSystemId subsystem, RIL_SOCKET_ID rid) {
...
extern void bootupGetBasebandVersion(RILSubSystemId subsystem, RIL_SOCKET_ID rid) {
...
/* ALPS00582073 START */
extern void bootupGetCalData(RILSubSystemId subsystem, RIL_SOCKET_ID rid)
{
    ATResponse *p_response = NULL;
    int err;
    RILChannelCtx *p_channel = getRILChannelCtx(subsystem, rid);
...
void requestGetDeviceId(void * data __unused, size_t datalen __unused, RIL_Token t)
{
    ...
    if (s_imei[rid] == NULL) {
        bootupGetImei(RIL_OEM, rid);
    }
    if (s_imeisv[rid] == NULL) {
        bootupGetImeisv(RIL_OEM, rid);
    }
...
int onCheckSetUnsolResponseFilter(void *data, size_t datalen, RIL_Token t){
    ...
    if ((radioState == RADIO_STATE_ON) || (radioState == RADIO_STATE_OFF)) {
        ...
        RIL_requestProxyTimedCallback(onRetrySetUnsolResponseFilter, (void*) mParam, &TIMEVAL_1,
            getRILChannelCtx(RIL_OEM, rid)->id, "onRetrySetUnsolResponseFilter");
        return 0;
    }
}
...
extern int rilOemMain(int request, void *data, size_t datalen, RIL_Token t)
{
    switch (request)
    {
        ...
        case RIL_REQUEST_GSM_DEVICE_IDENTITY:
            ...
            break;
        // World Phone store/reload modem type
...
void requestGetGcfMode(RIL_SOCKET_ID rid)
{
    ...
    err = at_send_command_singleline("AT+EPCT?", "+EPCT:", &p_response, getChannelCtxbyProxy(rid));
...
void requestSetTrm(void * data, size_t datalen  __unused, RIL_Token t) {
    ...
    if (isSvlteSupport() && mode != 3 && (mode != 19)) {
        LOGD("G requestSetTrm, ril.cdma.report.case ,1");
        property_set("ril.cdma.report.case", "1");
    }
