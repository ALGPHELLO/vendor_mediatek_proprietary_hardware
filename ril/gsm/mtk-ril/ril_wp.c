...
void onWorldModeChanged(char* urc, const RIL_SOCKET_ID rid) {
    ...
    if (false == isWorldModeRemoveSimReset()) {
        RIL_requestProxyTimedCallback(handleWorldModeChanged, mParam, &TIMEVAL_0,
                getRILChannelCtx(RIL_SIM, getMainProtocolRid())->id, "handleWorldModeChanged");
    } else {
        // notify world mode changed start
        status[0] = 0;
        LOGD("[onWorldModeChanged] state= %d", status[0]);
        // property_set("ril.nw.worldmode.state", "0");
        property_set("persist.radio.wm_state", "0");
        ...
        if (isCdmaSupport()) {
            ...
            RIL_requestProxyTimedCallback(handleWorldModeChangedNoSimReset, mParam, &TIMEVAL_0,
                    getRILChannelCtx(RIL_SIM, getMainProtocolRid())->id, "handleWorldModeChangedNoSimReset");
        }
    }
}
...
void requestResumeWorldModeChanged(void * data, size_t datelen, RIL_Token t) {
    ...
    err = at_send_command(cmd, &p_response, getRILChannelCtx(RIL_SIM, rid));

    queryActiveMode(rid, getRILChannelCtx(RIL_SIM, rid));
    ...
    property_set("persist.radio.wm_state", "1");
    ...
    if (err < 0 || p_response->success == 0) {
        ...
        if (mRetryResumeCount <= WP_COMMAND_MAX_RETRY_COUNT) {
            ...
            RIL_requestProxyTimedCallback(handleWorldModeChanged, param, &TIMEVAL_1S,
                    getRILChannelCtx(RIL_SIM, getMainProtocolRid())->id, "handleWorldModeReryChanged");
            return;
        }
    } else {
        status[0] = 1;
...
int allowForWorldModeSwitching(const int request) {
    if (bWorldModeSwitching == 1) {
        if (request == RIL_REQUEST_RADIO_POWER) {
            ...
            return 1;
        } else if ((request == RIL_REQUEST_SET_PREFERRED_NETWORK_TYPE) && isCdmaSupport()) {
...
bool isWorldPhoneOldVersion() {
    ...
    property_get("ro.mtk_md_world_mode_support", worldmode, "0");
...
void worldPhoneinitialize(const RIL_SOCKET_ID rid, RILChannelCtx *p_channel) {
    ...
    property_get("persist.radio.wm_state", worldModeState, "1");
...
void queryKeep3GMode(RIL_SOCKET_ID rid, RILChannelCtx *p_channel) {
    ...
    if (err == 0 && p_response->success != 0 && p_response->p_intermediates != NULL) {
        ...
        if (err >= 0) {
            ...
            if (0 == mode) {
                property_set("ril.nw.worldmode.keep_3g_mode", "0");
                LOGD("keep TD-SCDMA 3G mode mode");
            } else if (1 == mode) {
                property_set("ril.nw.worldmode.keep_3g_mode", "1");
