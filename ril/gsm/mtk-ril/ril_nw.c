...
int s_signal_response[MAX_SIM_COUNT][17] = {{0}};
...
// ALPS00353868 END
//ALPS03269579 START
#define PROPERTY_GSM_WFC_STATUS    "gsm.wfc.status"
int bEopsSupport = 0;
...
static const char PROPERTY_NW_MCCMNC[4][30] = {
    "ril.nw.operator.mccmnc.1",
    "ril.nw.operator.mccmnc.2",
    "ril.nw.operator.mccmnc.3",
    "ril.nw.operator.mccmnc.4",
};

static const char PROPERTY_NW_LTE_SIGNAL[MAX_SIM_COUNT][30] = {
...
int getSingnalStrength(char *line, int *response, const RIL_SOCKET_ID rid)
{
    ...
    int read_rssi_in_qdbm;
    int read_rscp_in_qdbm;
    int read_ecn0_in_qdb;
    int read_rsrp_in_qdbm;
    int read_rsrq_in_qdb;
    int read_rssnr_in_qdb;
    int read_act;

    int *sig1 = &response[0];    //mGsmSignalStrength
    int *sig2 = &response[1];    //mGsmBitErrorRate
    int *pCdmaDbm = &response[2];
    int *pCdmaEcio = &response[3];
    int *pEvdoDbm = &response[4];
    int *pEvdoEcio = &response[5];
    int *pEvdoSnr = &response[6];
    int *pLteSignalStrength = &response[7];
    int *rsrp_in_dbm = &response[8];         //mLteRsrp --> -1*read_rsrp_in_qdbm /4 = mLteRsrp
    int *rsrq_in_db = &response[9];          //mLteRsrq --> -1*read_rsrq_in_qdbm /4 = mLteRsrq
    int *rssnr_in_db = &response[10];        //mLteRssnr --> -1*read_rssnr_in_qdb /4 = mLteRssnr
    int *cqi = &response[11];                //mLteCqi
    int *timingAdvance = &response[12];
    int *tdscdma_rscp_in_dbm = &response[13];//mTdscdmaRscp
    ...
    *pEvdoEcio = -1;
    *pEvdoSnr = -1;
    *pLteSignalStrength = 99;
    *rsrp_in_dbm = INVALID;
    *rsrq_in_db = INVALID;
    *rssnr_in_db = INVALID;
    *cqi = INVALID;
    *timingAdvance = INVALID;
    *tdscdma_rscp_in_dbm = INVALID;

    int rscp_in_dbm = INVALID;
    ...
    if (err < 0) {
        ...
        rscp_in_dbm = (read_rscp_in_qdbm/4)*(-1);
        if (rscp_in_dbm > 120) {
            rscp_in_dbm = 120;
    ...
    err = at_tok_nextint(&line, &read_ecn0_in_qdb);
    if (err < 0) {
        read_ecn0_in_qdb = INVALID;
    }
    ...
    if(at_tok_hasmore(&line)){
        ...
        if (read_act == 7) {
            *rsrq_in_db = ((read_rsrq_in_qdb)/4)*(-1);
            if (*rsrq_in_db > 20) {
                *rsrq_in_db = 20;
            } else if (*rsrq_in_db < 3) {
                *rsrq_in_db = 3;
        }

            *rsrp_in_dbm = ((read_rsrp_in_qdbm)/4)*(-1);
            if (*rsrp_in_dbm > 140) {
                *rsrp_in_dbm = 140;
            ...
            if (err < 0) {//err w.r.t. rssnr
            } else if (read_rssnr_in_qdb != 0x7FFF) {
                *rssnr_in_db = ((read_rssnr_in_qdb) * 10) / 4;
                if (*rssnr_in_db > 300) {
                    *rssnr_in_db = 300;
                } else if (*rssnr_in_db < -200) {
                    *rssnr_in_db = -200;
                }
            }
        }
    ...
    if (hasMoreThanFiveElements != 1) {
        ...
        *tdscdma_rscp_in_dbm = rscp_in_dbm;
    } else {
        if ((read_act == 0) || (read_act == 1) || (read_act == 3)) {  // for GSM: <rxlevel>,<ber>
            ...
            *sig1 = ((read_rssi_in_qdbm/4)+113) / 2;
            updateSignalStrengthProperty(*rsrp_in_dbm, *rssnr_in_db, rid);
            ...
            *sig1 = 0;
            ...
            // when SignalStrength.getLteLevel() == SIGNAL_STRENGTH_NONE_OR_UNKNOWN
...
int queryRadioState(RILSubSystemId subsystem, RIL_SOCKET_ID rid)
{
    ...
    char *line;
    int ret;
    RILChannelCtx *pChannel = getRILChannelCtx(subsystem, rid);
...
void requestSignalStrength(void * data, size_t datalen, RIL_Token t)
{
    ...
    int err;
    int response[14] = {0};
    int default_response[14] = RIL_NW_INIT_SIGNAL;
    char *line;
                    ...
                    if ((err >= 0) &&( skip >= 0 && skip <= 4 && skip !=2)){
                        ...
                        if (err >= 0 && skip == 2)
                        {
                            ...
                            if (!at_tok_hasmore(&line)) {
                                ...
                                // report with {"", "", <PLMN>} when limited service.
                                RIL_onRequestComplete(t, RIL_E_SUCCESS, response, sizeof(response));
                                at_response_free(p_response);
                                return;
                            }
...
void requestSetNetworkSelectionManualWithAct(void * data, size_t datalen, RIL_Token t)
{
    ...
    if (datalen / sizeof(char *) == 3) {
        mode = (char*) args[2];
        if (mode[0] == '1'){
            LOGD("Semi auto network selection mode");
            isSemiAutoMode = true;
        }
    }
    ...
    ///M : [Network][C2K]add for band8 desense feature. @{
        ...
        if (cmd != NULL) {
            ...
            if ((err >= 0 && p_response->success != 0) ||
                ...
                p_response = NULL;
                detachCount = 0;
                RIL_requestProxyTimedCallback(onSetBandModeResult, NULL, &TIMEVAL_1,
                        getRILChannelCtx(RIL_SIM, getMainProtocolRid())->id, "onSetBandModeResult");
            }
        }
        at_response_free(p_response);
        ril_errno = RIL_E_SUCCESS;
...
void requestQueryCurrentBandMode(){
    ATResponse *p_response = NULL;
     ...
     char *line;
     err = at_send_command_singleline("AT+EPBSE?", "+EPBSE:", &p_response, getRILChannelCtx(RIL_SIM,getMainProtocolRid()));
...
void requestSetPreferredNetworkType(void * data, size_t datalen, RIL_Token t)
{
    ...
    property_get("persist.radio.simswitch", optr, "1");
...
static int getCellInfoListV12(char * line, int num, RIL_CellInfo_v12 * response, int slotId)
{
    ...
    */
    ...
    for(i=0 ; i < num ; i++) {
        ...
        LOGD("act=%d,cid=%d,mcc=%d,mnc=%d,lacTac=%d,pscPci=%d,sig1=%d,sig2=%d,"
                "sig1_dbm=%d,sig1_dbm=%d,ta=%d,rssnr=%d,cqi=%d,bsic=%d,arfcn=%d",
                act,cid,mcc,mnc,lacTac,pscPci,sig1,sig2,
...
void requestGetPacketSwitchBearer(RILSubSystemId subsystem, RIL_SOCKET_ID rid)
{
    ATResponse *p_response = NULL;
    int err;
    char *line;
    int ret;
    char *bearer = NULL;
    RILChannelCtx *p_channel = getRILChannelCtx(subsystem, rid);

    //ALPS02494171[
    err = at_send_command_singleline("AT+EPSB?", "+EPSB:", &p_response, p_channel);
    //ALPS02494171]
    ...
    if (rid == getMainProtocolRid()) {
        property_set("gsm.lte.ca.support", bearer);
    }
    free(bearer);
...
void requestSetPseudoCellMode(void * data, size_t datalen __unused, RIL_Token t)
{
    ...
    asprintf(&property, "persist.radio.apc.mode%d", rid);
    property_set(property, cmd);
    RLOGD("update property %s = %s", property, cmd);
    free(property);
    free(cmd);
    if (err < 0 || p_response->success == 0) {
        goto error;
...
void requestVoiceRadioTech(void * data, size_t datalen, RIL_Token t) {
    ...
    err = at_send_command_singleline("AT+EPSB?", "+EPSB:", &p_response, getRILChannelCtx(RIL_NW, rid));
...
void confirmRatBegin(void* param) {
    ...
    ATResponse *p_response = NULL;
    RILChannelCtx* p_channel = getRILChannelCtx(RIL_SIM, rild);
    bool ret = false;
    int err;
    ...
    if (ret) {
        ...
        RIL_requestProxyTimedCallback(confirmRatBegin, (void*)param, &TIMEVAL_5,
                getRILChannelCtx(RIL_SIM, rild)->id, "confirmRatBegin");
    }
}
...
void onConfirmRatBegin(char* urc, const RIL_SOCKET_ID rid) {
    ...
    RIL_requestProxyTimedCallback(confirmRatBegin, (void*)p_rilId, &TIMEVAL_0,
            getRILChannelCtx(RIL_SIM, rid)->id, "confirmRatBegin");
}
...
void onPsStateChanged(void* param) {
    ...
    RILChannelCtx *pChannel = getRILChannelCtx(subSysId, pInfo->rid);
    int response[3];
    ...
    if (is_cs == 1)
    {
        ...
        if (is_r8 == 1) {
            ...
            RIL_requestProxyTimedCallback (onPsStateChanged, (void*)pInfo, &TIMEVAL_0,
                getRILChannelCtx(RIL_NW, rid)->id, "onPsStateChanged");
        }
    }
...
void onRestrictedStateChanged(const RIL_SOCKET_ID rid)
{
    ...
    if (isSimInserted(rid) && !isCdmaSupport()) {
...
error:
    ...
    RLOGE("onNetworkScanResult ERROR: %d", err);
}
...
extern int rilNwMain(int request, void *data, size_t datalen, RIL_Token t)
{
    ...
    case RIL_REQUEST_DATA_REGISTRATION_STATE:
        if (!isEpdgSupport() ||
            ...
            LOGD("request gprs reg state: MAL case");
...
/// M: EPDG feature. Combime PS state from MAL @{
void requestMalGprsRegistrationState(void * data, size_t datalen, RIL_Token t)
{
    ...
    ************************************/
    ...
    /// M: EPDG feature. Query PS state from MAL @{
    response[0] = ril_mal_data_reg_status[rid];

    /// M: EPDG feature. Query PS state from MAL @{
    if (response[0] == 99 && ril_data_reg_status[rid] != 1 && ril_data_reg_status[rid] != 5) {
...
extern void onMalPsStateChanged(void *data, size_t datalen, RIL_Token t){
        ...
        if (stat == 99 || stat == 98) {
            ...
            // ALPS03269579 START
            asprintf(&mal_status, "%d", ril_mal_data_reg_status[rid]);
            LOGD("Set ril_mal_data_reg_status[%d] %s to property", rid, mal_status);
            property_set(PROPERTY_GSM_WFC_STATUS, mal_status);
            free(mal_status);
            // ALPS03269579 END
            stat = combineMalCgregState(rid);
        } else {
            //only 99 and 98 are valid value
            goto error;
        }
        /// @}

        if (isEpdgSupport() && isCdmaSupport()) {
...
void clearRilNWMalCache() {
    ...
    // ALPS03269579 START
    property_set(PROPERTY_GSM_WFC_STATUS, "");
    // ALPS03269579 END
}
/// @}
...
void requestGetPOLCapability(void *data, size_t datalen, RIL_Token t) {
    ...
    if (err < 0 || NULL == p_response) {
        RLOGE("requestGetPOLCapability Fail");
        goto error;
    }
    ...
    if (ret != RIL_E_SUCCESS) {
        RLOGD("requestGetPOLCapability: Error or no SIM inserted!");
        goto error;
    }
    ...
    if(err < 0) goto error;
    RLOGD("requestGetPOLCapability: found first ( ");
    temp = strchr(line, '-');
    ...
    line = temp;
    RLOGD("requestGetPOLCapability: %d , line is %s", result[0],line);
...
/*
    temp = strchr(line, ')');
    if (temp != NULL && line < temp ) {
        RLOGD("requestGetPOLCapability: find ), temp is %s", temp);
    }
    */
...
/*  line = temp;
    ...
    LOGD("requestGetPOLCapability: skip , line is %s", line);
    err = at_tok_char(&line);
    RLOGD("requestGetPOLCapability: line is %s", line);
...
void requestGetPOLList(void *data, size_t datalen, RIL_Token t)
{
        ...
        RLOGD("requestGetPOLList!! count is %d", count);
        ...
        for (i = 0,j=0, p_cur = p_response->p_intermediates
            ...
            line = p_cur->line;
            RLOGD("requestGetPOLList!! line is %s",line);
            err = at_tok_start(&line);
            if(err < 0) goto error;
            ...
            RLOGD("requestGetPOLList!! index is %s",response[j+0]);
            /* get format*/
            err = at_tok_nextstr(&line, &tmp);
            if (err < 0) goto error;
               sprintf(response[j+1], "%s", tmp);
            RLOGD("requestGetPOLList!! format is %s",response[j+1]);
            /* get oper*/
            err = at_tok_nextstr(&line, &tmp);
            if (err < 0) goto error;
            sprintf(response[j+2], "%s", tmp);
            RLOGD("requestGetPOLList!! oper is %s",response[j+2]);
            nAct = 0;
...
////            if(nAct == 0) { nAct = 1;} // No act value for SIM. set to GSM
            ...
            RLOGD("requestGetPOLList!! act is %s",response[j+3]);
        ...
        return;
    error:
        RLOGE("requestGetPOLList return error");
        at_response_free(p_response);
        if(response)
        {
            RLOGD("FREE!!");
...
void requestSetPOLEntry(void *data, size_t datalen, RIL_Token t)
{
    ...
    if (strings[1] == NULL) {
        ...
        RLOGD("requestSetPOLEntry Act = %d",nActTmp);
        ...
        if (nActTmp == 0) {
            RLOGD("requestSetPOLEntry no Act assigned,strings[2]=%s",strings[2]);
            asprintf(&cmd, "AT+CPOL=%s,2,\"%s\"", strings[0], strings[1]);
        }else{
            RLOGD("R8, MOLY and LR9 can supoort 7 arguments");
...
static void setModemCapability(void *param) {
    ...
    if (RatConfig_isLteFddSupported() == 1 && RatConfig_isC2kSupported() == 1
        ...
        at_send_command("AT+EGRAT=11", NULL,
                getDefaultChannelCtx(getMainProtocolRid()));
    ...
    // L/C/W/G (type 2)
        ...
        if (isSvlteCdmaOnlySetFromEngMode() == 3) {
            at_send_command("AT+ERAT=3,0", NULL, getDefaultChannelCtx(getMainProtocolRid()));
        } else if (!isOp016m()){
            at_send_command("AT+EGRAT=14", NULL, getDefaultChannelCtx(getMainProtocolRid()));
            at_send_command("AT+ERAT=14,0", NULL, getDefaultChannelCtx(getMainProtocolRid()));
        }
    // L/W/G (type 3)
...
void initNetworkMode(void *param) {
...
#endif
    RILChannelCtx *pChannel = getRILChannelCtx(subSysId, rid);
    if (RatConfig_isC2kSupported()
        ...
        if (getMainProtocolRid() == rid) {
            ...
            if (activeModem != 4 && activeModem != 6) {
                ...
                property_get("ro.mtk_tdd_data_only_support", prop_tdd_only, "0");
                RLOGD("is tdd data only support: %d", atoi(prop_tdd_only));
...
static int isEratExtSupport() {
    ...
    property_get("ril.nw.erat.ext.support", eratext, "-1");
...
static int switchLteRadioCapability(int svlteMode, int roaming, RIL_Token t) {
    ...
    if (roaming != ROAMING_MODE_JPKR_CDMA) {
        if ((err >= 0 && p_response != NULL && p_response->success != 0)
            ...
            property_get("persist.sys.lock.freq.lte", lockFreqProp, "0");
            int nLockFrep = atoi(lockFreqProp);
            LOGD("Lock LTE frequency: lockFreqProp = %s, nLockFrep = %d.", lockFreqProp, nLockFrep);
            if (nLockFrep == 1) {
                at_send_command("AT+EPBSE=16,2,64,0", NULL, getDefaultChannelCtx(getMainProtocolRid()));
            }
            // lock frequency for lab test. @}
        }
    }
...
static int switchRatMode(int preferNwType, int svlteMode, int roaming, RIL_Token t) {
    ...
    property_get("persist.sys.ct.ir.mode", lcgModeProp, "0");
...
bool isERATRetrySupport() {
    // only enable in Srlte project or Single SIM for Svlte project
    bool ret = isSrlteSupport() || (isSvlteSupport() && getSimCount() == 1);
    return ret;
}
...
static int switchSvlteRatMode(int preferNwType, int svlteMode, int volteState, int roaming, RIL_Token t) {
    ...
    property_get("persist.radio.ct.ir.engmode", lwgModeProp, "0");
...
static void setLteRadioCapability() {
    ...
    ATResponse *p_response = NULL;
    int i;
    err = asprintf(&cmd, "AT+CSRA=%d,%d,%d,%d,%d,%d,%d", params[0], params[1], params[2], params[3], params[4], params[5], params[6]);
    LOGD("setLteRadioCapability(), send command %s.", cmd);
    err = at_send_command(cmd, &p_response, getRILChannelCtx(RIL_NW, getMainProtocolRid()));
    free(cmd);
    at_response_free(p_response);
}
/// @}
...
static void onLteRadioCapabilityChanged(char* urc, const RIL_SOCKET_ID rid)
{
    ...
    if (rid == getMainProtocolRid()) {
        RIL_requestProxyTimedCallback(setLteRadioCapability, NULL, &TIMEVAL_0,
                    getRILChannelCtx(RIL_NW, getMainProtocolRid())->id, "setLteRadioCapability");
    }
}
/// @}
...
void enableLteBand8(RIL_SOCKET_ID rid, RILChannelCtx *p_channel) {
    ...
    property_get("persist.operator.optr", optr, "OM");
    property_get("persist.operator.seg", seg, "0");
    if (getMainProtocolRid() != rid || !isCdmaSupport() || !isSvlteSupport() || !(getSimCount() > 1) ||
