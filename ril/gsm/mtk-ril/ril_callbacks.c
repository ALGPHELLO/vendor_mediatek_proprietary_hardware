...
extern void initNetworkMode(void * param);
...
static void onRequest(int request, void *data, size_t datalen, RIL_Token t, RIL_SOCKET_ID socket_id __unused)
{
    ...
    if (radioState == RADIO_STATE_OFF &&
        !(request == RIL_REQUEST_RADIO_POWER ||
          ...
          /// @}
          // Verizon E911
          request == RIL_REQUEST_CURRENT_STATUS ||
         ...
         if (isWfcSupport() &&
              (request == RIL_REQUEST_DIAL ||
               ...
               request == RIL_REQUEST_IMS_EMERGENCY_DIAL ||
               request == RIL_REQUEST_SET_ECC_SERVICE_CATEGORY ||
...
static void resetSystemProperties(RIL_SOCKET_ID rid)
{
    ...
    // when modem reset, we should clear the md change apn data.
    // if not, FW will use this incorrect data to do IA. Please refer to the usage of
    // PROP_APN_CLASS_ICCID in DCT
    sprintf(prop, "ril.md_changed_apn_class.iccid%d", rid);
    property_set(prop, "");
    sprintf(prop, "ril.md_changed_apn_class%d", rid);
    property_set(prop, "");
...
void queryActiveMode(RIL_SOCKET_ID rid, RILChannelCtx *p_channel) {
    ...
    int err;
    char keep_3g[PROPERTY_VALUE_MAX] = {0};
    property_get("ril.nw.worldmode.keep_3g_mode", keep_3g, "-2");
    RLOGI("keep 3g mode:%s", keep_3g);

    if (p_channel == NULL){
        err = at_send_command_multiline("AT+CSRA?", "+CSRAA:", &p_response, getDefaultChannelCtx(rid));
    } else {
...
void queryBearer(RILSubSystemId subsystem, RIL_SOCKET_ID rid)
{
    ...
    // set 4G capability
    if (isLteSupport() && (((ret & 0x0100) > 0) || ((ret & 0x0080) > 0))) {
    ...
    memset(propValue, 0, PROPERTY_VALUE_MAX);
    property_get("ril.nw.worldmode.keep_3g_mode", propValue, "0");
    mode = atoi(propValue);
    LOGD("[RIL%d] keep_3g_mode = %d", rid+1, mode);
    if (mode == 0) {
        //check if support tplusw
        memset(propValue, 0, PROPERTY_VALUE_MAX);
        property_get("ril.simswitch.tpluswsupport", propValue, "0");
        mode = atoi(propValue);
    ...
    if (isCdmaSupport()) {
        if (isDualCTCard()) {
            ...
            property_get("persist.radio.cdma_slot", cdmaSlot_prop, "1");
...
void initMsimConfiguration(void *param){
    ...
    if (getMainProtocolRid() == rid) {
        if (getMdMaxSimCount()>= 4) {
            current_share_modem = getSimCount();
        } else if (getMdMaxSimCount() >= 3) {
            current_share_modem = getSimCount();
        } else if (getMdMaxSimCount() >= 2) {
            /********************************
            *  AT+ESADM=<mode>
            *  Always respond OK
            *
            *  <mode>Obitmap
            *  0x01: SIM1 is powered-on
            *  0x02: SIM2 is powered-on
            *  0x04: SIM3 is powered-on
            *  0x08: SIM4 is powered-on
            *******************************/
            current_share_modem = getSimCount();
        } else {
            current_share_modem = 1; // singal sim
        }
...
void setSilentReboot(void *param) {
    RIL_SOCKET_ID rid = *((RIL_SOCKET_ID *)param);
    ...
    *********************************/
    if (getMainProtocolRid() == rid) {
        property_get("ril.mux.report.case", property_value, "0");
        muxreport_case = atoi(property_value);
        LOGD("getprop ril.mux.report.case %d", muxreport_case);
        ...
        property_get("gsm.ril.eboot", property_value, "-1");
        auto_unlock_pin = atoi(property_value);
        RLOGD("getprop gsm.ril.eboot %d", auto_unlock_pin);
   ...
   /*  Call Waiting notifications */
    ...
    for (i = 0; i < TOTAL_FEATURE_NUMBER; i++) {
        ...
        if (DATA_L4C_LAST_PDN_ERROR_CAUSE == i) {
            ...
            property_set("ril.md.version", mdVer);
            free(mdVer);
        }
    }
    // common data @}
...
/* M: End - abnormal event logging for logger */
    ...
    if (getMainProtocolRid() == rid) {
        err = at_send_command_singleline("AT+ERAT=?", "+ERAT:", &p_response, getDefaultChannelCtx(rid));
        if (err == 0 && p_response->success != 0 && p_response->p_intermediates != NULL) {
            property_set("ril.nw.erat.ext.support", "1");
        } else {
            RLOGD("AT+ERAT=? not support");
            property_set("ril.nw.erat.ext.support", "0");
        }
        at_response_free(p_response);
    }

    /* check if modem support +EAPC?  send EAPC setting to modem if support APC */
    err = at_send_command_singleline("AT+EAPC?", "+EAPC:", &p_response, getDefaultChannelCtx(rid));
    if (err >= 0 && p_response->success != 0) {
        // set property if modem support APC, EM will check this property to show APC setting
        property_set("ril.apc.support", "1");
    ...
    if ((s_modemCapability[rid] & RAF_LTE) == RAF_LTE
        ...
        RLOGD("Test FD value=%s,%d", property_value, (int)(atof(property_value)));
    ...
    if ((atoi(hdVoiceEnabled) == 1)) {
        ...
        at_send_command("AT+EVOCD=0", NULL, pChannel);
    }
    /// @}
    ...
    initMsimConfiguration(param);
    ///M: [Network][C2K] add network C2K init network mode flow. @{
    initNetworkMode(param);
    /// @}
    setSilentReboot(param);
    ...
    if (getMainProtocolRid() == rid) {
        ...
        bootupGetIccid(subSysId, rid);  // main protocol
        for (i = 0; i < SIM_COUNT; i++) {
            if (i != rid) {
                bootupGetIccid(subSysId, RIL_SOCKET_1 + i); // sub protocols
            }
        }
        bootupGetIccid(subSysId, rid);
        bootupGetCalData(subSysId, rid);

        const char *logmuch_detect = "logmuch.detect.delay";
    ...
    bootupGetImei(subSysId, rid);
    bootupGetImeisv(subSysId, rid);
    bootupGetBasebandVersion(subSysId, rid);
    requestGetPacketSwitchBearer(subSysId, rid);
    ...
    //bootupSetRadio(rid);
    //remove ril init flag, it seems no used
...
static void updateEnhanceModemOffVersion() {
    ...
    if (s_enhanceModemOffVersion == 0) {
        property_set("ril.cdma.enhance.version", "0");
    } else {
        property_set("ril.cdma.enhance.version", "1");
    }
...
static void queryModemFeatureVersion(void)
{
    ...
    err = at_send_command_multiline(cmd, "+EMDVER:", &p_response, getDefaultChannelCtx(0));
    free(cmd);
    if (err == 0 && p_response->success != 0 && p_response->p_intermediates != NULL) {
        ...
        while (p_cur != NULL) {
            ...
            if (0 == strcmp(feature, "DSS_NO_RESET")) {
                    ...
                    property_set("ril.simswitch.no_reset_support", property_value);
                    free(property_value);
...
static void *mainLoop(void *param __unused)
{
    ...
    while (s_main_loop) {
        ...
        if (atoi(prop_value) == 0) {
            int dongleMode = SIM_SWITCH_MAL_PHONE_MODE;
            property_get(PROPERTY_SIM_SWITCH_DONGLE_MODE, prop_value, "0");
            dongleMode = atoi(prop_value);
            RLOGI("restart MAL if modem reset while sim switching, dongleMode=%d", dongleMode);
            if (SIM_SWITCH_MAL_PHONE_MODE == dongleMode) {
                property_set(PROPERTY_SIM_SWITCH_CONTROL_MAL, "1");
            } else {
                property_set(PROPERTY_SIM_SWITCH_CONTROL_MAL, "2");
            }
        }
        ...
        if (curr_share_modem >= 2) {
            ...
            for (i = 0; i < curr_share_modem; i++) {
                if (&s_pollSimId[i] != pMainSimId) {
                    RLOGI("Start initialized callback (MSMS - Others, %d)", s_pollSimId[i]);
                    RIL_requestTimedCallback(initializeCallback, (void *)&s_pollSimId[i],
                            &TIMEVAL_0);
                }
            }
            RLOGI("Start initialized callback (MSMS - Main, %d)", *pMainSimId);
            RIL_requestTimedCallback(initializeCallback, (void *)pMainSimId, &TIMEVAL_0);
        } else {
            RLOGI("Start initialized callback (SS)");
            RIL_requestTimedCallback(initializeCallback, (void *)&s_pollSimId[0], &TIMEVAL_0);
        }
...
void setSimSwitchProp(int SimId)
{
    ...
    if (strlen(tmp) > 0) {
        ...
        RLOGD("write simswitch iccid : %s", tmp);
    }
...
static void syncModemProtocolCapability() {
    ...
    if(!RatConfig_isWcdmaSupported()) {
        property_get("persist.radio.mtk_ps2_rat", prop_value, "G");
        if (strchr(prop_value, 'W') != NULL) {
            if (strchr(prop_value, 'L') != NULL) {
                property_set("persist.radio.mtk_ps2_rat", "L/G");
            } else {
                property_set("persist.radio.mtk_ps2_rat", "G");
            }
        }
        property_get("persist.radio.mtk_ps3_rat", prop_value, "G");
        if (strchr(prop_value, 'W') != NULL) {
            if (strchr(prop_value, 'L') != NULL) {
                property_set("persist.radio.mtk_ps3_rat", "L/G");
            } else {
                property_set("persist.radio.mtk_ps3_rat", "G");
            }
        }
        property_get("persist.radio.mtk_ps4_rat", prop_value, "G");
        if (strchr(prop_value, 'W') != NULL) {
            if (strchr(prop_value, 'L') != NULL) {
                property_set("persist.radio.mtk_ps4_rat", "L/G");
            } else {
                property_set("persist.radio.mtk_ps4_rat", "G");
            }
        }
    }
}
...
void updateTplusWVersion(RIL_SOCKET_ID rid) {
    ...
    if (getMainProtocolRid() == rid) {
        ...
        err = at_send_command_singleline(cmd, "+ESBP:", &p_response, getDefaultChannelCtx(rid));
        if (err == 0 && p_response->success != 0 && p_response->p_intermediates != NULL) {
            line = p_response->p_intermediates->line;
            err = at_tok_start(&line);
            if (err >= 0) {
                ...
                if (0 == mode) {
                    property_set("ril.simswitch.tpluswsupport", "0");
                    LOGD("don't support tplusw mode");
                } else if (1 == mode) {
                    property_set("ril.simswitch.tpluswsupport", "1");
