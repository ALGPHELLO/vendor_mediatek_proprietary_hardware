...
bool hoLtePdnSuccess[MAX_SIM_COUNT] = {0};
...
void setRspEranType(int eran_type, RIL_Data_Call_Response_v11 *pRspData,
        mal_datamngr_data_call_info_req_t* responseToMal, RIL_SOCKET_ID rid);
void createMalDataResponse(int interfaceId, int cid, RIL_Data_Call_Response_v11* response,
        mal_datamngr_data_call_info_req_t* responseToMal, RIL_SOCKET_ID rid);
...
void querySuggestionTimeFromRds(const reqSetupConf_t *pReqSetupConfig,
    RIL_Data_Call_Response_v11* responseFromRds, RIL_Token t);
...
#define MTK_CC33_SUPPORT "persist.data.cc33.support"
...
void ril_data_setflags(int s, struct ifreq *ifr, int set, int clr)
{
    ...
    return;
terminate:
    LOGE("ril_data_setflags: error in set %s:%d - %d:%s", flag, ret, errno, strerror(errno));
    free(flag);
    return;
}
...
static void requestOrSendDataCallList(RIL_Token *t, RIL_SOCKET_ID rid)
{
    ...
    if (t != NULL) {
        ...
        rilchnlctx = getChannelCtxbyProxy(rid);
    }
...
error:
    ...
    if (isEpdgSupport()) {
        if (reqSetupConf.retryCount == PDN_SETUP_THRU_MAX) { //only wifi rat and setupData fail.
            RIL_Data_Call_Response_v11* response = (RIL_Data_Call_Response_v11*)
                calloc(1, sizeof(RIL_Data_Call_Response_v11));
        ...
        if (strlen(iaProperty) == 0 && isApnDiff) {
            ...
            if (0 == check) {
                ...
                RIL_requestProxyTimedCallback (onAttachApnReset,
                       param,
                       &TIMEVAL_300_ms,
                       getRILChannelCtx(RIL_DATA, rid)->id,
                       "onAttachApnReset");
                goto error;
            }
            ...
            //update IA APN ICCID
            LOGV("IA: requestSetInitialAttachApn update IA ICCID [%s]", iccid);
...
set patch to invalid value");
            ...
            // M: OP17 IA Start
            getMSimProperty(rid, "gsm.ril.sim.op17", optr);
...
void requestSyncDataSettingsToMd(void* data, size_t datalen, RIL_Token t) {
    ...
    // M: Data Framework - CC 33
    if (isCC33Support() == 1) {
        asprintf(&cmd, "AT+EDSS=%d", dataOn);
        at_send_command(cmd, NULL, DATA_CHANNEL_CTX);
        free(cmd);
    }

    // AT+ECNCFG=<mobile_data>,<data_roaming>,[<volte>,<ims_test_mode>]
    asprintf(&cmd, "AT+ECNCFG=%d,%d", dataOn, dataRoamingOn);
    at_send_command(cmd, NULL, DATA_CHANNEL_CTX);
    free(cmd);
...
void onReAttachForSettingAttachApn(void* param) {
    ...
    reAttach->pDataChannel = getRILChannelCtx(RIL_DATA, getRILIdByChannelCtx(pChannel));
    char* apn = pInfo->apn;
...
bool isKeepDefaultPdn(int interfaceId, RIL_SOCKET_ID rid, char* apn) {
    ...
    if ((isOp01Support() && NULL != strcasestr(apn, "cmnet")) ||
        ...
        if (isEpdgSupport() || 0 == strcmp(dataRat, "LTE")) {
            return true;
        }
    } else if (0 == strcmp(operatorNumeric, "23430") || isEpdgSupport()) {
...
extern int rilDataUnsolicited(const char *s, const char *sms_pdu, RILChannelCtx* p_channel)
{
    ...
    RILChannelCtx* pDataChannel = getChannelCtxbyId(sCmdChannel4Id[rid]);
    ...
    if (strStartsWith(s, "+CGEV: NW DEACT") || strStartsWith(s, "+CGEV: ME DEACT") ||
        ...
        if (strStartsWith(s, "+CGEV: NW MODIFY")) {
            ...
            RIL_requestProxyTimedCallback(getAndSendPcoStatusWithParam, paramPco,
                &TIMEVAL_0, pDataChannel->id, "getAndSendPcoStatusWithParam");
            // M: [VzW] Data Framework @}
        } else
        ...
        if (strStartsWith(s, "+CGEV: NW DETACH")) {
            RIL_requestProxyTimedCallback(onNwDetach, &s_data_ril_cntx[rid],
                    &TIMEVAL_0, pDataChannel->id, "onNwDetach");
            return 1;
        }
    ...
    // M: [C2K][IRAT] URC start. {@
        ...
        RIL_requestProxyTimedCallback(onLwcgIratEvent, param, &TIMEVAL_0,
                pDataChannel->id, "onLwcgIratEvent");
        return 1;
    ...
    //M: [C2K][IRAT] URC end. @}
        ...
        return 1;
            ...
            RIL_requestProxyTimedCallback(onPcoStatus, param,
                    &TIMEVAL_0, pDataChannel->id, "onPcoStatus");
            return 1;
    } else if (strStartsWith(s, "+ELCE:")) {
        // LCE service
        onLceDataResult((char*) s, rid);
        return 1;
    // M: [LTE][Low Power][UL traffic shaping] @{
...
// LCE service start
void onLceDataResult(char* urc, RIL_SOCKET_ID rid)
{
    int ret;
...
error:
    LOGE("[%s] parsing error", __FUNCTION__);
...
void requestSetFdMode(void * data, size_t datalen, RIL_Token t) {
    ...
    if (paramNumber == 1) {
        ...
        snprintf(cmdLine, sizeof(cmdLine), "AT+EFD=%d,%d,%d", ((int *)data)[1], ((int *)data)[2], ((int *)data)[3]);
    } else {
        LOGE("requestSetFdMode: weird, should never be here!");
    }
    LOGD("requestSetFdMode: cmd = %s", cmdLine);
...
void onReattachForSimHotPlug(void* param) {
    ...
    reAttach->mode = ALWAYS_ATTACH_WITH_CHECK;
    reAttach->pDataChannel = getRILChannelCtx(RIL_DATA, rid);
...
int getDataEnable(RIL_SOCKET_ID rid)
{
    ...
    LOGI("getDataEnable: persist.radio.mobile.data=%s", dataEnable);
    if(strlen(dataEnable) == 0) {
        ...
        while (token != NULL) {
            LOGI("getDataEnable: token = %s, iccid = %s", token, maskIccid);
...
void onSimInsertChangedForData(RIL_SOCKET_ID rid, const char* urc) {
    ...
    if (strStartsWith(urc, "+ESIMS: 1,12")) {
        ...
        RIL_requestProxyTimedCallback(onReattachForSimHotPlug, param,
                &TIMEVAL_0, pDataChannel->id, "onReattachForSimHotPlug");
     }
}
...
void getPdpCapability(RIL_Token t)
{
    ...
    if (t) {
        ...
        err = at_send_command_multiline("AT+CGDCONT=?", "+CGDCONT:", &p_response, getDefaultChannelCtx(getMainProtocolRid()));
    }
    if (isATCmdRspErr(err, p_response)) {
    ...
    if (t) {
        ...
        LOGI("The ril token is null, use URC instead");
        at_send_command("AT+EPDNCTRANS=1", NULL, getDefaultChannelCtx(getMainProtocolRid()));
        err = at_send_command_singleline("AT+EGPCAP=?", "+EGPCAP:", &p_response, getDefaultChannelCtx(getMainProtocolRid()));
    }
...
void getL2pValue(RIL_Token t) {
    ...
    if (t) {
        ...
        LOGI("The ril token is null, use URC instead");
        err = at_send_command_singleline("AT+CGDATA=?", "+CGDATA:", &p_response, getChannelCtxbyProxy(RIL_SOCKET_1));
    }
...
int updatePdnAddressByCid(int cid, RILChannelCtx* rilchnlctx)
{
    ...
    LOGD("[%d]getPdpAdrrByCid response address [%d, %s, %s]", rid, cid,
            pdn_info[rid][responseCid].addressV4,
            pdn_info[rid][responseCid].addressV6);
    return 0;
error:
        ...
        for (i = 0; i < MAX_NUM_DNS_ADDRESS_NUMBER*2; i++) {
            ...
            if (out != NULL && strlen(out) > 0 && strcmp(NULL_IPV4_ADDRESS, out) != 0) {
                if (get_address_type(out) == IPV6) {
                    ...
                    LOGD("updateDns DNS%d V6 for cid%d %s", v6Count+1, responseCid,
                            pdn_info[rid][responseCid].dnsV6[v6Count]);
                    ++v6Count;
                } else {
                    strncpy(pdn_info[rid][responseCid].dnsV4[v4Count], out, MAX_IPV4_ADDRESS_LENGTH-1);
                    LOGD("updateDns DNS%d V4 for cid%d %s", v4Count+1, responseCid,
                            pdn_info[rid][responseCid].dnsV4[v4Count]);
                    ++v4Count;
                }
            }
        }
    ...
    for (p_cur = p_response->p_intermediates; p_cur != NULL; p_cur = p_cur->p_next) {
        ...
        for(i = 0; i<length; i++) {
            ...
            if (dotCount > 7 && innerDotCount == 16) {//the end of IPv6 address
                ...
                LOGD("updateTft get packet filter[%d] address & mask (IPv6) [%d, %s, %s]",
                    pkFilter->id, responseCid, pkFilter->address, pkFilter->mask);
                ...
                strncpy(pkFilter->mask, addressAndMask + i + 1,
                    MIN(sizeof(pkFilter->mask), (sizeof(addressAndMask) - i + 1))); //add one since there is a dot between address and mask
                LOGD("updateTft get packet filter[%d] address & mask (IPv4) [%d, %s, %s]",
                    pkFilter->id, responseCid, pkFilter->address, pkFilter->mask);
...
void convertToGlobalAddress(char* addressV6, char* prefix) {
    ...
    inet_pton(AF_INET6, addressV6, &ip);
        ...
        mal_datamngr_data_call_info_req_t* responseToMal, RIL_SOCKET_ID rid)
{
    ...
    responseToMal->eran_type = response->eran_type;
...
int createDataResponse(int interfaceId, int protocol, RIL_Data_Call_Response_v11* response, RIL_SOCKET_ID rid)
{
    ...
    for (i = 0; i < WIFI_MAX_PDP_NUM; i++) {
        if (pdn_info_wifi[rid][i].interfaceId == interfaceId &&
            ...
            if (hoLtePdnSuccess[rid] == true) {
                LOGD("[%d][%s] hoLtePdnSuccess: ignore wifi PDN", rid, __FUNCTION__);
                break;
            }
            /// @}
            isOverEpdg = true;
            ...
            response->eran_type = pdnInfo->eran_type;
        }
    }
    if (targetPdnIndex >= 0) {
...
void freeDataResponse(RIL_Data_Call_Response_v11* response)
{
    if (response->ifname != NULL) {
        LOGD("freeDataResponse: type=%s, ifname=%s, addresses=%s, dnses=%s, gateways=%s, pcscf=%s",
...
type=%s, ifname=%s, addresses=%s, dnses=%s, gateways=%s, pcscf=%s",
            ...
            response->dnses, response->gateways, response->pcscf);
}
...
QOS[qci=%d, dlGbr=%d, ulGbr=%d, dlMbr=%d, ulMbr=%d], PF number=%d, TftParameter[linked: %s]",
    ...
    if (response->tft.pfNumber > 0) {
        for (i=0; i<response->tft.pfNumber; i++) {
            LOGD("%s dedicate data call response CID%d-%d: TFT[id=%d, precedence=%d, bitmap=0x%08x, address=%s, \
...
spi=%08x, tos=%d, tosMask=%d, flowLabel=%05x, direction=%d, networkPfIdentifier=%d]",
                ...
                response->tft.pfList[i].id, response->tft.pfList[i].precedence, response->tft.pfList[i].bitmap, response->tft.pfList[i].address,
...
int queryMatchedPdnWithSameApn(const char* apn, int* matchedCidList, RILChannelCtx* rilchnlctx)
{
     ...
     */
    property_get("ril.pdn.reuse", reuse, "1");
    if (strcmp("0", reuse) == 0) {
        LOGI("[%d]queryMatchedPdnWithSameApn not reuse is set", rid);
        return 0;
    } else if (strcmp("2", reuse) == 0) {
        property_get("ril.pdn.name.reuse", reuseApnName, "");
        LOGD("[%d]queryMatchedPdnWithSameApn: ril.pdn.name.reuse = %s", rid, reuseApnName);
    }
...
int isCidTypeMatched(int cid, int protocol, RILChannelCtx* rilchnlctx)
{
    ...
    if (pdn_info[rid][cid].active != DATA_STATE_INACTIVE) {
        ...
        switch (protocol) {
            ...
            if ((strlen(pdn_info[rid][cid].addressV4) > 0
                    && strcmp(pdn_info[rid][cid].addressV4, NULL_IPV4_ADDRESS) != 0)
                    || (strlen(pdn_info[rid][cid].addressV6) > 0
                            && strcmp(pdn_info[rid][cid].addressV6,
                                    NULL_IPV4_ADDRESS))) {
                    result = 1;
                }
                break;
    ...
    if (result == 0) {
        LOGD("[%d]isCidTypeMatched result=%d [cid=%d, active=%d, protocol=%d, addrV4=%s, addrV6=%s]",
                rid, result, cid, pdn_info[rid][cid].active, protocol,
                pdn_info[rid][cid].addressV4, pdn_info[rid][cid].addressV6);
    }
    return result;
}
...
char* getConfigTftATCmd(int cid, PktFilter* pktFilter)
{
    ...
    if ((pktFilter->bitmap & PKT_BITMASK_IMC_BMP_V4_ADDR) || (pktFilter->bitmap & PKT_BITMASK_IMC_BMP_V6_ADDR)) {
        if (pktFilter->bitmap & PKT_BITMASK_IMC_BMP_V6_ADDR) {
            ...
            LOGD("getConfigTftATCmd convert IP format before: address[%s], mask[%s]", pktFilter->address, pktFilter->mask);
            ...
            LOGD("getConfigTftATCmd convert IP format after: address[%s], mask[%s]", pktFilter->address, pktFilter->mask);
        }
...
int clearQosTftContext(int cid, RILChannelCtx* rilchnlctx)
{
    ...
    if (max_pdn_support_4G > 0)
        asprintf(&cmd, "AT+CGEQOS=%d", cid);
    else
        asprintf(&cmd, "AT+CGEQREQ=%d", cid);
...
int convert_ipv6_address(char* pdp_addr, char* ipv6_addr, int isLinkLocal) {
    ...
    while (p) {
        value = atoi(p);
        snprintf(ipv6_addr, MAX_IPV6_ADDRESS_LENGTH, "%s%02X", ipv6_addr, value);
        p = strtok(NULL, ".");
        if (p == NULL) {
    ...
    LOGD("before convert_ipv6_address:%s", ipv6_addr);
    len = strlen(ipv6_addr);
    ipv6_addr[len-1] = '\0';
    LOGD("convert_ipv6_address:%s", ipv6_addr);
    ...
    RIL_Token t)
{
    ...
    // TODO: tmp using SIM_CHANNEL_CTX, need to use new channel_ctx (ask chunyi while merge back)
    RILChannelCtx* pRilchnlctx = getRILChannelCtx(RIL_SIM, rid);
    ...
    RIL_Data_Call_Response_v11 *response = calloc(1, sizeof(RIL_Data_Call_Response_v11));
    ...
    RIL_Token t)
{
    ...
    if (response == NULL) {
        LOGE("[%d][RILData_GSM] requestSetupDataCallOverIPv6: response shouldn't be null", rid);
        goto finish;
    }
    ...
    if (queryResult > 0 && isCidTypeMatched(activatedCidList[0], protocol, DATA_CHANNEL_CTX)) {
        ...
        property_get("ril.pdn.overrideApn", overrideApn, "");
        if (strlen(overrideApn) > 0) {
            ...
            property_set("ril.pdn.overrideApn", "");
        } else {
    ...
    RIL_Token t)
{
    ...
    RIL_Data_Call_Response_v11* response =
            (RIL_Data_Call_Response_v11*) malloc(sizeof(RIL_Data_Call_Response_v11));
    assert(response != NULL);
    mal_datamngr_data_call_info_req_t* responseToMal = (mal_datamngr_data_call_info_req_t*)
            calloc(1, sizeof(mal_datamngr_data_call_info_req_t));
    assert(responseToMal != NULL);
    ...
    if ((IPV4V6 + 1) == isAddrV4V6Bitmap) {
        ...
        int queryResult = queryMatchedPdnWithSameApn(requestedApn, activatedCidList, DATA_CHANNEL_CTX);
        if (queryResult > 0 && isCidTypeMatched(activatedCidList[0], protocol, DATA_CHANNEL_CTX)) {
            if (strlen(pdn_info[rid][activatedCidList[0]].addressV4) > 0
                    && strcmp(pdn_info[rid][activatedCidList[0]].addressV4, NULL_IPV4_ADDRESS) != 0) {
                isAddrV4V6Bitmap |= IPV4 + 1;
            }
            if (strlen(pdn_info[rid][activatedCidList[0]].addressV6) > 0
                    && strcmp(pdn_info[rid][activatedCidList[0]].addressV6, NULL_IPV4_ADDRESS) != 0) {
                isAddrV4V6Bitmap |= IPV6 + 1;
            }
...
error:
    ...
    if (CHANNEL_CLOSED == errPdnContAct) {
        ...
        RIL_onRequestComplete(t, RIL_E_GENERIC_FAILURE,
                response, sizeof(RIL_Data_Call_Response_v11));
    }
...
void requestOrSendDataCallListIpv6(RILChannelCtx* rilchnlctx, RIL_Token *t, RIL_SOCKET_ID rid)
{
    ...
    RIL_Data_Call_Response_v11* responses = (RIL_Data_Call_Response_v11*) calloc(1,
            maxCcmniNum * sizeof(RIL_Data_Call_Response_v11));
    ...
    if (activeCount > 0) {
        ...
        responsesOfActive = (RIL_Data_Call_Response_v11*)
                calloc(1, activeCount * sizeof(RIL_Data_Call_Response_v11));
...
int syncApnTableToMd(void* data, size_t datalen, RIL_Token t) {
    ...
    for (int i = 0; i < num; i++) {
        ...
        property_get("persist.radio.throttling_time", throttling_time, "900");
...
int getApnProfileId(int apnType) {
    ...
    switch (apnType) {
        ...
        case RIL_APN_TYPE_EMERGENCY:
            ...
            break;
        case RIL_APN_TYPE_DM:
            profileId = RIL_DATA_PROFILE_VENDOR_DM;
            break;
        case RIL_APN_TYPE_WAP:
            profileId = RIL_DATA_PROFILE_VENDOR_WAP;
            break;
        case RIL_APN_TYPE_NET:
            profileId = RIL_DATA_PROFILE_VENDOR_NET;
            break;
        case RIL_APN_TYPE_CMMAIL:
            profileId = RIL_DATA_PROFILE_VENDOR_CMMAIL;
            break;
        case RIL_APN_TYPE_TETHERING:
            profileId = RIL_DATA_PROFILE_TETHERED;
            break;
        case RIL_APN_TYPE_RCSE:
            profileId = RIL_DATA_PROFILE_VENDOR_RCSE;
            break;
    ...
    if (isMultipleImsSupport() || getMainProtocolRid() == rid) {
        ...
        for(int i = 0; i < dataCount; i++) {
            ...
            if (apnTypesBitmask == RIL_APN_TYPE_UNKNOWN ||
                ...
                for (int j = 0; j < (MAX_COUNT_APN_TYPE_ID - 3); j++) {
...
void onPdnDeactResult(char* urc, RIL_SOCKET_ID rid) {
    ...
    if (strlen(urc) > 0) {
        ...
        if (!hasSameInterfaceCid && !bMeDeact) {
            LOGI("[%d]Deactivate PDN CID/interfaceId/hoStart/eran is [%d, %d, %d, %d]",
                    rid, cid, interfaceId, bPdnHoStart, pdn_info[rid][cid].eran_type);
            // pdn_info[rid][cid].deactbyNw = 1;
            if (!bPdnHoStart && pdn_info[rid][cid].eran_type != RDS_RAN_WIFI) {
        ...
        if (pdn_info[rid][cid].interfaceId != INVALID_CID
                && bPdnHoStart && pdn_info[rid][cid].eran_type != RDS_RAN_WIFI) {
        ...
        if (!bMeDeact) {
            if (!bPdnHoStart) {
                if (pdn_info[rid][cid].eran_type != RDS_RAN_WIFI) {
                    clearPdnInfo(&pdn_info[rid][cid]);
                    ...
                    clearPdnInfo(&pdn_info[rid][cid]);
                    pdn_info[rid][cid].eran_type = RDS_RAN_WIFI;
    ...
    if (s_md_off) {
        ...
        requestOrSendDataCallListIpv6(getChannelCtxbyProxy(rid), NULL, rid);
    /// M: Ims Data Framework {@
    ...
    if (at_tok_hasmore(&line)) {
        ...
        if (pcoData.cid != -1) {
            ...
            if (name != NULL && strlen(name) > 0
                ...
                assert(pcoDataAttached.contents != NULL);
        ...
        int authType, const char *username, const char* password, RILChannelCtx *pChannel)
{
    ...
    // [mtu]
    property_get("ril.data.query.mtu", tmp, "1"); // [query mtu]  0:turn off ; 1: turn on
    queryMtu = atoi(tmp);
    ...
    if (max_pdn_support_4G > 0) {
        asprintf(&cmd, "AT+CGDCONT=%d,\"%s\",\"%s\",,0,0,0,%d,%d,%d",availableCid, pszIPStr,
                requestedApn, emergency_ind, pcscf_discovery_flag, signalingFlag);
        ...
        asprintf(&cmd, "AT+CGDCONT=%d,\"%s\",\"%s\",,0,0",availableCid, pszIPStr, requestedApn);
    }
    ...
    if (g_iaRoamingProtocol[iaSimId] != NULL) {
        asprintf(&cmd, "AT+EGDCONT=%d,\"%s\"", availableCid, g_iaRoamingProtocol[iaSimId]);
    ...
    *pConcatenatedBearerNum = concatenatedBearerNum;
    ...
    for (p_cur = p_response->p_intermediates; p_cur != NULL; p_cur = p_cur->p_next) {
        ...
        if (strStartsWith(line, " ME PDN ACT ")) {
            ...
            pdn_info[rid][availableCid].eran_type = RDS_RAN_MOBILE_3GPP;
            ++activatedPdnNum;
        }
    }
...
int getAttachApnSupport()
{
    ...
    if (attachApnSupport == -1) {
        ...
        err = at_send_command_singleline("AT+EGMR=0,16", "+EGMR:", &p_response,
                getChannelCtxbyProxy(getMainProtocolRid()));
        if (err < 0 || p_response->success == 0) {
...
to always attach mode");
            ...
            if (atoi(iaPwdFlag)) {
                ...
                property_get("ro.mtk_flight_mode_power_off_md", powerOffModem, "0");
...
void clearAttachApnCacheIfNecessary(RIL_SOCKET_ID rid, const char* urc)
{
            ...
            strStartsWith(urc, "+ESIMS: 1,8")) //DUALSIM_CONNECTED
    {
        ...
        RIL_requestProxyTimedCallback (onSetAttachApn, pChannel, &TIMEVAL_0, pChannel->id,
                "onSetAttachApn");
    }
...
void dumpPdnInfo(PdnInfo * pPdnInfo)
{
    if (pPdnInfo) {
        int i = 0;
        LOGD("[%s] pdn info: intfid=%d, primaryCid=%d, cid=%d, ddcid=%d, isDedicatedBearer=%d, \
isEmergency=%d, active=%d, signalingFlag=%d, apn=%s, addrV4=%s, addrV6=%s", __FUNCTION__,
            pPdnInfo->interfaceId, pPdnInfo->primaryCid, pPdnInfo->cid,
            pPdnInfo->ddcId, pPdnInfo->isDedicateBearer, pPdnInfo->isEmergency,
            pPdnInfo->active, pPdnInfo->signalingFlag, pPdnInfo->apn,
            pPdnInfo->addressV4, pPdnInfo->addressV6);
        for (; i < MAX_NUM_DNS_ADDRESS_NUMBER; i++) {
            LOGD("[%s] ipv4Dns[%d]: %s", __FUNCTION__, i, pPdnInfo->dnsV4[i]);
        }
        for (i = 0; i < MAX_NUM_DNS_ADDRESS_NUMBER; i++) {
            LOGD("[%s] ipv6Dns[%d]=%s", __FUNCTION__, i, pPdnInfo->dnsV6[i]);
        }
        for (i = 0; i < MAX_PCSCF_NUMBER; i++) {
            LOGD("[%s] pcscf[%d]=%s", __FUNCTION__, i, pPdnInfo->pcscf[i]);
        }
        LOGD("[%s] bearerId=%d, isNotDataPdn=%d, mtu=%d, eran_type=%d", __FUNCTION__,
            pPdnInfo->bearerId, pPdnInfo->isNoDataPdn, pPdnInfo->mtu, pPdnInfo->eran_type);
    }
}
...
bool isIpv6LinkLocal(const char *ipv6Addr) {
    if (ipv6Addr) {
        ...
        LOGD("isIpv6LinkLocal: ipv6 address: %s", ipv6Addr);
...
void storeModemSuggestedRetryTime(int time, RIL_Data_Call_Response_v11* response) {
...
void requestSetActivePsSimSlot(void * data, size_t datalen, RIL_Token t) {
    ...
    RILChannelCtx* p_channel = getRILChannelCtx(RIL_DATA, getMainProtocolRid());
    int err = 0;
...
int onPdnSyncFromSourceRat(void* param) {
    ...
    pdn_info[rid][cid].eran_type = RDS_RAN_MOBILE_3GPP;
    ...
    LOGI("[%d][RILData_GSM_IRAT] onPdnSyncFromSourceRat: cid = %d, interfaceId = %d, "
                "iRatPdnStatus = %d, pdnCount = %d, apn = %s, ipv4 = %s, ipv6 = %s.",
            rid, pSyncPdnInfo[cid].cid, pSyncPdnInfo[cid].interfaceId, pSyncPdnInfo[cid].pdnStatus,
            nReactPdnCount, pdn_info[rid][cid].apn, pSyncPdnInfo[cid].addressV4,
            pSyncPdnInfo[cid].addressV6);
    free(timeCallbackParam->urc);
    free(timeCallbackParam);
    return 0;
error:
        ...
        if (pdn_info[rid][i].interfaceId == interfaceId) {
            if (strlen(pSyncPdnInfo[i].addressV4) > 0) {
                if (strcmp(pSyncPdnInfo[i].addressV4, pdn_info[rid][i].addressV4) != 0) {
                    resetMask |= 0x01;
                    LOGI("[%d]IPv4 address lost during IRAT %d, old = %s, new=%s.", interfaceId,
                            rid, pSyncPdnInfo[i].addressV4, pdn_info[rid][i].addressV4);
                }
            }
            if (strlen(pSyncPdnInfo[i].addressV6) > 0) {
                if (strcmp(pSyncPdnInfo[i].addressV6, pdn_info[rid][i].addressV6) != 0) {
                    resetMask |= 0x02;
                    LOGI("[%d]IPv6 address lost during IRAT %d, old = %s, new=%s.", rid,
                            interfaceId, pSyncPdnInfo[i].addressV6, pdn_info[rid][i].addressV6);
                } else {
                    // IPv6 prefix may change even if the interface-id is not changed
                    resetMask |= 0x04;
                    LOGI("[%d]Only IPv6 address, need to update after IRAT %d, old = %s, new=%s.",
                            rid, interfaceId, pSyncPdnInfo[i].addressV6, pdn_info[rid][i].addressV6);
                }
            }
        }
    }
...
void onUpdateIratStatus(RIL_SOCKET_ID rid) {
    ...
    RILChannelCtx* pDataChannel = getChannelCtxbyProxy(rid);
...
void setRspEranType(int eran_type, RIL_Data_Call_Response_v11 *pRspData,
        mal_datamngr_data_call_info_req_t* responseToMal, RIL_SOCKET_ID rid)
{
    if (isImsSupport() && responseToMal != NULL) {
        int cid = responseToMal->defaultBearer.cid;
        if (cid >= WIFI_CID_OFFSET) {
            pdn_info_wifi[rid][cid - WIFI_CID_OFFSET].eran_type = eran_type;
        } else {
            pdn_info[rid][cid].eran_type = eran_type;
        }
        pRspData->eran_type = eran_type;
                ...
                if (isPdnHoStart(hoStatus.ucho_status)) {
                    ...
                    if (isHOSuccess(hoStatus.fgho_result)) {
                        ...
                        if (nWifiCid == INVALID_CID) {
                            ...
                            pPdnInfo->eran_type = RDS_RAN_WIFI;
    ...
    RIL_Token t)
{
    ...
    if (strcmp(pPdnInfo->addressV6, "::") != 0) {
        struct in6_addr ip;
        inet_pton(AF_INET6, pPdnInfo->addressV6, &ip);
...
void setOpPropertyBySim(RIL_SOCKET_ID rid, char *mccmnc) {
    ...
    const char *propConditionallyAttach = "gsm.ril.cond.attach";
    const char *propOp129Sim = "gsm.ril.sim.op129";
    if (isOp17MccMnc(mccmnc) || isOp129MccMnc(mccmnc)) {
        if (isOp17MccMnc(mccmnc)) {
            setMSimProperty(rid, "gsm.ril.sim.op17", "1");
        } else {
            property_set(propOp129Sim, "1");
        }
        property_set(propConditionallyAttach, "1");

        LOGD("[setOpPropertyBySim]-%d:, %s", rid, propConditionallyAttach);

        if (getMainProtocolRid() == rid &&
                !isInternationalRoamingEnabled() &&
                ((isWorldPhoneSupport() && bUbinSupport) || !isWorldPhoneSupport()) &&
                !bSetOpPropOneShot) {
            at_send_command("AT+ECOPS=1", NULL, getRILChannelCtx(RIL_SIM, rid));
            at_send_command("AT+EMSR=0,1", NULL, getRILChannelCtx(RIL_SIM, rid));
            property_set(propDataOpSusRsmMd, "1");
            LOGD("[setupOpPropertyCb]-%d: %s 1", rid, propDataOpSusRsmMd);
        } else if (!bSetOpPropOneShot) {
            property_set(propDataOpSusRsmMd, "0");
            LOGD("[setupOpPropertyCb]-%d: %s 0", rid, propDataOpSusRsmMd);
        }
        bSetOpPropOneShot = 1;
    }
}
...
void setOpPropertyBySbp(RIL_SOCKET_ID rid) {
    ...
    property_get("persist.radio.sim.sbp", sbpId, "0");
    int sbp = atoi(sbpId);
    LOGD("[setupOpPropertyCb]-%d: got d-sbp = %d", rid, sbp);
...
void setupOpPropertyCb(void* param) {
    ...
    if (hasCallback) {
        ...
        RIL_requestProxyTimedCallback(setupOpPropertyCb, paramNew, &TIMEVAL_500_MS,
                getRILChannelCtx(RIL_SIM, rid)->id, "setupOpPropertyCb");
    } else {
...
int setApnControlMode(RIL_SOCKET_ID rid) {
    ...
    property_get("ril.imshandover", imsHandOver, "0");
    ...
    if ((0 == strcmp(imsHandOver, "1")) || (0 == strcmp(imsHandOver, "2"))) {
        ...
        if (0 == strcmp(imsHandOver, "1")) {
            ...
            property_set("ril.imshandover", "0");
...
void querySuggestionTimeFromRds(const reqSetupConf_t *pReqSetupConfig,
    RIL_Data_Call_Response_v11* response, RIL_Token t) {
