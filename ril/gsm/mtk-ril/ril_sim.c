        ...
        switch (at_get_cme_error(p_response)) {
            ...
            case CME_SIM_WRONG:
                ...
                if (retryCounts.pin1 == 0 && retryCounts.puk1 == 0)
                {
                    ...
                    property_get("gsm.gcf.testmode", property_value, "0");
...
//MTK-END [mtkXXXXX][120208][APLS00109092] Replace "RIL_UNSOL_SIM_MISSING in RIL.java" with "acively query SIM missing status"
    ...
    if (sim_status == USIM_NOT_READY || sim_status == SIM_NOT_READY) {
    ...
    if (sim_status == SIM_ABSENT) {
        ...
        num_apps = 0;
    // MTK-START: AOSP SIM PLUG IN/OUT
    } else if (sim_status == UICC_NO_INIT) {
    ...
    // MTK-END
        ...
        // system properties.
        char eccSource[PROPERTY_VALUE_MAX] = {0};
        property_get("ril.ef.ecc.support", eccSource, "");
        if (strlen(eccSource) == 0 || atoi(eccSource) == 0) {
            int simId = getMappingSIMByCurrentMode(rid);
...
void resetSIMProperties(RIL_SOCKET_ID rid) {
    ...
    property_get("ril.ef.ecc.support", eccSource, "");
    if (strlen(eccSource) == 0 || atoi(eccSource) == 0) {
        property_set(PROPERTY_ECC_LIST[rid], NULL);
    }
}
...
void enforceQueryAllIccId() {
    ...
    for (i = 0; i < SIM_COUNT ; i++) {
        ...
        if (p_response->success == 0) {
            ...
            switch (at_get_cme_error(p_response)) {
                case CME_SUCCESS:
                    ...
                    break;
                case CME_SIM_NOT_INSERTED:
                    if (isSvlteSupport() || isSrlteSupport()) {
                    property_set(PROPERTY_ICCID_SIM[i], "");
                        RLOGD("enforceQueryAllIccId set ril.iccid.sim%d to NULL", i);
                    } else {
                        property_set(PROPERTY_ICCID_SIM[i], "N/A");
        ...
        if (ret != RIL_E_SUCCESS) {
            RLOGE("enforceQueryAllIccId: Error or no SIM inserted!");
            at_response_free(p_response);
            p_response = NULL;
            continue;
        }
        ...
        property_set(PROPERTY_ICCID_SIM[i], iccId);
...
bool bIsTc1()
{
    ...
    if (siTc1 < 0)
    {
        ...
        property_get("ro.mtk_tc1_feature", cTc1, "0");
        siTc1 = atoi(cTc1);
    }
...
void detectTestSim(RIL_SOCKET_ID rid)
{
    ...
    int err = at_send_command_singleline("AT+ETESTSIM?", "+ETESTSIM:", &p_response, getChannelCtxbyProxy(rid));
    if (err < 0 || p_response->success == 0)
...
void queryEfDir(RIL_SOCKET_ID rid) {
    ...
    do {
        err = at_send_command_singleline(cmd, "+CRSM:", &p_response, getChannelCtxbyProxy(rid));
        if (err < 0 || p_response->success == 0) {
...
static void queryIccId(void *param) {
    ...
    if (strStartsWith(urc, "+ESIMS: 1,9")) {
        RLOGD("RILd,RIL_UNSOL_VIRTUAL_SIM_ON");
        RIL_UNSOL_RESPONSE(RIL_UNSOL_VIRTUAL_SIM_ON, &sim_inserted_status, sizeof(int), rid);
...
detectSimDone:
    ...
    switch (cardPresent) {
        case 1:
            ...
            if (isSupportC2kSim()) {
                ...
                if (isUsimDetect[rid] == 0 || isUsimDetect[rid] == 1) {
                    ...
                    // Wait for +EUSIM
                    RIL_requestProxyTimedCallback (detectSim, mParam, &TIMEVAL_PIN_SIM, cid,
                            "detectSim");
                    acquire_wake_lock(PARTIAL_WAKE_LOCK, "sim_hot_plug");
                }
            } else {
                RLOGD("To query iccid");
                RIL_requestProxyTimedCallback (queryIccId, mParam, &TIMEVAL_PIN_SIM, cid,
                        "queryIccId");
                acquire_wake_lock(PARTIAL_WAKE_LOCK, "sim_hot_plug");
            }
            break;
        ...
        case 14:
            ...
            RIL_requestProxyTimedCallback (detectSim, mParam, &TIMEVAL_PIN_SIM, cid, "detectSim");
            acquire_wake_lock(PARTIAL_WAKE_LOCK, "sim_hot_plug");
            break;
        default:
...
static void onCdmaDetected(const char *s, RIL_SOCKET_ID rid) {
    ...
    const char *cardTypeSet = PROPERTY_RIL_CARD_TYPE_SET;
    const char *cardTypeSet_2 = PROPERTY_RIL_CARD_TYPE_SET_2;
    char card_type[20] = {0};
    char tmp[PROPERTY_VALUE_MAX] = {0};
    ...
    if ((isSvlteSupport() || isSrlteSupport()) && (isUsimDetect[rid] == 2 &&
        ...
        RIL_requestProxyTimedCallback(queryIccId, (void *)param, &TIMEVAL_PIN_SIM,
                p_cctx->id, "queryIccId");
    }

    property_get("ril.cfun.send", tmp, "0");
    property_get(PROPERTY_RIL_CT3G_ROAMING[rid], tmp2, "0");
    RLOGD("ril.cfun.send : %s, %s: %s", tmp, PROPERTY_RIL_CT3G_ROAMING[rid], tmp2);
    if (strcmp("1", tmp2) == 0) {
...
static void switchStkUtkMode(void *param) {
    ...
    RILChannelCtx* p_channel = getChannelCtxbyProxy(rid);
...
void switchStkUtkModeByCardType(RIL_SOCKET_ID rid) {
    ...
    if (strcmp(tmp, "SIM") == 0 || strcmp(tmp, "USIM") == 0) {
        ...
        RIL_requestProxyTimedCallback(switchStkUtkMode, param, NULL, p_cctx->id, funcString);
...
void onUsimDetected(const char *s, RIL_SOCKET_ID rid) {
     ...
     */
    char tmp[PROPERTY_VALUE_MAX] = {0};
    property_get("persist.radio.testsim.cardtype", tmp, "0");
    LOGD("onUsimDetected persist.radio.testsim.cardtype : %s", tmp);
...
void onSimInsertChanged(const char *s,RIL_SOCKET_ID rid)
{
    ...
    if ((supportC2k == 1) && strStartsWith(s, "+ESIMS: 0,11")) {
        ...
        param->rid = rid;
        char* funcString = "switchStkUtkMode";
        RIL_requestProxyTimedCallback(switchStkUtkMode, param, NULL, cid, funcString);
    }
    ...
    if (strStartsWith(s, "+ESIMS: 0,0") || strStartsWith(s, "+ESIMS: 0,13") || strStartsWith(s, "+ESIMS: 0,10") || strStartsWith(s, "+ESIMS: 0,11") || strStartsWith(s, "+ESIMS: 0,15")) {
        ...
        if (strStartsWith(s, "+ESIMS: 0,11") || strStartsWith(s, "+ESIMS: 0,13") || strStartsWith(s, "+ESIMS: 0,10")) {
            ...
            RLOGD("RILd,RIL_UNSOL_VIRTUAL_SIM_OFF");
            resetSIMProperties(rid);
            resetAidInfo(rid);
            RIL_UNSOL_RESPONSE(RIL_UNSOL_VIRTUAL_SIM_OFF, &sim_inserted_status, sizeof(int), rid);
            RIL_UNSOL_RESPONSE(RIL_UNSOL_RESPONSE_SIM_STATUS_CHANGED, NULL, 0, rid);
        } else {
    ...
    // MTK-END
        ...
        param->rid = rid;
        param->urc = urc;
        RIL_requestProxyTimedCallback (detectSim, param, &TIMEVAL_PIN_SIM, cid, "detectSim");
        acquire_wake_lock(PARTIAL_WAKE_LOCK, "sim_hot_plug");
        ...
        if (isCdmaLteDcSupport()) {
            RLOGD("AT+CFUN=1 send, ril.cfun.send set to 1");
            property_set("ril.cfun.send", "1");
...
//MTK-END [mtkXXXXX][120208][APLS00109092] Replace "RIL_UNSOL_SIM_MISSING in RIL.java" with "acively query SIM missing status"
    ...
    err = at_send_command_singleline(cmd, "+EPIN:", &p_response, getChannelCtxbyProxy(rid));
    free(cmd);
...
int getIccApplicationStatus(RIL_CardStatus_v6 **pp_card_status, RIL_SOCKET_ID rid, int sessionId) {
    static RIL_AppStatus app_status_array[] = {
        // ISIM_ABSENT = 0
        { RIL_APPTYPE_UNKNOWN, RIL_APPSTATE_UNKNOWN, RIL_PERSOSUBSTATE_READY,
          NULL, NULL, 0, RIL_PINSTATE_UNKNOWN, RIL_PINSTATE_UNKNOWN },
...
void requestSimReset(RIL_SOCKET_ID rid)
{
    ...
    int pivotSim;;
    int err;
    err = at_send_command_singleline("AT+ESIMS", "+ESIMS:", &p_response, getChannelCtxbyProxy(rid));
...
static void configModemStatus(RIL_SOCKET_ID rid) {
    ...
    property_get("gsm.ril.fulluicctype", cardType, "");
    ...
    if (invalidType == 0) {
        at_send_command(cmd, NULL, getRILChannelCtx(RIL_SIM, rid));
    }
    free(cmd);
}
...
static void getImsi(void *param)
{
    ...
    RILChannelCtx *p_cctx = getRILChannelCtx(RIL_SIM, param_t->rid);
    ATResponse *p_response = NULL;
    ...
    RLOGD("[op016m] Imsi: %s", p_response->p_intermediates->line);
...
int eraseCdmaCapabilityMode()
{
    ...
    property_get("persist.op016m.cdma.erase", erasecdma, "-1");
...
void checkPinState(const char *s, RIL_SOCKET_ID rid)
{
    if (hasCdmaCapability(s)) {
        ...
        RIL_requestProxyTimedCallback(checkPin,
            (void *)param, &TIMEVAL_PIN_SIM, p_cctx->id, "checkPin");
    }
}
/// @}
...
static int isOp096mSupport() {
    ...
    property_get("persist.operator.seg", seg, "");
    LOGD("isOp096mSupport isC2KSupport: %d, isCtaSupport: %d, optr: %s, seg: %s",
            isC2KSupport, isCtaSupport, optr, seg);
    return ((isC2KSupport == 1) && (isCtaSupport != 1)
...
int isPersistVsim()
{
    ...
    for (rid = RIL_SOCKET_1; rid < RIL_SOCKET_NUM; rid++) {
        getMSimProperty(rid, "persist.radio.external.sim", persist_vsim_inserted_prop);
...
int isVsimEnabledByRid(int rid)
{
    ...
    getMSimProperty(rid, "gsm.external.sim.enabled", vsim_enabled_prop);
    getMSimProperty(rid, "gsm.external.sim.inserted", vsim_inserted_prop);
