...
extern void initRILChannels(void)
{
    ...
    property_get("ril.atsend.timeout", property_value, DEFAULT_AT_TIMEOUT_MIN);
        ...
        RILChannelCtx *p_channel, RIL_Token ackToken)
{
    ...
    err = at_send_command_full_ack(command, RAW, NULL, NULL, 0, pp_outResponse, p_channel,
            ackToken);
    if (isChipTestMode() && strStartsWith(command, "AT+EFUN")){
        RLOGD("AT+EFUN is received, update radio state");
        int targetMode;
        if (sscanf(command, "AT+EFUN=%d", &targetMode) == 1) {
            RLOGD("update radio state with target mode %d", targetMode);
            updateRadioStateWithTargetMode(targetMode);
        } else {
            RLOGD("sscanf fail, wrong argument");
        }
    }
    return err;
}
