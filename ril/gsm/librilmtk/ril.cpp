...
static int responseVsimOperationEvent(Parcel &p, void *response, size_t responselen) {
    ...
    RLOGD("responseVsimOperationEvent: data_length=%d, data=%s", p_cur->data_length, p_cur->data);
...
static int responseDataCallListV11(Parcel &p, void *response, size_t responselen) {
    ...
    for (i = 0; i < num; i++) {
        ...
        RLOGD("ril.cpp eran_type %d", p_cur[i].eran_type);
        appendPrintBuf("%s[status=%d,retry=%d,cid=%d,%s,%s,%s,%s,%s,%s,%s,mtu=%d,eran_type=%d],", printBuf,
        p_cur[i].status,
        p_cur[i].suggestedRetryTime,
        p_cur[i].cid,
        (p_cur[i].active==0)?"down":"up",
