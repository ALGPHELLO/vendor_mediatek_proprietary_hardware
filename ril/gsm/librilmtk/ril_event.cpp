...
/* //hardware/ril/librilmtk/ril_event.cpp
**
...
void traverse_timer_list () {
    struct ril_event * list;
    list = timer_list.next;
    dlog("~~~~ traverse_timer_list ~~~~");
    while (list != &timer_list) {
        dlog("~~~~ list:%p, prev:%p, next:%p", list, list->prev, list->next);
        list = list->next;
    }
}
        ...
        if (n < 0) {
            ...
            if (errcode_tmp == 9) {
                ...
                for(i = 0; i < fd_max; i++) {
                 ...
                 // ALPS01509775: fd is damaged, use TRM to re-setup
