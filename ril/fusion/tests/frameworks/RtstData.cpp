 ...
 *****************************************************************************/
AssertionResult RtstExpectedRilReqAckItem::checkData() {
    int reqId;
    Parcel p;
    bool r = RtstEnv::get()->getExpectedRilReqAck(getSlotId(), reqId, p);
    if (!r) {
        return AssertionFailure() << "getExpectedRilReqAck time out";
    }
    if (reqId != getRilCmdId()) {
        return AssertionFailure() << "Check RIL Req Ack"
                                  << " req ID mismatch!"
                                  << " Expected: " << getRilCmdId()
                                  << " Actual: " << reqId;
    }
 ...
 *****************************************************************************/
AssertionResult RtstExpectedInitRilUrcItem::checkData() {
    int urcId;
    Parcel p;
    bool r = false;
    if (m_allSlot) {
        for (int i = 0; i < getSimCount(); i++) {
            r = RtstEnv::get()->getExpectedInitRilUrc(i, getRilCmdId(), urcId, p);
            if (r) {
               break;
            }
        }
    } else {
        r = RtstEnv::get()->getExpectedInitRilUrc(getSlotId(), getRilCmdId(), urcId, p);
    }
    if (!r) {
        return AssertionFailure() << "RtstExpectedInitRilUrcItem cannot find "
                                  << urcId;
    }
    return getDataSequency().checkParcel(p);
}
...
AssertionResult RtstExpectedAtItem::checkData() {
    ...
    RFX_LOG_D(TAG, "getExpectedAt channelId = %d, slotId = %d, expectedAT = %s",
 ...
 *****************************************************************************/
void RtstStatusItem::sendData() {
    RtstEnv::get()->setStatus(
        getSlotId(), getKey(), getValue());
}

/*****************************************************************************
 * Class RtstStatusCbItem
 *****************************************************************************/
void RtstStatusCbItem::sendData() {
     RtstEnv::get()->invokeStatusCallback(
            getSlotId(),
            m_classInfo,
            getKey(),
            getValue(),
 ...
 *****************************************************************************/
AssertionResult RtstExpectedStatusItem::checkData() {
    RfxVariant value;
    bool ret = RtstEnv::get()->getExpectedStatus(
            getSlotId(), getKey(), value, m_delay);
    if (!ret) {
        return AssertionFailure() << "Blocked Status Not Set";
    }

    if (value == getValue()) {
        return AssertionSuccess();
    } else {
        return AssertionFailure() << "Status mismatch!";
    }
}
...
AssertionResult RtstCase::stepRun(RtstItemBase * item, int num, ...) {
    ...
    va_end(argp);
    put(item);
    if (isStepRun() && (RtstEnv::get()->getMode() != RtstEnv::TEST_DATA_ONLY)) {
        return item->run();
    } else {
        return AssertionSuccess();
    }
...
AssertionResult RtstCase::runAt(unsigned int index) {
    if (isStepRun() && (RtstEnv::get()->getMode() != RtstEnv::TEST_DATA_ONLY)) {
        RFX_ASSERT(((index + 1) < m_caseItems.size()));
 ...
 * Class RtstInitAtRsp
 *****************************************************************************/
void RtstInitAtRsp::generateData(int num, va_list argp) {
    for (int i = 0; i < num; i++) {
        const char *data;
        data = va_arg(argp, const char* );
        m_atResponse.push(String8(data) + String8("\r"));
    }
}


RtstInitAtRsp *RtstInitAtRsp::fillData(int slotId, const char* atCmd, int num, ...) {
    RFX_LOG_D(TAG, "RtstInitAt::fillData %s", atCmd);
    RtstInitAtRsp *p = new RtstInitAtRsp();
    va_list argp;
    va_start(argp, num);
    p->generateData(num, argp);
    va_end(argp);
    RtstEnv::get()->setInitAtRsp(String8(atCmd), p, slotId);
    return p;
}
