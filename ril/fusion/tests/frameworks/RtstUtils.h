...
public:
    ...
    static void setStatusValueForGT(
        int slotId,
        const RfxStatusKeyEnum key,
        const RfxVariant &value,
        bool syncToRild = true
    );

    static void setStatusCallbackForGT(void (*statusCallback)(int slotId, const RfxStatusKeyEnum key,
            const RfxVariant &value));

    static const RfxVariant &getStatusValue(int slot, const RfxStatusKeyEnum key);

    static void registerBlockedStatus(const RfxStatusKeyEnum key) {
        s_blockedKeyList.add(key);
    }

    static void removeAllBlockedStatus() {
        s_blockedKeyList.clear();
    }

    static bool isBlockedStatusEmpty() {
        return s_blockedKeyList.isEmpty();
    }

    static bool isBlockedStatus(const RfxStatusKeyEnum key) {
        return (s_blockedKeyList.indexOf(key) < 0) ? false : true;
    }
...
private:
    ...
    static StatusListEntry * s_gt_status_list[MAX_SIM_COUNT + 1][RFX_STATUS_KEY_END_OF_ENUM];
    static RfxVariant s_dummy;
    static SortedVector<RfxStatusKeyEnum> s_blockedKeyList;
};
