...
int RtstFd::write(void* buf, int len)  const {
    ...
    RFX_LOG_D(TAG, "RtstSocket::write [fd = %d][len = %d]", mFd, len);
    return n;
exit:
...
int RtstFd::read(void* buf, int len) const {
    ...
    RFX_LOG_D(TAG, "RtstSocket::read [fd = %d][len = %d]", mFd, n);
    return n;
 ...
 *****************************************************************************/
 RtstSocketPair::RtstSocketPair() : m_s1(-1), m_s2(-1){
    ...
    RFX_LOG_D(TAG, "RtstSocketPair [%d][%d]", fd[0], fd[1]);
    m_s1.setFd(fd[0]);
    m_s2.setFd(fd[1]);
}