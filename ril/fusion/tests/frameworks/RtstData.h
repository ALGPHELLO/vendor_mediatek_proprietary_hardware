...
#define RTST_EXPECTED_RIL_REQ_ACK(_reqId) \
        RTST_EXPECTED_RIL_REQ_ACK_WITH_SLOT(_reqId, RFX_SLOT_ID_0)


#define RTST_EXPECTED_RIL_REQ_ACK_WITH_SLOT(_reqId, _slot) \
        _RTST_EXPECTED_RIL_REQ_ACK_WITH_SLOT(ril, _reqId, _slot)
...
#define RTST_AT_CMD(_channel, _at, _dataNum, ...) \
    RTST_AT_CMD_WITH_SLOT(RFX_SLOT_ID_0, _channel, _at, _dataNum, __VA_ARGS__)


#define RTST_AT_CMD_NO_ASSERT(_channel, _at, _dataNum, ...) \
    RTST_AT_CMD_WITH_SLOT_NO_ASSERT(RFX_SLOT_ID_0, _channel, _at, _dataNum, __VA_ARGS__)
 ...
 *         RTST_CASE_BEGIN();
 *         ...
 *         RTST_AT_CMD_WITH_SLOT(RFX_SLOT_ID_1, RIL_CMD_PROXY_1, "AT+ECSCB?", 2, "+ECSCB:0", "OK");
 *         ...
 *         RTST_CASE_END();
 *     }
 * </code>
...
#define RTST_URC_STRING(_urc) \
    RTST_URC_STRING_WITH_SLOT(RFX_SLOT_ID_0, _urc)
 ...
 *         RTST_CASE_BEGIN();
 *         ...
 *         RTST_URC_STRING_WITH_SLOT(RFX_SLOT_ID_1, "^SMMEMFULL:\"ME\"");
 *         ...
 *         RTST_CASE_END();
 *     }
 * </code>
...
/*
 * Macro to check the int RIL URC for all slot
 */
#define RTST_EXPECTED_INIT_RIL_URC_ALL_SLOT(_urcId, _dataNum, ...) \
        _RTST_EXPECTED_INIT_RIL_URC_ALL_SLOT(ril, _urcId, _dataNum, __VA_ARGS__)

/*
 * Macro to check the init RIL URC for all slot in NC way
 */
#define RTST_EXPECTED_INIT_RIL_URC_ALL_SLOT_NC(_urcId, _dataNum, ...) \
        _RTST_EXPECTED_INIT_RIL_URC_ALL_SLOT_NC(ril, _urcId, _dataNum, __VA_ARGS__)

/*
 * Helper macro to check init RIL URC for all slot without parameter
 */
#define RTST_EXPECTED_INIT_RIL_VOID_URC_ALL_SLOT(_urcId) \
        RTST_EXPECTED_INIT_RIL_URC_ALL_SLOT(_urcId, 1, RTST_VOID, "")


/*
 * Macro to check the RIL URC sent during init stage
 *
 *
 * PARAMTERS:
 *  _urcId:
 ...
 * </code>
 *
 * SEE ALSO: RTST_EXPECTED_INIT_RIL_URC_NC RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT
 *           RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT_NC  RTST_EXPECTED_INIT_RIL_VOID_URC
 *           RTST_EXPECTED_INIT_RIL_VOID_URC_WITH_SLOT
 */
#define RTST_EXPECTED_INIT_RIL_URC(_urcId, _dataNum, ...) \
    RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT(_urcId, RFX_SLOT_ID_0, _dataNum, __VA_ARGS__)


/*
 * The difference between RTST_EXPECTED_INIT_RIL_URC_NC and RTST_EXPECTED_INIT_RIL_URC is
 *    The RTST_EXPECTED_INIT_RIL_URC may add a data len info before the data
 *    according the following rule
 *         1. All data type are INT32s, will add the number of INT32
 *         2. All data type are STRING, and the number >= 2, will add the number of STRING
 *    The RTST_EXPECTED_INIT_RIL_URC_NC will not add the data len
 *
 * SEE ALSO: RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT, RTST_EXPECTED_INIT_RIL_URC
 *           RTST_EXPECTED_INIT_RIL_VOID_URC, RTST_EXPECTED_INIT_RIL_VOID_URC_WITH_SLOT
 *           RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT_NC
 */
#define RTST_EXPECTED_INIT_RIL_URC_NC(_urcId, _dataNum, ...) \
    RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT_NC(_urcId, RFX_SLOT_ID_0, _dataNum, __VA_ARGS__)

/*
 * Macro to check the RIL URC sent during init stage with slot ID
 *
 * PARAMTERS:
 *  _urcId:
 *      The RIL URC ID
 ...
 *         RTST_CASE_BEGIN();
 *         ...
 *         RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT(
 *             RIL_UNSOL_CDMA_CARD_INITIAL_ESN_OR_MEID,
 *             RFX_SLOT_ID_1,
 *             1,
 *             RTST_STRING,
 ...
 * </code>
 *
 * SEE ALSO: RTST_EXPECTED_INIT_RIL_URC_NC, RTST_EXPECTED_INIT_RIL_URC
 *           RTST_EXPECTED_INIT_RIL_VOID_URC, RTST_EXPECTED_INIT_RIL_VOID_URC_WITH_SLOT
 *           RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT_NC
 */
#define RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT(_urcId, _slot, _dataNum, ...) \
        _RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT(ril, _urcId, _slot, _dataNum, __VA_ARGS__)

/*
 * The difference between RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT_NC and
 *    RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT is
 *    The RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT may add a data len info before the data
 *    according the following rule
 *         1. All data type are INT32s, will add the number of INT32
 *         2. All data type are STRING, and the number >= 2, will add the number of STRING
 *    The RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT_NC will not add the data len
 *
 * SEE ALSO: RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT, RTST_EXPECTED_INIT_RIL_URC
 *           RTST_EXPECTED_INIT_RIL_VOID_URC, RTST_EXPECTED_INIT_RIL_VOID_URC_WITH_SLOT
 *           RTST_EXPECTED_INIT_RIL_URC_NC
 */
#define RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT_NC(_urcId, _slot, _dataNum, ...) \
        _RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT_NC(ril, _urcId, _slot, _dataNum, __VA_ARGS__)

/*
 * Helper macro for RIL URC during init stage without slot ID and parameters
 *
 * SEE ALSO: RTST_EXPECTED_INIT_RIL_URC, RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT
 *           RTST_EXPECTED_INIT_RIL_VOID_URC_WITH_SLOT
 *           RTST_EXPECTED_INIT_RIL_URC_NC, RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT_NC
 */
#define RTST_EXPECTED_INIT_RIL_VOID_URC(_urcId) \
        RTST_EXPECTED_INIT_RIL_VOID_URC_WITH_SLOT(_urcId, RFX_SLOT_ID_0)

/*
 * Helper macro for RIL URC during init stage without parameters and with slot ID
 *
 * SEE ALSO: RTST_EXPECTED_INIT_RIL_URC, RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT
 *           RTST_EXPECTED_INIT_RIL_VOID_URC
 *           RTST_EXPECTED_INIT_RIL_URC_NC, RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT_NC
 */
#define RTST_EXPECTED_INIT_RIL_VOID_URC_WITH_SLOT(_urcId, _slot) \
        RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT(_urcId, _slot, 1, RTST_VOID, "")
 ...
 * PARAMETERS
 *  _key:
 *      the key of the system property, such as "ril.fd.mode"
 *  _value:
 *      the value of the system property, such as "1"
 *
 * EXAMPLES
...
/*
 * Macro to mock the value of status
 *
 * PARAMETER
 *  _slot:
 *      slot ID
 *  _key:
 *      key of the status, SEE ALSO RfxStatusKeyEnum
 *  _value:
 *      value of the status, SEE ALSO RfxVariant
 *
 * EXAMPLES
 *
 * <code>
 ...
 * </code>
 *
 * SEE ALSO: RTST_EXPECTED_STATUS_VALUE, RTST_EXPECTED_STATUS_VALUE_WITH_DELAY
 */
#define RTST_STATUS_VALUE(_slot, _key, _value) \
        _RTST_STATUS_VALUE(ril, _slot, _key, _value)

/*
 * Macro to provide the expected status value
 * it will delay 10s to check
 *
 * PARAMETER
 *  _slot:
 ...
 *         RTST_CASE_BEGIN();
 *         ...
 *         RTST_EXPECTED_STATUS_VALUE(RFX_SLOT_ID_1, RFX_STATUS_KEY_NWS_MODE, RfxVariant(1));
 *         ...
 *         RTST_CASE_END();
 *     }
 * </code>
 *
 * SEE ALSO: RTST_STATUS_VALUE, RTST_EXPECTED_STATUS_VALUE_WITH_DELAY
 */
#define RTST_EXPECTED_STATUS_VALUE(_slot, _key, _value) \
        _RTST_EXPECTED_STATUS_VALUE(ril, _slot,_key,_value)

/*
 * Same as RTST_EXPECTED_STATUS_VALUE except you can change the default delay value
 * if you check the value after the macro RTST_EXPECTED_RIL_RESPONSE, you can set
 * the delay as 0 to speed up the test procedure
 */
#define RTST_EXPECTED_STATUS_VALUE_WITH_DELAY(_slot, _key, _value, _delay) \
        _RTST_EXPECTED_STATUS_VALUE_WITH_DELAY(ril, _slot, _key, _value, _delay)

/*
 * Register the status key if you want to check the status changes step by step
 *
 * If you register key A, when the status A is set it will block the telcore until
 * the test thread check the status A.
 *
 * <code>
 *    RTST_REGISTER_BLOCKED_STATUS(RFX_STATUS_KEY_MODE_SWITCH);
 * </code>
 */
#define RTST_REGISTER_BLOCKED_STATUS(_key) \
        RtstUtils::registerBlockedStatus(_key);


/*
 * Macro to provide the AT response when init procedure
 * You can use this macro in global or before the first RTST_CASE_BEGIN
 *
 * PARAMETER
 *   _tag:
 *      a name for this AT data, like A, B or C...
 *   _at:
 *      the AT send when init procedure such as AT+CGDCONT=?
 *   _dataNum
 *      number of AT response
 *   ...
 *     The AT response data
 * EXAMPLES
 *
 * <code>
 *    // In Global
 ...
 *
 * LIMITATION
 *    Because we use map to store the data, if you provide two same at with different
 *    response, the first response data will be overwrite by the second one
 */
#define RTST_INIT_AT_CMD(_tag, _at, _dataNum, ...) \
    static RtstInitAtRsp *_p##_tag##A = \
            RtstInitAtRsp::fillData(RFX_SLOT_ID_0, _at, _dataNum, __VA_ARGS__);\
    static RtstInitAtRsp *_p##_tag##B = \
            RtstInitAtRsp::fillData(RFX_SLOT_ID_1, _at, _dataNum, __VA_ARGS__)

#define RTST_INIT_AT_CMD_WITH_SLOT(_tag, _slot, _at, _dataNum, ...) \
    static RtstInitAtRsp *_p##_tag = RtstInitAtRsp::fillData(_slot, _at, _dataNum, __VA_ARGS__)
...
/*
 * Macro to invoke the status callback in RfxMainThread
 *
 * PRARAMETER
 *    _slotId:
 *        the slot ID
 *    _className:
 *        the class name of the class which has the status callback
 *    _key:
 *        the status key of the class registered
 *    _oldVaue:
 *        the old value when call the callback
 *    _newValue:
 *        the new vlaue when call the callback
 *
 * EXAMPLES
 *
 * <code>
 ...
 *
 */
#define RTST_INVOKE_STATUS_CB(_slotId, _className, _key, _oldValue, _newValue) \
        extern RfxClassInfo *get##_className##ClassInfo();                    \
        _RTST_INVOKE_STATUS_CB(ril, _slotId, get##_className##ClassInfo(),   \
...
// Internal Use
#define RTST_CASE_BEGIN_FOR_TEST(_rfx_assert, _step_test)    \
        _RTST_CASE_BEGIN_FOR_TEST(ril, _rfx_assert, _step_test)

// Internal Use
#define _RTST_CASE_BEGIN_FOR_TEST(_name, _rfx_assert, _step_test)        \
        do {                                                             \
...
#define _RTST_CASE_BEGIN(_name, _rfx_assert, _step_test)                                      \
        do {                                                                                  \
            ...
            for (int i = 0; i < getSimCount(); i++) {                                             \
                RTST_STATUS_VALUE(i, RFX_STATUS_KEY_MODEM_POWER_OFF, RfxVariant(false));      \
                RTST_STATUS_VALUE(i, RFX_STATUS_KEY_RADIO_STATE, RfxVariant(RADIO_STATE_ON)); \
            }
...
// Internal Use
#define _RTST_EXPECTED_RIL_REQ_ACK_WITH_SLOT(_name, _reqId, _slot)                              \
        do {                                                                                    \
            RtstExpectedRilReqAckItem *pItem = new RtstExpectedRilReqAckItem(_reqId, _slot);    \
...
// Internal Use
#define _RTST_EXPECTED_INIT_RIL_URC_ALL_SLOT(_name, _urcId, _dataNum, ...)                      \
        do {                                                                                    \
            RtstExpectedInitRilUrcItem *pItem = new RtstExpectedInitRilUrcItem(_urcId, -1);  \
            pItem->setForAllSlot();                                                             \
            AssertionResult r = _case##_name.stepRun(pItem, _dataNum, __VA_ARGS__);             \
            ASSERT_TRUE(r);                                                                     \
        } while(0)
...
// Internal Use
#define _RTST_EXPECTED_INIT_RIL_URC_ALL_SLOT_NC(_name, _urcId, _dataNum, ...)                   \
        do {                                                                                    \
            RtstExpectedInitRilUrcItem *pItem = new RtstExpectedInitRilUrcItem(_urcId, -1);     \
            pItem->setForAllSlot();                                                             \
            pItem->getDataSequency().setSeqCalType(RtstDataSequency::FORCE_NON_COUNTABLE);      \
...
// Internal Use
#define _RTST_EXPECTED_INIT_RIL_URC_WITH_SLOT_NC(_name, _urcId, _slot, _dataNum, ...)           \
        do {                                                                                    \
            RtstExpectedInitRilUrcItem *pItem = new RtstExpectedInitRilUrcItem(_urcId, _slot);  \
            pItem->getDataSequency().setSeqCalType(RtstDataSequency::FORCE_NON_COUNTABLE);      \
            AssertionResult r = _case##_name.stepRun(pItem, _dataNum, __VA_ARGS__);             \
...
// Internal Use
#define _RTST_STATUS_VALUE(_name, _slot, _key, _value)                                  \
        do {                                                                            \
            RtstStatusItem *pItem = new RtstStatusItem(_slot, _key, _value);            \
            AssertionResult r = _case##_name.stepRun(pItem, 0);                         \
            ASSERT_TRUE(r);                                                             \
        } while(0)

// Internal Use
#define _RTST_EXPECTED_STATUS_VALUE(_name, _slot, _key, _value)                              \
        do {                                                                                 \
            RtstExpectedStatusItem*pItem = new RtstExpectedStatusItem(_slot, _key, _value);  \
            AssertionResult r = _case##_name.stepRun(pItem, 0);                              \
            ASSERT_TRUE(r);                                                                  \
        } while(0)

// Internal Use
#define _RTST_INVOKE_STATUS_CB(_name, _slotId, _className, _key, _oldValue, _newValue)       \
        do {                                                                                 \
            RtstStatusCbItem *pItem =                                                        \
                    new RtstStatusCbItem(_slotId, _className, _key, _oldValue, _newValue);   \
            AssertionResult r = _case##_name.stepRun(pItem, 0);                              \
            ASSERT_TRUE(r);                                                                  \
        } while(0)

// Internal Use
#define _RTST_EXPECTED_STATUS_VALUE_WITH_DELAY(_name, _slot, _key, _value, _delay)           \
        do {                                                                                 \
            RtstExpectedStatusItem*pItem =                                                   \
                    new RtstExpectedStatusItem(_slot, _key, _value, _delay);                 \
            AssertionResult r = _case##_name.stepRun(pItem, 0);                              \
            ASSERT_TRUE(r);                                                                  \
        } while(0)
...
/*****************************************************************************
 * Class RtstExpectedRilReqAckItem
 *****************************************************************************/
/*
 * Expected RIL req ack item
 */
class RtstExpectedRilReqAckItem : public RtstRilItemBase {
// Constructor / Destructor
public:
    // Construct a expected RIL URC item
    RtstExpectedRilReqAckItem(
        int reqId,        // [IN] URC ID
        int slotId        // [IN] slot ID
    ) : RtstRilItemBase(reqId, slotId) {
    }

    virtual ~RtstExpectedRilReqAckItem() {}
...
public:
    ...
    RtstUrcStringItem(
        int slotId       // [IN] Slot ID
    ) : RtstAtItemBase(RIL_CMD_PROXY_URC, slotId) {
    }
...
/*****************************************************************************
 * Class RtstStatusItemBase
 *****************************************************************************/
/*
 * Class for base status item
 */
class RtstStatusItemBase : public RtstTelItemBase {
// External Method
public:
    // Get the key of the status
    //
    // RETURNS: the key of the status
    RfxStatusKeyEnum getKey() const {
        return m_key;
    }
    ...
    const RfxVariant & getValue() const {
        return m_value;
    }

// Constructor / Destructor
public:
    // Create the base item for status
    RtstStatusItemBase(
        int slot,             // [IN] Slot ID
        RfxStatusKeyEnum key, // [IN] The status key
        RfxVariant value      // [IN] The status value
    ) : RtstTelItemBase(slot), m_key(key), m_value(value) {
    }

    virtual ~RtstStatusItemBase() {}
...
/*****************************************************************************
 * Class RtstStatusItem
 *****************************************************************************/
/*
 * Class for status item
 */
class RtstStatusItem : public RtstStatusItemBase {
// Constructor / Destructor
public:
    // Create the item for status
    RtstStatusItem(
        int slot,             // [IN] Slot ID
        RfxStatusKeyEnum key, // [IN] The status key
        RfxVariant value      // [IN] The status value
    ) : RtstStatusItemBase(slot, key, value) {
    }

    virtual ~RtstStatusItem() {}
...
/*****************************************************************************
 * Class RtstStatusCbItem
 *****************************************************************************/
class RtstStatusCbItem : public RtstStatusItem {
...
public:
    ...
    RtstStatusCbItem(
        int slot,                        // [IN] Slot ID
        const RfxClassInfo *classInfo,   // [IN] the class infos
        RfxStatusKeyEnum key,            // [IN] The status key
        RfxVariant oldValue,             // [IN] The old status value
        RfxVariant newValue              // [IN] The new status value
    ) : RtstStatusItem(slot, key, oldValue), m_classInfo(classInfo), m_newValue(newValue) {
    }

    virtual ~RtstStatusCbItem() {}
...
/*****************************************************************************
 * Class RtstExpectedStatusItem
 *****************************************************************************/
/*
 * Status for expected status item
 */
class RtstExpectedStatusItem : public RtstStatusItemBase {
// Constructor / Destructor
public:
    // Create the item for status
    RtstExpectedStatusItem(
        int slot,             // [IN] Slot ID
        RfxStatusKeyEnum key, // [IN] The status key
        RfxVariant value,     // [IN] The status value
        int delay = 10
    ) : RtstStatusItemBase(slot, key, value), m_delay(delay){
    }

    virtual ~RtstExpectedStatusItem() {}
...
// External Method
public:
    static RtstInitAtRsp *fillData(int slotId, const char* atCmd, int number, ...);
    const Vector<String8> &getAtResponse() const {
        return m_atResponse;
    }

// Implementation
private:
    void generateData(int number, va_list argp);
    Vector<String8> m_atResponse;
};
