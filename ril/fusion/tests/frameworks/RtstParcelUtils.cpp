...
void dispatchVoiceRadioTech(Parcel& p, RequestInfo *pRI) {
#if 0
...
// When all RILs handle this request, this function can be removed and
// the request can be sent directly to the RIL using dispatchVoid.
void dispatchCdmaSubscriptionSource(Parcel& p, RequestInfo *pRI) {
#if 0
    RIL_RadioState state = CALL_ONSTATEREQUEST((RIL_SOCKET_ID)pRI->socket_id);
 ...
 */
int responseString(Parcel &p, void *response, size_t responselen) {
    /* one string only */
    startResponse;
...
int responseCallForwardsEx(Parcel &p, void *response, size_t responselen) {
#if 0
    int num;
...
int responseHardwareConfig(Parcel &p, void *response, size_t responselen)
{
   ...
   RLOGE("responseHardwareConfig weilun");
...
int responseSSData(Parcel &p, void *response, size_t responselen) {
#if 0
...
int responseEtwsNotification(Parcel &p, void *response, size_t responselen) {
#if 0
    if(NULL == response) {
        RLOGE("invalid response: NULL");
        return RIL_ERRNO_INVALID_RESPONSE;
...
int responsePcoIaData(Parcel &p, void *response, size_t responselen) {
  if (response == NULL) {
    RLOGE("responsePcoIaData: invalid NULL response");
    return RIL_ERRNO_INVALID_RESPONSE;
  }
  if (responselen != sizeof(RIL_PCO_Data_attached)) {
    RLOGE("responsePcoIaData: invalid response length %u, expecting %u",
