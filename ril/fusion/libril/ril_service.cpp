...
Return<void> RadioImpl::setModemImsCfg(int32_t serial, const hidl_string& keys,
   ...
   dispatchStrings(serial, mSlotId, RIL_REQUEST_SET_MD_IMSCFG, 3, keys.c_str(), values.c_str(),
...
Return<void> RadioImpl::setResponseFunctionsForAtci(
    ...
    RLOGD("setResponseFunctionsForAtci");
...
Return<void> RadioImpl::sendAtciRequest(int32_t serial, const hidl_vec<uint8_t>& data) {
    RLOGD("sendAtciRequest: serial %d", serial);
...
int radio::getIccCardStatusResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        // M @{
        s_cardState[slotId] = cardStatus.cardState;
        // M @}
...
int radio::eccPreferredRatResponse(int slotId, int responseType,
    ...
    if (radioService[slotId] != NULL && radioService[slotId]->mRadioResponseMtk != NULL) {
        ...
        RLOGE("eccPreferredRatResponse: radioService[%d]->mRadioResponseMtk == NULL",
                                                                           slotId);
    }
...
int radio::getVoiceRegistrationStateResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        if (response == NULL) {
               ...
               if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
        } else if (s_vendorFunctions->version <= 14 || true) {
            if (numStrings != 15) {
                RLOGE("getVoiceRegistrationStateResponse Invalid response: NULL");
                if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
            } else {
...
int radio::queryNetworkLockResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponseMtk != NULL) {
        ...
        if (response == NULL || responseLen != sizeof(RIL_SimMeLockCatInfo)) {
            RLOGE("queryNetworkLockResponse: Invalid response");
            if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
        } else {
...
int radio::setClipResponse(int slotId, int responseType, int serial, RIL_Errno e,
    ...
    if (isImsSlot(slotId)) {
        if (radioService[slotId]->mRadioResponseIms != NULL) {
            ...
            RLOGE("setClipResponse: radioService[%d]->mRadioResponseIms == NULL", slotId);
        }
    } else {
        if (radioService[slotId]->mRadioResponseMtk != NULL) {
            RadioResponseInfo responseInfo = {};
...
int radio::getColpResponse(int slotId, int responseType, int serial, RIL_Errno e,
    ...
    if (isImsSlot(slotId)) {
        if (radioService[slotId]->mRadioResponseIms != NULL) {
            ...
            RLOGE("getColpResponse: radioService[%d]->mRadioResponseIms == NULL", slotId);
        }
    } else {
        if (radioService[slotId]->mRadioResponseMtk != NULL) {
            RadioResponseInfo responseInfo = {};
...
int radio::getColrResponse(int slotId, int responseType, int serial, RIL_Errno e,
    ...
    if (isImsSlot(slotId)) {
        if (radioService[slotId]->mRadioResponseIms != NULL) {
            ...
            RLOGE("getColrResponse: radioService[%d]->mRadioResponseIms == NULL",
                    slotId);
        }
    } else {
        if (radioService[slotId]->mRadioResponseMtk != NULL) {
...
int radio::readUPBAnrEntryResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponseMtk != NULL) {
        ...
        hidl_vec<PhbEntryStructure> result;
        if (response == NULL || responseLen % sizeof(RIL_PhbEntryStructure*) != 0) {
            RLOGD("readUPBAnrEntryResponse: invalid response");
            if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
        } else {
            convertRilPhbEntryStructureToHal(response, responseLen, result);
        }
...
int radio::setModemImsCfgResponse(int slotId, int responseType,
    ...
    if (radioService[slotId]->mRadioResponseIms != NULL) {
        ...
        RLOGE("setModemImsCfgResponse: radioService[%d]->mRadioResponseIms == NULL",
                slotId);
    }
...
int radio::incomingCallIndicationInd(int slotId, int indicationType,
    ...
    if(mode >= 20) {   // Code Mode >= 20, IMS's Call Info Indication, otherwise CS's
        ...
        if (radioService[imsSlot] != NULL &&
            ...
            RLOGD("incomingCallIndicationInd: %s, %s, %s, %s, %s, %s, %s",
...
int radio::rilConnectedInd(int slotId,
                           ...
                           size_t responseLen) {
    if (radioService[slotId] != NULL && radioService[slotId]->mRadioIndicationMtk != NULL) {
        RLOGD("rilConnectedIndMtk");
        Return<void> retStatus = radioService[slotId]->mRadioIndicationMtk->rilConnected(
                convertIntToRadioIndicationType(indicationType));
        radioService[slotId]->checkReturnStatus(retStatus);
    } else {
        RLOGE("rilConnectedInd: radioService[%d]->mRadioIndicationMtk == NULL", slotId);
    }
...
int radio::pcoDataInd(int slotId,
    ...
    if (radioService[slotId] != NULL && radioService[slotId]->mRadioIndicationMtk != NULL) {
        ...
        RLOGD("pcoDataInd MTK");
        Return<void> retStatus = radioService[slotId]->mRadioIndicationMtk->pcoData(
                convertIntToRadioIndicationType(indicationType), pco);
        radioService[slotId]->checkReturnStatus(retStatus);
    } else if (radioService[slotId] != NULL && radioService[slotId]->mRadioIndication != NULL) {
        if (response == NULL || responseLen != sizeof(RIL_PCO_Data)) {
...
int radio::multiImsCountInd(int slotId,
    ...
    if (radioService[imsSlotId] != NULL &&
            radioService[imsSlotId]->mRadioIndicationIms != NULL) {
        if (response == NULL || responseLen < (sizeof(int))) {
            RLOGE("multiImsCountInd: invalid response");
            return 0;
        }
...
int radio::dataAllowedNotificationInd(int slotId, int indicationType,
        int token, RIL_Errno e, void *response, size_t responseLen) {
    RLOGE("Do NOT support dataAllowedNotificationInd!");
    return 0;
}
...
int radio::dedicatedBearerActivationInd(int slotId, int indicationType, int serial,
        RIL_Errno e, void *response, size_t responseLen) {
    RLOGD("dedicatedBearerActivationInd: Current is not support");
    return 0;
}

int radio::dedicatedBearerModificationInd(int slotId, int indicationType, int serial,
        RIL_Errno e, void *response, size_t responseLen) {
    RLOGD("dedicatedBearerModificationInd: Current is not support");
    return 0;
}

int radio::dedicatedBearerDeactivationInd(int slotId, int indicationType, int serial,
        RIL_Errno e, void *response, size_t responseLen) {
    RLOGD("dedicatedBearerDeactivationInd: Current is not support");
    return 0;
}
...
Return<void> RadioImpl::setWifiSignalLevel(int32_t serial, int32_t phoneId,
            int32_t rssi, int32_t snr) {
    RLOGD("%s: serial %d", __func__, serial);
    dispatchStrings(serial, phoneId, RIL_REQUEST_SET_WIFI_SIGNAL_LEVEL, 3,
...
Return<void> RadioImpl::setWifiEnabled(int32_t serial, int32_t phoneId,
    const hidl_string& ifName, int32_t isEnabled) {
    RLOGD("%s: serial %d", __func__, serial);
    dispatchStrings(serial, phoneId, RIL_REQUEST_SET_WIFI_ENABLED, 2,
        ifName.c_str(), std::to_string(isEnabled).c_str());
    return Void();
}
...
Return<void> RadioImpl::setWifiFlightModeEnabled(int32_t serial, int32_t phoneId,
    const hidl_string& ifName, int32_t isWifiEnabled, int32_t isFlightModeOn) {
    RLOGD("%s: serial %d", __func__, serial);
    dispatchStrings(serial, phoneId, RIL_REQUEST_SET_WIFI_ENABLED, 3,
...
Return<void> RadioImpl::setWifiAssociated(int32_t serial, int32_t phoneId,
        ...
        const hidl_string& apMac) {
    RLOGD("%s: serial %d", __func__, serial);
    dispatchStrings(serial, phoneId, RIL_REQUEST_SET_WIFI_ASSOCIATED, 4,
...
Return<void> RadioImpl::setWifiIpAddress(int32_t serial, int32_t phoneId,
    ...
    RLOGD("%s: serial %d", __func__, serial);
    dispatchStrings(serial, phoneId, RIL_REQUEST_SET_WIFI_IP_ADDRESS, 3,
...
Return<void> RadioImpl::setLocationInfo(int32_t serial, int32_t phoneId,
    ...
    RLOGD("%s: serial %d", __func__, serial);
    dispatchStrings(serial, phoneId, RIL_REQUEST_SET_GEO_LOCATION, 10,
            accountId.c_str(),
            broadcastFlag.c_str(),
            latitude.c_str(),
            longitude.c_str(),
...
Return<void> RadioImpl::setLocationInfoWlanMac(int32_t serial, int32_t phoneId,
    ...
    RLOGD("%s: serial %d", __func__, serial);
    dispatchStrings(serial, phoneId, RIL_REQUEST_SET_GEO_LOCATION, 11,
            accountId.c_str(),
            broadcastFlag.c_str(),
            latitude.c_str(),
            longitude.c_str(),
...
Return<void> RadioImpl::setEmergencyAddressId(int32_t serial,
        int32_t phoneId, const hidl_string& aid) {
    RLOGD("%s: serial %d", __func__, serial);
    dispatchStrings(serial, phoneId, RIL_REQUEST_SET_EMERGENCY_ADDRESS_ID, 1, aid.c_str());
    return Void();
}
...
Return<void> RadioImpl::setNattKeepAliveStatus(int32_t serial, int32_t phoneId,
        ...
        const hidl_string& dstIp, int32_t dstPort) {
    RLOGD("%s: serial %d", __func__, serial);
    dispatchStrings(serial, phoneId, RIL_REQUEST_SET_NATT_KEEP_ALIVE_STATUS, 6,
            ifName.c_str(),
            enable ? "1" : "0",
            srcIp.c_str(),
...
int radio::onWifiMonitoringThreshouldChanged(int slotId, int indicationType,
    ...
    if (radioService[slotId] != NULL && radioService[slotId]->mRadioIndicationMtk != NULL) {
        if (response == NULL) {
            RLOGE("%s: invalid response", __func__);
            return 0;
        }
        hidl_vec<int32_t> indStgs;
        int *pInt = (int *) response;
        int numInts = responselen / sizeof(int);
        RLOGE("onWifiMonitoringThreshouldChanged responselen: %d, sizeof(int): %lu, numInts: %d",
        ...
        RLOGE("%s: radioService[%d]->mRadioIndicationMtk == NULL", __func__, slotId);
    }
...
int radio::onWifiPdnActivate(int slotId, int indicationType, int token,
    ...
    if (radioService[slotId] != NULL && radioService[slotId]->mRadioIndicationMtk != NULL) {
        ...
        Return<void> retStatus = radioService[slotId]->mRadioIndicationMtk->onWifiPdnActivate(
                convertIntToRadioIndicationType(indicationType), indStgs);
        radioService[slotId]->checkReturnStatus(retStatus);
    } else {
        RLOGE("%s: radioService[%d]->mRadioIndicationMtk == NULL", __func__, slotId);
...
int radio::onWfcPdnError(int slotId, int indicationType, int token,
    ...
    if (radioService[slotId] != NULL && radioService[slotId]->mRadioIndicationMtk != NULL) {
        ...
        int *pInt = (int *) response;
        int numInts = responselen / sizeof(int);
        RLOGE("onWfcPdnError responselen: %d, sizeof(int): %lu, numInts: %d",
                responselen, (unsigned long)sizeof(int), numInts);
        indStgs.resize(numInts);
        for (int i = 0; i < numInts; i++) {
            indStgs[i] = (int32_t) pInt[i];
        }
        Return<void> retStatus = radioService[slotId]->mRadioIndicationMtk->onWfcPdnError(
                convertIntToRadioIndicationType(indicationType), indStgs);
        radioService[slotId]->checkReturnStatus(retStatus);
    } else {
        RLOGE("%s: radioService[%d]->mRadioIndicationMtk == NULL", __func__, slotId);
...
int radio::onPdnHandover(int slotId, int indicationType, int token,
    ...
    if (radioService[slotId] != NULL && radioService[slotId]->mRadioIndicationMtk != NULL) {
        ...
        int *pInt = (int *) response;
        int numInts = responselen / sizeof(int);
        RLOGE("onPdnHandover responselen: %d, sizeof(int): %lu, numInts: %d",
                responselen, (unsigned long)sizeof(int), numInts);
        indStgs.resize(numInts);
        for (int i = 0; i < numInts; i++) {
            indStgs[i] = (int32_t) pInt[i];
            RLOGE("onPdnHandover indStgs[%d]: %d", i, indStgs[i]);
        }
        Return<void> retStatus = radioService[slotId]->mRadioIndicationMtk->onPdnHandover(
                convertIntToRadioIndicationType(indicationType), indStgs);
        radioService[slotId]->checkReturnStatus(retStatus);
    } else {
        RLOGE("%s: radioService[%d]->mRadioIndicationMtk == NULL", __func__, slotId);
...
int radio::onWifiRoveout(int slotId, int indicationType, int token,
    ...
    if (radioService[slotId] != NULL && radioService[slotId]->mRadioIndicationMtk != NULL) {
        ...
        for (int i = 0; i < numStrings; i++) {
            ...
            RLOGE("onWifiRoveout indStgs[%d]: %s", i, resp[i]);
        }
        Return<void> retStatus = radioService[slotId]->mRadioIndicationMtk->onWifiRoveout(
                convertIntToRadioIndicationType(indicationType), indStgs);
        radioService[slotId]->checkReturnStatus(retStatus);
    } else {
        RLOGE("%s: radioService[%d]->mRadioIndicationMtk == NULL", __func__, slotId);
...
int radio::onLocationRequest(int slotId, int indicationType, int token,
    ...
    if (radioService[slotId] != NULL && radioService[slotId]->mRadioIndicationMtk != NULL) {
        ...
        for (int i = 0; i < numStrings; i++) {
            indStgs[i] = convertCharPtrToHidlString(resp[i]);
            RLOGE("onLocationRequest indStgs[%d]: %s", i, resp[i]);
        }
        Return<void> retStatus = radioService[slotId]->mRadioIndicationMtk->onLocationRequest(
                convertIntToRadioIndicationType(indicationType), indStgs);
        radioService[slotId]->checkReturnStatus(retStatus);
    } else {
        RLOGE("%s: radioService[%d]->mRadioIndicationMtk == NULL", __func__, slotId);
...
int radio::onWfcPdnStateChanged(int slotId, int indicationType, int token,
    ...
    if (radioService[slotId] != NULL && radioService[slotId]->mRadioIndicationMtk != NULL) {
        ...
        Return<void> retStatus = radioService[slotId]->mRadioIndicationMtk->onWfcPdnStateChanged(
                convertIntToRadioIndicationType(indicationType), indStgs);
        radioService[slotId]->checkReturnStatus(retStatus);
    } else {
        RLOGE("%s: radioService[%d]->mRadioIndicationMtk == NULL", __func__, slotId);
...
int radio::onNattKeepAliveChanged(int slotId, int indicationType, int token,
    ...
    if (radioService[slotId] != NULL && radioService[slotId]->mRadioIndicationMtk != NULL) {
        ...
        for (int i = 0; i < numStrings; i++) {
            indStgs[i] = convertCharPtrToHidlString(resp[i]);
            RLOGE("onNattKeepAliveChanged indStgs[%d]: %s", i, resp[i]);
        }
        Return<void> retStatus = radioService[slotId]->mRadioIndicationMtk->onNattKeepAliveChanged(
                convertIntToRadioIndicationType(indicationType), indStgs);
        radioService[slotId]->checkReturnStatus(retStatus);
    } else {
        RLOGE("%s: radioService[%d]->mRadioIndicationMtk == NULL", __func__, slotId);
...
int radio::onTxPowerIndication(int slotId, int indicationType, int token,
        RIL_Errno err, void *response, size_t responseLen) {
    if (radioService[slotId] != NULL && radioService[slotId]->mRadioIndicationMtkV2_1 != NULL) {
        if (response == NULL) {
            RLOGE("%s: invalid response", __func__);
            return 0;
        }
        ...
        int *pInt = (int *) response;
        int numInts = responseLen / sizeof(int);
        RLOGE("onTxPowerIndication responselen: %d, sizeof(int): %lu, numInts: %d",
                responseLen, (unsigned long)sizeof(int), numInts);
        indTxPower.resize(numInts);
        for (int i = 0; i < numInts; i++) {
            indTxPower[i] = (int32_t) pInt[i];
        }
        Return<void> retStatus = radioService[slotId]->mRadioIndicationMtkV2_1
                ->onTxPowerIndication(
