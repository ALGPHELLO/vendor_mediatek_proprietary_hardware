...
RtcAgpsNSlotController::RtcAgpsNSlotController() :
    ...
    logV(AGPS_TAG, "[RtcAgpsNSlotController]Constructor 0x%x", this);
}

RtcAgpsNSlotController::~RtcAgpsNSlotController() {
    logV(AGPS_TAG, "[RtcAgpsNSlotController]Destructor 0x%x", this);
}

void RtcAgpsNSlotController::onInit() {
    RfxController::onInit();

    logV(AGPS_TAG, "[RtcAgpsNSlotController]onInit(). this=%x!", this);
...
void RtcAgpsNSlotController::onDeinit() {
    logV(AGPS_TAG, "[RtcAgpsController]onDeinit(). this=%x!", this);
    for (int i = 0; i < RfxRilUtils::getSimCount(); i++) {
...
bool RtcAgpsNSlotController::onHandleUrc(const sp<RfxMessage>& message) {
    ...
    case REQUEST_DATA_CONNECTION:
        ...
        } else if (isOp09()|| isCtSimCard(slot_id)){
...
int RtcAgpsNSlotController::getDefaultDataSlotId() {
    ...
    rfx_property_get("persist.radio.data.iccid", defaultData, "");
    if (strlen(defaultData) == 0 || strcmp("N/A", defaultData) == 0) {
        return RFX_SLOT_ID_UNKNOWN;
    }

    for (i = 0; i < SIM_COUNT; i++) {
        rfx_property_get(PROPERTY_ICCID_SIM[i], iccid, "");
...
void RtcAgpsNSlotController::setConnectionState(int state) {
    ...
    sp<RfxMessage> request = RfxMessage::obtainRequest(m_activeSlotId,
        RFX_MSG_REQUEST_AGPS_TCP_CONNIND, RfxIntsData(&state, sizeof(int)), RADIO_TECH_GROUP_C2K);
    RFX_LOG_D(AGPS_TAG, "[RtcAgpsNSlotController]setConnectionState token=%d", request->getToken());
    requestToMcl(request);
}

bool RtcAgpsNSlotController::isOp09() {
    char property_value[RFX_PROPERTY_VALUE_MAX] = { 0 };
    int ct6m = 0;
    int op09 = 0;
    rfx_property_get("ro.ct6m_support", property_value, "0");
    ct6m = atoi(property_value);
    rfx_property_get("ro.operator.optr", property_value, "none");
    op09 = (strcmp("OP09", property_value) == 0) ? 1 : 0;
    RFX_LOG_D(AGPS_TAG, "[RtcAgpsNSlotController]op09(%d), ct6m(%d)", op09, ct6m);
...
bool RtcAgpsNSlotController::isCtSimCard(int slotId) {
    bool ret = false;
    int cdma_card_type = getStatusManager(slotId)->getIntValue(
    ...
    logV(AGPS_TAG, "[RtcAgpsNSlotController]isCtSimCard(%d)", cdma_card_type);
    return ret;
}
