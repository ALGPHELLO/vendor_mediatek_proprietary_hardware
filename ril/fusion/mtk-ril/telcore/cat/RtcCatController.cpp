...
void RtcCatController::onInit() {
    ...
    mPendingCmdRec.cmdType = 0;
    mPendingCmdRec.cmdQualifier = 0;
    ...
    // Register callbacks for CDMA 3G card CDMA capability changed
    getStatusManager()->registerStatusChanged(RFX_STATUS_KEY_CDMA3G_SWITCH_CARD,
        RfxStatusChangeCallback(this, &RtcCatController::onCdma3GSwitchCard));
}

void RtcCatController::onCdma3GSwitchCard(RfxStatusKeyEnum key,
    ...
    String8 prop("cdma.3g.switch.card");
...
void RtcCatController::onCardTypeChanged(RfxStatusKeyEnum key,
    ...
    if ((oldState > 0) && (newState <= 0)) {
        isStkServiceRunning = getStatusManager()->getBoolValue(RFX_STATUS_KEY_IS_CAT_RUNNING, false);
        //cmdType = getStatusManager()->getIntValue(RFX_STATUS_KEY_STK_CACHE_CMD_TYPE, 0);
        logD(RFX_LOG_TAG, "onCardTypeChanged isStkServiceRunning %d, cmdType %d, cmdQual %d",
            isStkServiceRunning, mPendingCmdRec.cmdType, mPendingCmdRec.cmdQualifier);
        if (!isStkServiceRunning && mPendingCmdRec.cmdType != 0) {
            sendResponseForCardMissing(mPendingCmdRec.cmdType, mPendingCmdRec.cmdQualifier);
            mPendingCmdRec.cmdType = 0;
            mPendingCmdRec.cmdQualifier = 0;
            getStatusManager()->setIntValue(RFX_STATUS_KEY_STK_CACHE_CMD_TYPE, 0);
        }
        if (mProCmdRec.needResponse) {
            logD(RFX_LOG_TAG, "card missing and current pending cmd is %d", mProCmdRec.cmdType);
            if (CMD_SETUP_CALL == mProCmdRec.cmdType) {
                sendResponseForCardMissing(mProCmdRec.cmdType, 0);
                mProCmdRec.needResponse = false;
            }
        }
    ...
    switch (msg_id) {
        ...
        case RFX_MSG_REQUEST_STK_SEND_TERMINAL_RESPONSE:
            ...
            // BIP @}
                ...
                // Todo: may ignore the current response in this case
...
void RtcCatController::handleSimRefresh(char* urc) {
    ...
    switch(urc[9]) {
        ...
        case '6':
            ...
            pSimRefreshRspWithType->simRefreshRsp->result = SESSION_RESET;
...
bool RtcCatController::isChipTestMode_Stk() {
...
void RtcCatController::parseStkCmdQualifier(char* cmd, int* cmdQual) {
    ...
    *cmdQual = getStkCommandType(&cmd[typePos]);
    logD(RFX_LOG_TAG, "current stk command qualifier is %d .", *cmdQual);
}
...
void RtcCatController::parseStkCmdType(const sp<RfxMessage>& message, int* cmdType) {
    ...
    if (stkCmd != NULL) {
        ...
        logD(RFX_LOG_TAG, "parseStkCmdType cmd:%s fail", cmd);
        return;
    }

    if (cmd) {
        logD(RFX_LOG_TAG, "parseStkCmdType cmd:%s ", cmd);
        ...
        // check command type
        *cmdType = getStkCommandType(&cmd[typePos]);
        logD(RFX_LOG_TAG,"parseStkCmdType cmdType:%d", *cmdType);
        //free(cmd);
    } else {
...
void RtcCatController::handleEventNotify(const sp<RfxMessage>& message) {
    ...
    if (stkCmd != NULL) {
        ...
        logE(RFX_LOG_TAG, "handleEventNotify :%s fail", cmd);
        return;
    }
...
void RtcCatController::handleProactiveCommand(const sp<RfxMessage>& message) {
    ...
    if (stkCmd != NULL) {
        ...
        logE(RFX_LOG_TAG, "handleProactiveCommand :%s fail", cmd);
        return;
    }

    if ((!isChipTestMode_Stk() && false == isStkServiceRunning) || pSimRefreshRspWithType != NULL) {
        if (cmd != NULL) {
            urc_len = strlen(cmd);
        } else {
        ...
        parseStkCmdType(cmd, &cmdType);
        mPendingCmdRec.cmdType = cmdType;
        getStatusManager()->setIntValue(RFX_STATUS_KEY_STK_CACHE_CMD_TYPE, cmdType);
        parseStkCmdQualifier(cmd, &cmdQual);
        mPendingCmdRec.cmdQualifier= cmdQual;
        logD(RFX_LOG_TAG, "Debug!STK service is not running yet. mPendingCmdRec.cmdType = %d, mPendingCmdRec.cmdQualifier = %d",
            mPendingCmdRec.cmdType, mPendingCmdRec.cmdQualifier);
        pProactive_cmd = pProCmd;
        isProaCmdQueued = true;
        logD(RFX_LOG_TAG, "STK service is not running yet.[%p]",pProactive_cmd);
        return;
    }
...
void RtcCatController::requestHandleStkServiceIsRunning() {
    ...
    if (true == isEventNotifyQueued) {
        logD(RFX_LOG_TAG, "STK service is running: isEventNotifyQueued: true");
        int index = 0;
        while (index < EVENT_NOTIFY_BUFFER_LEN &&
               NULL != getStkQueuedEventNotify(index)) {
            char *cmd = (char *)getStkQueuedEventNotify(index);
            if (NULL != cmd) {
                logD(RFX_LOG_TAG, "Notify Cmd:[%d][%s]", strlen(cmd), cmd);
                handleStkCommand(cmd, CMD_TYPE_NOTIFY);
    ...
    if (true == isProaCmdQueued) {
        logD(RFX_LOG_TAG, "STK service is running: isProaCmdQueued: true");
        char *cmd = pProactive_cmd;
        if (NULL != cmd) {
            logD(RFX_LOG_TAG, "Proactive Cmd:[%d][%s]", strlen(cmd), cmd);
            handleStkCommand(cmd, CMD_TYPE_PROACTIVE);
            freeStkQueuedProactivCmd();
            isProaCmdQueued = false;
            mPendingCmdRec.cmdType = 0;
...
void RtcCatController::sendStkQueuedCmd() {
...
void RtcCatController::setStkCachedEventNotify(char* cmd) {
    ...
    if (EVENT_NOTIFY_BUFFER_LEN == i) {
        ...
        logD(RFX_LOG_TAG, "setStkCachedEventNotify: [index:%d][%p][%s]", i, pEvent_notify, pEvent_notify);
    }
}
