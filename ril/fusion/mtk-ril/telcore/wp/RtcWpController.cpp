        ...
        if (state == 0) {
            ...
            if (blockedState == NO_BLOCK_WP_CHANGED) {
                ...
                if (blockStatusKeyValue == WP_LOCK_IDLE) {
                    ...
                    handleWorldModeChangeStart(true);
...
void RtcWpController::onRequestRadioOffDone(int slotId) {
    ...
    if (closeRadioCount == RfxRilUtils::getSimCount()) {
        closeRadioCount = 0;
...
void RtcWpController::requestRadioOn() {
    for (int slotId = RFX_SLOT_ID_0; slotId < RfxRilUtils::getSimCount(); slotId++) {
        ...
        radioController->moduleRequestRadioPower(backupRadioPower[slotId], NULL,
...
void RtcWpController::handleWorldModeChangeStart(bool flag) {
    // send start urc to RILJ
    if (true == flag) {
        int callingSlotId = getCallingSlotId();
        logD(WP_LOG_TAG, "handleWorldModeChangeStart flag=%d, callingSlotId=%d", flag, callingSlotId);
        if (callingSlotId >= RFX_SLOT_ID_0) {
...
void RtcWpController::processWorldModeResumingResponse(const sp<RfxMessage>& message) {
    ...
    if (message->getError() != RIL_E_SUCCESS) {
        ...
        if (mRetryResumeCount < WP_COMMAND_MAX_RETRY_COUNT) {
            handleWorldModeChangeStart(false);
            return;
        }
