...
#define PROP_NAME_OPERATOR_ID_OP09      9
#define PROP_NAME_OPERATOR_ID_OP12      12  // not use D-sbp
#define PROP_NAME_OPERATOR_ID_OP20      20  // not use D-sbp
...
bool RtcRedialController::handleAtciDialRequest(const char* data, int dataLength) {
    ...
    logD(LOG_REDIAL_TAG, "ATCI number %s", callNumber);
...
bool RtcRedialController::notifyRilRequest(const sp<RfxMessage>& message) {
    ...
    switch (msgId) {
        ...
        case RFX_MSG_REQUEST_CURRENT_STATUS:
            mIsCurrentStatusSent = true;
            increaseEmcsCount();
            logD(LOG_REDIAL_TAG, "mInEmergencyMode:%s", boolToString(mInEmergencyMode));
            if (mInEmergencyMode) {
...
void RtcRedialController::handleEmergencyCallbackMode(int mode) {
    ...
    // There is anther mo call coming
    if (mMoCall != NULL) {
    ...
    if (mode == EMERGENCY_CALLBACK_MODE_EXIT) {
        setRatMode(REDIAL_RAT_OP_RESUME);
        // exit ECBM maybe twice
        mEcbm = EMERGENCY_CALLBACK_MODE_NONE;
    }
}
...
void RtcRedialController::checkMoCallAndFree() {
    if (mMoCall != NULL) {
        logE(LOG_REDIAL_TAG, "has a mocall: %d, %s", mMoCall->getCallId(), mMoCall->getNumber());
...
int RtcRedialController::getGlobalRatMode() {
    ...
    if (RfxRilUtils::isC2kSupport() && RatConfig_isC2kSupported()) {
        ...
        rfx_property_get("persist.radio.c_capability_slot", temp_str, "1");
...
void RtcRedialController::setEmergencyMode(bool emergencyMode) {
    ...
    if (mInEmergencyMode == emergencyMode ||
            (emergencyMode == false && !decreaseEmcsCountAndIsZero())) {
        return;
    }
...
void RtcRedialController::setRatMode(RedialRatOp ratOp) {
    ...
    if (ratOp == REDIAL_RAT_OP_RESUME &&
        ...
        logD(LOG_REDIAL_TAG, "setRatMode, pending to resume due to call count not zeor");
...
MoCallContext::MoCallContext(const sp<RfxMessage>& msg, bool isEcc) {
    ...
    if (DBG) {
        RFX_LOG_D(LOG_REDIAL_TAG, "MoCallContext Constructor(message:%s, clientId:%d, isEcc:%s), \
number =%s",
...
MoCallContext::MoCallContext(char *callNumber, bool isEcc) {
    ...
    if (DBG) {
        RFX_LOG_D(LOG_REDIAL_TAG, "MoCallContext Constructor(CallNumber:%s, isEcc:%s)",
...
MoCallContext::~MoCallContext() {
    // free memory
    RFX_LOG_D(LOG_REDIAL_TAG, "MoCallContext destructor");
    if (mNumber != NULL) {
        free(mNumber);
    }
...
void MoCallContext::setCallId(int callId) {
    if (DBG) {
        RFX_LOG_D(LOG_REDIAL_TAG, "MoCallContext setCallId:%d", callId);
    }
    mCallId = callId;
}
...
RfxRedialData *MoCallContext::getRedialData() {
    ...
    if (DBG) {
        RFX_LOG_D(LOG_REDIAL_TAG, "MoCallContext data null(number:%s,mDialData:%s)",
