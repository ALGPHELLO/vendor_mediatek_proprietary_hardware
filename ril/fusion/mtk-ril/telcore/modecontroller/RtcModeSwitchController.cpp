...
int RtcModeSwitchController::calculateOMCCapabilitySlot() {
    ...
    switch (realCCardCount) {
        ...
        default: {
            ...
            rfx_property_get("persist.radio.simswitch", property_value, "1");
            int capabilitySlot = atoi(property_value) - 1;
            if (containsCdma(mSwitchInfo->card_type[capabilitySlot])
    ...
    RFX_LOG_D(RTC_MODE_CONTROLLER_TAG, "[SMC][calculateOMCCapabilitySlot] %d", cCapabilitySlot);
    return cCapabilitySlot;
}

int RtcModeSwitchController::calculateOP09ACCapabilitySlot() {
    int cCapabilitySlot = RFX_SLOT_ID_0;
    RFX_LOG_D(RTC_MODE_CONTROLLER_TAG,
            "[SMC][calculateOP09ACCapabilitySlot] %d", cCapabilitySlot);
    return cCapabilitySlot;
}

int RtcModeSwitchController::calculateCCapabilitySlot() {
    char property_value[PROPERTY_VALUE_MAX] = { 0 };
    rfx_property_get("persist.operator.optr", property_value, OPERATOR_OM);

    if (strncmp(property_value, OPERATOR_OP09, strlen(OPERATOR_OP09)) == 0) {
        memset(property_value, 0, sizeof(property_value));
        rfx_property_get("persist.operator.seg", property_value, "");
        if (strncmp(property_value, SPEC_OP09_A, strlen(SPEC_OP09_A)) == 0) {
            return calculateOP09ACCapabilitySlot();
        }
    }
    return calculateOMCCapabilitySlot();
}
