...
bool isSimulateUSSI()
{
    ...
    rfx_property_get("persist.ims.ussi.simulate", property_value, "0");
...
void setErrorMessageFromXcap(int slotId, int errorCode, const char* message) {
    char *fullMsg;
    ...
    for (;len > 0; len -= (PROP_VALUE_MAX - 1)) {
        RFX_LOG_D(RFX_LOG_TAG, "len: [%d], max: [%d]", len, PROP_VALUE_MAX);

        char tmpStr[PROP_VALUE_MAX] = {0};
        int copyLen = 0;
        if (len >= PROP_VALUE_MAX - 1) {
            copyLen = PROP_VALUE_MAX - 1;
        }
