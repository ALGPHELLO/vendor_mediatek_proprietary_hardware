...
bool RmcEmbmsURCHandler::isHvolteDisable() {
    ...
    rfx_property_get("persist.embms.hvolte.disable", prop, "0");
...
void RmcEmbmsURCHandler::onEmbmsSessionActiveUpdate(const sp<RfxMclMessage>& msg) {
    ...
    logD(LOG_TAG, "[onEmbmsSessionActiveUpdate]%s", line->getLine());
...
void RmcEmbmsURCHandler::onEmbmsSaiListUpdate(const sp<RfxMclMessage>& msg) {
    ...
    int err = 0;
    RfxAtLine *line = msg->getRawUrc();
    logD(LOG_TAG, "[onEmbmsSaiListUpdate]%s", line->getLine());
...
void RmcEmbmsURCHandler::onEmbmsSessionListUpdate(const sp<RfxMclMessage>& msg) {
    ...
    */
    ...
    int err;
    RfxAtLine *line = msg->getRawUrc();
    logD(LOG_TAG, "[onEmbmsSessionListUpdate]%s", line->getLine());
    ...
    if (num == index) {
        logD(LOG_TAG, "sendEvent RFX_MSG_EVENT_EMBMS_POST_SESSION_UPDATE");
...
void RmcEmbmsURCHandler::onEmbmsSrvStatus(const sp<RfxMclMessage>& msg) {
    ...
    logD(LOG_TAG, "[onEmbmsSrvStatus]%s", line->getLine());
