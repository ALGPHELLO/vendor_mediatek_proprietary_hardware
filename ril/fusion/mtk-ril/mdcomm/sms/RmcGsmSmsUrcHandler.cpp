...
error:
    ...
    logE(mTag, "onNewSms check fail, errAckType %d", errAckType);
}
...
error:
    ...
    logE(mTag, "onNewSmsStatusReport check fail, errAckType %d", errAckType);
}
...
void RmcGsmSmsUrcHandler::handleNewSmsOnSim(const sp<RfxMclMessage>& msg) {
    ...
    getMclStatusManager()->setIntValue(RFX_STATUS_KEY_GSM_INBOUND_SMS_TYPE, SMS_INBOUND_3GPP_CMTI);
