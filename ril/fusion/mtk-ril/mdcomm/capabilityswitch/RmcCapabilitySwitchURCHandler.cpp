...
Mutex RmcCapabilitySwitchURCHandler::m_switch_urc_channel_mutex;
...
void RmcCapabilitySwitchURCHandler::onHandleUrc(const sp<RfxMclMessage> &msg) {
    ...
    if(strStartsWith(msg->getRawUrc()->getLine(), "+ESIMMAP:")) {
        handleSimSwitchUrc(msg);
    }
}

void RmcCapabilitySwitchURCHandler::handleSimSwitchUrc(const sp<RfxMclMessage>& msg) {
    int err;
    int new_major_slot;
    int old_major_slot = getNonSlotMclStatusManager()->getIntValue(
            RFX_STATUS_KEY_MAIN_CAPABILITY_SLOT, 0);
    //parse urc info
    logI(RFX_LOG_TAG, "handleSimSwitchUrc begin, old_major_slot=%d", old_major_slot);
    ...
    if (err < 0) {
        logE(RFX_LOG_TAG, "handleSimSwitchUrc err %d %d", err, new_major_slot);
        return;
    }
    if (checkIfSwitch()) {
...
bool RmcCapabilitySwitchURCHandler::checkIfSwitch() {
    bool ret = false;
    m_switch_urc_channel_mutex.lock();
    ret = getNonSlotMclStatusManager()->
    ...
    m_switch_urc_channel_mutex.unlock();
    return ret;
}
