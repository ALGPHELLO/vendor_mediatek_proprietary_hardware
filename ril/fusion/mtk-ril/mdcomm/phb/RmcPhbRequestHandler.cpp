        ...
        if (entry->alphaId != NULL || entry->number != NULL) {
            ...
            if (i != len || asciiNotSupported) {
                ...
                if (encodeUCS2_0x81(line, alphaId, sizeof(alphaId))) {  // try UCS2_0x81 coding
                    ...
                    logD(RFX_LOG_TAG, "alphaId length=%d, line length=%d", sizeof(alphaId), strlen(line));
...
void RmcPhbRequestHandler:: requestEditUPBEntry(const sp<RfxMclMessage>& msg) {
    ...
    RIL_Errno ret = RIL_E_GENERIC_FAILURE;
    RilPhbCpbwEncode encodeMethod;
    size_t datalen = msg->getData()->getDataLength();
...
void RmcPhbRequestHandler:: requestWriteUpbGrpEntry(const sp<RfxMclMessage>& msg) {
    ...
    size_t datalen = msg->getData()->getDataLength();
    int paramCount = datalen/sizeof(int);
    logD(RFX_LOG_TAG, "requestWriteUpbGrpEntry . datalen is %d, size of int is %d", datalen, sizeof(int));
    if (paramCount < 1) {
    ...
    selectPhbStorage(RIL_PHB_ADN);
        if (entry->text != NULL || entry->number != NULL) {
            ...
            if (i != len) {
                ...
                if (encodeUCS2_0x81(line, alphaId, sizeof(alphaId))) {  // try UCS2_0x81 coding
                    ...
                    // UCS2 coding
                    encodeMethod = CPBW_ENCODE_UCS2;
                    logD(RFX_LOG_TAG, "requestWritePhoneBookEntryExt - alphaId length = %d, line length = %d",
                ...
                if (bIsTc1() == true) {
                    ...
                    encodeMethod = CPBW_ENCODE_GSM7BIT;
                    memcpy(alphaId, line, ((sizeof(alphaId) < strlen(line)) ? sizeof(alphaId) : strlen(line)));
                    logD(RFX_LOG_TAG, "requestWritePhoneBookEntryExt - sizeof(alphaId) = %d , strlen(line) = %d",
                            sizeof(alphaId), strlen(line));
        ...
        if (ret == RIL_E_SUCCESS && maxAnrNum > 0) {
            ...
            if (bIsTc1() == true) {
                ...
                if (entry->adnumber != NULL && strlen(entry->adnumber) > 0) {
                    cmd.append(String8::format("AT+EPBUM=2,0,%d,%d,\"%s\",%d,0", entry->index, maxAnrNum,
                            entry->adnumber, entry->adtype));
                } else {
        ...
        if (ret == RIL_E_SUCCESS && maxEmailNum > 0) {
            ...
            if (bIsTc1() == true) {
                if (entry->email != NULL && strlen(entry->email) > 0) {
                    ...
                    cmd.append(String8::format("AT+EPBUM=2,1,%d,%d,\"%s\"", entry->index, 1, entry->email));
                } else {
