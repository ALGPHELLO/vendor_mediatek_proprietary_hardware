        ...
        if (recvLen != size) {
            if (recvLen == -1) {
                ...
                if (errno != EAGAIN && errno != EINTR) {
                    ...
                    RFX_LOG_E(RFX_LOG_TAG, "[VT RIL RECV] Try again (not error). ret = %d", errno);
                    continue;
                }
