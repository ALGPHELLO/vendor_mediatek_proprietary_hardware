...
void RmcCatCommonRequestHandler::requestStkEventNotify(const sp<RfxMclMessage>& msg) {
    ...
    int cmdType = ((int *) msg->getData()->getData())[0];
    int cmdData = ((int *) msg->getData()->getData())[1];
...
void RmcCatCommonRequestHandler::sendStkBtSapResponseComplete(const sp<RfxMclMessage>& msg,
    ...
    if ((success = pb_get_encoded_size(&encoded_size, fields, data)) &&
        ...
        if(success) {
            logD(RFX_LOG_TAG, "[BTSAP] sendStkBtSapResponseComplete, Size: %d (0x%x) Size as written: 0x%x",
        ...
        logD(RFX_LOG_TAG, "Not sending response type %d: encoded_size: %u. encoded size result: %d",
