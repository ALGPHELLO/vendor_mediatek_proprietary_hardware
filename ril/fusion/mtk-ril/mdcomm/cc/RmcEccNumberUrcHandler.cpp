...
void RmcEccNumberUrcHandler::handleNetworkEcc(const sp<RfxMclMessage>& msg) {
    ...
    if (ss.find("+CEN1") == 0) {
        ...
        if (ptr == NULL) {
            ...
            logD(RFX_LOG_TAG, "[%s] append number %s to ecc list: %s", __FUNCTION__,
                number, newEccList);
        } else {
            ...
            int len = ptr - eccList;
            strncpy(newEccList, eccList, len);

            ptr = strstr(ptr + 1, ";");
            if (ptr != NULL) {
                // Copy the mapping string after finding number
                len = sizeof(newEccList) - strlen(newEccList);
                strncat(newEccList, ptr, len - 1);
            }

            sprintf(newEccList, "%s;%s,%d", newEccList, number, category);
            logD(RFX_LOG_TAG, "[%s] update number %s in ecc list: %s", __FUNCTION__,
                number, newEccList);
        }
