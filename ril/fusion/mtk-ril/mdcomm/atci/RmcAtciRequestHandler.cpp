...
const char *RmcAtciRequestHandler::ENABLE_URC_PROP = "persist.service.atci_urc.enable";
...
void RmcAtciRequestHandler::onHandleRequest(const sp<RfxMclMessage>& msg) {
...
void RmcAtciRequestHandler::onHandleEvent(const sp<RfxMclMessage>& msg) {
    int messageId = msg->getId();
    switch (messageId) {
...
void RmcAtciRequestHandler::handleOemHookAtciInternalRequest(const sp<RfxMclMessage>& msg) {
    ...
    logD(LOG_TAG_ATCI, "[%s] data: %s", __FUNCTION__, data);
    ...
    for (RfxAtLine* line = atResponse->getIntermediates(); line != NULL; line = line->getNext()) {
        logD(LOG_TAG_ATCI, "[%s] intermediate: <%s>", __FUNCTION__, line->getLine());
        ret.appendFormat("%s\r\n", line->getLine());
    }
    logD(LOG_TAG_ATCI, "[%s] finalResponse: <%s>", __FUNCTION__,
            atResponse->getFinalResponse()->getLine());
    ret.appendFormat("%s\r\n", atResponse->getFinalResponse()->getLine());
    logD(LOG_TAG_ATCI, "[%s] result: <%s>", __FUNCTION__, ret.string());
