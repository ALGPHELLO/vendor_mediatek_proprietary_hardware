...
RfxNetworkScanResultData::RfxNetworkScanResultData(void *data, int length) : RfxBaseData(data, length)  {
    if (data != NULL) {
        ...
        if (pData->network_infos_length > 0) {
            int countCell = pResult->network_infos_length / sizeof(RIL_CellInfo_v12);
            pData->network_infos = (RIL_CellInfo_v12 *) calloc(countCell, sizeof(RIL_CellInfo_v12));
            if (pData->network_infos == NULL) goto error;
            memcpy(pData->network_infos, pResult->network_infos, pData->network_infos_length);
        }
        m_data = pData;
        m_length = length;
        return;
...
RfxNetworkScanResultData::~RfxNetworkScanResultData() {
    if (m_data) {
        if (((RIL_NetworkScanResult *)m_data)->network_infos != NULL) {
            free(((RIL_NetworkScanResult *)m_data)->network_infos);
        }
        free(m_data);
    }
}
