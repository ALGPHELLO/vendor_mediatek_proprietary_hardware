...
mSendToMainProtocol = %d", m_type, RFX_ID_TO_STR(m_id), m_id, m_channel_id,
        m_slot_id, m_client_id, m_token, (m_raw_urc==NULL? NULL: m_raw_urc->getLine()),
        (m_raw_urc2==NULL? NULL: m_raw_urc2->getLine()), m_err, mSendToMainProtocol);
}
...
sp<RfxMclMessage> RfxMclMessage::obtainEvent(int id, const RfxBaseData &data, int channel_Id,
                int slot_id, int client_id, int token, nsecs_t delayTime, int priority) {
    sp<RfxMclMessage> msg = new RfxMclMessage();
    msg->m_type = EVENT;
    msg->m_id = id;
