...
String8 RfxMessage::toString() const {
    return String8::format(
            ...
            pTimeStamp = %" PRId64, typeToString(type), groupToString(source), groupToString(dest),
            RFX_ID_TO_STR(pId), pId, pToken, RFX_ID_TO_STR(id), id, token, slotId, clientId, error,
            isSentOnCdmaCapabilitySlot, sendToMainProtocol, pTimeStamp);
}
