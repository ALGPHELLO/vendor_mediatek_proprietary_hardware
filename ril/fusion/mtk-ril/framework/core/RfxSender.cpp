...
RfxSender::RfxSender(int fd, int channel_id, RfxChannelContext *context) :
    ...
    rfx_property_get("ril.atsend.timeout", property_timeout, DEFAULT_AT_TIMEOUT_MIN);
...
sp<RfxAtResponse> RfxSender::atSendCommandRawAck (const char *command,
    ...
    sp<RfxAtResponse> outResponse = atSendCommandFullAck(command, RAW,
            NULL, 0, ackToken);
    if (RfxRilUtils::isChipTestMode() && RfxMisc::strStartsWith(command, "AT+EFUN")){
        printLog(ERROR, String8::format("AT+EFUN is received, update radio state"));
        int targetMode;
        if (sscanf(command, "AT+EFUN=%d", &targetMode) == 1) {
            printLog(DEBUG, String8::format("update radio state with target mode %d", targetMode));
            // TODO: FIXME, use RfxMclStatusManager to update Radio state
            // updateRadioStateWithTargetMode(targetMode);
        } else {
            printLog(DEBUG, String8::format("sscanf fail, wrong argument"));
        }
    }
    return outResponse;
}
...
on channel %d, tid:%lu, AT cmd: %s, AT command timeout: %lldms", m_channel_id, m_threadId, command,
timeoutMsec));
                    #ifdef HAVE_AEE_FEATURE
...
last AT cmd: %s, AT command timeout: %lldms", m_channel_id, m_threadId, command, timeoutMsec));
                    exit(0);
                }
                free(pErrMsg);
            } else {
                if (atoi(modemException) != 1) {
                    LOG_ALWAYS_FATAL("AT command pending too long, assert!!! on channel %d,\
tid:%lu, AT cmd: %s, AT command timeout: %lldms", m_channel_id, m_threadId, command, timeoutMsec);
                } else {
                    LOG_ALWAYS_FATAL("Modem already exception, assert!!! on channel %d, tid:%lu,\
last AT cmd: %s, AT command timeout: %lldms", m_channel_id, m_threadId, command, timeoutMsec);
                }
            }
        }
...
int RfxSender::sendUserData(int clientId, unsigned char* data, size_t length) {
    ...
    unsigned char* header = RfxFragmentEncoder::encodeHeader(
            RfxFragmentData(RfxFragmentEncoder::VERSION, clientId, 0, length));
...
int RfxSender::writeline(const char *s) {
    ...
    if (RfxRilUtils::isUserLoad()) {
        ...
        for (i = 0; i < NUM_ELEMS(s_atcmd_log_reduction); i++) {
            if (RfxMisc::strStartsWith(s, s_atcmd_log_reduction[i])) {
                printLog(DEBUG, String8::format("AT> %s (%s tid:%lu)\n", s, mName, m_threadId));
                break;
            }
        }
        if (i == NUM_ELEMS(s_atcmd_log_reduction)) {
            printLog(INFO, String8::format("AT> %s (%s tid:%lu)\n", s, mName, m_threadId));
        }
    }
