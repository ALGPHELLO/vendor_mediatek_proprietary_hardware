...
unsigned char* RfxController::pii(const char* input) {

    if (input == NULL) {
        RFX_LOG_I(RFX_LOG_TAG, "pii: input is null");
        return NULL;
    }

    // according to Rlog, input should executes sha1 and then use base64 to encode.
    uint8_t sha1Result[SHA_DIGEST_LENGTH];
    unsigned char *outText = NULL;
    size_t b64Len = 0;

    SHA1((const uint8_t *) input, strlen(input), sha1Result);
    if (!EVP_EncodedLength(&b64Len, sizeof(sha1Result))) {
        RFX_LOG_E(RFX_LOG_TAG, "pii: can not get length of base64");
        return NULL;
    }
    outText = (unsigned char *) OPENSSL_malloc(b64Len);
    if (outText == NULL) {
        RFX_LOG_E(RFX_LOG_TAG, "pii: malloc fail");
        return NULL;
    }
