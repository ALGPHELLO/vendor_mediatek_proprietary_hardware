...
void RfxChannelManager::initMuxPath() {
    ...
    if (RFX_SLOT_COUNT >= 2) {
        ...
        if(targetSim*RIL_CHANNEL_OFFSET > getSupportChannels()) {
            ...
            for(i = 0; i < RIL_CHANNEL_OFFSET-1; i++) {
                strncpy(mux_path_tmp[i], muxPath[(targetSim-1)*RIL_CHANNEL_OFFSET+i],
                        RIL_SUPPORT_CHANNELS_MAX_NAME_LEN);
                strncpy(muxPath[(targetSim-1)*RIL_CHANNEL_OFFSET+i], muxPath[i],
                        RIL_SUPPORT_CHANNELS_MAX_NAME_LEN);
...
void RfxChannelManager::urcRegisterDone() {
    ...
    sSelf->mUrcCount++;
    if (RfxRilUtils::getSimCount() == sSelf->mUrcCount) {
