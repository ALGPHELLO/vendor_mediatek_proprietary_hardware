...
bool RfxReader::threadLoop() {
    ...
    printLog(INFO, String8::format("threadLoop. RfxReader %s init, channel id = %d", mName,
            m_channel_id));
...
void RfxReader::readerLoop() {
    ...
    printLog(ERROR, String8::format("%s Closed", mName));
    exit(-1);
}
...
char* RfxReader::readline(char *buffer) {
    ...
    while (p_eol == NULL) {
        if (0 == MAX_AT_RESPONSE - (p_read - buffer)) {
            printLog(DEBUG, String8::format("ERROR: Input line exceeded buffer\n"));
        ...
        do {
            ...
            if (RfxRilUtils::isUserLoad() != 1) {
                if (count < 0) {
                    ...
                    // raw data
                    char data[MAX_AT_RESPONSE] = {0};
                    char temp[10] = {0};
                    for (int i = 0; i < count; i++) {
                        snprintf(temp, 4, "%02x ", p_read[i]);
                        strncat(data, temp, strlen(temp));
                    }
                    printLog(DEBUG, String8::format("%s\n", data));
...
void RfxReader::readerLoopForFragData() {
    ...
    for (;;) {
        ...
        handleUserDataEvent(aggregateData, fragData.getDataLength());
        free(aggregateData);
        free(tmpData);
    }
    free(header);
...
void RfxReader::handleUserDataEvent(char *data, size_t length) {
    printLog(INFO, String8::format("handleUserDataEvent, len:%zu", length));
    // create Message and set to MclDispatcherThread
