	      ...
	      while (RTA_OK (rta, rtasize))
		{
		  ...
		  switch (rta->rta_type)
		    ...
		    case IFLA_ADDRESS:
		      ...
		      if (rta_payload < sizeof (ifas[ifa_index].addr.sl.sll_addr))   //resolvecd coverity cr 
			{
		    ...
		    case IFLA_BROADCAST:
		      ...
		      if (rta_payload < sizeof (ifas[ifa_index].broadaddr.sl.sll_addr))      //resolvecd coverity cr
			{
	      ...
	      while (RTA_OK (rta, rtasize))
		{
		  ...
		  switch (rta->rta_type)
			...
			switch (ifam->ifa_family)
			  ...
			  default:
			    //if (rta_payload <= sizeof (ifas[ifa_index].addr))
			    if (rta_payload < sizeof (sa->sa_data))                   //resolved coverity cr
			...
			default:
			   //if (rta_payload <= sizeof (ifas[ifa_index].addr))
			   if (rta_payload < sizeof (ifas[ifa_index].addr.sa.sa_data))        //resolved coverity cr
			...
			default:
			  //if (rta_payload <= sizeof (ifas[ifa_index].addr))
			  if (rta_payload < sizeof (ifas[ifa_index].broadaddr.sa.sa_data))      //resolved coverity cr
