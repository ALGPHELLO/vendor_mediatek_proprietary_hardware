...
struct LbsDeathRecipient : virtual public hidl_death_recipient
{
    ...
    virtual void serviceDied(uint64_t cookie, const wp<IBase>& who) override {
      ...
      ALOGE("Abort due to ILBS hidl service failure");
    }
};
...
void hidl_request_gps(bool turnOn) {
    ...
    if (lbsHal != nullptr) {
        ...
        int size = (_offset < sizeof(_buff)/sizeof(_buff[0]))? _offset : sizeof(_buff)/sizeof(_buff[0]);
...
static int hw_test_3332(const int fd)
{
    ...
    if (bytewrite == sizeof(cmd)) {
        ...
        MND_ERR("write error, write API return is %ld, error message is %s\n", bytewrite, strerror(errno));
        return 1;
    }
}
