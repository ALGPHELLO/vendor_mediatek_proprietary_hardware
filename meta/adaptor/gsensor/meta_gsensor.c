...
{  
	...
	else if((err = gsensor_calibration(fd, arg->delay, arg->num, arg->tolerance, &cur)) != 0)
	{
		MGSLOGD("calibrate acc: %d\n", err);
		return err;
	}
	else if((err = gsensor_set_cali(fd, &cur)) != 0)
	{    
		MGSLOGD("set calibration fail: (%s) %d\n", strerror(errno), err);
		return err;
	}
	else if((err = gsensor_get_cali(fd, &cur)) != 0)
	{    
...
int gs_exec_read_raw(HwmData *dat)
{
	...
	else if((err = gsensor_read(fd, &cur)) != 0)
	{
...
int gs_exec_read_nvram(HwmData *dat)
{
	...
	else if((err = gsensor_read_nvram(&cur)) != 0)
	{
