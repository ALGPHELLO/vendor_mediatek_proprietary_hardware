            ...
            switch(nfc_msg_loc->type)
            {
               ...
               {                
                   ...
                   if(gTagAction == NFC_EM_OPT_ACT_WRITE)
                   {
                       ...
                       META_LOG("nfc_cnf.result.m_nNfc_als_readerm_opt_rsp.result :%d",nfc_cnf.result.m_nNfc_als_readerm_opt_rsp.result);
                   }
...
//                   META_LOG("META_NFC_CMD,MTK_NFC_EM_ALS_READER_MODE_OPT_RSP,[%zu,%zu,%zu]",
//                    sizeof(nfc_cnf),
//                    sizeof(nfc_cnf.result.m_nNfc_als_readerm_opt_rsp),
//                    sizeof(nfc_cnf.result.m_nNfc_als_readerm_opt_rsp.ndef_read));

                   
                   META_LOG("META_NFC_CMD,MTK_NFC_EM_ALS_READER_MODE_OPT_RSP,[%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x]",
               ...
               case MTK_NFC_EM_ALS_READER_MODE_NTF:
               {
                   ...
                   nfc_cnf.header.token = gBackUpToken;
                    ...
                    memcpy(&nfc_cnf.result.m_nNfc_als_readerm_ntf.Uid, locPtr->Uid , sizeof(nfc_cnf.result.m_nNfc_als_readerm_ntf.Uid));
                   ...
                   META_LOG("META_NFC_CMD:NFC MTK_NFC_EM_ALS_READER_MODE_NTF ,isNDEF :%d,result:%d,UidLen:%d",
...
/* disable useless log message and fix coverity :Untrusted loop bound issue
                   {
                      int i;
...
*/
               ...
               case MTK_NFC_EM_POLLING_MODE_NTF:
               {
                   ...
                   memcpy(&nfc_cnf.result.m_nNfc_polling_func_ntf.ntf.reader.Uid, locPtr->ntf.reader.Uid, 32);
            ...
            else 
            {
                ...
                META_LOG("META_NFC_CMD:Don't Write to PC MSGID.");  //remove nfc_msg.msg_type printing
            }
        }
    }
    bStop_ReadThread = 1;
...
int META_NFC_init(void)
{
    ...
    else if (pid == 0)  /*child process*/
    {
        ...
        META_LOG("nfc_open: execute: %s\n", "/system/bin/nfcstackp");
        err = execl("/system/bin/nfcstackp", "nfcstackp", "NFC_TEST_MODE", NULL);
        if (err == -1)
        {
...
void META_NFC_deinit()
{
    ...
    err = execl("kill /system/bin/nfcstackp", "nfcservice", NULL);
    if (err == -1)
    {
       ...
       */
       case NFC_OP_ALS_READER_MODE:
       {
           ...
           META_LOG("%02x,%02x,%02x,%02x,%02x,%02x,%02x,",
                   ...
                   */
           META_LOG("readerm_req.action:%02x",req->cmd.m_nNfc_als_readerm_req.action);
           META_LOG("readerm_req.supporttype:%02x",req->cmd.m_nNfc_als_readerm_req.supporttype);
           META_LOG("readerm_req.typeA_datarate:%02x",req->cmd.m_nNfc_als_readerm_req.typeA_datarate);
           META_LOG("readerm_req.typeB_datarate:%02x",req->cmd.m_nNfc_als_readerm_req.typeB_datarate);
           META_LOG("readerm_req.typeF_datarate:%02x",req->cmd.m_nNfc_als_readerm_req.typeF_datarate);
           META_LOG("readerm_req.typeV_datarate:%02x",req->cmd.m_nNfc_als_readerm_req.typeV_datarate);
           META_LOG("readerm_req.typeV_subcarrier:%02x",req->cmd.m_nNfc_als_readerm_req.typeV_subcarrier);
       ...
       case NFC_OP_ALS_P2P_MODE:
       {
           ...
           META_LOG("%02x,%02x,%02x,%02x,%02x,%02x,%02x,",
                   ...
                   */
           META_LOG("p2p_req.action:%02x",req->cmd.m_nNfc_als_p2p_req.action);
           META_LOG("p2p_req.isDisableCardM:%02x",req->cmd.m_nNfc_als_p2p_req.isDisableCardM);
           META_LOG("p2p_req.mode:%02x",req->cmd.m_nNfc_als_p2p_req.mode);
           META_LOG("p2p_req.role:%02x",req->cmd.m_nNfc_als_p2p_req.role);
           META_LOG("p2p_req.supporttype:%02x",req->cmd.m_nNfc_als_p2p_req.supporttype);
           META_LOG("p2p_req.typeA_datarate:%02x",req->cmd.m_nNfc_als_p2p_req.typeA_datarate);
           META_LOG("p2p_req.typeF_datarate:%02x",req->cmd.m_nNfc_als_p2p_req.typeF_datarate);
       ...
       case NFC_OP_ALS_CARD_MODE:
       {
           ...
           META_LOG("%02x,%02x,%02x,%02x,",
                ...
                */
           META_LOG("cardm_req.action:%02x",req->cmd.m_nNfc_als_cardm_req.action);
           META_LOG("cardm_req.fgvirtualcard:%02x",req->cmd.m_nNfc_als_cardm_req.fgvirtualcard);
           META_LOG("cardm_req.supporttype:%02x",req->cmd.m_nNfc_als_cardm_req.supporttype);
           META_LOG("cardm_req.SWNum:%02x",req->cmd.m_nNfc_als_cardm_req.SWNum);
           META_NFC_SEND_SOCKET(msg);
       }
       break;
       ...
       break;
        case NFC_OP_CHIP_VERSION:
        {
            ...
            //   0x02: 6605 (default)
            //   0xFF: Error
            nfc_cnf.op = NFC_OP_CHIP_VERSION;

            nfc_cnf.result.m_nNfc_chip_Version_rsp.chip_ver = 0x02;
