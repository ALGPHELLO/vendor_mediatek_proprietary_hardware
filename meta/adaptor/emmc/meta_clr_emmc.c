...
#include <fs_mgr.h> /* for fs_mgr_XXX APIs usage */
...
#define NVDATA_PARTITION "/vendor/nvdata"
...
static void list_services_usage(const char *root)
{
	...
	if (!is_permissive())
		LOGE("Can not query all services which are using %s in enforcing mode, please set selinux to permissive mode\n", root);
...
static int format_root_device(const char *root)
{
	...
	if (!fstab) {
		LOGE("Failed in  fs_mgr_read_fstab_default\n");
		ret = -1;
		goto out;
	}

	rec = fs_mgr_get_entry_for_mount_point(fstab, root);
	if (!rec) {
	...
	if (devp) {
		p = realloc(rec->blk_device, strlen(devp)+1);
		if(p) {
			strcpy(p, devp);
			rec->blk_device = p;
		}
	...
	crypt_footer = fs_mgr_is_encryptable(rec) && !strcmp(rec->key_loc, "footer");
	...
	if (ret != 0) {
#if defined(MTK_GPT_SCHEME_SUPPORT) || defined(MNTL_SUPPORT)
		LOGE("format_volume: %s failed on %s\n",
			strcmp("f2fs", rec->fs_type)? "make_extf4fs":"make_f2fs", rec->blk_device);
#else
		LOGE("format_volume: make_ext4fs failed\n");
#endif
		ret = -3;
	}
	else {
#ifdef MTK_GPT_SCHEME_SUPPORT
		LOGE("format_volume: %s success on %s\n",
			strcmp("f2fs", rec->fs_type)? "make_extf4fs":"make_f2fs", rec->blk_device);
#else
		LOGE("format_volume: make_ext4fs success\n");
#endif
	}
out:
	if (fstab)
...
{
	int result = 0, ret = 0, alloc_len = 0, pre_len, name_len;
	DIR *d = NULL;
	struct dirent *de;
	char *files = NULL;
	...
	pre_len = strlen("/system/bin/rm -r /data/");
	...
	while ((de = readdir(d)) != NULL) {
		...
		if (name_len + pre_len + 1 > alloc_len) {
			files = realloc(files, (name_len + pre_len + 1) * sizeof(char));
			if (!files) {
				LOGE("cant re-allocate memory for %d bytes\n", (name_len + 30) * sizeof(char));
				ret = -1;
				goto out;
			}
			alloc_len = name_len + pre_len + 1;
		}
		snprintf(files, pre_len + name_len, "/system/bin/rm -r /data/%s", de->d_name);
		files[pre_len + name_len] = 0;
		LOGD("the file is %s\n", de->d_name);
		if (system(files)) {
...
int format_internal_sd_partition()
{
	...
	rec = fs_mgr_get_entry_for_mount_point(fstab, "/intsd");
	if (!rec) {
	...
	if (get_mount_rtn || !strstr(mnt_media_dir, MEDIA_DIR)) {
		...
		free(source);
		asprintf(&source, "/system/bin/newfs_msdos %s", rec->blk_device);
		rtn = system(source);
#else
...
void META_CLR_EMMC_OP(FT_EMMC_REQ *req)
{
	...
	case FT_EMMC_OP_CLEAR_WITHOUT_TCARD:
		ret = clear_emmc_nomedia_entry(DATA_PARTITION);
