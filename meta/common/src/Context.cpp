...
bool Context::IsModemSupport(int idx)
{
	...
	sprintf(szProperty, "ro.boot.opt_md%d_support", idx);
...
unsigned int Context::dumpDataInHexString(const unsigned char* con, int length, unsigned int bytesPerRow)
{
	...
	if(getLogLevel() || getPropValue("persist.meta.dumpdata") == 1)
	{
		int i = 0;
		unsigned int j = 0;
...
const char* Context::makepath(unsigned char file_ID)
{
	...
	else
	{
		...
		if(file_ID == 0)
		{
			return "/vendor/nvdata/AllMap";
		}
		else if(file_ID == 1)
		{
			return "/vendor/nvdata/AllFile";	
		}
		else
		{
			META_LOG("[Meta] makepath error: invalid file_ID %d! ", file_ID);
...
#else
		META_LOG("MTK_EXTMD_NATIVE_DOWNLOAD_SUPPORT create modem thread after native download complete");	
#endif
	}
...
#else
		META_LOG("[Meta] MTK_EXTMD_NATIVE_DOWNLOAD_SUPPORT create modem thread after native download complete");	
#endif
	}
}
...
int Context::ChangeModemMode(int mode)  // 1:modem to normal  2:modem to meta
{
	...
	{
		META_LOG("[Meta] To check modem status before send AT Command by ATCI");
		usleep(100*1000);
		nRetry++;
	...
	MSocket *pSocket = getSocket(SOCKET_AT_ATCI);
	if(pSocket == NULL)
	{
		pSocket = createSocket(SOCKET_AT_ATCI);
		if(pSocket != NULL)
		{
			int bInit = pSocket->initReservedClient("adb_atci_socket", 1);
			if(bInit == 0)
			{
				delSocket(SOCKET_AT_ATCI);
				return -1;
			}
		}
		else
	...
	property_get("persist.radio.simswitch",strSimStatus,"unknown");
	...
	//=============================================//
	//Workaround for Sylvia L+L issue
	if(mode == 2)
	{
		setATRespFlag(1);
		META_LOG("[META] Set single SIM");
		pSocket->send_msg("AT+EFUN=1\r");
		META_LOG("[Meta] Send AT+EFUN=1");
		while(getATRespFlag()!=0)
		{
			usleep(100*1000);
			nRetry++;
	...
	{
		setATRespFlag(1);
		META_LOG("[META] Set dual SIM");
		pSocket->send_msg("AT+EFUN=3\r");
		META_LOG("[Meta] Send AT+EFUN=3");
		while(getATRespFlag()!=0)
		{
			usleep(100*1000);
			nRetry++;
...
int Context::notifyModemDoRF(int mdIdx)
{
	...
	//write bootprof
	writeBootprof("[META] checkMdStatus mtk.md1.status=ready.");
	...
	writeBootprof("[META] Wait Modem urc +EIND: 128");
	...
	writeBootprof("[META] AT+EMETACFG=1 return OK.");
...
WIFI_PARA Context::getWifiPara()
{
    ...
    property_get("persist.atm.ipaddress", strIpAddr, "0,0,0,0");
...
int Context::getModemModeSwitching()
{
	return m_modemModeSwitching;
}
	
void Context::setModemModeSwitching(int modemModeSwitching)
{
	m_modemModeSwitching = modemModeSwitching;
}
...
int getModemModeSwitching()
{
	return Context::instance()->getModemModeSwitching();
}
	
void setModemModeSwitching(int modemModeSwitching)
{
	return Context::instance()->setModemModeSwitching(modemModeSwitching);
}
