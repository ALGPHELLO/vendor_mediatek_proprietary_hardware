...
private:
	int SwitchMdloggerMode(FT_TARGETLOG_CTRL_REQ *req);
	int	SwitchMobilelogMode(FT_TARGETLOG_CTRL_REQ *req);
	int TargetLogPulling(FT_TARGETLOG_CTRL_REQ *req);
	int GetTargetLogPullingStatus(FT_TARGETLOG_CTRL_REQ *req);
	unsigned int QueryMdloggerStatus();
	unsigned int GetLogPropValue(char *key);
	unsigned int SetModemLogFilter(FT_TARGETLOG_CTRL_REQ *req);
	void* GetLoggerObj(unsigned int type, const char * service);
