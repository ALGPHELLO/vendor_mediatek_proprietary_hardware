...
#define SOCKET_NAME_RIL "rild-dongle"
#define SOCKET2_NAME_RIL "rild2"
#define SOCKET3_NAME_RIL "rild3"
#define SOCKET4_NAME_RIL "rild4"

#define SOCKET_NAME_RIL_DEBUG "rild-dongle-debug"

#define ANDROID_WAKE_LOCK_NAME "radio-interface-dongle"


#define PROPERTY_RIL_IMPL "gsm.version.ril-impl-dongle"
...
static int responseSimStatus(Parcel &p, void *response, size_t responselen) {
    ...
    if (responselen == sizeof (RIL_CardStatus_v6)) {
		...
		RLOGE("responseSimStatus: RIL_CardStatus_v6");
		...
		//not care ims sub app index
        p.writeInt32(-1);

        sendSimStatusAppInfo(p, p_cur->num_applications, p_cur->applications);
    } else if (responselen == sizeof (RIL_CardStatus_v5)) {

		RLOGE("responseSimStatus: RIL_CardStatus_v5");
...
static void startListen(RIL_SOCKET_ID socket_id, SocketListenParam* socket_listen_p) {
    ...
    memset(socket_name, 0, sizeof(char)*32);

    switch(socket_id) {
        case RIL_SOCKET_1:
            strncpy(socket_name, RIL_getRilSocketName(), 31);
            break;
    #if (SIM_COUNT >= 2)
        case RIL_SOCKET_2:
            strncpy(socket_name, SOCKET2_NAME_RIL, 31);
            break;
    #endif
    #if (SIM_COUNT >= 3)
        case RIL_SOCKET_3:
            strncpy(socket_name, SOCKET3_NAME_RIL, 31);
            break;
    #endif
    #if (SIM_COUNT >= 4)
        case RIL_SOCKET_4:
            strncpy(socket_name, SOCKET4_NAME_RIL, 31);
            break;
    #endif
        default:
