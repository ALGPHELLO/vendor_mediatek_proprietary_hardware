...
bool UsbSelect::searchParam(const char *buf, const char *key, char *value, int len)
{
    ...
    if (!p) {
        ...
        RLOGD(" not found the value of %s", key);
        return false;
    }    

    //str = strstr(p, "=");
    const char *str = strstr(p, "=");
    if (!str) {
...
bool UsbSelect::findDevice(char *vid, char *pid, struct usbdevice *tmpdevice)
{
    ...
    if (NULL == vid || NULL == pid || NULL == tmpdevice) {
        RLOGD(" param is wrong");
    }
    RLOGD("idvendor:%s, idproduct:%s\n", vid, pid);
...
int UsbSelect::handleUsbAutoSelect(NetlinkEvent *evt)
{
    ...
    if (NULL == evt) {
        RLOGD("handleUsbAutoSelect: NetlinkEvent is null");
        return -1;
    }
    ...
    if (NULL == buf && strlen(buf) < 9) {
        RLOGD("handleUsbAutoSelect: The Param of PRODUCT is wrong in NetlinkEvent");
        return -1;
    }
    RLOGD("handleUsbAutoSelect: The PRODUCT is: %s ", buf);
    ...
    if (strtoul(lastDevice.VID, NULL, 16) == strtoul(vid, NULL, 16) &&
        ...
        RLOGD("handleUsbAutoSelect: NetlinkEvent is the same of last one");
        ...
        if (found) {
            ...
            RLOGD("The 3Gdongle(vid = %s,pid = %s) is not found", vid, pid);
            return 2;
...
void UsbSelect::handleDongle(struct usbdevice *tmpdevice)
{
	...
	RLOGD("handleDongle: new 3Gdongle insert, the libpath is: %s", tmpdevice->libpath);
	...
	if(strtoul("12D1", NULL, 16) == strtoul(tmpdevice->VID, NULL, 16)) {
		...
		while(1) {
			if ( ( property_get("tedongle.mark_switchuser", mark_switchuser, NULL) > 0)
				   && (mark_switchuser[0] == '1') ) {
				RLOGD("The property of rild.mark_switchuser is 1");
				break;
		...
		while(1) {
			if ( ( property_get("tedongle.mark_switchuser", mark_switchuser, NULL) > 0)
				   && (mark_switchuser[0] == '0') ) {
				RLOGD("The property of rild.mark_switchuser is 0");
				break;
...
int UsbSelect::handleUsbRemove(NetlinkEvent *evt)
{
    ...
    if (NULL == evt) {
        RLOGD("handleUsbRemove: NetlinkEvent is null");
        return -1;
    }
    ...
    if (NULL == buf && strlen(buf) < 9) {
        RLOGD("handleUsbRemove: The Param of PRODUCT is wrong in NetlinkEvent");
        return -1;
    }
    RLOGD("handleUsbRemove: The PRODUCT is: %s ", buf);
    ...
    if (strtoul(lastRemovedDevice.VID, NULL, 16) == strtoul(vid, NULL, 16) &&
        ...
        RLOGD("handleUsbRemove: NetlinkEvent is the same of last removed one");
