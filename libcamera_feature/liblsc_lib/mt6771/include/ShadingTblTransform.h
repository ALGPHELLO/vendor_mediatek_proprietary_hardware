...
#define SHADINGTBLTRANSFORM_REVISION 8013001
...
//#define BWSENSOR // config if the sensor is black-white or RGB
...
#if SHADING_VER == 5
#define MAX_TIL_COEFF_SIZE        ((LSCTRANS_MAX_TIL_GRID_NUM-1)*(LSCTRANS_MAX_TIL_GRID_NUM-1)*LSCTRANS_SUPPORT_CH_NUM*12/2*4) // pack every 2 coef into 4 bytes
#else
...
#if (defined(PC_SIM) && (defined(linux) || defined(__linux)))
#define LINUXVER
#endif
...
#define MEDIA_PATH ""
#define OUTPUT_SL2 0
#define OUTPUT_SOURCEGAINTBL 0
#define OUTPUT_MISC 0
#else
#define MEDIA_PATH ".\\data"
#define OUTPUT_SL2 1
#define OUTPUT_SOURCEGAINTBL 1
#define OUTPUT_MISC 1
#endif
#define CUSTOMER_RELEASE 0
//#define TOOL_Version 2.3// Add offset for tuning
//#define TOOL_Version 2.4// Add SHADING_VER 5
#define TOOL_Version 4.0// official release for SHADING_VER 4 platform
...
typedef enum
{
  SHADING_GAIN_TYPE_MODE0, // u2.8
  SHADING_GAIN_TYPE_MODE1, // u3.7
  SHADING_GAIN_TYPE_MODE2, // u1.9
  SHADING_GAIN_TYPE_DEFAULT,
} SHADING_GAIN_TYPE;
...
typedef struct SHADING_TBL_SPEC_PSO
{
  unsigned int            img_width;   // orig/target img width
  unsigned int            img_height;  // orig/target img height
  unsigned int            offset_x;    // (0,0) at the upper left corner
  ...
  unsigned int            ratio_idx;     // index for the compensation strength (0~20)//(0~10)
  SHADING_GRGB_SAME grgb_same;     // whether to use the same gains for Gr and Gb
  unsigned short            *tableR;         // output table for R gain
  unsigned short            *tableGr;      // output table for G1 gain
  unsigned short            *tableGb;      // output table for G2 gain
  unsigned short            *tableB;         // output table for B gain
  SHADING_TYPE      data_type;   // input/output table type
} SHADING_TBL_SPEC_PSO;
...
{
  SHADING_TBL_SPEC      input;
  SHADING_TBL_SPEC_PSO  output;
  SHADING_AFN_T         afn;
  void                  *working_buff_addr;
  unsigned int          working_buff_size;
} SHADIND_TRFM_CONF_PSO;
...
typedef struct
{
    ...
    unsigned int u4Addarg;
    unsigned char u2Tbltype;
}LSC_CAL_INI_PARAM_LINUX_T;
#endif
...
LSC_RESULT shading_transform_new(SHADIND_TRFM_CONF shading_conf);
LSC_RESULT shading_transform_new(SHADIND_TRFM_CONF_PSO shading_conf, SHADING_GAIN_TYPE gain_type = SHADING_GAIN_TYPE_DEFAULT);
LSC_RESULT shading_transform(SHADIND_TRFM_CONF shading_conf);
LSC_RESULT shading_align_golden(SHADIND_ALIGN_CONF trans);
#ifdef LINUXVER
LSC_RESULT LscGaintoHWTbl(float *p_pgn_float, unsigned int *p_lsc_tbl, int grid_x, int grid_y, int RawImgW, int RawImgH, void* WorkBuf, int BufSize, const LSC_CAL_INI_PARAM_LINUX_T &IniParamLinux );
#else
...
MINT32 mrLSC_Calibrate(LSC_CALI_INFO_T cali_info, MUINT8 a_u1Mode, MUINT16 a_u2SVDTermNum, MUINT8 GrGb_equal);
...
public:
    ...
    static bool LscVirtualChannel(LSC_RESVERED_STRUCT *Content);

    static LSC_RESVERED_STRUCT file_reversion;
