...
#define TSF_ALGO_VERSION 3.3
#define MTKTSF_REVISION 7453001
...
// Memory Buffer Info
struct MTK_TSF_MEM_BUF_INFO_T
{
    MUINT32     size;
    MUINTPTR    VA;
    MUINTPTR    PA;
    MTK_TSF_MEM_BUF_INFO_T(
        MUINT32     _size = 0,
        MUINTPTR    _VA = 0,
        MUINTPTR    _PA = 0)
    : size(_size)
...
public:
    virtual void        doNotifyCb (MINT32  _msgType) = 0;

public:
    enum ECb_T
    {
        eID_NOTIFY_TSF_IN    = 0,
        eID_NOTIFY_TSF_OUT   = 1
    };
};
...
typedef struct
{
    ...
    MUINT32                    StatStrideSize;  // input stat's stride size
    MUINT32                    StatFullSize;    // input stat's full size
    MTK_BAYER_ORDER_ENUM       BayerOrder;
    MTK_TSF_LSC_PARAM_STRUCT   *pLscConfig;
    MBOOL                      WithOTP; // with/without OTP data, algorithm could get NVRAM shadingTbl(gain)
    ...
    MTKTSF_AFN_T               afn;
    MTK_TSF_MEM_BUF_INFO_T     buf_tsfi;
    MTK_TSF_MEM_BUF_INFO_T     buf_tsfo;
    ITsfCb*                    pCb;
...
typedef struct
{
    ...
    MUINT32                     u4TblSize; // in byte
    MINT32                      ExifData[296+7]; // For TSF V2.0
