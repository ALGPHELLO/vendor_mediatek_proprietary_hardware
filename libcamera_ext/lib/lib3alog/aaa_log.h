...
public:
    ...
    static class _init
    {
        ...
        _init()
        {
            ...
            property_get("debug.3alog.enable", value, DEFAULT_LOG);    // check property
            itmp = atoi(value);
            ...
            {   // check if off log manually
                GET_PROP_OFF("debug.af.enable", LOG_ON);
                GET_PROP_OFF("debug.ae.enable", LOG_ON);
                GET_PROP_OFF("debug.awb.enable", LOG_ON);

                bEn = true;
            }
            else if( itmp == 0 ) // disable log
            {   // check if on log manually
                GET_PROP_ON("debug.af.enable", LOG_OFF);
                GET_PROP_ON("debug.ae.enable", LOG_OFF);
                GET_PROP_ON("debug.awb.enable", LOG_OFF);
