...
getInstance()
{
    MTKWarp *pAppGlesWarp = NULL;

    LOGD("[%s] getInstance \n", LOG_TAG);
    if (pAppGlesWarp == NULL) {
        pAppGlesWarp = new AppGlesWarp();
    }
/*
#ifndef SIM_MAIN
...
void
AppGlesWarp::
destroyInstance(MTKWarp* pAppGlesWarp)
{
    if (pAppGlesWarp) {
        delete pAppGlesWarp;
    }
...
MRESULT AppGlesWarp::WarpInit(MUINT32 *InitInData, MUINT32 *InitOutData)
{
    LOGD("[%s] WarpInit \n", LOG_TAG);

    GLES_WARP_IMG_EXT_INFO* core = &warp_obj.core_info;
    ...
    core->ImgFmt = (GLES_WARP_IMAGE_ENUM)input->ImgFmt;
    core->OutImgFmt =  (GLES_WARP_IMAGE_ENUM)input->OutImgFmt;
    ...
    core->GB_Width = input->Width;
    core->GB_Height = input->Height;
    ...
    //initial GPU warping
    core->DisableEGLImageInit = input->DisableEGLImageInit;
    printf(" warp_obj.GlesWarpingInit \n");
    warp_obj.GlesWarpingInit();
...
MRESULT AppGlesWarp::WarpMain(void)
{
    LOGD("[%s] WarpMain \n", LOG_TAG);
...
MRESULT AppGlesWarp::WarpReset(void)
{
    LOGD("[%s] WarpReset \n", LOG_TAG);
    WarpState = WARP_STATE_STANDBY;
    //gWarpCurrImageNum = 0;
...
MRESULT AppGlesWarp::WarpFeatureCtrl(MUINT32 FeatureID, void* pParaIn, void* pParaOut)
{
    MRESULT RetCode = S_WARP_OK;
    ...
    void *pSetBufferAddr = (void *)pParaIn;
    GLES_WARP_IMG_EXT_INFO* core = &warp_obj.core_info;
    switch ( FeatureID )
    {
    case WARP_FEATURE_ADD_IMAGE:
        ...
        else
        {
            ...
            core->TuningPara.demo = ImgInfo->pTuningPara->Demo;
            ...
            core->SrcGraphicBuffer = ImgInfo->SrcGraphicBuffer;
            core->DstGraphicBuffer = ImgInfo->DstGraphicBuffer;
    ...
    case WARP_FEATURE_GET_LOG:
        // TBD
        break;
    case WARP_FEATURE_INIT_EGLIMAGE:
         //GLES_WARP_IMG_EXT_INFO* core = &warp_obj.core_info;
         //core->ImgFmt = (GLES_WARP_IMAGE_ENUM)ImgInfo->ImgFmt;
         //core->OutImgFmt =  (GLES_WARP_IMAGE_ENUM)ImgInfo->OutImgFmt;
         core->InitBufferCount = ImgInfo->InitBufferCount;
         core->InitBufferType = ImgInfo->InitBufferType;
         printf(" core->InitBufferType %d\n", core->InitBufferType);
         core->SrcGraphicBuffer = (void*) ImgInfo->SrcGraphicBuffer;
         core->DstGraphicBuffer = (void*) ImgInfo->DstGraphicBuffer;
         warp_obj.InitEGLImage();
        break;
    default:
