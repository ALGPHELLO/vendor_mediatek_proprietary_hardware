...
typedef struct vpu_st_sett_harris {
    int S32_Scale;
    int S32_Offset;
};
...
typedef struct vpu_st_sett_canny {
    int S32_Scale;
    int S32_Offset;
};

typedef struct vpu_st_sett_k15 {
    float F32_Scale;
    int S32_RX;
    int S32_RY;
    int S32_Width;
...
typedef struct vpu_st_sett_k14 {
    int S32_Width;
    int S32_Height;
    int S32_Stride;
};
...
/*
    ...
    MY_GET_TIME(&sub_start_time, NULL);
    algo = vpu->getAlgo("vpu_lib_d2d_harris");
    MY_GET_TIME(&sub_end_time, NULL);
    MY_DIFF_TIME(&temp_time, sub_start_time, sub_end_time);
...
int vpu_enque_histogram(int i4ImgInFD,int i4ImgOutFD,int  width,int height)
{
    ...
    // 1.1 init algo of vpu
    MY_GET_TIME(&sub_start_time, NULL);
    algo = vpu->getAlgo("vpu_lib_d2d_histogram");
    if (algo == NULL)
    {
        MY_LOGW("fail to get vpu algo handle");
        goto EXIT;
...
int vpu_enque_canny(int i4ImgInFD,int i4ImgWkFD,int i4ImgOutFD,int  width,int height, int pad_size)
{
    ...
    // 1.1 init algo of vpu
    MY_GET_TIME(&sub_start_time, NULL);
    algo = vpu->getAlgo("vpu_lib_d2d_canny");
    if (algo == NULL)
    {
        MY_LOGW("fail to get vpu algo handle");
        goto EXIT;
...
int vpu_enque_histogram_data_partition(int i4ImgInFD,int i4ImgOutFD,int  width,int height,int loop)
{
    ...
    while(cnt < loop)
    {
        ...
        {
            deque_req = vpu->deque();
            MY_LOGD("deque_req = (0x%lx) %d",(unsigned long)deque_req);
...

{
    ...
    int param1;
    int result, i, j, enq_count0 = 0, deq_count0 = 0, cnt = 0;
    ...
    // 1.1 init algo of vpu
    MY_GET_TIME(&sub_start_time, NULL);
    algo0 = vpu0->getAlgo("vpu_flo_d2d_k14");
    if (algo0 == NULL)
    {
    ...
    algo1 = vpu1->getAlgo("vpu_flo_d2d_k15");
    if (algo1 == NULL)
    {
...
int vpu_enque_swnr(int i4InOutFD,int i4InFD,int i4OutFD,int i4WeightFD,int width, int height, int i4Run, UINT8 *prVaInOut, int ion_drv_handle)
{
   VpuStream  *vpu     = NULL;
    ...
    // 1.1 init algo of vpu
    MY_GET_TIME(&sub_start_time, NULL);
    algo = vpu->getAlgo("vpu_lib_d2d_swnr");
    if (algo == NULL)
    {
        MY_LOGW("fail to get vpu algo handle");
        goto EXIT;
...
{
    UINT8 *prVaIn1, *prVaIn2, *prVaOut1,*prVaOut2,*prVaOut3;
    ...
    else
    {
        ...
        if (argc < 1)
        {
            MY_LOGD("Usage: simUT_test -c <testAlgo> -r <Runtimes> -m <testCase>");
            MY_LOGD("<testAlgo> 0: HarrisCorner, 1: Histogram, 2:CannyEdge, 3:Histogram task partition");
            MY_LOGD("<testCase> 0: Normal Single Core, 1:N/A, 2: N/A, 3:N/A");
        }
    }
...
#define BITTRUE_PATTERN "data/Output_1920x1080__5.raw"
        ...
        // flush cache
        //ion_cache_sync_flush_range( i4ImgInFD);
        //ion_cache_sync_flush_range( i4ImgOutFD);
