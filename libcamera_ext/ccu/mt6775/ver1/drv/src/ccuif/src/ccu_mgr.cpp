...
int CcuMgr::ccuBoot()
{
    ...
    if(_ccuBootCnt == 0)
    {
        ...
        rlt = m_pDrvCcu->init("CcuMgr", m_logBuffers[0].mva, m_logBuffers[1].mva, (MINT8 *)m_logBuffers[0].va, (MINT8 *)m_logBuffers[1].va);
        if(rlt == false)
        {
        ...
        for(int i=CCU_CAM_TG_MIN_IDX ; i<CCU_CAM_TG_MAX_IDX ; i++)
            for(int j = 0; j<2 ;j++)
                for(int k = 0; k<3 ;k++)
                {
                    import_buf.memID[p++] = m_AfBufferList[i].AF_Buf[j][k].share_fd;
                }
        import_buf.memID[p++] = m_logBuffers[0].share_fd;
        import_buf.memID[p++] = m_logBuffers[1].share_fd;
...
void CcuMgr::_checkCcuEnable()
{
    ...
    property_get("debug.ccu_mgr_ccu.enable", value, "0");
