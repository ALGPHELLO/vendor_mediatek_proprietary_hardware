...
void *CcuDrvImp::ccuAddrToVa(MUINT32 ccuAddr)
{
    ...
    else if(ccuAddr >= CCU_CACHE_BASE) //the address in located in DDR
    {
        ...
        vaOut = (void *)(m_ddrBuf.va + ccuAddrOffset);
    }
    else
    {
    ...
    LOG_DBG("dmembase(%p), ddrBase(%p), ccuAddr(%x), offset(%x), vaOut(%p)", ccuDmemBasePtr, m_ddrBuf.va, ccuAddr, ccuAddrOffset, vaOut);
...
MBOOL CcuDrvImp::setCcuRun()
{
    ...
    //Set remap offset
    MUINT32 remapOffset = m_ddrBuf.mva - CCU_CACHE_BASE;
...
MBOOL CcuDrvImp::allocateDdrBuffer()
{
    LOG_DBG("+:%s\n",__FUNCTION__);

    m_ddrBuf.size = CCU_CACHE_SIZE;
    this->pCcuMem->ccu_malloc(this->pCcuMem->gIonDevFD, m_ddrBuf.size, &m_ddrBuf.fd, &m_ddrBuf.va);
    if ( NULL == m_ddrBuf.va )
    {
        LOG_ERR("ccu_malloc fail: %p", m_ddrBuf.va);
        return false;
    }

    this->pCcuMem->mmapMVA(m_ddrBuf.fd, &m_ddrBuf.ionHandle, &m_ddrBuf.mva, CCU_DDR_BUF_MVA_UPPER_BOUND, CCU_DDR_BUF_MVA_LOWER_BOUND);
    if ( 0 == m_ddrBuf.mva ) {
        LOG_ERR("mmapMVA(m_ddrBuf.mva) Error: %x", m_ddrBuf.mva);
        return false;
    }
    if (( m_ddrBuf.mva < CCU_DDR_BUF_MVA_LOWER_BOUND) || ( m_ddrBuf.mva > CCU_DDR_BUF_MVA_UPPER_BOUND))
    {
        AEE_ASSERT_CCU_USER("ION allocated CCU DDR buffer MVA exceeds requested region, expected(0x%x~0x%x), actual(0x%x)",
            CCU_DDR_BUF_MVA_LOWER_BOUND, CCU_DDR_BUF_MVA_UPPER_BOUND, m_ddrBuf.mva);
    }


    LOG_DBG("CCU DDR buffer:va(%p),mva(%x),sz(%d)\n", m_ddrBuf.va, m_ddrBuf.mva, m_ddrBuf.size);
    LOG_DBG("-:%s\n",__FUNCTION__);
...
MBOOL CcuDrvImp::RealUninit(const char* userName)
{
    ...
    if(this->pCcuMem)
    {
        LOG_DBG_MUST("+:release ddr buffers\n");
        if(false == this->pCcuMem->munmapMVA(m_ddrBuf.ionHandle))
        {
            LOG_ERR("munmap ddr MVA fail:ion_handle_in_data\n");
        }
        if(false == this->pCcuMem->ccu_free(this->pCcuMem->gIonDevFD, m_ddrBuf.size, m_ddrBuf.fd, m_ddrBuf.va)) {
            LOG_ERR("ccu_free_ddr_buffer fail\n");
        }
        LOG_DBG_MUST("-:release ddr buffer\n");
...
MBOOL CcuDrvImp::loadCCUBin()
{
    ...
    *(ccuCtrlBasePtr) = 0x0;
    ...
    ret = loadBin(ddrPath, (unsigned int*)m_ddrBuf.va, ddrBuf, CCU_CACHE_SIZE, 0, BIN_CCU);
    if(ret == MFALSE)
        goto LOAD_CCU_BIN_EXIT;
...
MBOOL CcuDrvImp::loadSensorBin(WHICH_SENSOR_T sensorDev, uint8_t sensor_slot)
{
    LOG_DBG("+:%s\n",__FUNCTION__);
    ...
    if ((sensor_slot == 1))
        ret = loadBin(sensorBinPath, (unsigned int *)m_ddrBuf.va, sensorBinBuf, 0, SENSOR_BIN_OFFSET_SLOT_1, BIN_SENSOR_BIN_1);
    else if ((sensor_slot == 2))
        ret = loadBin(sensorBinPath, (unsigned int *)m_ddrBuf.va, sensorBinBuf, 0, SENSOR_BIN_OFFSET_SLOT_2, BIN_SENSOR_BIN_2);
    if(ret == MFALSE)
...
MBOOL CcuDrvImp::GetSensorName(MINT32 kdrvFd, WHICH_SENSOR_T sensorDev, char *sensorName)
{
    ...
    LOG_DBG_MUST("ccu main2 sensor name: %s\n", sensorNames[2]);
    ...
    switch(sensorDev)
    {
        ...
        case WHICH_SENSOR_MAIN_2:
        {
            memcpy(sensorName, sensorNames[2], strlen(sensorNames[2])+1);
            break;
        }
...
MBOOL CcuDrvImp::initI2cController(enum CCU_I2C_CHANNEL i2c_controller_id)
{
    if(ioctl(this->m_Fd, CCU_IOCTL_SET_I2C_MODE, i2c_controller_id) < 0)
    {
