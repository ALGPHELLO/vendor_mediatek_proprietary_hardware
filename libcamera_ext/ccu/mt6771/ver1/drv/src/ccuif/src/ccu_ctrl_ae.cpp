...
int CcuCtrlAe::init(MUINT32 sensorIdx, ESensorDev_T sensorDev)
{
	...
	if (!m_pDrvCcu->loadSensorBin(ccuSensorDev, sensor_slot))
    {
...
static enum CCU_I2C_CHANNEL whichSesnorToI2cChannel(WHICH_SENSOR_T whichSensor)
{
    if(whichSensor == WHICH_SENSOR_MAIN)
    {
        return CCU_I2C_CHANNEL_MAINCAM;
    }
    else if(whichSensor == WHICH_SENSOR_SUB)
    {
        return  CCU_I2C_CHANNEL_SUBCAM;
    }
    else if(whichSensor == WHICH_SENSOR_MAIN_2)
    {
        return  CCU_I2C_CHANNEL_MAINCAM2;
    }
    else
    {
        LOG_ERR("Sensor Type is not main nor sub : %x\n", whichSensor);
        return CCU_I2C_CHANNEL_UNDEF;
    }
}
...
bool CcuCtrlAe::ccuCtrlPreprocess_SensorInit(void *inDataPtr, void *inDataBuf)
{
    ...
    i2cControllerId = whichSesnorToI2cChannel(infoIn->eWhichSensor);
    ...
    i2cBufIoArg.i2c_controller_id = i2cControllerId;
    ...
    if((infoIn->eWhichSensor == WHICH_SENSOR_MAIN) && (sensorI2cSlaveAddr[0] == -1))
        slaveAddrValid = false;
    else if((infoIn->eWhichSensor == WHICH_SENSOR_SUB) && (sensorI2cSlaveAddr[1] == -1))
        slaveAddrValid = false;
    else if((infoIn->eWhichSensor == WHICH_SENSOR_MAIN_2) && (sensorI2cSlaveAddr[2] == -1))
        slaveAddrValid = false;

    if(slaveAddrValid == false)
    {
        LOG_ERR("[CCU If]Main  Sensor slave addr : %d\n", sensorI2cSlaveAddr[0]);
        LOG_ERR("[CCU If]Sub   Sensor slave addr : %d\n", sensorI2cSlaveAddr[1]);
        LOG_ERR("[CCU If]Main2 Sensor slave addr : %d\n", sensorI2cSlaveAddr[2]);
    ...
    {
        compatInfoIn->u16FPS = currentFpsList[IMGSENSOR_SENSOR_IDX_MAIN];
        compatInfoIn->sensorI2cSlaveAddr = sensorI2cSlaveAddr[0];
    }
    else if(infoIn->eWhichSensor == WHICH_SENSOR_MAIN_2)
    {
        compatInfoIn->u16FPS = currentFpsList[IMGSENSOR_SENSOR_IDX_MAIN2];
        compatInfoIn->sensorI2cSlaveAddr = sensorI2cSlaveAddr[2];
    }
    else if(infoIn->eWhichSensor == WHICH_SENSOR_SUB)
    {
        compatInfoIn->u16FPS = currentFpsList[IMGSENSOR_SENSOR_IDX_SUB];
    ...
    LOG_DBG_MUST("ccu sensor in, compatInfoIn->eWhichSensor: %x\n", compatInfoIn->eWhichSensor);
...
bool CcuCtrlAe::ccuCtrlPostprocess_SensorInit(void *outDataPtr, void *inDataBuf)
{
    ...
    i2cControllerId = whichSesnorToI2cChannel(compatInfoIn->eWhichSensor);
    if(i2cControllerId == CCU_I2C_CHANNEL_UNDEF)
    {
        m_pDrvCcu->PrintReg();
