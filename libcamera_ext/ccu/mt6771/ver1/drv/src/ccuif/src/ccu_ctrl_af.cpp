...
void CcuCtrlAf::ccu_af_deque_afo_buffer(AFAcquireBuf_Reg_INFO_OUT_AP_T* rAFAcquireBufout)
{
    ...
    //ion_cache_sync_flush_all(m_pBufferList->m_IonDevFD);//flush all
    ...
    if(i==3 || i==4)
    {
        ...
        for(k = 0; k < Half_AFO_BUFFER_COUNT; k++)
        {
            ...
            {
                rAFAcquireBufout->AFOBufAddr = AFO_BUF[j][k].va;
                LOG_DBG("CCU_AFOBufs CCU to AP = 0x%x\n", rAFAcquireBufout->AFOBufAddr);
            }
        }
        ion_cache_sync_flush_range_va(m_pBufferList->m_IonDevFD,rAFAcquireBufout->AFOBufAddr, AFO_BUF_SIZE);
    }
    else if (i == 5)
    {
