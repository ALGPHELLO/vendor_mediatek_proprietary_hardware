...
MBOOL AbsCcuCtrlBase::resolveCmdIOBuf(const struct CcuCtrlParamDescriptor *paramDescptr, CcuIOBufInfo *inDataBufInfo, CcuIOBufInfo *outDataBufInfo)
{
    ...
    m_pDrvCcu->getIpcIOBuf((void **)&inDataBufInfo->addr_ap, (void **)&outDataBufInfo->addr_ap, &inDataBufInfo->addr_ccu, &outDataBufInfo->addr_ccu);
...
int AbsCcuCtrlBase::ccuControl(ccu_msg_id msgId, void *inDataPtr, void *outDataPtr)
{
    ...
    if(inDataBufInfo.bufType == CCU_SRAM_IO)
    {
        ...
        MUINT32 *interIoBuf = (MUINT32 *)inDataBufInfo.addr_ap_interm;
        MUINT32 *realIoBuf = (MUINT32 *)inDataBufInfo.addr_ap;
        ...
        if(msgId == MSG_TO_CCU_SENSOR_INIT)
        {
            ...
            LOG_DBG_MUST("interIoBuf compatInfoIn->eWhichSensor: %x\n", InterCompatInfoIn->eWhichSensor);
            ...
            LOG_DBG_MUST("realIoBuf compatInfoIn->eWhichSensor: %x\n", RealCompatInfoIn->eWhichSensor);
    ...
    if(paramDescptr->need_output)
    {
        ...
        memcpy(outDataPtr, outDataBufInfo.addr_ap, paramDescptr->output_non_ptr_total_size);
    }

    if(msgId == MSG_TO_CCU_SENSOR_INIT)
    {
        COMPAT_SENSOR_INFO_IN_T *RealCompatInfoIn = (COMPAT_SENSOR_INFO_IN_T *)inDataBufInfo.addr_ap;
        LOG_DBG_MUST("realIoBufAft compatInfoIn->eWhichSensor: %x\n", RealCompatInfoIn->eWhichSensor);
