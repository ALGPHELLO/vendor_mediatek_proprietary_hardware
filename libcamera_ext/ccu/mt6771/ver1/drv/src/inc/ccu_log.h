 ...
 ************************************************************************************************/
//#ifndef _CCUIF_LOG_H_   // Can't use Header Guard because we use undef-redefine to apply new DBG_LOG_TAG and DBG_LOG_LEVEL for each file.
//#define _CCUIF_LOG_H_
...
#define BASE_LOG_VRB(fmt, arg...)       do { ALOGD(DBG_LOG_TAG "[%s] " fmt NEW_LINE_CHAR, __func__, ##arg); } while(0)  // <Verbose>: Show more detail debug information. E.g. Entry/exit of private function; contain of local variable in function or code block; return value of system function/API...
#define BASE_LOG_DBG(fmt, arg...)       do { ALOGD(DBG_LOG_TAG "[%s] " fmt NEW_LINE_CHAR, __func__, ##arg); } while(0)  // <Debug>: Show general debug information. E.g. Change of state machine; entry point or parameters of Public function or OS callback; Start/end of process thread...
#define BASE_LOG_INF(fmt, arg...)       do { ALOGI(DBG_LOG_TAG "[%s] " fmt NEW_LINE_CHAR, __func__, ##arg); } while(0)  // <Info>: Show general system information. Like OS version, start/end of Service...
#define BASE_LOG_WRN(fmt, arg...)       ALOGW(DBG_LOG_TAG "[%s] WARNING: " fmt NEW_LINE_CHAR, __func__, ##arg)  // <Warning>: Some errors are encountered, but after exception handling, user won't notice there were errors happened.
#define BASE_LOG_ERR(fmt, arg...)       ALOGE(DBG_LOG_TAG "[%s, %s, line%04d] ERROR: " fmt NEW_LINE_CHAR, __FILE__, __func__, __LINE__, ##arg)  // When MP, will only show log of this level. // <Fatal>: Serious error that cause program can not execute. <Error>: Some error that causes some part of the functionality can not operate normally.
...
#define CCUIF_TRACE_CALL() ATRACE_CALL()
#define CCUIF_TRACE_BEGIN(name) ATRACE_BEGIN(name)
#define CCUIF_TRACE_END() ATRACE_END()
#else
#define CCUIF_TRACE_CALL()
...
#define DECLARE_DBG_LOG_VARIABLE(ModuleName)    \
    bool ModuleName ## _DbgLogEnable_VERBOSE   = true; \
    bool ModuleName ## _DbgLogEnable_DEBUG     = true; \
    ...
    bool CCU_DEFUALT_LOG_LV3 = false; \
...
#define EXTERN_DBG_LOG_VARIABLE(ModuleName)    \
    ...
    extern bool CCU_DEFUALT_LOG_LV3; \
...
#define DBG_LOG_CONFIG(GroupName, ModuleName)                                       \
    do {                                                                            \
        char acDbgLogLevel[PROPERTY_VALUE_MAX] = {'\0'};                            \
                                                                                    \
        if(CCU_DEFUALT_LOG_LV3)                                                       \
            property_get("debug." #GroupName "." #ModuleName, acDbgLogLevel, "3");  \
        else                                                                        \
            property_get("debug." #GroupName "." #ModuleName, acDbgLogLevel, "4");  \
                                                                                    \
...
#define DBG_LOG_SET_DEFAULT_LV3() do {CCU_DEFUALT_LOG_LV3 = true;} while(0)
...
#undef LOG_ERR_MUST
#undef LOG_AST_MUST
#define LOG_VRB(fmt, arg...)        do { if (ccu_drv_DbgLogEnable_VERBOSE) { BASE_LOG_VRB(fmt, ##arg); } } while(0)
#define LOG_DBG(fmt, arg...)        do { if (ccu_drv_DbgLogEnable_DEBUG  ) { BASE_LOG_DBG(fmt, ##arg); } } while(0)
#define LOG_INF(fmt, arg...)        do { if (ccu_drv_DbgLogEnable_INFO   ) { BASE_LOG_INF(fmt, ##arg); } } while(0)
#define LOG_WRN(fmt, arg...)        do { if (ccu_drv_DbgLogEnable_WARN   ) { BASE_LOG_WRN(fmt, ##arg); } } while(0)
#define LOG_ERR(fmt, arg...)        do { if (ccu_drv_DbgLogEnable_ERROR  ) { BASE_LOG_ERR(fmt, ##arg); } } while(0)
#define LOG_AST(cond, fmt, arg...)  do { if (ccu_drv_DbgLogEnable_ASSERT ) { BASE_LOG_AST(cond, fmt, ##arg); } } while(0)
#define LOG_VRB_MUST(fmt, arg...)        do { BASE_LOG_VRB(fmt, ##arg); } while(0)
#define LOG_DBG_MUST(fmt, arg...)        do { BASE_LOG_DBG(fmt, ##arg); } while(0)
#define LOG_INF_MUST(fmt, arg...)        do { BASE_LOG_INF(fmt, ##arg); } while(0)
#define LOG_WRN_MUST(fmt, arg...)        do { BASE_LOG_WRN(fmt, ##arg); } while(0)
#define LOG_ERR_MUST(fmt, arg...)        do { BASE_LOG_ERR(fmt, ##arg); } while(0)
#define LOG_AST_MUST(cond, fmt, arg...)  do { BASE_LOG_AST(cond, fmt, ##arg); } while(0)
