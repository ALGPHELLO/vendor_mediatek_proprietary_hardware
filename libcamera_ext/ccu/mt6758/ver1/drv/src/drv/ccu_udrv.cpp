...
MBOOL CcuDrvImp::init(const char* userName)
{
    ...
    if(this->m_UserCnt > 0)
    {
        if(this->m_UserCnt < MAX_USER_NUMBER){
            strncpy((char*)this->m_UserName[this->m_UserCnt], userName, MAX_USER_NAME_SIZE);
...
MBOOL CcuDrvImp::loadCCUBin()
{
    ...
    MBOOL ret = MTRUE;
    unsigned int pmemBuf[CCU_PMEM_SIZE];
    unsigned int dmemBuf[CCU_DMEM_SIZE];
    char *pmPath = "/system/vendor/bin/lib3a.ccu.pm";
    char *dmPath = "/system/vendor/bin/lib3a.ccu.dm";

    //========================= Load PMEM binary ===========================
    LOG_INF("Load PM");
	ret = loadBin(pmPath, gCcuDrvObj[CCU_PMEM].m_pCcuHwRegAddr, pmemBuf, CCU_PMEM_SIZE, 0);
    if(ret == MFALSE)
        goto LOAD_CCU_BIN_EXIT;

    //========================= Load DMEM binary ===========================
    LOG_INF("Load DM");
    ret = loadBin(dmPath, gCcuDrvObj[CCU_DMEM].m_pCcuHwRegAddr, dmemBuf, CCU_DMEM_SIZE, CCU_DMEM_OFFSET);
    if(ret == MFALSE)
        goto LOAD_CCU_BIN_EXIT;
...
MBOOL CcuDrvImp::loadSensorBin(WHICH_SENSOR_T sensorDev)
{
    LOG_DBG("+:%s\n",__FUNCTION__);
    ...
    ret = loadBin(sensorPmPath, gCcuDrvObj[CCU_PMEM].m_pCcuHwRegAddr, sensorPmBuf, 0, SENSOR_PM_OFFSET);
    if(ret == MFALSE)
        goto LOAD_SENSOR_BIN_EXIT;

    //========================= Load Sensor DM binary ===========================
    LOG_INF("Load Sensor DM");
    ret = loadBin(sensorDmPath, gCcuDrvObj[CCU_DMEM].m_pCcuHwRegAddr, sensorDmBuf, 0, SENSOR_DM_OFFSET);
    if(ret == MFALSE)
        goto LOAD_SENSOR_BIN_EXIT;
...
MBOOL CcuDrvImp::GetSensorName(MINT32 kdrvFd, WHICH_SENSOR_T sensorDev, char *sensorName)
{
    ...
    LOG_DBG_MUST("ccu main sensor name is: %s\n", sensorNames[0]);
    LOG_DBG_MUST("ccu sub sensor name is: %s\n", sensorNames[1]);
...
void CcuDrvImp::DumpSramLog()
{
    ...
    *(ccuLogPtr_1 + CCU_LOG_SIZE - 1) = '\0';
    *(ccuLogPtr_2 + CCU_LOG_SIZE - 1) = '\0';
    *(isrLogPtr + CCU_ISR_LOG_SIZE - 1) = '\0';
    LOG_WRN("---------CHUNK 1-----------\n%s", ccuLogPtr_1);
    LOG_WRN("---------CHUNK 2-----------\n%s", ccuLogPtr_2);
    LOG_WRN("---------ISR LOG-----------\n%s", isrLogPtr);
