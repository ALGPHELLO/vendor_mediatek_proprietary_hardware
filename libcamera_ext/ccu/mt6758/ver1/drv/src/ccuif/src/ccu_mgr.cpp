...
#define AFO_BUF_SIZE ((AFO_WIN_NUM + AFO_STRIDE_NUM) * AFO_WIN_NUM * AFO_WIN_DATA)
static volatile int tempcount;
...
int CcuMgr::ccuUninit()
{
    ...
    if(_ccuInitCnt == 1)
    {
        ...
        for(int i=0 ; i<6 ; i++)
        {
            buffer = &(m_BufferList.AF_Buf[i]);

            LOG_DBG("uninit_AFO_buffers[%d]\n", i);

            ret = this->_uninitBuffer(buffer);
            if(ret == MFALSE)
            {
                LOG_ERR("uninit_AFO_buffers[%d] fail\n", i);
                break;
            }
        }
        ...
        for(int i=0 ; i<6 ; i++)
        {
            buffer = &(m_BufferList.AF_Reg[i]);

            LOG_DBG("init_AF_regs[%d]\n", i);
...
MBOOL CcuMgr::_initAFBuffers()
{
    ...
    for(int i=0 ; i<6 ; i++)
    {
        ...
        //map buffer from m_BufferList
        buffer = &(m_BufferList.AF_Buf[i]);
        buffer->size = AFO_BUF_SIZE;//0x7a0*0x2f;
        ret = this->_initBuffer(buffer);
        if(ret == MFALSE)
        {
