...
void NetlinkEventHandler::onEvent(NetlinkEvent *evt) {
    ...
    if (strcmp(subsys, "net") == 0) {
        ...
        if (action == NetlinkEvent::Action::kAddressUpdated ||
            ...
            char *flags = const_cast<char *>(evt->findParam("FLAGS"));
            int flag = stringToBinaryBase(flags, 10, &err);
            if (err < 0) {
                NA_LOG_E("[%s] error occurs when converting flag to binary"
                        ", err = %d", __FUNCTION__, err);
                return;
            }

            char *valids = const_cast<char *>(evt->findParam("VALID"));
            int valid = stringToBinaryBase(valids, 10, &err);
            if (err < 0) {
                NA_LOG_E("[%s] error occurs when converting valid to binary"
                        ", err = %d", __FUNCTION__, err);
                return;
            }
            ...
            enqueueReqInfo(NETAGENT_IO_CMD_IPUPDATE, address, ipv6PrefixLength, interfaceId, flag, valid, act);
        }
#ifdef MTK_IPV6_NORA_FEATURE
        else if (action == NetlinkEvent::Action::kNoRA) {
            char *flags = const_cast<char *>(evt->findParam("FLAGS"));
            int flag = stringToBinaryBase(flags, 10, &err);
            if (err < 0) {
                NA_LOG_E("[%s][NoRA] error occurs when converting flag to binary"
                        ", err = %d", __FUNCTION__, err);
                return;
            }

            // flag = 1 RA init fail, valid = -1,
            // flag = 0 RA refresh fail, valid = -2.
            int valid = (flag == 0) ? -2 : -1;

            enqueueReqInfo(NETAGENT_IO_CMD_RA, (char *)NULL_IPV6_ADDRESS,
                    INVALID_IPV6_PREFIX_LENGTH, interfaceId, flag, valid, ACTION_NO_RA);
        }
#endif
    }
}

void NetlinkEventHandler::enqueueReqInfo(NA_CMD cmd, char *address, int ipv6PrefixLength, int interfaceId, int flag, int valid, ACTION action) {
    if (m_pNetAgent == NULL) {
    ...
    pNetlinkEventObj->flag = flag;
    pNetlinkEventObj->valid = valid;
...
int NetlinkEventHandler::getFlag(void *obj, NA_RA *flag) {
    NetlinkEventReqInfo *pReqInfo = reinterpret_cast<NetlinkEventReqInfo *>(obj);
    ...
    // flag = 1 RA init fail,
    // flag = 0 RA refresh fail.
    *flag = (pReqInfo->flag == 0) ? NETAGENT_IO_NO_RA_REFRESH : NETAGENT_IO_NO_RA_INITIAL;
    return NETLINK_RET_SUCCESS;
}

int NetlinkEventHandler::getValid(void *obj, int *valid) {
    NetlinkEventReqInfo *pReqInfo = reinterpret_cast<NetlinkEventReqInfo *>(obj);
...
int NetlinkEventHandler::setLastReqInfo(void *obj) {
    ...
    pNetlinkEventObj->flag = pReqInfo->flag;
    pNetlinkEventObj->valid = pReqInfo->valid;
    ...
    for (int i = 0; i < size; i++) {
        ...
        if (pTemp->interfaceId == pReqInfo->interfaceId) {
            if ((pTemp->cmd != pReqInfo->cmd) ||
                    ...
                    (pTemp->flag != pReqInfo->flag) ||
                    (pTemp->valid != pReqInfo->valid) ||
...
const char *NetlinkEventHandler::actionToString(ACTION action) {
    switch (action) {
        ...
        case ACTION_NO_RA: return "NoRA";
        default: return "Unknown";
    }
}

String8 NetlinkEventHandler::reqInfoToString(NetlinkEventReqInfo *pReqInfo) {
    return String8::format("ReqInfo <- {interfaceId=%d, cmd=%s, action=%s, flag=%d, valid=%d"
