...
static char * tx_msc_response_cache = NULL;
pthread_t tx_msc_response_thread;
...
pthread_mutex_t write_tx_msc_response_lock;
pthread_mutex_t write_tx_msc_response_ready_lock;
pthread_cond_t write_tx_msc_response_signal = PTHREAD_COND_INITIALIZER;
...
/* ptty channel config.
 *
 * channel id 32~34: used by MD for customer features.
 * channel id 61~40: used by MUX channel expansion.
 * channel id 21,22,23 now no used, 25 is used by MD data card
 * channel id 63~62: reserved by spec.
 */
...
#if defined (MTK_MUX_CHANNEL_64)
    { 61, 512, "/dev/radio/pttycmd7" },
    { 60, 512, "/dev/radio/pttycmd8" },
    { 59, 512, "/dev/radio/pttycmd9" },
    { 58, 512, "/dev/radio/pttycmd10" },
    { 43, 512, "/dev/radio/pttycmd11" },
#endif
#if (SIM_COUNT >= 2)
...
#if defined (MTK_MUX_CHANNEL_64)
    { 57, 512, "/dev/radio/ptty2cmd7" },
    { 56, 512, "/dev/radio/ptty2cmd8" },
    { 55, 512, "/dev/radio/ptty2cmd9" },
    { 54, 512, "/dev/radio/ptty2cmd10" },
    { 42, 512, "/dev/radio/ptty2cmd11" },
#endif
#endif
...
#if defined (MTK_MUX_CHANNEL_64)
    { 53, 512, "/dev/radio/ptty3cmd7" },
    { 52, 512, "/dev/radio/ptty3cmd8" },
    { 51, 512, "/dev/radio/ptty3cmd9" },
    { 50, 512, "/dev/radio/ptty3cmd10" },
    { 41, 512, "/dev/radio/ptty3cmd11" },
#endif
#endif
...
#if defined (MTK_MUX_CHANNEL_64)
    { 49, 512, "/dev/radio/ptty4cmd7" },
    { 48, 512, "/dev/radio/ptty4cmd8" },
    { 47, 512, "/dev/radio/ptty4cmd9" },
    { 46, 512, "/dev/radio/ptty4cmd10" },
    { 40, 512, "/dev/radio/ptty4cmd11" },
#endif
#endif
...
#ifdef FUSION_RILD
    {45, 512, "/dev/radio/pttynwcmd"},
    {44, 512, "/dev/radio/pttynwurc"},
#endif
        ...
        if (c != prefix_length) {
            ...
            if (c < 0) {
                ...
                if (writeErrno==ETXTBSY || writeErrno==ENODEV) {
                    LOGMUX(LOG_ERR, "write to MD get ETXTBSY/ENODEV (%d), drop data (%d)", errno, length);
...
static int setup_pty_interface(Channel *channel)
{
    ...
    if (poll_thread_arg == NULL) {
        LOGMUX(LOG_ERR, "OOM");
        return GSM0710MUXD_OUT_OF_MEMORY;
    }
...
{
    LOGMUX(LOG_DEBUG, "Enter, LackofAtciChannel=%d",LackofAtciChannel);
        ...
        if ((len = read(channel->fd, buf, sizeof(buf))) >= 0) {
            ...
            case EAGAIN:
                ...
                if (read_action_count == 0) {
                    ...
                    //skip the below assert because RILD will reopen channels due to SIM switch feature.
...
close_pty_channel:
    LOGMUX(LOG_ERR, "Appl. dropped connection, device %s shutting down. Set to be reopened", channel->ptsname);
...
static int poll_modem_ready(int    serial_device_fd, int to)
{
    ...
    LOGMUX(LOG_INFO, "gsm0710muxd: wait for modem boot up");
    ...
    do {
        ...
        LOGMUX(LOG_INFO, "Selected %d", sel);
        if (FD_ISSET(serial_device_fd, &rfds)) {
            memset(buf, 0, sizeof(buf));
            len = read(serial_device_fd, buf, sizeof(buf));
            SYSCHECK(len);
            LOGMUX(LOG_INFO, "Read %d bytes from serial device", len);
 ...
 * Return:   0 on success (OK-response), -1 otherwise
 */
static int chat(int serial_device_fd, char * cmd, int to __unused)
{
    LOGMUX(LOG_DEBUG, "Enter");
    ...
    while (1) {
        ...
        if (sel < 0) {
            ...
            if (FD_ISSET(serial_device_fd, &rfds)) {
                ...
                syslogdump("<s ", buf, len);
                gsm0710_buffer_write(buf_for_bootup, buf, len);
                LOGMUX(LOG_DEBUG, "serial device buff: %s", buf_for_bootup->data);
                //errno = 0;
 ...
 * Return:   0 on success (OK-response), -1 otherwise
 */
static int chatCmux(int serial_device_fd, char * cmd, int to __unused)
{
...
void write_tx_msc_response(void * input __unused) {
    while (1) {
        pthread_mutex_lock(&write_tx_msc_response_ready_lock);
        if (tx_msc_response_cache == NULL) {
            pthread_cond_wait(&write_tx_msc_response_signal, &write_tx_msc_response_ready_lock);
        }
        pthread_mutex_unlock(&write_tx_msc_response_ready_lock);

        char * tmp = NULL;
        pthread_mutex_lock(&write_tx_msc_response_lock);
        int length = strlen(tx_msc_response_cache);
        LOGMUX(LOG_DEBUG, "enter, data length:%d", length);
        tmp = (char *)calloc(1, sizeof(char) * length);
        memcpy(tmp, tx_msc_response_cache, length);
        free(tx_msc_response_cache);
        tx_msc_response_cache = NULL;
        pthread_mutex_unlock(&write_tx_msc_response_lock);
        ...
        rx_thread_write_frame(0, (const unsigned char *)tmp, length, GSM0710_TYPE_UIH);

        free(tmp);
        LOGMUX(LOG_DEBUG, "end");
    }
    ...
    if (frame->length > 0) {
        ...
        if ((type & GSM0710_CR) == GSM0710_CR) {
            ...
            case GSM0710_CONTROL_CLD:
                LOGMUX(LOG_DEBUG, "The mobile station requested mux-mode termination");
                /* Modify by LS */
            ...
            case GSM0710_CONTROL_MSC:
                ...
                if (i + 1 < frame->length) {
                    ...
                    if ((signals & GSM0710_SIGNAL_FC) == GSM0710_SIGNAL_FC) {
                        LOGMUX(LOG_DEBUG, "No frames allowed, channel id=%d,tx_fc_off=%d"
                        ...
                        //op.arg |= USSP_CTS;
                        LOGMUX(LOG_DEBUG, "Frames allowed, channel id=%d,tx_fc_off=%d",channel->id,channel->tx_fc_off);
            ...
            if (supported) {
                ...
                LOGMUX(LOG_DEBUG, "start to send msc response");
                pthread_mutex_lock(&write_tx_msc_response_lock);
                if (tx_msc_response_cache != NULL) {
                    free(tx_msc_response_cache);
                }
                tx_msc_response_cache = (char *)calloc(1, sizeof(char) *  frame->length);
                memcpy(tx_msc_response_cache, frame->data, frame->length);
                pthread_mutex_unlock(&write_tx_msc_response_lock);
                pthread_cond_signal(&write_tx_msc_response_signal);
            }
        } else {
...
terminate:
    LOGMUX(LOG_DEBUG, "Device polling thread terminated");
...
int start_muxer(Serial *serial)
{
    ...
    if (poll_modem_ready(serial->fd, 10) < 0)
        LOGMUX(LOG_ERR, "Wait +EIND: 128 timeout! Continue...");

    if (chat(serial->fd, "AT", 1) < 0) {
        LOGMUX(LOG_ERR, "Modem does not respond to AT commands, trying close mux mode");
        #if 0
...
int watchdog(Serial *serial)
{
    ...
    case MUX_STATE_INITILIZING:
        ...
        /**
          * Create a thread for sending tx msc response to modem.
          * Reason: There may be a dead lock:
          * 1. When Modem Cmux buffer (the data cached by it will be send to AP MUXD) is full,
          *     cmux will go to sleep until cmux buffer is no full.
          * 2. If AP CCCI buffer is also full at the same time, AP MUXD cannot write frame to it.
          *     And Ap Muxd will keep the write_frame_Lock.
          * 3. AP MUXD read frame from in_buffer, and the frame is a COMMAND(NO_FRAME_ALLOWED).
          *     When AP Muxd finish setting flag for the MSC, try to write tx_msc_reaponse to modem
          *     faile as the write_frame_lock is kept by step 2.
          * 4. Step 3 is executing in MD rx thread, it is waiting at write_frame_lock unlock, but at
          *     the same time, MD tx thread is also waiting cmux buffer free with write_frame_lock.
          * Finally, it causes a dead lock beetween write frame and read frame. We need to create a
          * new thread to send tx_msc_response to avoid the dead lock.
          */
        if (create_thread(&tx_msc_response_thread, write_tx_msc_response, NULL) != 0) {
            LOGMUX(LOG_ERR, "Could not create thread for write msc response");
        }
...
void *poll_thread(void *vargp)
{
    ...
    while (1) {
        ...
        if (select((poll_thread_arg->fd) + 1, &fdr, &fdw, NULL, NULL) > 0) {
            LOGMUX(LOG_DEBUG, "poll_thread select found");
...
int main(int argc, char *argv[])
{
    ...
    property_set("ril.muxreport.run", "0");
