...
public:
    Return<void> onEvent(const Event &e) {
        ...
        //ALOGD("timestamp[%d] data[0][%f] data[1][%f] data[2][%f] data[3][%f]", (int)gRGBWTimeStamp, gRGBWValue[0], gRGBWValue[1], gRGBWValue[2], gRGBWValue[3]);
...
int rgbwLightSensorEnable(int enable)
{
    ...
    if (enable == 1) {
        ...
        ALOGD("enable RGBW sensor ret[%d]", ret);
    } else {
        ret = gSensorEventQueue->disableSensor(gRGBWSensorHandle);
        ALOGD("disable RGBW sensor ret[%d]", ret);
    }
...
int rgbwLightSensorInit(void)
{
    ...
    {
        ALOGD("get DefaultSensor FAIL! %d", gRGBWSensorHandle);
        return -1;
    }
...
void rgbwLightSensorDeInit(void)
{
    ...
    if (gSensorEventQueue != NULL)
    {
        ...
        if (!ret.isOk())
        {
            ALOGD("disable rgbw sensor fail");
        }
        gSensorEventQueue = NULL;
    }
