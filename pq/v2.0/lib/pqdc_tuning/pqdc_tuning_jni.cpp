...
static int getIndex(PQ_DC_index_t a_eIndex)
{
    ...
    service->getTuningField(MOD_DC_SWREG, getPQDCOffset(a_eIndex),
...
static jboolean setIndex(PQ_DC_index_t a_eIndex , unsigned int a_u4Index)
{
    ...
    service->setTuningField(MOD_DC_SWREG, 0xffff, PROXY_TUNING_OVERWRITTEN);
    service->setTuningField(MOD_DC_SWREG, getPQDCOffset(a_eIndex), a_u4Index);
    service->setTuningField(MOD_DC_SWREG, 0xfffe, 0);
#else
