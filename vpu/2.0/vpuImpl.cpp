...
)
{
    ...
    return false;
#else
		MY_LOGW("2.0 do not support this function in 2.0 version");
		return false;
#endif
}
/******************************************************************************
...
{
	MY_LOGI("+ 0x%x/0x%x/0x%x", extra_param.opp_step,\
		extra_param.freq_step, extra_param.bw);
	printf("setExtraParam + 0x%x/0x%x/0x%x", extra_param.opp_step,\
		extra_param.freq_step, extra_param.bw);
...
getAlgo(std::string name, int fd)
{
	...
	printf("getAlgo(%s) in ...", name.c_str());
    if ((iter = mvVpuAlgoMap.find(name)) == mvVpuAlgoMap.end())    // not in Pool
    {
		...
		printf("fd: %d, algo name %s ...", fd, algo_n->name);
    ...
    for (buf_idx = 0 ; buf_idx < mNative.buffer_count ; buf_idx++)
    {
        ...
        for (plane_idx = 0 ; plane_idx < buf_n->plane_count ; plane_idx++)
        {
			...
			else
			{
	            ...
	            ATRACE_END();
				printf("ion_import: buffer[%d], plane[%d], share_fd %d, ion_handle 0x%lx\n", buf_idx, plane_idx,
					req_buf->plane[plane_idx].share_fd, (unsigned long)ion_handle);
		...
		req_n->algo_id[0], (unsigned long)req_n->request_id, req_n->frame_magic, core_index);
{
	...
	printf("[vpu] corrrrrrre index(0x%x/0x%x/0x%x)\n\n\n",req_n->requested_core, core_index, req_n->occupied_core);
...
)
{
	...
	MY_LOGI("[vpu] setPower opp_step_%d/core_%d\n", pwr.opp, core_index);
