...
LOCAL_C_INCLUDES:= \
  $(TOP)/kernel-4.4/drivers/misc/mediatek/vpu/2.0/ \
  $(TOP)/$(MTK_PATH_SOURCE)/hardware/m4u/$(LC_MTK_PLATFORM) \
  $(TOP)/$(MTK_PATH_SOURCE)/hardware/vpu/include/ \
  $(TOP)/$(MTK_PATH_SOURCE)/hardware/vpu/2.0/platform/$(TARGET_BOARD_PLATFORM)/ \
...
HAVE_EFUSE_BOND_VERSION := 71
LOCAL_CFLAGS += -DHAVE_EFUSE_BOND_VERSION=$(HAVE_EFUSE_BOND_VERSION)
else ifneq (,$(filter $(strip $(MTK_PLATFORM)), mt6775))
HAVE_EFUSE_BOND_VERSION := 75
else
LOCAL_CFLAGS := 
endif
