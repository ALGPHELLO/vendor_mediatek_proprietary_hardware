        ...
        if (!handlesLocal[i].in_use) {
            switch (slave_address) {
                ...
                case 0x6E:
                    ...
                    error = Tfa98xx_ReadRegister16(i, TFA98XX_REVISIONNUMBER, &rev);
...
#define dateLen (13)
	...
	char date[dateLen + 1];
	char buffer[dateLen * 4];
	char month[4] = "";
	int day = 0;
	int year = 0;
	int i;

	if (!handle_is_open(handle)) return Tfa98xx_Error_NotOpen;

	//Get the release date
	error = Tfa98xx_DspGetParam(handle, 1, 0xFF, dateLen * 4, (unsigned char *) buffer);
	if (error != Tfa98xx_Error_Ok) return error;

	//create date as ASCII string
	for (i = 0; i < dateLen; i++) date[i] = buffer[i * 3 + 2 + 6];
	date[dateLen - 2] = '\0';

	//split into date parts
	if (sscanf(date, "%3s %d %d", month, &day, &year) == 3) {
		//Anything after 2012/12/19 has 67 words, else 55
		if (year > 2012) *pParamCount = 67;
		else if (year < 2012) *pParamCount = 55;
		else if (day <= 19) *pParamCount = 55;			//The year = 2012, anything <= 19 must be 55
		else if ((month[0] | 0x20) != 'd') *pParamCount = 55;	//Year = 2012, month < december, must be 55
		else *pParamCount = 67;	//year = 2012, month = 12 and day > 19. Must be 67
	} else {
                 ...
                 const unsigned char *bytes)
{
        ...
        while (index < length) {
                ...
                i2c_error = NXP_I2C_Write(handlesLocal[handle].slave_address, size, bytes + index);
...
Tfa98xx_Error_t
Tfa98xx_DspSupportFramework(Tfa98xx_handle_t handle, 
						 int *pbSupportFramework)
{
	int featureBits[2] = { 0, 0 };
	Tfa98xx_Error_t error = Tfa98xx_Error_Ok;
	
	assert(pbSupportFramework != 0);
...
{
        NXP_I2C_Error_t i2c_error;
        int bytes2write = num_bytes + 1;        /* subaddress followed by the data */
                       ...
                       unsigned char subaddress, unsigned short *pValue)
{
        ...
        i2c_error =
            NXP_I2C_WriteRead(handlesLocal[handle].slave_address, bytes2write, write_data, bytes2read, read_buffer);
                 ...
                 unsigned char subaddress, int num_bytes, unsigned char data[])
{
        ...
        write_data[0] = subaddress;
        i2c_error =
            NXP_I2C_WriteRead(handlesLocal[handle].slave_address, bytes2write, write_data, num_bytes, data);
        if (classify_i2c_error(i2c_error) != Tfa98xx_Error_Ok) {
                return classify_i2c_error(i2c_error);
        } else {
                return Tfa98xx_Error_Ok;
                             ...
                             Tfa98xx_AmpInputSel_t input_sel)
{
        ...
        if (error == Tfa98xx_Error_Ok) {
                error =
                    Tfa98xx_WriteRegister16(handle, TFA98XX_I2SREG, value);
        }
        return error;
}
                            ...
                            Tfa98xx_OutputSel_t output_sel)
{
        ...
        if (error == Tfa98xx_Error_Ok) {
                ...
                case Tfa98xx_I2SOutputSel_DcdcFFwdCur:
                        ...
                        value |=
                            (output_sel << TFA98XX_I2S_SEL_REG_DOLS_POS);
                        break;
                default:
                        error = Tfa98xx_Error_Bad_Parameter;
                }
        ...
        if (error == Tfa98xx_Error_Ok) {
                error = Tfa98xx_WriteRegister16(handle, TFA98XX_I2S_SEL_REG, value);
        }
        return error;
}
                             ...
                             Tfa98xx_OutputSel_t output_sel)
{
        ...
        if (error == Tfa98xx_Error_Ok) {
                ...
                case Tfa98xx_I2SOutputSel_DcdcFFwdCur:
                        /* enum definition matches the HW registers */
                        value |=
                            (output_sel << TFA98XX_I2S_SEL_REG_DORS_POS);
                        break;
                default:
                        error = Tfa98xx_Error_Bad_Parameter;
                }
                                  ...
                                  const unsigned char *pSpeakerBytes)
{
        ...
        if (pSpeakerBytes != 0) 
        {
            ...
            error = Tfa98xx_DspSetParamVarWait(handle, MODULE_SPEAKERBOOST,
                                        SB_PARAM_SET_LSMODEL, length,
                                        pSpeakerBytes, TFA98XX_WAITRESULT_NTRIES_LONG);
        } 
        else 
        {
            error = Tfa98xx_Error_Bad_Parameter;
                                          ...
                                          const unsigned char *pSpeakerBytes)
{
        ...
        if (pSpeakerBytes != 0) 
        {
            /* by design: keep the data opaque and no interpreting/calculation */
			/* Use long WaitResult retry count */
            error = Tfa98xx_DspSetParamMultipleVarWait(handle_cnt, handles,
                                                MODULE_SPEAKERBOOST,
                                                SB_PARAM_SET_LSMODEL, length,
                                                pSpeakerBytes,
...
#if 0
      {
        ...
        {
            error = Tfa98xx_DspSupportDrc(handles[i], &bSupportDrc);
            ...
            if (bSupportDrc)
            {
                /* Need to set AgcGainInsert back to PRE, as the SetConfig forces it to POST */
                error = Tfa98xx_DspSetAgcGainInsert( handles[i], Tfa98xx_AgcGainInsert_PreDrc);
            }
        }
      }
#endif
                                 ...
                                 int length, unsigned char *pSpeakerBytes)
{
	return Tfa98xx_DspReadSpkrParams(handle, 
					SB_PARAM_GET_LSMODEL, length, pSpeakerBytes);
}
...
{
	return Tfa98xx_DspReadSpkrParams(handle, 
				SB_PARAM_GET_XMODEL, length, pSpeakerBytes);
}
...
Tfa98xx_Error_t Tfa98xx_DspSupporttCoef(Tfa98xx_handle_t handle, int *pbSupporttCoef)
{
        ...
        assert(pbSupporttCoef != 0);

        error = Tfa98xx_DspGetSwFeatureBits(handle, featureBits);
        if (error == Tfa98xx_Error_Ok) 
        {
           /* easy case: new API available => not the 87 */
           *pbSupporttCoef = (featureBits[0] & FEATURE1_TCOEF) == 0; /* bit=0 means tCoef expected */
        }
        else if (error == Tfa98xx_Error_RpcParamId) 
        {
        /* feature bits available in MTP directly */
        error = Tfa98xx_ReadRegister16(handle, TFA98XX_MTP+6, &featureBits1);
        if (error == Tfa98xx_Error_Ok)
        {
            *pbSupporttCoef = (featureBits1 & FEATURE1_TCOEF) == 0; /* bit=0 means tCoef expected */
        }
                                       ...
                                       unsigned char *pConfigBytes)
{
   ...
   {
#if 0
      const unsigned char temp[TFA98XX_CONFIG_LENGTH+TFA98XX_PRESET_LENGTH];
      error = Tfa98xx_DspGetParam(  handle, MODULE_SPEAKERBOOST, 
                                    SB_PARAM_GET_CONFIG_PRESET, 
                                    (TFA98XX_CONFIG_LENGTH+TFA98XX_PRESET_LENGTH), temp);
      if( error == Tfa98xx_Error_Ok)
      {
          for(int i=0; i<length; i++)
          {
              pConfigBytes[i] = temp[i];
          }
      }
#else
   /* Here one can keep it simple by reading only the first length bytes from DSP memory */

      error = Tfa98xx_DspGetParam(  handle, MODULE_SPEAKERBOOST, 
                                    SB_PARAM_GET_CONFIG_PRESET, length, pConfigBytes);
#endif
   }
   else 
   {
...
{
   Tfa98xx_Error_t error = Tfa98xx_Error_Ok;
   /* Here one cannot keep it simple as in Tfa98xx_DspReadConfig(), since */
   /* we are interested by the LAST length bytes from DSP memory          */
   ...
   if (pPresetBytes != 0) 
   {
      ...
      if( error == Tfa98xx_Error_Ok)
      {
		  ...
		  for( i=0; i<length; i++)
          {
              pPresetBytes[i] = temp[TFA98XX_CONFIG_LENGTH+i];
          }
      }
   }
   else 
...
Tfa98xx_Error_t Tfa98xx_SetVolume(Tfa98xx_handle_t handle, float voldB)
{
        ...
        if (voldB <= 0.0f) {
                 ...
                 */
                error =
                    Tfa98xx_ReadRegister16(handle, TFA98XX_AUDIO_CTR,
                                           &value);
        } else {
                error = Tfa98xx_Error_Bad_Parameter;
        }
...
Tfa98xx_Error_t Tfa98xx_SetMute(Tfa98xx_handle_t handle, Tfa98xx_Mute_t mute)
{
        ...
        if (error != Tfa98xx_Error_Ok)
                return error;
        error =
            Tfa98xx_ReadRegister16(handle, TFA98XX_SYS_CTRL,
                                   &sysctrl_value);
        if (error != Tfa98xx_Error_Ok)
                return error;
        switch (mute) {
        ...
        if (error != Tfa98xx_Error_Ok)
                return error;
        error =
            Tfa98xx_WriteRegister16(handle, TFA98XX_SYS_CTRL,
                                    sysctrl_value);
        return error;
}
...
static Tfa98xx_Error_t waitResult(Tfa98xx_handle_t handle, int waitRetryCount)
{
        Tfa98xx_Error_t error;
        ...
        while ((error == Tfa98xx_Error_Ok) && ((cf_status & 0x0100) == 0) && (tries < waitRetryCount));     /* don't wait forever, DSP is pretty quick to respond (< 1ms) */
                    ...
                    const unsigned char data[])
{
	...
	return Tfa98xx_DspSetParamVarWait(handle, module_id, param_id, num_bytes, data, TFA98XX_WAITRESULT_NTRIES);
}
...
Tfa98xx_DspSetParamMultiple(int handle_cnt,
                            ...
                            int num_bytes, const unsigned char data[])
{
	return Tfa98xx_DspSetParamMultipleVarWait(handle_cnt, handles, module_id, param_id, num_bytes, data, TFA98XX_WAITRESULT_NTRIES);
}
                    ...
                    unsigned char param_id, int num_bytes, unsigned char data[])
{
        ...
        if (error == Tfa98xx_Error_Ok) {
                ...
                while ((error == Tfa98xx_Error_Ok) && ((cf_status & 0x0100) == 0) && (tries < TFA98XX_WAITRESULT_NTRIES));     /* don't wait forever, DSP is pretty quick to respond (< 1ms) */
