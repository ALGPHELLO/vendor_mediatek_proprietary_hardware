...
void SCPSpiPollingHandle(uint32_t spi_id, uint32_t spi_base)
{
    ...
    if (NULL == chip_config_intr) {
        SPI_ERR_ISR("chip_config is NULL!!\n");
        return;
    }
...
void startspiIrqTimer(struct spi_transfer *xfer)
{
    if(((xfer->id) >= 0) && ((xfer->id) < SPI_HOST_NUM))
    {
        if(irqTimer[xfer->id].timerHandle == 0)
        {
            ...
            if(irqTimer[xfer->id].timerHandle == 0)
                SPI_ERR("%s %d timTimerSet failed!\n", __func__, __LINE__);
        }
        else
        {
            SPI_ERR("%s %d Assert:SPI%d timerHandler!=0 before startTimer!\n", __func__, __LINE__, xfer->id);
            configASSERT(0);
        }
    }
    else
        SPI_ERR("%s %d illegal xfer->id=%d\n", __func__, __LINE__, xfer->id);
}
...
static int spi_fifo_sync(struct spi_transfer *xfer)
{
    ...
    do {
        i++;
        if (i > 10000000L) {
            SPI_ERR("spi_scp polling timeout!\n");
            break;
        }
