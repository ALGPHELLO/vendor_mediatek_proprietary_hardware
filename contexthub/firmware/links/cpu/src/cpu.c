...
uint64_t cpuIntsOff(void)
{
    ...
    uint64_t ret = (uint64_t)uxCriticalNesting;
    if (!is_in_isr()) {
        taskENTER_CRITICAL();
        ret = (uint64_t)uxCriticalNesting - 1; /* remember the value before enter cirtical */
    }
    return ret;
#endif
}
...
void cpuIntsRestore(uint64_t state)
{
    ...
    extern unsigned long uxCriticalNesting;
    if (!is_in_isr()) {
        configASSERT((unsigned long)state == (uxCriticalNesting - 1))
        taskEXIT_CRITICAL();
    }
#endif
}
