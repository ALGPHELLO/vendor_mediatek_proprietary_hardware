...
#define MAX_NUM_BLOCKS      128         /* times 256 = 32768 bytes */
#else
...
static bool initSensors()
{
    ...
    // mtk add, we don't use chre default simpleQ, we use contexthub_fw.c share dram
    ...
    if (mNumSensors != 0) {
        ...
        osLog(LOG_INFO, "initSensors: Active sensor number is %d, bypass mem allocate funtion to prevernt scp assert\n", mNumSensors);
        return false;
    }
    ...
    for (i = SENS_TYPE_INVALID + 1, j = 0; i <= SENS_TYPE_LAST_USER && j < mNumSensors; i++) {
        ...
        //if chre task can premption in future, below condition need reconsider
        if ((si = sensorFind(i, 0, &handle)) != NULL && !(si->flags1 & SENSOR_INFO_FLAGS1_LOCAL_ONLY) && (sensorGetInitComplete(handle))) {
            mSensorList[i - 1] = j;
...
static void hostIntfAddBlock(struct HostIntfDataBuffer *data)
{
     ...
     * we do not need nanohubPrefetchTx, simpleQEnqueue will send data to dram
     * so we directly setinterrupt
     */
...
static void queueFlush(struct ActiveSensor *sensor)
{
    ...
    simpleQueueEnqueue(mOutputQ, &sensor->buffer, sizeof(uint32_t) + sensor->buffer.length, sensor->discard);
    resetBuffer(sensor);
    /* sensor->curSamples must not reinitialize to 0 when flush sensor, sensor->curSamples = 0
     * will update sensor->firstTime to a new loop batch, then batch timeout will be delayed.
     * mWakeupBlocks and mNonWakeupBlocks should reinitialize to 0.
     * if we sensor->curSamples = 0 issue will occre, for example:
     * in system time 50000000000, acc rate = 200hz, latency = 10s enable, normally timeout is
     * 60000000000, 70000000000 ......
     * but in system time 55000000000 app call flush, so this branch will running.
     * if in this branch we set curSamlples = 0, sensor->firstTime = nowtime = 55000000000,
     * next batch timeout is nowtime + 10s = 65000000000, if we frequency call flush, batch will
     * never happen.
     */
    ...
    if (evtType == EVT_APP_START) {
...
#endif
        osLog(LOG_INFO, "hostintf: %lld, chreType:%d, rate:%u, latency:%lld, cmd:%d!\n",
        ...
        if (cmd->sensType > SENS_TYPE_INVALID && cmd->sensType <= SENS_TYPE_LAST_USER && mSensorList[cmd->sensType - 1] < MAX_REGISTERED_SENSORS) {
            ...
            if (sensor->sensorHandle) {
                if (cmd->cmd == CONFIG_CMD_FLUSH) {
                    ...
                    /* sensor->curSamples must reinitialize to 0 when disable sensor, sensor->curSamples = 0,
                     * means this sensor buffer don't store any events, next enable sensor->firstTime = enabletime.
                     * mWakeupBlocks and mNonWakeupBlocks should reinitialize to 0.
                     * if we don't set sensor->curSamples = 0, issue will occur, for example:
                     * first acc rate = 200hz, latency = 0s, in system time 50000000000 we disable acc,
                     * curSamples may be 6, sensor->firstTime may be systemtime 50000000000.
                     * after 10s time, second acc rate = 200hz, latency = 10s, because first sensor->firstTime = 50000000000,
                     * and system not time is 50000000000 + 10000000000 = 60000000000 > sensor->firstTime, so batch timeout,
                     * only 6 events transfer to ap, second time when we enable sensor will immediately report data, disorganize
                     * sensor batch flow.
                     */
                    mWakeupBlocks = 0;
                    mNonWakeupBlocks = 0;
        ...
        if (sensor->sensorHandle) {
            ...
            if (sensor->firstTime &&
                ...
                /* mtk add, we should at here to push data to dram, and wakeup ap */
                simpleQueueEnqueue(mOutputQ, &sensor->buffer, sizeof(uint32_t) + sensor->buffer.length, sensor->discard);
                resetBuffer(sensor);
                /* sensor->curSamples must reinitialize to 0 when batch timeout, sensor->curSamples = 0 will
                 * update sensor->firstTime to a new loop batch.
                 * mWakeupBlocks and mNonWakeupBlocks should reinitialize to 0.
                 * for example:
                 * acc rate = 200hz, latency = 10s, assume mTotalBlock = 1024
                 * when if condition true, this branch will running, in this branch we set curSamlples = 0,
                 * sensor->firstTime = nowtime, next batch timeout is nowtime + 10s.
                 */
                mWakeupBlocks = 0;
                mNonWakeupBlocks = 0;
                sensor->curSamples = 0;
                hostIntfSetInterrupt(sensor->interrupt);
                mark_timestamp((evtType & 0xFF), (evtType & 0xFF), HOSTINTF, rtcGetTime());
            } else if (mWakeupBlocks + mNonWakeupBlocks >= mTotalBlocks) {
                /* mtk add, we should at here to push data to dram, and wakeup ap */
                simpleQueueEnqueue(mOutputQ, &sensor->buffer, sizeof(uint32_t) + sensor->buffer.length, sensor->discard);
                resetBuffer(sensor);
                /* sensor->curSamples must not reinitialize to 0 when events >= mTotalBlocks, sensor->curSamples = 0
                 * will update sensor->firstTime to a new loop batch, then batch timeout will be delayed forever.
                 * mWakeupBlocks and mNonWakeupBlocks should reinitialize to 0.
                 * for example:
                 * acc rate = 200hz, latency = 10s, assume mTotalBlock = 1024
                 * there will be 2000 events in 10s, 2000 events > mTotalBlocks, so this branch will running.
                 * if in this branch we set curSamlples = 0, when 1024 events is ready, sensor->firstTime = nowtime,
                 * next batch timeout is nowtime + 10s, but every 1024 events will update this sensor->firstTime,
                 * so batch timeout will never happen.
                 */
                mWakeupBlocks = 0;
                mNonWakeupBlocks = 0;
...
void hostIntfSetInterrupt(uint32_t bit)
{
   ...
   if (mHostIntfTid) {
         ...
         * nanohubPrefetchTx will clear this bit, but we don't need nanohubPrefetchTx
         * simpleQEnqueue already send data to dram
         */
        if (!atomicBitsetGetBit(mInterruptMask, bit)) {
            mInterruptCntWkup++;
            apIntSet(true);
        } else {
            mInterruptCntNonWkup++;
        }
    }
#else
...
void hostIntfSetInterruptMask(uint32_t bit)
{
    ...
    osLog(LOG_ERROR, "hostIntfSetInterruptMask: %d\n", bit);
    if (mHostIntfTid) {
        if (!atomicBitsetGetBit(mInterruptMask, bit)) {
...
void hostIntfClearInterruptMask(uint32_t bit)
{
    ...
    osLog(LOG_ERROR, "hostIntfClearInterruptMask: %d\n", bit);
    if (mHostIntfTid) {
        if (atomicBitsetGetBit(mInterruptMask, bit)) {
