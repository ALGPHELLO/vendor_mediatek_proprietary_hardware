...
TEST(CdmaSmsTest, testRemoveZero) {

    // has header
    unsigned char data1[] = {
        0x00, 0x03, 0x20, 0x00, 0xe8, 0x01, 0x12, 0x48, 0x90, 0x28, 0x00, 0x1b, 0x88, 0x10, 0x17,
        0x27, 0xa1, 0xe3, 0xd2, 0x71, 0xb4, 0xde, 0x6f, 0x31, 0x98};
    unsigned char data1_c[] = {
        0x00, 0x03, 0x20, 0x00, 0xe8, 0x01, 0x12, 0x48, 0x90, 0x28, 0x00, 0x1b, 0x88, 0x10, 0x17,
        0x27, 0xa1, 0xe3, 0xd2, 0x71, 0xb4, 0xde, 0x6f, 0x31, 0x98};
    int len = removeZero(data1, sizeof(data1)/sizeof(data1[0]));
    ASSERT_EQ(len, 25);
    for (int i = 0; i < 25; i++) {
        ASSERT_EQ(data1[i], data1_c[i]);
    }
    // No header, 7bit asiic, has trailing zero, user data is the last
    unsigned char data2[] = {
        0x00, 0x03, 0x20, 0x00, 0xf0, 0x01, 0x25, 0x11, 0x3c, 0x4c, 0xd9, 0xf3, 0xf3,
        0xc9, 0x9b, 0x46, 0x8e, 0x79, 0x34, 0x68, 0xf1, 0x8f, 0x46, 0x8c, 0x5e, 0xb2,
        0x68, 0xc5, 0x93, 0x26, 0x6d, 0x1a, 0x32, 0x66, 0xcf, 0xa3, 0x26, 0x7d, 0x1a,
        0x32, 0x63, 0xd1, 0xa8, 0x00};

    unsigned char data2_c[] = {
        0x00, 0x03, 0x20, 0x00, 0xf0, 0x01, 0x24, 0x11, 0x3c, 0x4c, 0xd9, 0xf3, 0xf3,
        0xc9, 0x9b, 0x46, 0x8e, 0x79, 0x34, 0x68, 0xf1, 0x8f, 0x46, 0x8c, 0x5e, 0xb2,
        0x68, 0xc5, 0x93, 0x26, 0x6d, 0x1a, 0x32, 0x66, 0xcf, 0xa3, 0x26, 0x7d, 0x1a,
        0x32, 0x63, 0xd1, 0xa8};
    len = removeZero(data2, sizeof(data2)/sizeof(data2[0]));
    ASSERT_EQ(len, 43);
    for (int i = 0; i < 43; i++) {
        ASSERT_EQ(data2[i], data2_c[i]);
    }
    // No header, 7bit asiic, has trailing zero, user data is not the last
    unsigned char data3[] = {
        0x00, 0x03, 0x20, 0x00, 0xf0, 0x01, 0x25, 0x11, 0x3c, 0x4c, 0xd9, 0xf3, 0xf3,
        0xc9, 0x9b, 0x46, 0x8e, 0x79, 0x34, 0x68, 0xf1, 0x8f, 0x46, 0x8c, 0x5e, 0xb2,
        0x68, 0xc5, 0x93, 0x26, 0x6d, 0x1a, 0x32, 0x66, 0xcf, 0xa3, 0x26, 0x7d, 0x1a,
        0x32, 0x63, 0xd1, 0xa8, 0x00, 0x06, 0x01, 0x00};

    unsigned char data3_c[] = {
        0x00, 0x03, 0x20, 0x00, 0xf0, 0x01, 0x24, 0x11, 0x3c, 0x4c, 0xd9, 0xf3, 0xf3,
        0xc9, 0x9b, 0x46, 0x8e, 0x79, 0x34, 0x68, 0xf1, 0x8f, 0x46, 0x8c, 0x5e, 0xb2,
        0x68, 0xc5, 0x93, 0x26, 0x6d, 0x1a, 0x32, 0x66, 0xcf, 0xa3, 0x26, 0x7d, 0x1a,
        0x32, 0x63, 0xd1, 0xa8, 0x06, 0x01, 0x00};

    len = removeZero(data3, sizeof(data3)/sizeof(data3[0]));
    ASSERT_EQ(len, 46);
    for (int i = 0; i < 46; i++) {
        ASSERT_EQ(data3[i], data3_c[i]);
    }

    unsigned char data4[] = {
        0x00, 0x03, 0x20, 0x00, 0x40, 0x01, 0x04, 0x10, 0x14, 0x8d, 0x20
    };

    unsigned char data4_c[] = {
        0x00, 0x03, 0x20, 0x00, 0x40, 0x01, 0x04, 0x10, 0x14, 0x8d, 0x20
    };

    len = removeZero(data4, sizeof(data4)/sizeof(data4[0]));
    ASSERT_EQ(len, 11);
    for (int i = 0; i < 11; i++) {
        ASSERT_EQ(data4[i], data4_c[i]);
    }

    unsigned char data5[] = {
        0x00, 0x03, 0x20, 0x00, 0xe0, 0x01, 0x06, 0x20, 0x16, 0xc1, 0xee, 0xf0, 0x70
    };
    unsigned char data5_c[] = {
        0x00, 0x03, 0x20, 0x00, 0xe0, 0x01, 0x06, 0x20, 0x16, 0xc1, 0xee, 0xf0, 0x70
    };
    len = removeZero(data5, sizeof(data5)/sizeof(data5[0]));
    ASSERT_EQ(len, 13);
    for (int i = 0; i < 13; i++) {
        ASSERT_EQ(data5[i], data5_c[i]);
    }
...
TEST(CdmaSmsTest, RIL_REQUEST_CDMA_SEND_SMS_0) {
    ...
    RTST_AT_CMD(SMS_CHANNEL,
        "AT+CMGS=\"0\", \"0000021002040702c4d48a19a088060100082b00032000f00124113c4cd9f3f3c99b468e793468f18f468c5eb268c593266d1a3266cfa3267d1a3263d1a8\"",
        1,
        "^HCMGSS:32");
...
TEST(CdmaSmsTest, RIL_REQUEST_CDMA_SEND_SMS_3) {
    ...
    RTST_AT_CMD(SMS_CHANNEL,
        ...
        1,
        "^HCMGSF:16,1");
    RTST_EXPECTED_RIL_RESPONSE_NC(RIL_REQUEST_CDMA_SEND_SMS, RIL_E_GENERIC_FAILURE, 3,
            RTST_INT32, "0",
            RTST_INT32, "-1",
            RTST_INT32, "16"
            );
