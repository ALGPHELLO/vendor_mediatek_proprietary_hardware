...
void ViaHandler::handleCdmaSubscription(RfxBaseHandler* handler, char **p_response, RIL_Errno *result) {
    String8 mTag = String8("RmcCdmaSimRequest");
    int err;
    int skip = 0;
    int sidnidCount = 0;
    ...
    *result = RIL_E_SIM_ERR;
    ...
    do {
        ...
        RFX_LOG_D(mTag, "handleCdmaSubscription CNUM: %s, Sid: %s, Nid: %s, VIMIN: %s, VPRLID: %s",
...
void ViaHandler::handleCdmaPrlChanged(const sp<RfxMclMessage>& msg, RfxBaseHandler* handler, int slotId) {
    ...
    char slotIdStr[32];
    char prlKey[32] = "cdma.prl.version";
    char prlValue[32];
...
int ViaHandler::getCdmaLocationInfo(RfxBaseHandler* handler,
    ...
    if (err < 0) {
        ...
        char *system_id = NULL;
        int systemId = (*voice_reg_state_cache).system_id;
        asprintf(&system_id, "%d", systemId);
        if (system_id != NULL) {
            if (systemId != mSystemId) {
               mSystemId = systemId;
               RFX_LOG_D(LOG_TAG, "getCdmaLocationInfo, set systemId is %d.", mSystemId);
               rfx_property_set("cdma.operator.sid", system_id);
            }
            free(system_id);
        }
    }
