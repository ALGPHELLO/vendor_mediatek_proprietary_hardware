...
#define SOCKET_NAME_RIL_DEBUG "rildc-debug"

// M: For remote process call from rpc socket
#define SOCKET_NAME_RPC "rpc"
// M: For ATCI daemon connection
#define SOCKET_NAME_ATCI_C2K "rild-atci-c2k"


#define ANDROID_WAKE_LOCK_NAME "radio-interface-c2k"
...
// M: Disable some google default log
#define DEBUG 0
/*******************************************************************/

// M: C2K has reportConn callback
...
// M: Add mediatek proprietary socket
...
// M: Add mediatek proprietary event
...
/**
 * ===================================================================
 * M: To fix build error after GSM/C2K RIL commands integration start.
 * Define GSM only command's dispatch/response function here to avoid build error.
 */
#define RIL_ERRNO_INVALID_RESPONSE -1
...
/**
 * M: To fix build error after GSM/C2K RIL command integration end.
 * ===================================================================
 */
// MTK-START: SIM GBA
...
//    {0, NULL, NULL, RIL_SUPPORT_CHANNELS}
};
...
static CommandInfo s_c2k_local_commands[] = {
#include "c2k_ril_local_commands.h"
};

static UnsolResponseInfo s_c2k_local_unsolResponses[] = {
#include "c2k_ril_local_urc_commands.h"
};
...
extern void sendPendedUrcs(RIL_SOCKET_ID socket_id, int fdCommand);
extern void cacheUrc(int unsolResponse, const void *data, size_t datalen, RIL_SOCKET_ID socket_id);
...
pthread_mutex_t s_pendingUrcMutex[SIM_COUNT];
int s_fdCommand[SIM_COUNT];
// @}
...
issueLocalRequest(int request, void *data, int len, RIL_SOCKET_ID socket_id) {
    ...
    if (request >= RIL_LOCAL_REQUEST_VENDOR_C2K_BASE) {
        for (i = 0; i < (int32_t)NUM_ELEMS(s_c2k_local_commands); i++) {
            if (request == s_c2k_local_commands[i].requestNumber) {
                pRI->pCI = &(s_c2k_local_commands[i]);
                hasMatched = true;
                break;
            }
        }
    ...
    if (!hasMatched) {
        RLOGE("issueLocalRequest unsupported request code %d token %d", request);
        free(pRI);
        return;
    }
...
processCommandBuffer(void *buffer, size_t buflen, int owner, RIL_SOCKET_ID socket_id) {
    ...
    *pendingRequestsHook = pRI;
    ...
    // M: Create Proxy for RequestInfo
    RequestInfoProxy * proxy = (RequestInfoProxy *) calloc(1, sizeof(RequestInfoProxy));
    if (proxy == NULL) {
        RLOGE("Memory allocation failed for RequestInfoProxy, request %s", requestToString(request));
        return 0;
    }
    proxy->p = new Parcel();
    ...
    if (request == RIL_LOCAL_C2K_REQUEST_AT_COMMAND_WITH_PROXY_CDMA) {
        int32_t parcelProxyId;
        status = proxy->p->readInt32(&parcelProxyId);
        assert(status == NOERROR);
        RLOGD("Always send to AT_CHANNEL on CDMA");
        proxyId = DEFAULT_CHANNEL;
    }

    /* Save dispatched proxy in RequestInfo */
    proxy->pRI->cid = (RILChannelId) proxyId;
    int queueSize =  enqueueProxyList(&s_Proxy[proxyId], proxy);
    if (0 != queueSize) {
        RLOGI("%s is busy. %s queued. total:%d requests pending", proxyIdToString(proxyId),
...
static void dispatchFdMode(Parcel &p, RequestInfo *pRI) {
    RLOGE("dispatchFdMode not support in c2k rild!");
}
...
sendResponseRaw (const void *data, size_t dataSize, RIL_SOCKET_ID socket_id) {
    int* pFd = &s_ril_param_socket.fdCommand;
...
#if (SIM_COUNT >= 2)
    if (socket_id == RIL_SOCKET_2) {
        pFd = &s_ril_param_socket2.fdCommand;
        ...
        pFd = &s_ril_param_socket3.fdCommand;
        ...
        pFd = &s_ril_param_socket4.fdCommand;
    ...
    ret = blockingWrite(*pFd, (void *)&header, sizeof(header));
    ...
    ret = blockingWrite(*pFd, data, dataSize);
...
/// M: Ims Data Framework {@
static int responseSetupDedicateDataCall(Parcel &p, void *response, size_t responselen) {
    RLOGD("responseSetupDedicateDataCall, Current is not support");
    return 0;
}
/// @}
...
static int responseIratStateChange(Parcel &p, void *response, size_t responselen)
{
    RLOGD("responseIRatStateChange: 0 responselen = %d, sizeof(RIL_Pdn_IratInfo) = %d.",
...
static int
responseGetSmsSimMemStatusCnf(Parcel &p, void *response, size_t responselen) {
    int numInts = 0;
...
static int responseNetworkScanResult(Parcel &p, void *response, size_t responselen) {
    ...
    int i;
    for (i = 0; i < p_result->network_infos_length; i++) {
        p.writeInt32((int)p_cur->cellInfoType);
        p.writeInt32(p_cur->registered);
        p.writeInt32(p_cur->timeStampType);
        p.writeInt64(p_cur->timeStamp);
...
static void dispatchPhbEntry(Parcel &p, RequestInfo *pRI) {
    ...
    appendPrintBuf("%s type=%d,index=%d,number=%s,ton=%d,alphaId=%s", printBuf,
            p_Record.type, p_Record.index, (char*)p_Record.number, p_Record.ton,
            (char*)p_Record.alphaId);
    closeRequest;
    printRequest(pRI->token, pRI->pCI->requestNumber);
...
static int responsePhbEntries(Parcel &p, void *response, size_t responselen) {
    ...
    for (int i = 0 ; i < num ; i++) {
       ...
       appendPrintBuf("%s type=%d, index=%d, number=%s, ton=%d, alphaId=%s", printBuf,
                p_cur.type, p_cur.index, (char*)p_cur.number, p_cur.ton,
        (char*)p_cur.alphaId);
    }
    closeResponse;
...
RequestInfoProxy* createRpcRequestInfoProxy(RequestInfo* pRI, void *buffer, size_t buflen, int* pId) {
    ...
    RLOGD("createRpcRequestInfoProxy method, request in parcel = %d", temBuffer);
    status = pRequest->p->readInt32(&temBuffer);
    RLOGD("createRpcRequestInfoProxy method, token in parcel = %d", temBuffer);

    pRequest->p_next = NULL;
    pRequest->pRI = pRI;
    RLOGD("createRpcRequestInfoProxy method, request = %d", request);

#ifdef ADD_MTK_REQUEST_URC
    if (request >= RIL_LOCAL_REQUEST_VENDOR_C2K_BASE) {
        proxyId = s_c2k_local_commands[pRI->pCI->requestNumber -
                RIL_LOCAL_REQUEST_VENDOR_C2K_BASE].proxyId;
    } else if (request >= RIL_REQUEST_VENDOR_BASE) {
        proxyId = s_mtk_commands[pRI->pCI->requestNumber - RIL_REQUEST_VENDOR_BASE].proxyId;
    } else {
        proxyId = s_commands[pRI->pCI->requestNumber].proxyId;
    }
#else /* ADD_MTK_REQUEST_URC */
    proxyId = s_commands[pRI->pCI->requestNumber].proxyId;
#endif /* ADD_MTK_REQUEST_URC */
    RLOGD("createRpcRequestInfoProxy method end, proxyId = %d", proxyId);
...
//c+w
    ...
    LOGD("PARCEL_COMMAND_BYTES is 1024 now. request=%d", request);
    if (request == RIL_REQUEST_C2K_RPC_SET_MODEM_THERMAL) {
        request = RIL_REQUEST_SET_MODEM_THERMAL;
    }
    if (request < 1 || request >= (int32_t)NUM_ELEMS(s_commands)) {
    #ifdef ADD_MTK_REQUEST_URC
            if (request > (RIL_LOCAL_REQUEST_VENDOR_C2K_BASE + (int32_t)NUM_ELEMS(s_c2k_local_commands)) ||
                ...
                request < RIL_LOCAL_REQUEST_VENDOR_C2K_BASE))
    #endif /* ADD_MTK_REQUEST_URC */
        {
            LOGE("processRpcCommandsCallback unsupported request code %d token %d", request, token);
            Parcel p;
            p.writeInt32 (RESPONSE_SOLICITED);
            p.writeInt32 (token);
            p.writeInt32 (RIL_E_REQUEST_NOT_SUPPORTED);
            send_num = send(s_fdRpcCommand,p.data(),p.dataSize(),MSG_DONTWAIT);
            LOGD("onRequestUnsupportComplete send_num = %d", send_num);
            if (send_num < 0) {
                LOGE("onRequestUnsupportComplete error = %s", strerror(errno));
            }
...
#ifdef ADD_MTK_REQUEST_URC
        if (request >= RIL_LOCAL_REQUEST_VENDOR_C2K_BASE) {
            pRI->pCI = &(s_c2k_local_commands[request - RIL_LOCAL_REQUEST_VENDOR_C2K_BASE]);
        } else if (request >= RIL_REQUEST_VENDOR_BASE) {
...
// M: RPC socket callback function
...
eventLoop(void *param) {
    ...
    if (isSrlteSupport()) {
        ...
        /**
         * In SVLTE project, rild is a child process created by statusd. So we must
...
RIL_register_socket (RIL_RadioFunctions *(*Init)(const struct RIL_Env *, int, char **),RIL_SOCKET_TYPE socketType, int argc, char **argv) {
    ...
    if(Init) {
        ...
        switch(socketType) {
            case RIL_SAP_SOCKET:
                RilSapSocket::initSapSocket("msap_c2k_socket1", UimFuncs);

#if (SIM_COUNT >= 2)
                RilSapSocket::initSapSocket("msap_c2k_socket2", UimFuncs);
#endif

#if (SIM_COUNT >= 3)
                RilSapSocket::initSapSocket("msap_c2k_socket3", UimFuncs);
#endif

#if (SIM_COUNT >= 4)
                RilSapSocket::initSapSocket("msap_c2k_socket4", UimFuncs);
#endif
                break;
            default:
                break;
...
RIL_onRequestComplete(RIL_Token t, RIL_Errno e, void *response, size_t responselen) {
    ...
    if (pRI->cancelled == 0) {
        ...
        if (pRI->pCI->requestNumber == RIL_REQUEST_SET_MODEM_THERMAL) {
...
#endif
{
    ...
    fdCommand = s_ril_param_socket.fdCommand;
#if (SIM_COUNT >= 2)
        if (soc_id == RIL_SOCKET_2) {
            fdCommand = s_ril_param_socket2.fdCommand;
        }
#if (SIM_COUNT >= 3)
        if (soc_id == RIL_SOCKET_3) {
            fdCommand = s_ril_param_socket3.fdCommand;
        }
#endif
#if (SIM_COUNT >= 4)
        if (soc_id == RIL_SOCKET_4) {
            fdCommand = s_ril_param_socket4.fdCommand;
        }
#endif
#endif
    ...
    if (unsolResponse >= RIL_LOCAL_C2K_UNSOL_VENDOR_BASE) {
        for (i = 0; i < (int32_t)NUM_ELEMS(s_c2k_local_unsolResponses); i++) {
            if (unsolResponse == s_c2k_local_unsolResponses[i].requestNumber) {
                pUnsolRi = &s_c2k_local_unsolResponses[i];
                break;
            }
        }
    ...
    switch (wakeType) {
        case WAKE_PARTIAL:
            RLOGD("RIL_onUnsolicitedResponse call grabPartialWakeLock");
            grabPartialWakeLock();
...
error_exit:
    if (shouldScheduleTimeout) {
        RLOGD("RIL_onUnsolicitedResponse call releaseWakeLock");
        releaseWakeLock();
    }
}
...
*/
    switch(request) {
        ...
        case RIL_REQUEST_CONFIG_MODEM_STATUS_C2K: return "RIL_REQUEST_CONFIG_MODEM_STATUS_C2K";
 ...
 */
static int responseNotSupport(Parcel &p, void *response, size_t responselen) {
    RLOGE("C2K: response URC not support!!");
    return RIL_ERRNO_INVALID_RESPONSE;
}
...
void cacheUrc(int unsolResponse, const void *data, size_t datalen, RIL_SOCKET_ID socket_id){
    ...
    if (unsolResponse != RIL_LOCAL_C2K_UNSOL_CDMA_CARD_TYPE_NOTIFY) {
