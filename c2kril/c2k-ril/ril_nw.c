...
extern int rilNwUnsolicited(const char *s, const char *sms_pdu) {
    ...
    if (strStartsWith(s, "^SYSINFO:")) {
       ...
       return 1;
        ...
        RIL_requestProxyTimedCallback(repollNetworkState, NULL, &TIMEVAL_0, getDefaultChannelCtx()->id);
        repollinqueue = 1;
        return 1;
        ...
        /*iccid is readable after modem boot up or exec +VRESETUIM complete*/
            ...
            if (!iccid_readable) {
                ...
                if (UIM_STATUS_CARD_INSERTED == uimInsertedStatus) {
                    RIL_requestProxyTimedCallback(queryIccid, NULL, &TIMEVAL_0, getChannelCtxbyId(NW_CHANNEL)->id);
                }
            }
                                        ...
                                        size_t datalen, RIL_Token t)
{
    ...
    if (err < 0) {
        ...
        property_set("cdma.operator.sid", response.system_id);
    }
    err = at_tok_nextstr(&line, &response.network_id);
    if (err < 0) goto error;
...
static void queryIccid(void *param) {
    ...
    err = at_send_command_singleline("AT+VICCID?", "+VICCID:", &p_response, getChannelCtxbyId(NW_CHANNEL));
    if ((err < 0) || (p_response->success == 0)) {
...
static void requestSetRoamingPreference(void *data, size_t datalen, RIL_Token t) {
    ...
    if (requrstRoamingPreferencesType == 0){
        ...
        LOGE("[%s]Not support RoamingPreferencesType=%s", __func__, requrstRoamingPreferencesType);
    }
...
static int isCdmaRegSuspendEnabled(void)
{
    ...
    err = at_send_command_singleline("AT+VREGCTR?", "+VREGCTR:", &p_response, getChannelCtxbyId(NW_CHANNEL));
    if ((err < 0) || (p_response->success == 0))
    {
