...
static int getCdmaBroadcastLanguageConfig(RIL_CDMA_BroadcastSmsConfigInfo **infos,
        int *count, RIL_Token t);
static void setEmptyConfig(RIL_CDMA_BroadcastSmsConfigInfo ***infos, int *count);

void rilSmsOnSmsReady(void *arg)
{
    SMS_UNUSED(arg);
    /*  SMS PDU mode */
    at_send_command("AT+CMGF=0", NULL, getChannelCtxbyId(SMS_CHANNEL));
    if (isSvlteSupport() == 1) {
        at_send_command("AT+CNMI=1,1,0,2,0", NULL, getChannelCtxbyId(SMS_CHANNEL));
        readAllUnreadSmsOnME();
    } else {
        at_send_command("AT+CNMI=1,2,0,2,0", NULL, getChannelCtxbyId(SMS_CHANNEL));
    }
}
...
static void oemRequestSendCdmaSMS(void *data, size_t datalen, RIL_Token t)
{
    ...
    if (strStartsWith(respline, "^HCMGSF:")) {
        ...
        if (err >= 0 && err_class == 2) {
            // error class = 2: Temporary failure
            // While it's temporary failure, it should response re-try to AOSP for AOSP retry flow
...
error:
    ...
    int rilError = RIL_E_GENERIC_FAILURE;
    if ((p_response != NULL) &&
...
static int getSmsSimMemStatus(int *totalMem, int *usedMem)
{
    ...
    /* get sim info */
    at_send_command("AT+CPMS=\"SM\"", NULL, getChannelCtxbyId(SMS_CHANNEL));
    ...
    *totalMem = skip_int;
    ...
    if (simStatusChanged) {
        ...
        at_send_command(cmd, NULL, getChannelCtxbyId(SMS_CHANNEL));
        free(cmd);
        cmd = NULL;
    }
...
static void requestReadDelNewSms(void* param)
{
    ...
    if (NULL != pNewMsgStorage) {
      ...
      if (cmd1 != NULL) {
          at_send_command(cmd1, NULL, getChannelCtxbyId(SMS_CHANNEL));
          free(cmd1);
      }

      asprintf(&cmd2, "AT+CMGR=%d", pNewMsgStorage->sms_index);
      if (cmd2 != NULL) {
          at_send_command(cmd2, NULL, getChannelCtxbyId(SMS_CHANNEL));
          free(cmd2);
      }

      asprintf(&cmd3, "AT+CMGD=%d", pNewMsgStorage->sms_index);
      if (cmd3 != NULL) {
          at_send_command(cmd3, NULL, getChannelCtxbyId(SMS_CHANNEL));
          free(cmd3);
      }

      at_send_command("AT+CPMS=\"ME\"", NULL, getChannelCtxbyId(SMS_CHANNEL));
      free(pNewMsgStorage);
    }
}
