...
static void lockAtChannel(RILChannelId current_channel) {
    for (int i = 0; i < RIL_SUPPORT_CHANNELS; i++) {
        LOGD("Sim switch lock channel[%d]", i);
        if (i != current_channel) {
            RILChannelCtx* p_channel = getChannelCtxbyId(i);
...
static void unlockAtChannel(RILChannelId current_channel) {
    for (int i = 0; i < RIL_SUPPORT_CHANNELS; i++) {
        if (i != current_channel) {
            RILChannelCtx* p_channel = getChannelCtxbyId(i);
        ...
        case RC_PHASE_APPLY:
            if (isSvlteSupport() == 1 || isVsimEnabled() == 1 || isPersistVsim() == 1) {
                ...
                if (isDssNoResetSupport() && hasCdmaCard()) {
                    RILChannelId current_channel = RIL_queryMyChannelId(t);
                    lockAtChannel(current_channel);
                    responseRc->status = RC_STATUS_SUCCESS;
                    ...
                    unlockAtChannel(current_channel);
                    break;
                }
            }
            responseRc->status = RC_STATUS_SUCCESS;
...
static void requestScreenState(void *data, size_t datalen, RIL_Token t)
{
  ...
  // The ePDG/RNS framework need to monitoring the 1X signal strength across the threshold.
  ...
  if(s_screenState == 1)
  {
      ...
      // The ePDG/RNS framework need to monitoring the 1X signal strength across the threshold.
      // So we send command to adjust signaling threshold to MD3 whenever screen on/off.
      property_get("net.handover.th1x", threshold, "");
      if (strlen(threshold) != 0) {
          err = at_send_command("AT+ECSQTH=1", NULL, OEM_CHANNEL_CTX);
          if (err < 0) {
              LOGD("requestScreenState(), send AT+ECSQTH fail");
          }
      } else {
          LOGD("net.handover.th1x is empty");
      }
  ...
  else if(s_screenState == 0)
  {
     ...
     // system("echo LCD > /sys/class/power_supply/twl4030_bci_bk_battery/device/status_off");
      ...
      property_get("net.handover.th1x", threshold, "");
      if (strlen(threshold) != 0) {
          asprintf(&cmd, "AT+ECSQTH=1,%s", threshold);
          LOGD("requestScreenState(), adjust signaling threshold %s", cmd);
          err = at_send_command(cmd, NULL, OEM_CHANNEL_CTX);
          free(cmd);
          if (err < 0) {
              LOGD("requestScreenState(), send AT+ECSQTH fail");
...
static int configEiratMode() {
    ...
    if (cdmaslotid == currMajorSim) {
        ...
        if (isSvlteCdmaOnlySetFromEngMode() == 1) {
            ril_errno = at_send_command("AT+EIRATMODE=3", NULL, getChannelCtxbyId(NW_CHANNEL));
        } else if (isSvlteLcgSupport()) {
            ril_errno = at_send_command("AT+EIRATMODE=2", NULL, getChannelCtxbyId(NW_CHANNEL));
        } else {
            ril_errno = at_send_command("AT+EIRATMODE=1", NULL, getChannelCtxbyId(NW_CHANNEL));
        }
        if (ril_errno < 0) {
            return ril_errno;
        }
        if (!isEratExtSupport()) {
            ril_errno = at_send_command("AT^PREFMODE=8", NULL, getChannelCtxbyId(NW_CHANNEL));
        }
        if (ril_errno < 0) {
            return ril_errno;
        }
        ...
        ril_errno = at_send_command("AT+EIRATMODE=0", NULL, getChannelCtxbyId(NW_CHANNEL));
        if (ril_errno < 0) {
            return ril_errno;
        }
        if (!isEratExtSupport()) {
            ril_errno = at_send_command("AT^PREFMODE=2", NULL, getChannelCtxbyId(NW_CHANNEL));
            if (ril_errno < 0) {
                return ril_errno;
            }
        }
...
/// M: OP01 6M or CMCC OM @{
static int isOp016mSupportExt() {
    int isC2KSupport = 0;
    int isCmccOMSupport = 0;
    char c2k[PROPERTY_VALUE_MAX] = {0};
    char cmccom[PROPERTY_VALUE_MAX] = {0};
    char optr[PROPERTY_VALUE_MAX] = {0};

    property_get("ro.mtk_c2k_support", c2k, "0");
    isC2KSupport = atoi(c2k);
    property_get("ro.cmcc_light_cust_support", cmccom, "0");
    isCmccOMSupport = atoi(cmccom);

    property_get("persist.operator.optr", optr, "");

    return (isC2KSupport == 1) && (((strcmp("OP01", optr) == 0) ? 1 : 0) || isCmccOMSupport == 1);
}
/// @}
...
void requestModeSwitchSetTrm(void *data, size_t datalen, RIL_Token t)
{
    ...
    property_set("ril.cdma.emdstatus.send", "0");
    RLOGD("power off modem, set ril.cdma.emdstatus.send to 0");
