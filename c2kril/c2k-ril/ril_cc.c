...
extern void combineDataAttach(RILChannelCtx *p_channel);
...
extern int rilCcMain(int request, void *data, size_t datalen, RIL_Token t) {
    switch (request) {
        ...
        case RIL_REQUEST_DIAL:
            if (oemVcallOutSupportEnable(g_oem_support_flag) ||
                    !oemSupportEnable(g_oem_support_flag)) {
...
static void requestGetCurrentCalls(void *data, size_t datalen, RIL_Token t) {
    ...
    if (needRepoll) {
        RIL_requestProxyTimedCallback(sendCallStateChanged, NULL,
                &TIMEVAL_CALLSTATEPOLL, getChannelCtxbyId(CC_CHANNEL)->id);
    }
    return;
...
static void requestDial(void *data, size_t datalen, RIL_Token t) {
    ...
    if (!isUserLoad()) {
        LOGD("%s: p_dial->address:%s, clir:%s, num len:%d, addrNoPlusCode:%s, origNumber:%s",
            __FUNCTION__, p_dial->address, clir, len, addrNoPlusCode, origNumber);
    }
...
static void requestSendFlash(void *data, size_t datalen, RIL_Token t) {
    ...
    //Do not show private information in user load, ex: phone number
    if (!isUserLoad()) {
        LOGD("%s, number len:%d, number:%s, formatNumber:%s, origNumber:%s",
            __FUNCTION__, len, number, formatNumber, origNumber);
    }
    ...
    if (len == 0) {
        ...
        if (len < 16) {
            ...
            //Do not show private information in user load, ex: phone number
            if (!isUserLoad()) {
                LOGD("formatString:%s", formatString);
            }
...
static void turnOnRadio(void *param) {
    ATResponse *p_response = NULL;
    int err = 0;
    char *cmd = NULL;
    int radioOn = 0;
    ...
    if (cmd != NULL) {
        ...
        if ((psParm->enable && !radioOn) || (!psParm->enable && radioOn)) {
            ...
            err = at_send_command(cmd, &p_response, getChannelCtxbyId(NW_CHANNEL));
            if (err < 0 || p_response == NULL || p_response->success == 0) {
...
static void requestEmergencyDial(void *data, size_t datalen, RIL_Token t) {
    ...
    LOGD("%s: p_dial->address:%s, clir:%s, num len:%d, formatNum:%s, origNum:%s",
            __FUNCTION__, p_dial->address, clir, len, formatNumber, origNumber);

    inemergency = 1;
    psParam.enable = 1;
    turnOnRadio((void*) &psParam);
...
void onCallingPartyNumberInfo(char *urc) {
    ...
    if (strStartsWith(urc, "+CLIP")) {
        ...
        //Do not show private information in user load, ex: phone number
        if (!isUserLoad()) {
            LOGD("%s, RIL_CDMA_CALLING_PARTY_NUMBER_INFO_REC: len:%d buf:%s number_type:%d \
...
void onRedirectingNumberInfo(char *urc) {
    ...
    if (strStartsWith(urc, "+REDIRNUM")) {
        ...
        LOGD("%s, RIL_CDMA_REDIRECTING_NUMBER_INFO_REC: len:%d buf:%s number_type:%d \
...
void onExtendedDisplayInfo(char *urc) {
    ...
    if (strStartsWith(urc, "+CEXTD")) {
        ...
        if (info != NULL &&
            ...
            //Do not show private information in user load, ex: phone number
            if (!isUserLoad()) {
                LOGD("%s, RIL_CDMA_EXTENDED_DISPLAY_INFO_REC: alpha_len:%d alpha_buf:%s",
                        __FUNCTION__,
...
void onDisplayAndSignalsInfo(char *urc) {
    ...
    if (strStartsWith(urc, "+CFNM")) {
        ...
        if (display != NULL &&
            ...
            LOGD("%s, RIL_CDMA_DISPLAY_INFO_REC: alpha_len:%d alpha_buf:%s", __FUNCTION__,
                    cdmainfo->infoRec[numOfRecs].rec.display.alpha_len,
                    cdmainfo->infoRec[numOfRecs].rec.display.alpha_buf);
...
void requestRedial(void *data, size_t datalen, RIL_Token t, int isEmergency) {
    ...
    LOGD("%s: p_reDial->address:%s, num len:%d, formatNum:%s, origNum:%s, addrNoPlus:%s",
            __FUNCTION__, p_reDial->address, len, formatNumber, origNumber, addrNoPlusCode);
    if (origNumber == NULL) {
    ...
    LOGD("[C2K RILD]requestRedial, command:%s", cmd);
    ret = at_send_command(cmd, &p_response, CC_CHANNEL_CTX);
    free(cmd);
