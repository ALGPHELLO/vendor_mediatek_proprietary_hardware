...
int writeToChannel(int fd, char* buffer);
...
void constructChannelMapping() {
    ...
    for(i = 0; i < pseudoChannelNumber; i++) {
        ...
        RLOGI("constructChannelMapping, pseudoChannels[%d] = %x, indexMapped = %d",
                i, physicalChannels[indexMapped].p_channelMutex, indexMapped);
    }
}
