...
void fillMalDataCallResponse(RIL_Data_Call_Response_v11* response,
...
void requestSetupDataCall(void * data, size_t datalen, RIL_Token t) {
    ...
    response->eran_type = RDS_RAN_MOBILE_3GPP2;
...
error:
    ...
    RIL_onRequestComplete(t, RIL_E_NETWORK_ERR, response, sizeof(RIL_Data_Call_Response_v11));
...
void requestOrSendDataCallList(RIL_Token t) {
    ...
    RIL_Data_Call_Response_v11* responses = (RIL_Data_Call_Response_v11*) calloc(1,
            nPdnInfoSize * sizeof(RIL_Data_Call_Response_v11));
...
void dumpDataResponse(RIL_Data_Call_Response_v11* response, const char* reason) {
    LOGD("%s data call response: status=%d, suggestedRetryTime=%d, interfaceId=%d, active=%d, \
            ...
            response->dnses, response->gateways, response->pcscf, response->eran_type);
}
...
void openSocketsForNetworkInterfaceConfig() {
    ...
    if (sock_fd_v4 < 0) {
        ...
        LOGD("Allocate sock_fd_v4 = %d", sock_fd_v4);
    }
    ...
    if (sock_fd_v6 < 0) {
        ...
        LOGD("Allocate sock6_fd_v6 = %d", sock_fd_v6);
    }
}
...
int definePdnContext(const RIL_Default_Bearer_VA_Config_Struct *pImsParam,
    ...
    property_get("persist.operator.optr", optrValue, "0");
    ...
    if (strcmp(optrValue, "OP20") == 0 &&
        ...
        if (AUTHTYPE_NOT_SET == authType) {
            asprintf(&cmd, "AT+CGPRCO=%d,\"tethering\",\"tethering\",\"\",\"\",0,1,1,%d",
                    availableCid, isIaApn);
        } else {
            asprintf(&cmd, "AT+CGPRCO=%d,\"tethering\",\"tethering\",\"\",\"\",%d,1,1,%d",
...
int updatePdnAddressByCid(int cid, RILChannelCtx *pChannel) {
    ...
    LOGI("[RILData_Query] getPdnAdrrByCid cid= %d, ipv4 = %s, ipv6 = %s.", cid,
...
void configureNetworkInterface(int interfaceId, int isUp) {
    ...
    for (i = 0; i < nPdnInfoSize; i++) {
        if (pPdnInfo[i].interfaceId == interfaceId) {
            if (isUp) {
                LOGI("[RILData_Util] configureNetworkInterface %s with %s(v4) and %s(v6)",
...
int convertIpv6Address(char* pdp_addr, char* ipv6_addr, int isLinkLocal) {
    ...
    if (isLinkLocal == 1) {
        strncpy(ipv6_addr, IPV6_PREFIX, MAX_IPV6_ADDRESS_LENGTH - 1);
    } else {
        strncpy(ipv6_addr, NULL_ADDR, MAX_IPV6_ADDRESS_LENGTH - 1);
    }
...
int isInitialAttachApn(const char *requestedApn) {
    ...
    if (strlen(iaProperty) > 0) {
        ...
        //LOGD("[RILData_C2K]isInitialAttachApn apnParameter=%s.", apnParameter);
        ...
        if (pPdnInfo[i].interfaceId == interfaceId) {
            if (strlen(pSyncPdnInfo[i].addressV4) > 0) {
                if (strcmp(pSyncPdnInfo[i].addressV4, pPdnInfo[i].addressV4) != 0) {
                    resetMask |= 0x01;
                    LOGI("IPv4 address lost during IRAT %d, old = %s, new=%s.",
                            interfaceId, pSyncPdnInfo[i].addressV4, pPdnInfo[i].addressV4);
                }
            }
            if (strlen(pSyncPdnInfo[i].addressV6) > 0) {
                if (strcmp(pSyncPdnInfo[i].addressV6, pPdnInfo[i].addressV6) != 0) {
                    resetMask |= 0x02;
                    LOGI("IPv6 address lost during IRAT %d, old = %s, new=%s.",
                            interfaceId, pSyncPdnInfo[i].addressV6, pPdnInfo[i].addressV6);
                 } else {
                    // IPv6 prefix may change even if the interface-Id is not
                    resetMask |= 0x04;
                    LOGI("Only IPv6 address, need to update after IRAT %d, old = %s, new=%s.",
                      interfaceId, pSyncPdnInfo[i].addressV6, pPdnInfo[i].addressV6);
                }
            }
        }
    ...
    for (p_cur = p_response->p_intermediates; p_cur != NULL; p_cur = p_cur->p_next) {
        ...
        if (responseCid == cid) {
            ...
            if (strlen(out) > 0) {
                ...
                LOGD("AT+CGCONTRDP response prim-addr=%s", out);
            }
            ...
            if (strlen(out) > 0) {
                strncpy(pPdnInfo[cid].pcscf[count], out, MAX_IPV6_ADDRESS_LENGTH - 1);
                pPdnInfo[cid].pcscf[count++][MAX_IPV6_ADDRESS_LENGTH - 1] = '\0';
                LOGD("AT+CGCONTRDP response sec-addr=%s", out);
            }
