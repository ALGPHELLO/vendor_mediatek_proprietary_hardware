...
typedef struct {
    ...
    int network_exist;  // only for ECC feature
    int sysinfo_roam_status;  // backup roam_status from ^SYSINFO
} RIL_UNSOL_Msg_Cache;
...
#ifdef FLASHLESS_SUPPORT
void waitForAdjustPortPath(int interface_num, char devicePath[], int devicePathLen);
#else
