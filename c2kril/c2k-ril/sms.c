...
int RILEncodeCdmaSmsPdu(RIL_CDMA_SMS_Message* p_message, char** s_pdu, char** s_number)
{
    ...
    *s_number = (char*)malloc(p_message->sAddress.number_of_digits + 1);
    ...
    p_message->uBearerDataLen = removeZero(p_message->aBearerData, p_message->uBearerDataLen);

    if (isSupportBearerReplyOpt()) {
...
int removeZero(unsigned char* bytePdu, unsigned short len) {
...
bool isSupportBearerReplyOpt() {
    char property_value[PROPERTY_VALUE_MAX] = { 0 };
    property_get("persist.operator.optr", property_value, "0");
    // Verizon don't support bearer reply option
