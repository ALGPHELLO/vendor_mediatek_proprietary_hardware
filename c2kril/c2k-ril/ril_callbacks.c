...
void trigger_update_cache()
{
    ...
    at_send_command("AT+EIPRL?", NULL, getDefaultChannelCtx());
    at_send_command("AT+EFCELL?", NULL, getDefaultChannelCtx());
}
...
int isMalSupported()
{
    ...
    if (is_mal_supported == -1) {
        ...
        property_get("persist.mtk_ims_support", ims_prop, "0");
        property_get("persist.mtk_epdg_support", epdg_prop, "0");
...
static void atcommand_init()
{
    ...
    /*  No auto-answer */
    at_send_command("ATS0=0", NULL, getChannelCtxbyId(DEFAULT_CHANNEL));

    /*  Extended errors */
    at_send_command("AT+CMEE=1", NULL, getChannelCtxbyId(DEFAULT_CHANNEL));

    /*  Network registration events */
    err = at_send_command("AT+CREG=2", &p_response, getChannelCtxbyId(DEFAULT_CHANNEL));

    /* some handsets -- in tethered mode -- don't support CREG=2 */
    if (err < 0 || p_response->success == 0) {
        at_send_command("AT+CREG=1", NULL, getChannelCtxbyId(DEFAULT_CHANNEL));
    }
    ...
    at_send_command(cmd, NULL, getChannelCtxbyId(DEFAULT_CHANNEL));

    /*Send URC +VMCCMNC when MCC/MNC changed in network */
    at_send_command("AT+VMCCMNC=1", NULL, getChannelCtxbyId(DEFAULT_CHANNEL));

    /*disable CBP wakeup AP because of ETS message*/
    at_send_command("AT+VUSBETS=0", NULL, getChannelCtxbyId(DEFAULT_CHANNEL));

    /* set cp rm interface protocol - Relay Layer Rm interface, PPP */
    at_send_command("AT+CRM=1", NULL, getChannelCtxbyId(DEFAULT_CHANNEL));
    ...
    if (getSvlteProjectType() >= SVLTE_PROJ_SC_3M
        ...
        at_send_command("AT+EIRATMODE=0", NULL, getChannelCtxbyId(DEFAULT_CHANNEL));
    } else if (isCdmaLteDcSupport()) {
        if (isSvlteCdmaOnlySetFromEngMode() == 1) {
            at_send_command("AT+EIRATMODE=3", NULL, getChannelCtxbyId(DEFAULT_CHANNEL));
            // M: [VzW] Data Framework
            // Try to disable VZW APN if it needs
            syncDisabledApnToMd();
        } else if (isSvlteLcgSupport()) {
            at_send_command("AT+EIRATMODE=2", NULL, getChannelCtxbyId(DEFAULT_CHANNEL));
        } else {
            at_send_command("AT+EIRATMODE=1", NULL, getChannelCtxbyId(DEFAULT_CHANNEL));
        }
    }

    /*enable +VSER URC*/
    at_send_command("AT+VSER=1", NULL, getChannelCtxbyId(DEFAULT_CHANNEL));

    /*add for new service_state mechanism*/
    at_send_command("AT^PREFMODE?", NULL, getChannelCtxbyId(DEFAULT_CHANNEL));
    ...
    if ((atoi(hdVoiceEnabled) == 1)) {
        at_send_command("AT+EVOCD=1", NULL, getDefaultChannelCtx());
    } else {
        at_send_command("AT+EVOCD=0", NULL, getDefaultChannelCtx());
    }
    /// @}

    /*enable femtocell URC*/
    if (isFemtocellSupport()) {
        at_send_command("AT+EFCELL=1", NULL, getChannelCtxbyId(DEFAULT_CHANNEL));
    }
}
...
void syncDisabledApnToMd() {
    ...
    int err = 0;
    ATResponse *p_response = NULL;
    RILChannelCtx* pChannel = getChannelCtxbyId(DEFAULT_CHANNEL);
...
static void resetSystemProperties()
{
    ...
    switch (slotid) {
        case 1:
            ...
            // Backward compatible for not support EF ECC URC
            if (strlen(eccSource) == 0 || atoi(eccSource) == 0) {
                property_set("ril.cdma.ecclist", "");
                LOGD("C2K set ril.cdma.ecclist to NULL");
            }
            break;
        case 2:
            ...
            // Backward compatible for not support EF ECC URC
            if (strlen(eccSource) == 0 || atoi(eccSource) == 0) {
                property_set("ril.cdma.ecclist1", "");
                LOGD("C2K set ril.cdma.ecclist1 to NULL");
            }
            break;
        default:
...
static void initializeCallback(void *param __unused)
{
    ...
    if ((0 != cpbypass) && (0 == bypass_ets_only))
    {
        ...
        at_close(getChannelCtxbyId(DEFAULT_CHANNEL));
        s_closed = 1;
    ...
    if (s_port > 0 || s_device_socket) {
        ...
        for (i = 0; i < RIL_SUPPORT_CHANNELS; i++) {
            ...
            while (p_channel->fd < 0) {
                do {
                    LOGD("muxpath[%d] = %s", i, s_mux_path[i]);
                ...
                LOGD("Channel %d path=%s, fd=%d, mutex = %x", i, s_mux_path[i], p_channel->fd,
...
void setIccidProperty(VIA_ICCID_TYPE type, char* pIccid)
{
    ...
    if (getActiveSvlteModeSlotId() == 1) {
        ...
        LOGD("set true iccid %s to ril.iccid.sim1", maskIccid);
    } else if (getActiveSvlteModeSlotId() == 2) {
        property_set("ril.iccid.sim2", iccidStr);
        LOGD("set true iccid %s to ril.iccid.sim2", maskIccid);
    } else {
...
void cacheUrcIfNeeded(const char *s, const char *sms_pdu) {
    ...
    if (strStartsWith(s, "+REGPAUSEIND:")) {
        const int socket_id = getDefaultChannelCtx()->id;
