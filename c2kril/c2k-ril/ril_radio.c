...
extern int rilRadioUnsolicited(const char *s, const char *sms_pdu) {
...
void updateRadioStatus(void *param) {
...
void requestShutdown(void *data, size_t datalen, RIL_Token t) {
    int err;
    ATResponse *p_response = NULL;
    ATResponse *p_rfs_response = NULL;
    ATResponse *p_cbp_version_response = NULL;
...
static void requestSetThermalModem(void *data, size_t datalen, RIL_Token t)
{
    ...
    property_get("ril.cdma.enhance.version", enhanceMDVersion, "0");
    enhanceMDVersionVal = atoi(enhanceMDVersion);

    LOGD("%s: modem_on = %d, ril.cdma.enhance.version = %d.", __FUNCTION__, modem_on, enhanceMDVersionVal);
    if (modem_on) {
...
void requestResetRadio(void * data, size_t datalen, RIL_Token t)
{
    ...
    if (s_md3_off != 1) {
        ...
        err = at_send_command_with_specified_timeout("AT+EPOF",
                POWEROFF_RETRY_TIMES, &p_response, RADIO_CHANNEL_CTX);
        RLOGI("%s: AT+EPOF, err=%d, p_response->success=%d", __FUNCTION__, err, p_response->success);
        if (err < 0 || p_response->success == 0) goto error;

        property_set("ril.cdma.emdstatus.send", "0");
        property_set("cdma.ril.eboot", "1");
        RLOGI("%s: power off modem, set ril.cdma.emdstatus.send to 0, cdma.ril.eboot to 1", __FUNCTION__);
