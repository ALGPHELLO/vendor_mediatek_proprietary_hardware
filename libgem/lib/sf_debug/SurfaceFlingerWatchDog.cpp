...
public:
    RTTDumper()
        : mLastFinishTime(0)
        , mUseProcessCallstack(MTK_SF_USE_PROCESSCALLSTACK)
    {
    }
...
private:
    ...
    bool threadLoop() {
        ...
        if (property_get("debug.sf.watchdog_callstack", buffer, NULL) > 0) {
            mUseProcessCallstack = atoi(buffer);
        }

        property_get("vold.encryption.type", buffer, "-1");
        ALOGD("[SF-WD-RTT] vold.encryption.type: %s (%d) mUseProcessCallstack:%d",
               buffer, strcmp(buffer,"default") != 0, mUseProcessCallstack);
        // append SurfaceFlinger rtt information to rtt file
        if (mUseProcessCallstack || strcmp(buffer,"default") != 0) {
            dumpProcess();
        } else {
            dumpProcess(getpid());

            // append HWC rtt information to rtt file
            property_get("debug.sf.hwc_pid", buffer, "0");
