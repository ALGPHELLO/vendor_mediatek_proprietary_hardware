...
status_t FpsInfo::setSingleLayer(const int32_t single)
{
    ...
    if (res != NO_ERROR) {
        ALOGW("Failed to set single layer %d: wrong state %d", single, mState);
        return res;
    }
...
status_t FpsInfo::setNumberDisplay(const int32_t number)
{
    ...
    res = connectServiceLocked();
    if (res != NO_ERROR) {
        ALOGW("Failed to set number of display %d: wrong state %d", number, mState);
        return res;
    }
...
status_t FpsInfo::setHwcInfo(const HwcInfo& info)
{
    ...
    if (res != NO_ERROR) {
        ALOGW("Failed to set HwcInfo[s:%d|d:%d]: wrong state %d",
                info.singleLayer, info.numDisplay, mState);
        return res;
    }
...
status_t FpsInfo::setForegroundInfo(int32_t pid, String8& packageName)
{
    ...
    res = connectServiceLocked();
    if (res != NO_ERROR) {
        ALOGW("Failed to set fg info [%d|%s]: wrong state %d", pid, packageName.string(), mState);
        return res;
    }
...
status_t FpsInfo::setWindowFlag(const int32_t flag, const int32_t mask)
{
    ...
    res = connectServiceLocked();
    if (res != NO_ERROR) {
        ALOGW("Failed to set window flag [%08x|%08x]: wrong state %d", flag, mask, mState);
        return res;
    }
...
status_t FpsInfo::forbidAdjustingVsync(bool forbid)
{
    ...
    res = connectServiceLocked();
    if (res != NO_ERROR) {
        ALOGW("Failed to forbid adjusting vsync [%d]: wrong state %d", forbid, mState);
        return res;
    }
