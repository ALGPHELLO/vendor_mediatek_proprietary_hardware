...
void MtkOmxVenc::EncodeFunc(OMX_BUFFERHEADERTYPE *pInputBuf, OMX_BUFFERHEADERTYPE *pOutputBuf)
{
    ...
    else    // encode normal frame
    {
...

        {
            ...
            property_get("mtk.omx.venc.dynamicfps", value, "1");
...
void MtkOmxVenc::DLMetaModeEncodeVideo(OMX_BUFFERHEADERTYPE *pInputBuf, OMX_BUFFERHEADERTYPE *pOutputBuf)
{
    ...
    if (OMX_FALSE == mSeqHdrEncoded)
    {
...
#if 1 // dump bs
        {
            ...
            property_get("mtk.omx.venc.dl.dump.bs", value, "0");
            bDumpLog = atoi(value);
...
VAL_BOOL_T MtkOmxVenc::EncSettingMPEG4Enc()
{
    ...
    mEncDrvSetting.u4Profile = VENC_DRV_MPEG_VIDEO_PROFILE_MPEG4_SIMPLE;
    mEncDrvSetting.u4Level = VENC_DRV_VIDEO_LEVEL_3;
...
VAL_BOOL_T MtkOmxVenc::EncSettingH264Enc()
{
    ...
    if (OMX_TRUE == IsConstantBitrate())
    {
        mExtraEncDrvSetting.u4BitrateMode = 1; // constant bitrate
    }
    else
    {
        mExtraEncDrvSetting.u4BitrateMode = 0;
    }

    if (mIsViLTE)
    {
        property_get("mtk.venc.i.interval", value, "0");
...
OMX_BOOL MtkOmxVenc::InitEncParams()
{
    ...
    char value[PROPERTY_VALUE_MAX];
    property_get("mtk.omx.venc.bufcount", value, "8");
...
OMX_BOOL MtkOmxVenc::InitAvcEncParams()
{
    ...
    //mOutputPortDef.nBufferSize = MTK_VENC_DEFAULT_OUTPUT_BUFFER_SIZE_AVC;
    {
        ...
        int bufK;
        char value[PROPERTY_VALUE_MAX];
        bufK = property_get("mtk.omxvenc.bufk", value, "0");
        bufK = atoi(value);
...
OMX_BOOL MtkOmxVenc::InitHevcEncParams()
{
    ...
    //mOutputPortDef.nBufferSize = MTK_VENC_DEFAULT_OUTPUT_BUFFER_SIZE_HEVC;
    {
        ...
        int bufK;
        char value[PROPERTY_VALUE_MAX];
        bufK = property_get("mtk.omxenc.bufk", value, "0");
        bufK = atoi(value);
        MTK_OMX_LOGE("bufk = %d", bufK);
        if (bufK == 0)
        {
...
OMX_BOOL MtkOmxVenc::InitSecEncParams(void)
{
    ...
    if (svp)
    {
        ...
        else
        {
           ...
           if(uIsInHouseReady)
           {
               property_get("ro.mtk_venc_inhouse_ready", value, "0");
               MTK_OMX_LOGE("%s ro.mtk_venc_inhouse_ready %s",__FUNCTION__,value);
               if (atoi(value))
               {
           ...
           {
               property_get("ro.mtk_microtrust_tee_support", value, "0");
               if (atoi(value))
               {//Microtrust TEE
...
{
    char value[PROPERTY_VALUE_MAX];
    property_get("mtk.omxvenc.mbaff", value, "2");
...
bool MtkOmxVenc::dumpOutputBuffer(OMX_BUFFERHEADERTYPE *pOutputBuf, OMX_U8 *aOutputBuf, OMX_U32 aOutputSize)
{
    ...
    else
    {
        char value[PROPERTY_VALUE_MAX];
        VAL_BOOL_T bDumpLog = 0;
        property_get("mtk.omx.venc.dump.bs", value, "0");
        bDumpLog = atoi(value);
        ...
        if (mIsViLTE)
        {
            ...
            property_get("mtk.omx.venc.vilte.dump.bs", value, "0");
            bDump = atoi(value);
            if (bDump)
            {
