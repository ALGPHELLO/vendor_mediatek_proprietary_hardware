...
MtkOmxVenc::MtkOmxVenc()
{
    ...
    {
        property_get("mtk.omxvenc.input.scaling", value, "0");
        mInputScalingMode = (OMX_BOOL)atoi(value);
        property_get("mtk.omxvenc.input.scaling.max.wide", value, "1920");
        mMaxScaledWide = (OMX_U32)atoi(value);
        property_get("mtk.omxvenc.input.scaling.max.narrow", value, "1088");
    ...
    mDrawStripe = false;
    {
        property_get("mtk.omxvenc.drawline", value, "0");
        mDrawStripe = (bool)atoi(value);
    }
    mDumpInputFrame = false;
    {
        property_get("mtk.omxvenc.dump", value, "0");
        mDumpInputFrame = (OMX_BOOL) atoi(value);
    }

    mDumpCts = false;
    {
        property_get("mtk.dumpCts", value, "0");
        mDumpCts = (OMX_BOOL) atoi(value);
    }

    mRTDumpInputFrame = false;
    {
        property_get("mtk.omxvenc.rtdump", value, "1");
        mRTDumpInputFrame = (OMX_BOOL) atoi(value);
    }

    mDumpColorConvertFrame = false;
    {
        property_get("mtk.omxvenc.ccdump", value, "0");
        mDumpColorConvertFrame = (OMX_BOOL) atoi(value);
    }
    mDumpCCNum = 5;
    {
        property_get("mtk.omxvenc.ccdumpnum", value, "5");
        mDumpCCNum = atoi(value);
    }
    {
        property_get("mtk.omxvenc.secrgb", value, "0");
        if (atoi(value))
        {
            mDumpFlag |= DUMP_SECURE_INPUT_Flag;
        }
        property_get("mtk.omxvenc.secyuv", value, "0");
        if (atoi(value))
        {
            mDumpFlag |= DUMP_SECURE_TMP_IN_Flag;
        }
        property_get("mtk.omxvenc.secbs", value, "0");
        if (atoi(value))
        {
            mDumpFlag |= DUMP_SECURE_OUTPUT_Flag;
        }
        property_get("mtk.omxvenc.dumpyv12test", value, "0");
        if (atoi(value))
        {
    ...
    {
        property_get("wfd.dummy.enable", value, "1");
    ...
    mDumpDLBS = OMX_FALSE;
    {
        property_get("mtk.omxvenc.dump.dlbs", value, "0");
        mDumpDLBS = (OMX_BOOL) atoi(value);
    }

    mIsMtklog = OMX_FALSE;
    {
        property_get("debug.MB.running", value, "0");
        mIsMtklog = (OMX_BOOL) atoi(value);
    }

    watchdogTimeout = 2000;
    {
        property_get("mtk.omxvenc.watchdog.timeout", value, "2000");
    ...
    property_get("mtk.omxvenc.vilte", value, "0");
    ...
    property_get("mtk.omxvenc.avpf", value, "0");
    ...
    property_get("mtk.omxvenc.wfd.record", value, "0");
    mRecordBitstream  = (OMX_BOOL) atoi(value);
    property_get("mtk.omxvenc.wfd.loopback", value, "0");
    ...
    switch (nParamIndex)
    {
        ...
        case OMX_IndexVendorMtkOmxVencSetScenario:
        {
            ...
            if (tmpVal == OMX_VIDEO_MTKSpecificScenario_ViLTE)
            {
                ...
                //reduce ViLTE mem usage for 512M project
                ...
                if (0 == strcmp(value, "0x20000000"))
                {
                    ...
                    MTK_OMX_LOGD("@@ ViLTE on 512M project, reduce input/output buffer count to %d/%d", mInputPortDef.nBufferCountActual, mOutputPortDef.nBufferCountActual);
                }
            }
        ...
        case OMX_IndexVendorMtkOmxVencPrependSPSPPS:
        {
            ...
            if (1)//always turn on
            {
                ...
                {// temp solution to fix unsupport prepend issue for mt8167
                    ...
                    property_get("mtk.omxvenc.unsupportprepend", value, "0");
    ...
    switch (nParamIndex)
    {
        ...
        case OMX_IndexParamVideoPortFormat:
        {
            ...
            if (pPortFormat->nPortIndex == mInputPortFormat.nPortIndex)
            {
                ...
                MTK_OMX_LOGD("[GetParameter] OMX_IndexParamVideoPortFormat index = %lu, color format = %d\n",
    ...
    else if (nPortIndex == mOutputPortDef.nPortIndex)
    {
        ...
        *ppBufferHdr = mOutputBufferHdrs[mOutputBufferPopulatedCnt] =
        ...
        if (OMX_TRUE == mStoreMetaDataInOutBuffers)
        {
            ...
            char value[PROPERTY_VALUE_MAX];
            int bsSize = 0;
            property_get("mtk.omx.venc.bssize", value, "0");
            bsSize = atoi(value);
            if (0 != bsSize)
            {
