...
MtkOmxVdec::MtkOmxVdec()
{
    ...
    VAL_CHAR_T szBuildWithMJC[PROPERTY_VALUE_MAX];
    property_get("ro.mtk_clearmotion_support", szBuildWithMJC, 0);
    int i4BuildWithMJC = atoi(szBuildWithMJC);
    property_get("mtk.mjc.fake.engine.support", szBuildWithMJC, "0");
    mMJCFakeEngine = (atoi(szBuildWithMJC) == 1) ? OMX_TRUE : OMX_FALSE;

    MTK_OMX_LOGU("Build With MJC:%d. Fake Engine(%d)", i4BuildWithMJC, mMJCFakeEngine);
...
//#endif
    ...
    mMJCLog = (VAL_BOOL_T) atoi(mMJCValue);
    ...
    property_get("mtk_deinterlace_support", mDIValue, "1");
...
//#endif
    ...
    */
    ...
    property_get("mtk.omxvdec.output.checksum", value2, "0");
    ...
    property_get("mtk.dumpCts", value2, "0");
...
//#endif
    ...
    property_get("mtk.omxvdec.avpf", value, "1");
    mAVPFEnable = (OMX_BOOL) atoi(value);

    property_get("mtk.omxvdec.avpf.plithres", value, "1");
    if (0 != atoi(value))
    {
    ...
    property_get("mtk.omxvdec.avpf.resendplithres", value, "10");
    if (0 != atoi(value))
    {
...
//#ifdef MTK_CLEARMOTION_SUPPORT
    ...
    MTK_OMX_LOGU("mOutputBufferHdrsCnt:%d, mOutputPortDef.nBufferCountActual:%d, MAX_MIN_UNDEQUEUED_BUFS:%d, FRAMEWORK_OVERHEAD:%d, TOTAL_MJC_BUFFER_CNT:%d, MAX_TOTAL_BUFFER_CNT:%d",
...
//#endif
    ...
    MTK_OMX_LOGU("allocate mFrameBuf: 0x%08x mOutputPortDef.nBufferCountActual:%d MAX_TOTAL_BUFFER_CNT:%d sizeof(FrmBufStruct):%d", mFrameBuf, mOutputPortDef.nBufferCountActual, MAX_TOTAL_BUFFER_CNT, sizeof(FrmBufStruct));
...
//#ifdef MTK_CLEARMOTION_SUPPORT
    ...
    if (mMJCEnable == OMX_TRUE && VAL_TRUE == mMJCCreated)
    {
        ...
        if (mMJCLog)
        {
            MTK_OMX_LOGU("[MJC] signal mMJCSem to terminate MJC thread");
        }
    }
//#endif
    ...
    if (IS_PENDING(MTK_OMX_OUT_PORT_ENABLE_PENDING))
    {
        ...
        MTK_OMX_LOGU("signal mOutPortAllocDoneSem (%d)", get_sem_value(&mOutPortAllocDoneSem));
    }
    if (IS_PENDING(MTK_OMX_IDLE_PENDING))
    {
        SIGNAL(mInPortAllocDoneSem);
        MTK_OMX_LOGU("signal mInPortAllocDoneSem (%d)", get_sem_value(&mInPortAllocDoneSem));
        SIGNAL(mOutPortAllocDoneSem);
        MTK_OMX_LOGU("signal mOutPortAllocDoneSem (%d)", get_sem_value(&mOutPortAllocDoneSem));
    }
...
{
    OMX_ERRORTYPE err = OMX_ErrorNone;
    MTK_OMX_LOGU("MtkOmxVdec::SendCommand cmd=%s", CommandToString(Cmd));
    ...
    switch (Cmd)
    {
        case OMX_CommandStateSet:   // write 8 bytes to pipe [cmd][nParam1]
            ...
            {
                MTK_OMX_LOGU("set MTK_OMX_VDEC_IDLE_PENDING");
                SET_PENDING(MTK_OMX_IDLE_PENDING);
            }
            else if (nParam1 == OMX_StateLoaded)
            {
                MTK_OMX_LOGU("set MTK_OMX_VDEC_LOADED_PENDING");
    ...
    switch (nParamIndex)
    {
        case OMX_IndexParamPortDefinition:
        {
            ...
            else if (pPortDef->nPortIndex == mOutputPortDef.nPortIndex)
            {
                ...
                //] MJPEG
                MTK_OMX_LOGU("nStride = %d,  nSliceHeight = %d, PicSize = %d, ori_nBufferCountActual = %d", mOutputPortDef.format.video.nStride, mOutputPortDef.format.video.nSliceHeight,
...
//#endif
                    ...
                    MTK_OMX_LOGU("MaxDPBNumber = %d,  OutputBuffers = %d (%d) ,PicSize=%d", MaxDPBNumber, ActualOutBuffNums, mOutputPortDef.nBufferCountActual, mOutputPortDef.nBufferSize);
...
//#endif
                ...
                if ( /*(OMX_FALSE == mIsUsingNativeBuffers) &&*/ (OMX_TRUE == mCrossMountSupportOn) )
                {
                    ...
                    MTK_OMX_LOGU("original nBufferCountActual after adjust = %d(+%d) ",
                        mOutputPortDef.nBufferCountActual, mMaxColorConvertOutputBufferCnt);
                }
            }
            else
        ...
        case OMX_IndexParamVideoPortFormat:
        {
            ...
            else if (pPortFormat->nPortIndex == mOutputPortFormat.nPortIndex)
            {
                ...
                else
                {
                    ...
                    {
                        MTK_OMX_LOGU("Disable FlexibleYUV Flow for surface format, MediaCodecInfo.CodecCapabilities.COLOR_FormatSurface in CTS case");
                        mbYUV420FlexibleMode = OMX_FALSE;
                    }
                }
            }
        ...
        case OMX_IndexParamVideoRv:
        {
            ...
            MTK_OMX_LOGU("Set pRvType nPortIndex=%d", pRvType->nPortIndex);
        ...
        case OMX_IndexVendorMtkOmxVdecThumbnailMode:
        {
            ...
            MTK_OMX_LOGU("Set thumbnail mode enable? %s", (pThumbnailMode->nU32==1?"yes":"no"));
                ...
                (mCodecId == MTK_VDEC_CODEC_ID_MPEG4))
            {
                ...
                MTK_OMX_LOGU("SetParameter OMX_IndexVendorMtkOmxVdecThumbnailMode mOutputPortDef.nBufferCountActual %d", mOutputPortDef.nBufferCountActual);
            }
            }
...
//#endif
        ...
        case OMX_IndexVendorMtkOmxVdecConcealmentLevel:
        {
            ...
            MTK_OMX_LOGU("Set concealment level %d", mConcealLevel);
            break;
        }
#if 0//def MTK S3D SUPPORT
        ...
        case OMX_IndexVendorMtkOmxVdecACodecColorConvertMode:
        {
            ...
            MTK_OMX_LOGU("OMX_IndexVendorMtkOmxVdecACodecColorConvertMode mACodecColorConvertMode: %d", mACodecColorConvertMode);
            break;
        }
        case OMX_IndexVendorMtkOmxVdecFixedMaxBuffer:
        {
            MTK_OMX_LOGU("deprecated, only turn this on when legacy mode");
            break;
        }
        ...
        case OMX_GoogleAndroidIndexEnableAndroidNativeBuffers:
        {
            ...
            {
                MTK_OMX_LOGU("OMX_GoogleAndroidIndexEnableAndroidNativeBuffers enable(%d)", pEnableNativeBuffers->enable);
        ...
        case OMX_GoogleAndroidIndexUseAndroidNativeBuffer:
        {
            ...
            if (OMX_TRUE == mIsSecureInst)
            {
                ...
                gralloc_extra_query(_handle, GRALLOC_EXTRA_GET_SECURE_HANDLE, &secure_buffer_handle);
#endif
                MTK_OMX_LOGU("@@ _handle(0x%08X), secure_buffer_handle(0x%08X)", _handle, secure_buffer_handle);
                mOutputUseION = OMX_FALSE;
            ...
            if (OMX_TRUE == mOutputUseION)
            {
                MTK_OMX_LOGU("buffer_handle_t(0x%08x), ionFd(0x%08X)", _handle, ion_buf_fd);
                ...
                if (MTK_VDEC_CODEC_ID_AVC == mCodecId || MTK_VDEC_CODEC_ID_HEVC == mCodecId)
                {
                    ...
                    if (OMX_TRUE == QueryDriverFormat(&qinfoIn, &qinfoOut))
                    {
                        ...
                        MTK_OMX_LOGU(" AVC meDecodeType = %d", meDecodeType);
                    }
                    else
                    {
...
//#endif
                ...
                if (OMX_TRUE == mIsSecureInst)
                {
                    ...
                    (MtkVideoRegisterSharedMemory_Ptr *) dlsym(mCommonVdecInHouseLib, MTK_COMMON_SEC_VDEC_IN_HOUSE_REGISTER_SHARED_MEMORY);
                        ...
                        secHandle = pfnMtkVideoAllocateSecureFrameBuffer_Ptr(bufferSize, 512, "svp_vdec_set_UseNativeBuf");
                        va_share_handle = pfnMtkVideoRegisterSharedMemory_Ptr(buffer, bufferSize, "svp_vdec_set_UseNativeBuf");
                    }
                    else //TRUSTONIC_TEE
                    {
                        // get secure buffer handle from gralloc (DEBUG ONLY)
                        ...
                        MTK_OMX_LOGU("mIonOutputBufferInfo[%d].secure_handle(0x%08X) %d", mIonOutputBufferCount, secHandle, mIonOutputBufferCount);
                    }
        ...
        case OMX_IndexVendorMtkOmxVideoUseIonBuffer:
        {
            ...
            MTK_OMX_LOGU("@@ mIsClientLocally(%d), mIsFromGralloc(%d), VA(0x%08X), FD(%d), size(%d)", mIsClientLocally, mIsFromGralloc, pUseIonBufferParams->virAddr, pUseIonBufferParams->Ionfd, pUseIonBufferParams->size);
            int ret;
            OMX_U8 *buffer;
        ...
        case OMX_IndexVendorMtkOmxVideoSetClientLocally:
        {
            ...
            MTK_OMX_LOGU("@@ mIsClientLocally(%d)", mIsClientLocally);
            break;
        }
        ...
        case OMX_IndexVendorMtkOmxVdecNoReorderMode:
        {
            ...
            MTK_OMX_LOGU("Set No Reorder mode enable, %d", pVideoNoReorderInfo->nU32);
        ...
        case OMX_GoogleAndroidIndexStoreMetaDataInBuffers:
        {
            ...
            if (pStoreMetaDataInBuffersParams->nPortIndex == mOutputPortFormat.nPortIndex)
            {
                ...
                MTK_OMX_LOGU("@@ mStoreMetaDataInBuffers(%d) for port(%d)", mStoreMetaDataInBuffers, pStoreMetaDataInBuffersParams->nPortIndex);
            }
            else
            {
                err = OMX_ErrorBadPortIndex;
        ...
        case OMX_GoogleAndroidIndexstoreANWBufferInMetadata:
        {
            ...
            if (pStoreMetaDataInBuffersParams->nPortIndex == mOutputPortFormat.nPortIndex)
            {
                ...
                {
                    MTK_OMX_LOGU("@@ OMX_GoogleAndroidIndexstoreANWBufferInMetadata return un support by setting property");
                    err = OMX_ErrorUnsupportedIndex;
                    goto EXIT;
...
//ANDROID_N
        case OMX_GoogleAndroidIndexEnableAndroidNativeHandle:
        {
            ...
            {
                MTK_OMX_LOGE("@@ OMX_GoogleAndroidIndexEnableAndroidNativeHandle: invalid port index");
                err = OMX_ErrorBadParameter;
                goto EXIT;
            }
            ...
            if (NULL != pEnableNativeBuffers)
            {
                MTK_OMX_LOGU("OMX_GoogleAndroidIndexEnableAndroidNativeHandle enable(%d)", pEnableNativeBuffers->enable);
                //maybe
                ...
                if(!mIsSecUsingNativeHandle)
                {
                    ...
                    MTK_OMX_LOGU("OMX_GoogleAndroidIndexEnableAndroidNativeHandle not support, enable(%d)", pEnableNativeBuffers->enable);
                    goto EXIT;
                }
            }
            else
...
//endof ANDROID_N
        ...
        {
            MTK_OMX_LOGU("Sets skip reference check mode!");
        ...
        case OMX_IndexVendorMtkOmxVdecLowLatencyDecode:
        {
            ...
            MTK_OMX_LOGU("Sets low latency decode mode! %d", pLowLatencyDecodeInfo->nU32);
        ...
        {
            MTK_OMX_LOGU("Set 16x slowmotion mode");
...
//#endif
        ...
        case OMX_IndexVendorMtkOmxVdecSetScenario:
        {
            ...
            MTK_OMX_LOGU("@@ set vdec scenario %lu, mViLTESupportOn %d, mCrossMountSupportOn %d", tmpVal, mViLTESupportOn, mCrossMountSupportOn);
            break;
        }
        ...
        case OMX_IndexVendorMtkOmxVdecANW_HWComposer:
        {
            if (mDecoderInitCompleteFlag == OMX_FALSE) {
                ...
                MTK_OMX_LOGU("@@ set OMX_IndexVendorMtkOmxVdecANW_HWComposer %d", mANW_HWComposer);
            }
            break;
        }
        ...
        case OMX_IndexVendorMtkOmxVdecTaskGroup:
        {
            ...
            MTK_OMX_LOGU("@@ set av task group %u", mEnableAVTaskGroup);
            break;
        }
        ...
        case OMX_IndexParamPortDefinition:
        {
            ...
            else if (pPortDef->nPortIndex == mOutputPortDef.nPortIndex)
            {
                ...
                if (mMinUndequeuedBufsFlag == OMX_TRUE)
                {
                    ...
                    MTK_OMX_LOGU("nBufferCountActual %d (%i)", mOutputPortDef.nBufferCountActual, mMinUndequeuedBufsDiff);
                    mMinUndequeuedBufsFlag = OMX_FALSE;
                }
                ...
                if (OMX_COLOR_FormatYUV420Planar == mOutputPortFormat.eColorFormat)
                {
                    ...
                    if (mIsUsingNativeBuffers && mNativeWindowHDRInternalConvert == OMX_FALSE)
                    {
                        ...
                        if (VDEC_DRV_DECODER_MTK_SOFTWARE == meDecodeType) //VPX VP9 sw
                        {
                            ...
                            MTK_OMX_LOGU("[MJC][HAL_PIXEL_FORMAT_I420] -> HAL_PIXEL_FORMAT_YUV_PRIVATE");
                            }
                        }
                        else
                        {
...
//#if  ((defined MTK_CLEARMOTION_SUPPORT) || (defined MTK_POST_PROCESS_FRAMEWORK_SUPPORT) || (defined MTK_DEINTERLACE_SUPPORT))
                            ...
                            {
                            pPortDef->format.video.eColorFormat = (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_YUV_PRIVATE;
                            MTK_OMX_LOGU("[MJC][HAL_PIXEL_FORMAT_NV12_BLK] -> HAL_PIXEL_FORMAT_YUV_PRIVATE");
                            }
                            ...
                            if( ( OMX_TRUE == mMJCEnable ) ||( OMX_TRUE == mDeInterlaceEnable )/*||( OMX_TRUE == mUsePPFW )*/ )
                            {
                                ...
                                else
                                {
                                    if (OMX_TRUE == mbIs10Bit)
                                    {
                                        ...
                                        MTK_OMX_LOGU("[MJC][HAL_PIXEL_FORMAT_NV12_BLK] -> HAL_PIXEL_FORMAT_YUV_PRIVATE_10BIT");
                                    }
                                    else
                                    {
                                        pPortDef->format.video.eColorFormat = (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_YUV_PRIVATE;
...
//#endif
                ...
                else if (OMX_MTK_COLOR_FormatYV12 == mOutputPortFormat.eColorFormat)
                {
                    ...
                    if (mIsUsingNativeBuffers && mNativeWindowHDRInternalConvert == OMX_FALSE)
                    {
                        ...
                        else if (VDEC_DRV_DECODER_MTK_SOFTWARE == meDecodeType) //VPX VP9 sw
                        {
                            ...
                            {
                            pPortDef->format.video.eColorFormat = (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_YUV_PRIVATE;
                            MTK_OMX_LOGU("[MJC][HAL_PIXEL_FORMAT_YV12] -> HAL_PIXEL_FORMAT_YUV_PRIVATE");
                            }
                        }
                        else //TBD
                        {
                ...
                else if (OMX_COLOR_FormatYUV420SemiPlanar == mOutputPortFormat.eColorFormat)  // for NV12
                {
                    ...
                    if (mIsUsingNativeBuffers && mNativeWindowHDRInternalConvert == OMX_FALSE)
                    {
                        ...
                        {
                            pPortDef->format.video.eColorFormat = (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_I420;
                            MTK_OMX_LOGE("[HAL_PIXEL_FORMAT_NV12] -> HAL_PIXEL_FORMAT_I420");
                        }
                        else if (VDEC_DRV_DECODER_MTK_SOFTWARE == meDecodeType)  // H.264
                        {
                            // Video codec requires 16 align for vertical stride, not standard YV12
                            pPortDef->format.video.eColorFormat = (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_YCbCr_420_888;
                            MTK_OMX_LOGE("[STANDARD][HAL_PIXEL_FORMAT_NV12] -> HAL_PIXEL_FORMAT_YCbCr_420_888 -1");
                            if( (OMX_TRUE == mMJCEnable) || (OMX_TRUE == mDeInterlaceEnable) )
                            {
                                pPortDef->format.video.eColorFormat = (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_YUV_PRIVATE;
                                MTK_OMX_LOGE("[MJC][HAL_PIXEL_FORMAT_NV12] -> HAL_PIXEL_FORMAT_YUV_PRIVATE -1");
                            }
                        }
                        else //TBD
                        {
                            // Video codec requires 16 align for vertical stride, not standard YV12
                            pPortDef->format.video.eColorFormat = (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_YCbCr_420_888;
                            MTK_OMX_LOGE("[STANDARD][HAL_PIXEL_FORMAT_NV12] -> HAL_PIXEL_FORMAT_YCbCr_420_888 -2");
                            if( (OMX_TRUE == mMJCEnable) || (OMX_TRUE == mDeInterlaceEnable) )
                            {
                                pPortDef->format.video.eColorFormat = (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_YUV_PRIVATE;
                                MTK_OMX_LOGE("[MJC][HAL_PIXEL_FORMAT_NV12] -> HAL_PIXEL_FORMAT_YUV_PRIVATE -2");
                            }
                        }
                    }
                    else
                    {
                        ...
                        pPortDef->format.video.nStride = pPortDef->format.video.nFrameWidth;
                        pPortDef->format.video.nSliceHeight = pPortDef->format.video.nFrameHeight;
                        MTK_OMX_LOGE("UsingConverted, FrameWidth(%d), FrameHeight(%d)", pPortDef->format.video.nFrameWidth, pPortDef->format.video.nFrameHeight);
                    }
                }
                ...
                MTK_OMX_LOGU("32x32 Aligned! mOutputPortDef.nBufferSize(%d), nStride(%d), nSliceHeight(%d) nBufferCountActual(%d)",
        ...
        case OMX_IndexParamVideoRv:
        {
            ...
            MTK_OMX_LOGU("Get pRvType nPortIndex=%d", pRvType->nPortIndex);
        ...
        case OMX_GoogleAndroidIndexGetAndroidNativeBufferUsage:
        {
            ...
            if (pNativeBuffersUsage->nPortIndex == mOutputPortFormat.nPortIndex)
            {
                ...
                if(OMX_TRUE == mIsSecureInst) {
                    ...
                    MTK_OMX_LOGU("OMX_GoogleAndroidIndexGetAndroidNativeBufferUsage %d, %x", mIsSecureInst, pNativeBuffersUsage->nUsage);
                }
                if (mIsHDRVideo == OMX_TRUE) {
                    if (mANW_HWComposer == OMX_TRUE) {
                        mNativeWindowHDRInternalConvert = OMX_FALSE;
                    } else {
                        mNativeWindowHDRInternalConvert = OMX_TRUE;
                    }

                    MTK_OMX_LOGU("OMX_GoogleAndroidIndexGetAndroidNativeBufferUsage HDR video %d, mNativeWindowHDRInternalConvert %d, %x",
                        mIsHDRVideo, mNativeWindowHDRInternalConvert, pNativeBuffersUsage->nUsage);
                }
#endif
            }
            else
        ...
        case OMX_GoogleAndroidIndexDescribeColorFormat:
        {
                ...
                (HAL_PIXEL_FORMAT_YCbCr_420_888 == describeParams->eColorFormat) || (OMX_COLOR_FormatYUV420Flexible == describeParams->eColorFormat)) )
            {
                ...
                MTK_OMX_LOGU("client query OMX_COLOR_FormatYUV420Flexible mbYUV420FlexibleMode %d, ret: %d", mbYUV420FlexibleMode, err_return);
            }
            else
            {
...
//ANDROID_N
        ...
        default:
        {
            MTK_OMX_LOGU("MtkOmxVdec::GetParameter unsupported nParamIndex");
            err = OMX_ErrorUnsupportedIndex;
            break;
        }
    }
                                    ...
                                    OMX_IN OMX_PTR ComponentConfigStructure)
{
    ...
    switch (nConfigIndex)
    {
        ...
        case OMX_IndexVendorMtkOmxVdecPriorityAdjustment:
        {
            ...
            {
                mPropFlags |= MTK_OMX_VDEC_ENABLE_PRIORITY_ADJUSTMENT;
                MTK_OMX_LOGU("MtkOmxVdec::SetConfig -> enable priority adjustment");
            }
            else
            {
                mPropFlags &= ~MTK_OMX_VDEC_ENABLE_PRIORITY_ADJUSTMENT;
        ...
        case OMX_IndexVendorMtkOmxVdecSeekMode:
        {
            ...
            MTK_OMX_LOGU("Set seek mode enable, %lld", mSeekTargetTime);
        ...
        case OMX_IndexVendorMtkOmxVdecSwitchBwTVout:
        {
            ...
            if (*((OMX_BOOL *)ComponentConfigStructure) == OMX_TRUE)
            {
                MTK_OMX_LOGU("MtkOmxVdec::SetConfig -> Switch TV out TRUE");
                bEnable = VAL_TRUE;
                eVDecDrvSetParam(mDrvHandle, VDEC_DRV_SET_TYPE_SET_SWITCH_TVOUT, (VAL_BOOL_T *)&bEnable, NULL);
            }
            else
            {
                MTK_OMX_LOGU("MtkOmxVdec::SetConfig -> Switch TV out FALSE");
                bEnable = VAL_FALSE;
                eVDecDrvSetParam(mDrvHandle, VDEC_DRV_SET_TYPE_SET_SWITCH_TVOUT, (VAL_BOOL_T *)&bEnable, NULL);
            }
            break;
        ...
        case OMX_IndexVendorMtkOmxVdecAVSyncTime:
        {
            ...
            //MTK_OMX_LOGU("MtkOmxVdec::SetConfig set avsync time %lld", time);
            mAVSyncTime = time;
            break;
        }
        ...
        case OMX_IndexVendorMtkOmxVdecSlowMotionSpeed:
        {
            ...
            MTK_OMX_LOGU("Set 16x slowmotion speed(%d)", paramInfo->nS32);
...
//#endif
        ...
        case OMX_IndexVendorMtkOmxVdecSlowMotionSection:
        {
            ...
            MTK_OMX_LOGU("Sets 16x slowmotion section(%lld ~ %lld)", pParam->StartTime, pParam->EndTime);
//#ifdef MTK_CLEARMOTION_SUPPORT
            if (mMJCEnable == OMX_TRUE)
            {
...
//ANDROID_N
        ...
        case OMX_GoogleAndroidIndexDescribeColorAspects:
        {
            ...
            mDescribeColorAspectsParams.sAspects = pColorAspectsParams->sAspects;
            memcpy(&mDescribeColorAspectsParams.sAspects, &pColorAspectsParams->sAspects, sizeof(pColorAspectsParams->sAspects));
...
//#ifdef MTK_CLEARMOTION_SUPPORT
            ...
            if ((mMJCEnable == OMX_TRUE) && (mUseClearMotion == OMX_TRUE) && (mMJCScalerByPassFlag == OMX_FALSE))
            {
                ...
                MTK_OMX_LOGU("[HEVC][MJC] from MJC, GetCropInfo() u4CropLeft %d, u4CropRight %d, u4CropTop %d, u4CropBottom %d\n",
...
//#endif
            {
                if (VDEC_DRV_MRESULT_OK != eVDecDrvGetParam(mDrvHandle, VDEC_DRV_GET_TYPE_GET_FRAME_CROP_INFO, NULL, &temp_ccop_info))
                {
                    MTK_OMX_LOGU("[WARNING] Cannot get param: VDEC_DRV_GET_TYPE_GET_FRAME_CROP_INFO");
                    err = OMX_ErrorBadParameter;
                    break;
                }
                ...
                MTK_OMX_LOGU("OMX_IndexVendorMtkOmxVdecGetCropInfo GetCropInfo() from Codec, u4CropLeft %d, u4CropRight %d, u4CropTop %d, u4CropBottom %d\n",
        ...
        case OMX_IndexConfigCommonOutputCrop:
        {
            ...
            {
                MTK_OMX_LOGU("mCropWidth : %d , mCropHeight : %d", mCropWidth, mCropHeight);
                return OMX_ErrorUndefined;
            }
            ...
            else
            {
                ...
                MTK_OMX_LOGU("GetCropInfo() from Codec, u4CropLeft %d, u4CropRight %d, u4CropTop %d, u4CropBottom %d\n",
                             temp_ccop_info.u4CropLeft, temp_ccop_info.u4CropRight, temp_ccop_info.u4CropTop, temp_ccop_info.u4CropBottom);
            }
            break;
        }
...
//ANDROID_N
        ...
        default:
            MTK_OMX_LOGU("MtkOmxVdec::GetConfig Unknown config index: 0x%08X", nConfigIndex);
            err = OMX_ErrorUnsupportedIndex;
            break;
    }
                                   ...
                                   OMX_INOUT OMX_STATETYPE *pState)
{
    ...
    *pState = mState;
    ...
    MTK_OMX_LOGU("MtkOmxVdec::GetState (mState=%s)", StateToString(mState));
                                         ...
                                         OMX_IN OMX_U32 nSizeBytes)
{
        ...
        *ppBufferHdr = mInputBufferHdrs[mInputBufferPopulatedCnt] = (OMX_BUFFERHEADERTYPE *)MTK_OMX_ALLOC(sizeof(OMX_BUFFERHEADERTYPE));
        ...
        if (OMX_TRUE == mIsSecureInst)
        {
            ...
            else if(INHOUSE_TEE == mTeeType)
            {
                ...
                (*ppBufferHdr)->pBuffer = (OMX_U8 *)pfnMtkVideoAllocateSecureBuffer_Ptr(nSizeBytes, 512, "svp_vdec_in_AllocBuffer");  // allocate secure buffer from TEE
                //#endif
            }
            else
            {
        ...
        if (mInputBufferPopulatedCnt == mInputPortDef.nBufferCountActual)
        {
            ...
            MTK_OMX_LOGU("AllocateBuffer:: input port populated");
        }
    }
    else if (nPortIndex == mOutputPortDef.nPortIndex)
    {
        ...
        if (0 == mOutputBufferPopulatedCnt)
        {
            ...
            MTK_OMX_MEMSET(mFrameBuf, 0x00, arraySize);
            MTK_OMX_MEMSET(mFrameBufInt, 0x00, arraySize);
            MTK_OMX_LOGU("AllocateBuffer:: clear mFrameBuf");
        }
        ...
        *ppBufferHdr = mOutputBufferHdrs[mOutputBufferPopulatedCnt] = (OMX_BUFFERHEADERTYPE *)MTK_OMX_ALLOC(sizeof(OMX_BUFFERHEADERTYPE));
        ...
        if (OMX_TRUE == mIsSecureInst)
        {
            ...
            if (INHOUSE_TEE == mTeeType)
            {
                ...
                (*ppBufferHdr)->pBuffer = (OMX_U8 *)pfnMtkVideoAllocateSecureBuffer_Ptr(nSizeBytes, 512, "svp_vdec_out_AllocBuffer");  // allocate secure buffer from TEE
                //#endif
            }
            else
            {
            ...
            MTK_OMX_LOGU("AllocateBuffer hSecureHandle = 0x%08X", (*ppBufferHdr)->pBuffer);
        }
        else
        {
        ...
        if (OMX_FALSE == mStoreMetaDataInBuffers && OMX_FALSE == mIsSecureInst)
        {
            ...
            MTK_OMX_LOGU("MtkOmxVdec::AllocateBuffer Out port_idx(0x%X), idx[%d], pBuffHead(0x%08X), pBuffer(0x%08X), mOutputUseION(%d)",
        ...
        if (mOutputBufferPopulatedCnt == mOutputPortDef.nBufferCountActual)
        {
            ...
            MTK_OMX_LOGU("AllocateBuffer:: output port populated");
        }
    }
    else
    {
                                    ...
                                    OMX_IN OMX_U8 *pBuffer)
{
    ...
    if (nPortIndex == mInputPortDef.nPortIndex)
    {
        ...
        *ppBufferHdr = mInputBufferHdrs[mInputBufferPopulatedCnt] = (OMX_BUFFERHEADERTYPE *)MTK_OMX_ALLOC(sizeof(OMX_BUFFERHEADERTYPE));
        ...
        //MTK_OMX_LOGU ("UB port (0x%X), idx[%d] (0x%08X)(0x%08X)", (unsigned int)nPortIndex, (int)mInputBufferPopulatedCnt, (unsigned int)mInputBufferHdrs[mInputBufferPopulatedCnt], (unsigned int)pBuffer);
        MTK_OMX_LOGU("UB port (0x%X), idx[%d] (0x%08X)(0x%08X), mInputUseION(%d)", (unsigned int)nPortIndex, (int)mInputBufferPopulatedCnt, (unsigned int)mInputBufferHdrs[mInputBufferPopulatedCnt], (unsigned int)pBuffer, mInputUseION);
    ...
    else if (nPortIndex == mOutputPortDef.nPortIndex)
    {
        ...
        if (0 == mOutputBufferPopulatedCnt)
        {
            ...
            MTK_OMX_MEMSET(mFrameBuf, 0x00, arraySize);
            MTK_OMX_MEMSET(mFrameBufInt, 0x00, arraySize);
            MTK_OMX_LOGU("UseBuffer:: clear mFrameBuf");
        }
        *ppBufferHdr = mOutputBufferHdrs[mOutputBufferPopulatedCnt] = (OMX_BUFFERHEADERTYPE *)MTK_OMX_ALLOC(sizeof(OMX_BUFFERHEADERTYPE));
        (*ppBufferHdr)->pBuffer = pBuffer;
        (*ppBufferHdr)->nAllocLen = nSizeBytes;
        ...
        if (OMX_FALSE == mStoreMetaDataInBuffers)
        {
            ...
            {
                mIonOutputBufferCount++;
                MTK_OMX_LOGU("[UseBuffer] mIonOutputBufferCount (%d), u4BuffHdr(0x%08x), LINE:%d", mIonOutputBufferCount, (*ppBufferHdr), __LINE__);
            }
            ...
            MTK_OMX_LOGU("UB port (0x%X), idx[%d] (0x%08X)(0x%08X), mOutputUseION(%d)", (unsigned int)nPortIndex, (int)mOutputBufferPopulatedCnt, (unsigned int)mOutputBufferHdrs[mOutputBufferPopulatedCnt], (unsigned int)pBuffer, mOutputUseION);
            ...
            {
                OMX_U32 u4PicAllocSize = mOutputPortDef.format.video.nSliceHeight * mOutputPortDef.format.video.nStride;
                MTK_OMX_LOGU("mOutputUseION: %d, u4PicAllocSize %d", mOutputUseION, u4PicAllocSize);
                memset((*ppBufferHdr)->pBuffer + u4PicAllocSize, 128, u4PicAllocSize / 2);
                memset((*ppBufferHdr)->pBuffer, 0x10, u4PicAllocSize);
            }
                ...
                || (mUseClearMotion && (OMX_TRUE == mSeqInfoCompleteFlag) && ((mInputPortFormat.eCompressionFormat == OMX_VIDEO_CodingAVC) || (mInputPortFormat.eCompressionFormat == OMX_VIDEO_CodingHEVC))))
            {
                if (mOutputBufferPopulatedCnt < MTK_MJC_DEFAULT_OUTPUT_BUFFER_COUNT)
                {
                    ...
                    else
                    {
                        ...
                        if (mMJCLog)
                        {
                            MTK_OMX_LOGU("[MJC] [UseBuffer][MJC BUFFER][%d] Va = 0x%08X, Pa = 0x%08X, Size = 0x%08X, Hdr = 0x%08X",
                                         mMJCBufferCount,
                                         mMJCBufferInfo[mMJCBufferCount].u4VA,
                                         mMJCBufferInfo[mMJCBufferCount].u4PA,
                                         mMJCBufferInfo[mMJCBufferCount].u4Size,
                ...
                else
                {
                    ...
                    if (mOutputBufferPopulatedCnt < TOTAL_MJC_BUFFER_CNT)
                    {
                        ...
                        else
                        {
                            ...
                            if (mMJCLog)
                            {
                                MTK_OMX_LOGU("[MJC] [UseBuffer][SCALAR BUFFER][%d] Va = 0x%08X, Pa = 0x%08X, Size = 0x%08X, Hdr = 0x%08X",
                                             mScalerBufferCount,
                                             mScalerBufferInfo[mScalerBufferCount].u4VA,
                                             mScalerBufferInfo[mScalerBufferCount].u4PA,
                                             mScalerBufferInfo[mScalerBufferCount].u4Size,
                                     ...
                                     OMX_IN OMX_BUFFERHEADERTYPE *pBuffHead)
{
    ...
    if ((nPortIndex == MTK_OMX_INPUT_PORT) && bAllowFreeBuffer)
    {
        ...
        if (OMX_TRUE == mIsSecureInst)
        {
            MTK_OMX_LOGU("FreeBuffer: hSecureHandle(0x%08X)", pBuffHead->pBuffer);
            OMX_U32 i = 0;
    ...
    if ((nPortIndex == MTK_OMX_OUTPUT_PORT) && bAllowFreeBuffer)
    {
        ...
        if (OMX_FALSE == mStoreMetaDataInBuffers)
        {
            ...
            else
            {
                ...
                if (ret < 0)
                {
                    MTK_OMX_LOGU("[WARNING][Output][FreeBuffer], pBuffer %x, LINE: %d\n", pBuffHead->pBuffer, __LINE__);
                }
            }
//#ifdef MTK_CLEARMOTION_SUPPORT
            if (mMJCEnable == OMX_TRUE)
...
//#endif
        ...
        else
        {
            ...
            if (OMX_TRUE == mPortReconfigInProgress)
            {
                ...
                {
                    MTK_OMX_LOGU("waiting flush decoder done...");
                    SLEEP_MS(5);
                    count++;
                    if (count == 100)
                    {
    ...
    switch (nNewState)
    {
        case OMX_StateIdle:
            if ((mState == OMX_StateLoaded) || (mState == OMX_StateWaitForResources))
            {
                ...
                MTK_OMX_LOGU("wait on mInPortAllocDoneSem(%d), mOutPortAllocDoneSem(%d)!!", get_sem_value(&mInPortAllocDoneSem), get_sem_value(&mOutPortAllocDoneSem));
        ...
        case OMX_StateLoaded:
            ...
            if (mState == OMX_StateIdle)    // IDLE  to LOADED
            {
                if (IS_PENDING(MTK_OMX_LOADED_PENDING))
                {
                    ...
                    MTK_OMX_LOGU("wait on mInPortFreeDoneSem(%d) %d, mOutPortFreeDoneSem(%d) %d "
...
OMX_ERRORTYPE MtkOmxVdec::HandlePortEnable(OMX_U32 nPortIndex)
{
    ...
    if (nPortIndex == MTK_OMX_INPUT_PORT || nPortIndex == MTK_OMX_ALL_PORT)
    {
        ...
        {
            MTK_OMX_LOGU("Wait on mInPortAllocDoneSem(%d)", get_sem_value(&mInPortAllocDoneSem));
            WAIT_T(mInPortAllocDoneSem);
    ...
    if (nPortIndex == MTK_OMX_OUTPUT_PORT || nPortIndex == MTK_OMX_ALL_PORT)
    {
        if (IS_PENDING(MTK_OMX_OUT_PORT_ENABLE_PENDING))
        {
            MTK_OMX_LOGU("Wait on mOutPortAllocDoneSem(%d)", get_sem_value(&mOutPortAllocDoneSem));
            WAIT_T(mOutPortAllocDoneSem);
            CLEAR_PENDING(MTK_OMX_OUT_PORT_ENABLE_PENDING);

            if (mState == OMX_StateExecuting && mPortReconfigInProgress == OMX_TRUE)
            {
                MTK_OMX_LOGU("mPortReconfigInProgress as FALSE");
                mPortReconfigInProgress = OMX_FALSE;
            }
        }
        mCallback.EventHandler((OMX_HANDLETYPE)&mCompHandle,
...
OMX_BOOL MtkOmxVdec::FlushInputPort()
{
    ...
    MTK_OMX_LOGU("FlushInputPort -> mNumPendingInput(%d)", (int)mNumPendingInput);
    while (mNumPendingInput > 0)
    {
        MTK_OMX_LOGU("Wait input buffer release....%d", mNumPendingInput);
        SLEEP_MS(1);
        cnt ++;
        if (cnt > 2000)
        {
...
OMX_BOOL MtkOmxVdec::FlushOutputPort()
{
    ...
    MTK_OMX_LOGU("-FlushOutputPort -> mNumPendingOutput(%d)", (int)mNumPendingOutput);
...
OMX_ERRORTYPE MtkOmxVdec::HandlePortDisable(OMX_U32 nPortIndex)
{
    ...
    if (nPortIndex == MTK_OMX_OUTPUT_PORT || nPortIndex == MTK_OMX_ALL_PORT)
    {
        ...
        MTK_OMX_LOGU("MtkOmxVdec::HandlePortDisable mOutputPortDef.bPopulated(%d)", mOutputPortDef.bPopulated);
        ...
        else
        {
            if (get_sem_value(&mOutPortFreeDoneSem) > 0)
            {
                ...
                if (0 == retVal)
                {
                    MTK_OMX_LOGU("@@ OutSem -- (OK)");
                }
                else if (-1 == retVal)
                {
                    MTK_OMX_LOGU("@@ OutSem errno %d-- (EAGAIN %d)", errno, EAGAIN);
                }
            }
        }
        ...
        if (OMX_TRUE == mPortReconfigInProgress)
        {
            ...
            if ( (OMX_TRUE == mCrossMountSupportOn) )
            {
                ...
                mOutputPortDef.nBufferCountActual = mReconfigOutputPortBufferCount;
                MTK_OMX_LOGU("during OMX_EventPortSettingsChanged nBufferCountActual after adjust = %d(+%d) ",
                    mOutputPortDef.nBufferCountActual, mMaxColorConvertOutputBufferCnt);
            }
...
OMX_ERRORTYPE MtkOmxVdec::HandlePortFlush(OMX_U32 nPortIndex)
{
    ...
    MTK_OMX_LOGU("@@ mNumPendingInput(%d), mNumPendingOutput(%d), mPortReconfigInProgress(%d), mNeedDecodeWhenFlush %d", mNumPendingInput, mNumPendingOutput, mPortReconfigInProgress, mNeedDecodeWhenFlush);
    if ((nPortIndex == MTK_OMX_INPUT_PORT || nPortIndex == MTK_OMX_ALL_PORT) &&
        (!mEmptyThisBufQ.empty() || (mDecoderInitCompleteFlag == OMX_FALSE && mInputBuffInuse == OMX_TRUE)) && (mCodecId == MTK_VDEC_CODEC_ID_AVC || mCodecId == MTK_VDEC_CODEC_ID_MPEG4 || mCodecId == MTK_VDEC_CODEC_ID_HEVC))
    {
        ...
        while ((!mEmptyThisBufQ.empty() || (mDecoderInitCompleteFlag == OMX_FALSE && mInputBuffInuse == OMX_TRUE) || mNeedDecodeWhenFlush) && (!mFillThisBufQ.empty()) && (bIsHEVC_ETBQcontainsPPS || bIsH264_ETBQcontainsPPS || bIsMP4_ETBQcontainsPPS))  // etb queue may contain pps
        {
            ...
            {
                MTK_OMX_LOGU("@@ ignore pending check when decoding error %d, PendingI %d, PendingO %d", mErrorInDecoding, mNumPendingInput, mNumPendingOutput);
                break;
            }
            if (u4Count > 20)
            {
                if (OMX_TRUE == mPortReconfigInProgress)
                {
                    MTK_OMX_LOGU("@@ timeout(%d) skip pending check in PortReconfigInProgress, mNeedDecodeWhenFlush %d", u4Count, mNeedDecodeWhenFlush);

                    // allow decoding thread to consume next buffer
                    if (mRestorePortReconfigFlag == OMX_FALSE)
                    {
                        mPortReconfigInProgress = OMX_FALSE;
                        MTK_OMX_LOGU("@@ allow decoding thread to consume next buffer, mFramesDecoded %d, mNeedDecodeWhenFlush %d", mFramesDecoded, mNeedDecodeWhenFlush);
            ...
            SLEEP_MS(1);
            u4Count++;
            MTK_OMX_LOGU("@@ waiting for input being consumed, mNumPendingInput(%d) mNeedDecodeWhenFlush %d", mNumPendingInput, mNeedDecodeWhenFlush);
    ...
    if (nPortIndex == MTK_OMX_INPUT_PORT || nPortIndex == MTK_OMX_ALL_PORT)
    {
        ...
        MTK_OMX_LOGU("get lock before FlushInputPort\n");
        LOCK_T(mDecodeLock);
        FlushInputPort();
        mInputFlushALL = OMX_FALSE;
        UNLOCK(mDecodeLock);
    ...
    if (nPortIndex == MTK_OMX_OUTPUT_PORT || nPortIndex == MTK_OMX_ALL_PORT)
    {
        MTK_OMX_LOGU("get lock before FlushOutputPort\n");
        LOCK_T(mDecodeLock);
        FlushOutputPort();
        UNLOCK(mDecodeLock);
...
void MtkOmxVdec::ReturnPendingOutputBuffers()
{
    ...
    for (size_t i = 0 ; i < mBufColorConvertDstQ.size() ; i++)
    {
        ...
        MTK_OMX_LOGU("FBD (Pending_CCDQ) (0x%08X) (0x%08X) %lld (%u) flags(0x%08x)",
    ...
    for (size_t i = 0 ; i < mBufColorConvertSrcQ.size() ; i++)
    {
        ...
        MTK_OMX_LOGU("FBD (Pending_CCSQ) (0x%08X) (0x%08X) %lld (%u) flags(0x%08x)",
...
{
    MTK_OMX_LOGU("--- ETBQ: mNumPendingInput %d; mEmptyThisBufQ.size() %d", (int)mNumPendingInput, mEmptyThisBufQ.size());
...
{
    MTK_OMX_LOGU("--- FTBQ: mNumPendingOutput %d; mFillThisBufQ.size() %d,  CCDst: %d, CCSrc: %d",
...
OMX_U32 MtkOmxVdec::AllocateIonBuffer(int IonFd, OMX_U32 Size, VdecIonBufInfo *IonBufInfo)
{
    ...
    MTK_OMX_LOGU("ION allocate Size (%d), u4VA(0x%08X), share_fd(%d), VA(0x%08X), PA(0x%08X)",
...
OMX_BOOL MtkOmxVdec::DescribeFlexibleColorFormat(DescribeColorFormatParams *params)
{
    ...
    MTK_OMX_LOGU("DescribeFlexibleColorFormat %d fmt %x, W/H(%d, %d), WS/HS(%d, %d), (%d, %d)", sizeof(size_t), fmt, imageInfo.mWidth, imageInfo.mHeight,
    ...
    if (OMX_TRUE == mOutputAllocateBuffer || OMX_TRUE == needColorConvertWithNativeWindow())
    {
            ...
            (OMX_FALSE == direct_dequeue))
        {
            ...
            else
            {
                ...
                MTK_OMX_LOGU("@@ QueueOutputColorConvertDstBuffer (0x%08X) (0x%08X) index %d, mNumFreeAvailOutput %d, mIsFree %d, CCBsize %d",
...
void MtkOmxVdec::HandleVendorMtkOmxVdecUseClearMotion(OMX_BOOL mForceAllocateBuffer)
{
    ...
    if (OMX_TRUE == mIsSecureInst)
    {
        MTK_OMX_LOGU("BYPASS MJC for SVP");
        mUseClearMotion = OMX_FALSE;
        mMJCReconfigFlag = OMX_FALSE;
        MJC_MODE mMode;
        mMode = MJC_MODE_BYPASS;
...
OMX_BOOL MtkOmxVdec::EnableCodecDriverUFO()
{
    ...
    {
        MTK_OMX_LOGD("MJC is off (need Enable UFO)(OMX_IndexParamPortDefinition)");
        bMJCOff = VAL_TRUE;
    }
    ...
    if ((mCodecId == MTK_VDEC_CODEC_ID_HEVC || mCodecId == MTK_VDEC_CODEC_ID_AVC || mCodecId == MTK_VDEC_CODEC_ID_VP9)
         ...
         && mIsSecureInst == OMX_FALSE
         && ((mOutputPortDef.format.video.nStride * mOutputPortDef.format.video.nSliceHeight > 1920 * 1088) || ((mOutputPortDef.format.video.nStride * mOutputPortDef.format.video.nSliceHeight == 1920 * 1088) && bMJCOff == VAL_TRUE)))
    {
