...
OMX_BOOL MtkOmxVdec::InitHEVCSecParams() //HEVC.SEC.M0
{
    ...
    if (svp)
    {
        ...
        {
            property_get("ro.mtk_blowfish_tee_support", value, "0");
            if (atoi(value))
            {
                svp_mode = 3;
   ...
   else // TRUSTONIC_TEE
   {
       ...
       property_get("ro.mtk_tee_gp_support", value, "0");
...
OMX_BOOL MtkOmxVdec::DecodeVideoEx(OMX_BUFFERHEADERTYPE *pInputBuf)//, OMX_BUFFERHEADERTYPE *pOutputBuf)
{
    ...
    if (mDrvHandle != NULL)
    {
        ...
        property_get("mtk.omxvdec.setVideotsThreshold", setVideotsThreshold, "1000000");
...
//#endif
        ...
        else if (VDEC_DRV_MRESULT_NEED_MORE_OUTPUT_BUFFER == rResult)
        {
            ...
            MTK_OMX_LOGU("%s, %lld, 0x%X %d (0x%X), (%lld ms, %lld ms, %lld ms), %d (NEED_MORE_OUTPUT_BUFFER)",
        ...
        else
        {
            ...
            _out_time = getTickCountMs();
            if (!(mSeekMode == OMX_TRUE && mSeekTargetTime > 0)) {
            MTK_OMX_LOGU("%s (%d), %lld, 0x%X %d (0x%X), (%lld ms, %lld ms, %lld ms), %d",
...
//#endif
                        ...
                        mCodecId == MTK_VDEC_CODEC_ID_S263))
    {
        ...
        MTK_OMX_LOGU("InitVideoDecodeHW() %d", nFrameSize);
    ...
    if (mViLTESupportOn == OMX_TRUE)
    {
        ...
        property_get("mtk.vdec.vtdroperrframe", dropErrframeProp, "0");
...
//#endif
    ...
    MTK_OMX_LOGU("InitVideoDecodeHW(): meDecodeType=%d, mThumbnailMode=%d, color format=%x, nStride=%d, nSliceHeight=%d ",
    ...
    MTK_OMX_LOGU("VAL_CHIP_ID: %d", u4ChipName);
...
//#endif
    ...
    MTK_OMX_LOGU("[Info][InitVideoDecodeHW] codec property 0x%08x\n", mCodecProperty);
...
{
    MTK_OMX_LOGU("+DeInitVideoDecodeHW");
                    ...
                    (OMX_TRUE == mIsSecureInst && pFrameBuf->rSecMemHandle == mFrameBuf[i].frame_buffer.rSecMemHandle))
                {
                    ...
                    if (OMX_TRUE == bGetResolution || mLegacyMode == OMX_TRUE)
                    {
				        ...
				        if (mLegacyMode)
				        {
                            ...
                            if (VDEC_DRV_MRESULT_OK == eVDecDrvGetParam(mDrvHandle, VDEC_DRV_GET_TYPE_GET_FRAME_CROP_INFO, NULL, &temp_ccop_info))
                            {
                                OMX_U32 realCropWidth, realCropHeight;

                                realCropWidth = temp_ccop_info.u4CropRight - temp_ccop_info.u4CropLeft + 1;
                                realCropHeight = temp_ccop_info.u4CropBottom - temp_ccop_info.u4CropTop + 1;

                                if (mCropWidth != realCropWidth || mCropHeight != realCropHeight ||
                                    mCropLeft != temp_ccop_info.u4CropLeft || mCropTop != temp_ccop_info.u4CropTop)
                                {
                                    mCropLeft = temp_ccop_info.u4CropLeft;
                                    mCropTop = temp_ccop_info.u4CropTop;
                                    mCropWidth = realCropWidth;
                                    mCropHeight = realCropHeight;
                                    MTK_OMX_LOGU("GET_FRAME_CROP_INFO : mCropLeft %d, mCropTop %d, mCropWidth %d, mCropHeight %d\n",
                                            mCropLeft, mCropTop, mCropWidth, mCropHeight);
                            ...
                            else if (mCropWidth-mCropLeft != temp_pic_info.u4Width || mCropHeight-mCropTop != temp_pic_info.u4Height)
				            {
				                mCropLeft = 0;
				                mCropTop = 0;
				                mCropWidth = temp_pic_info.u4Width;
				                mCropHeight = temp_pic_info.u4Height;
				                MTK_OMX_LOGE("GetInfo() mCropLeft %d, mCropTop %d, mCropWidth %d, mCropHeight %d\n",
				                        mCropLeft, mCropTop, mCropWidth, mCropHeight);
...
OMX_BOOL MtkOmxVdec::dumpInputBuffer(OMX_BUFFERHEADERTYPE *pInputBuf, OMX_U8 *aInputBuf, OMX_U32 aInputSize)
{
...

    {
        ...
        if (OMX_TRUE == mViLTESupportOn)
        {
            ...
            property_get("mtk.omx.vdec.vilte.dump.bs", value, "0");
            bDump = atoi(value);
            if (bDump)
            {
