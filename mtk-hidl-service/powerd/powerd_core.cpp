...
static int load_api(void)
{
    ...
    if (perfCusUserRegScn == NULL) {
        ALOGE("perfCusUserRegScn error: %s", dlerror());
        dlclose(handle);
        return -1;
    }
...
int powerd_req(void * pMsg, void ** pRspMsg)
{
    ...
    vpRsp = (struct tPowerData *) malloc(sizeof(struct tPowerData));
    if (vpRsp == NULL) {
...
*/
        ...
        case POWER_MSG_QUERY_INFO:
            ...
            if(vpData->pBuf) {
                ...
                ALOGI("[powerd_req] POWER_MSG_QUERY_INFO: cmd:%d, param:%d", vpQueryData->cmd, vpQueryData->param);
        ...
        case POWER_MSG_SCN_CONFIG:
            if(vpData->pBuf) {
                vpScnData = (struct tScnData*)(vpData->pBuf);
                ALOGI("[powerd_req] POWER_MSG_SCN_CONFIG: hdl:%d, cmd:%d, param1:%d, param2:%d",
