                         ...
                         size_t bytes)
{
    ...
    ALOGD("out_write() wrote %zd bytes %zd frames", written_bytes, written_frames);
                       ...
                       size_t bytes)
{
...

    {
        ...
        while ((remaining_frames > 0) && (attempts < MAX_READ_ATTEMPTS)) {
            ...
            if (frames_read > 0) {
                ...
                ALOGD("  in_read (att=%d) got %zd frames, remaining=%zu",
                             attempts, frames_read, remaining_frames);
            } else {
                attempts++;
                ALOGG("  in_read read returned %zd", frames_read);
    ...
    ALOGD("in_read returns %zu", bytes);
    return bytes;
