...
#define MHAL_IOCTL_JPEG_ENC_START                   0x2007  ///< To start jpeg encode
 ...
 */
typedef enum {
    JPEG_ENC_FORMAT_YUY2,
    JPEG_ENC_FORMAT_UYVY,
    JPEG_ENC_FORMAT_NV12,
    JPEG_ENC_FORMAT_NV21,
    JPEG_ENC_FORMAT_YV12,
   
    JPEG_ENC_FORMAT_UNKNOWN
...
typedef struct
{
    // time for wait IRQ, unit : millisecond
    MINT32 timeout;

    // src/dst buffer address
    MUINT8 *srcAddr;
    MUINT8 *srcChromaAddr;
    MUINT8 *dstAddr;

    // src/dst FD for ION mode
    MINT32 srcFD;
    MINT32 srcChromaFD;
    MINT32 dstFD;

    // src/dst buffer size
    MUINT32 srcBufferSize;
    MUINT32 srcChromaBufferSize;
    MUINT32 dstBufferSize;

    // image width/height for encode
    MUINT32 imgWidth;
    MUINT32 imgHeight;
    MUINT32 imgStride;

    // whether to write SOI marker or not
    MUINT32 enableSOI;
    // quality setting for encode
    MUINT32 quality;
    // src buffer format: YUY2, UYVY, NV12, NV21, YV12
    JPEG_ENC_FORMAT_ENUM encFormat;  

    // encoded size, this is output for encoder
    MUINT32 *encSize;
