...
JpgSwDec::JpgSwDec()
{
    ...
    cinfo = new jpeg_decompress_struct_MTK;
    memset(cinfo, 0, sizeof(jpeg_decompress_struct_MTK));
...
bool JpgSwDec::create()
{
    ...
    cinfo->err = jpeg_std_error_MTK(&sw_err);
    ...
    jpeg_create_decompress_MTK(cinfo);
...
bool JpgSwDec::parse()
{
    ...
    int status = jpeg_read_header_MTK(cinfo, true);
    if (status != JPEG_HEADER_OK_MTK) {
    ...
    if(!cinfo->progressive_mode){
      ...
      if(cinfo->comps_in_scan == 3 && cinfo->jpeg_color_space == JCS_YCbCr_MTK){
...
bool JpgSwDec::Config(JUINT32 decMode)
{
    ...
    if(fQuality)
      cinfo->dct_method = JDCT_ISLOW_MTK;
    else
    ...
    cinfo->out_color_space = JCS_RGB_MTK;

    cinfo->dither_mode = JDITHER_NONE_MTK;

    //if (config == SkBitmap::kARGB_8888_Config) {

        cinfo->out_color_space = JCS_RGBA_8888_MTK;
        
    //} else if (config == SkBitmap::kRGB_565_Config) {
    //    cinfo.out_color_space = JCS_RGB_565;
    //    if (this->getDitherImage()) {
    //        cinfo.dither_mode = JDITHER_ORDERED_MTK;
    //    }
    //}
    ...
    if (!jpeg_start_decompress_MTK(cinfo)) {
   ...
   gettimeofday(&t1, NULL);
    ...
    for ( i=0; cinfo->output_scanline < cinfo->output_height && i<decRowNum; i++) 
    {
        ...
        for( j=0; cinfo->output_scanline < cinfo->output_height && j < LumaMcuHeight;j++ ){
         ...
         row_count = jpeg_read_scanlines_MTK(cinfo, &rowptr, 1);
