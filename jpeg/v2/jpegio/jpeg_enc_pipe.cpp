...
#define JPGENC_PORT_READ M4U_PORT_JPGENC_RDMA
#define JPGENC_PORT_WRITE M4U_PORT_JPGENC_BSDMA
#endif
...
bool JpgEncHal::LevelLock(EncLockType type)  
{
    ...
    else
    {
        ...
        { //lock video success
            ...
            } while((type == JPEG_ENC_LOCK_HW_ONLY) && (fEncoderType == JPEG_ENC_SW));
        }
    }
...
{  
    ...
    if (fEncFormat == kENC_YUY2_Format || fEncFormat == kENC_UYVY_Format)
    {
        ...
        fSrcMinBufferStride  = TO_CEIL((width2 << 1), 128);
    #else
    ...
    {
    #ifdef JENC_CORRECT_ALIGNMENT_CHECK
        fSrcMinBufferStride    = TO_CEIL((width2), 128);
    #else
...
bool JpgEncHal::alloc_ion()
{
    ...
    if (fIsSrc2p && (fSrcFD == fSrcFD2))
    {
        ...
        if (0x0 == fSrcIonPA)
        {
            if( !imgIon_getPA(fIonDevFD, fSrcFD, JPGENC_PORT_READ, fSrcIonVA, fEncSrcBufSize + fEncCbCrBufSize, &fSrcIonHdle, &fSrcIonPA))
                return false;
        }
    ...
    else
    {
        if (0x0 == fSrcIonPA)
        {
            if( !imgIon_getPA(fIonDevFD, fSrcFD, JPGENC_PORT_READ, fSrcIonVA, fEncSrcBufSize, &fSrcIonHdle, &fSrcIonPA))
                return false;
        }
        ...
        if( fIsSrc2p )
        {
            if (0x0 == fSrcChromaIonPA)
            {
                ...
                if( !imgIon_getPA(fIonDevFD, fSrcFD2, JPGENC_PORT_READ, fSrcChromaIonVA, fEncCbCrBufSize, &fSrcChromaIonHdle, &fSrcChromaIonPA))
                    return false;
            }
        }
    }
...
bool JpgEncHal::onSwEncode(JUINT32 *encSize)
{
    ...
    struct jpeg_compress_struct_MTK cinfo;
    struct jpeg_error_mgr_MTK jerr;
    unsigned int i, j, encLines;
    JSAMPROW_MTK y[16],cb[16],cr[16];           
    JSAMPARRAY_MTK data[3];           
    data[0] = y;
    data[1] = cb;
    data[2] = cr;

    /// Step 1: allocate and initialize JPEG compression object
    cinfo.err = jpeg_std_error_MTK(&jerr);

    cinfo.client_data = NULL;
    jpeg_create_compress_MTK(&cinfo);

    /// Step 2: set parameters for compression
    cinfo.image_width      = image_width;    /* image width and height, in pixels */
    cinfo.image_height     = image_height;
    cinfo.input_components = 3;          /* # of color components per pixel */
    cinfo.in_color_space   = JCS_YCbCr_MTK;  /* colorspace of input image */
    cinfo.encode_speed = SLOW_MTK;           /* enable multi-thread jpeg encode by 2x */
    ...
    jpeg_set_quality_MTK(&cinfo, quality, TRUE_MTK /* limit to baseline-JPEG values */);
    cinfo.en_soi = fIsAddSOI ;
    cinfo.dct_method = JDCT_IFAST_MTK;

    /// Step 3: specify data destination (eg, a memory)
    jpeg_mem_dest_MTK(&cinfo, (unsigned char **)&fDstAddr, (unsigned long *)&fDstSize);
        ...
        case kENC_NV12_Format:  /// YUV420, 2x2 subsampled , interleaved V/U plane
            {
                ...
                if (cinfo.encode_speed == SLOW_MTK)
                {
                ...
                jpeg_start_compress_MTK(&cinfo, TRUE_MTK);
                ...
                for (j = 0; j < image_height; j += 16)
                {
                    ...
                    jpeg_write_raw_data_MTK(&cinfo, data, 16);
       ...
       case kENC_UYVY_Format:
            {
                ...
                for (j = 0; j < image_height; j += 8)
                {
                    ...
                    jpeg_write_raw_data_MTK(&cinfo, data, 8);
                }
                free(mcuTempY);
                free(mcuTempU);
                free(mcuTempV);
...
bool JpgEncHal::start(JUINT32 *encSize)
{
    ...
    if (JPEG_ENC_HW == fEncoderType)
    {
        ...
        property_get("jpegEncode.forceEnable.SW", value, "0");
        forceToUseSWEnc = atol(value);

    #ifdef JENC_CORRECT_ALIGNMENT_CHECK
        // NV12/21 must 128x16 align
        if ((kENC_NV12_Format == fEncFormat || kENC_NV21_Format == fEncFormat) &&
            ((fSrcBufStride < ALIGN(fDstWidth, 128, 7)) || ((fEncSrcBufSize / fSrcBufStride) < ALIGN(fDstHeight, 16, 4))))
        {
            switchToSw = true;
            JPG_DBG(" JpgEncHal::start YUV420 Non-MCU align => force to use sw");
        }

        // YUY2 must 128x8 align
        if (kENC_YUY2_Format == fEncFormat &&
            ((fSrcBufStride < ALIGN(fDstWidth, 128, 7)) || ((fEncSrcBufSize / fSrcBufStride) < ALIGN(fDstHeight, 8, 3))))
        {
            switchToSw = true;
